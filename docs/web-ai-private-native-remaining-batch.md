...
| Private full-conversation public sharing | Standard personal-account non-project create/publish transaction, selected-branch binding, native confirmation and validated Copy/share result implemented | 69 focused sharing/delete/mutation Node cases pass; new native policy and consumer/MCP tests await grouped execution | Source-only for grouped APK; [contract and other sharing scopes](chatgpt-private-conversation-share.md); no actual link created yet |
...
| Private upload reservation allocation and claim | Current official experiment, single-use scope-bound slot, allocation concurrent with native byte/image preparation, zero-wait route selection and final processing integrated into the existing uploader | 170 focused Node cases pass, including exact allocation/claim bodies, PDF/image metadata, cancellation, expiry, late replies and unchanged temporary/project uploads | Source-only for grouped APK; [contract](chatgpt-private-upload-reservations.md); live claim and measured speed pending; native picker-open prewarm not wired yet |
...
| Attachment upload | Earlier small-text private upload verified; production text-file delivery passed on 1541; native byte handoff and ordinary/temporary/writable-project image/PDF transactions packaged; ingest/read-only projects, [common documents](chatgpt-private-document-upload.md), [multipart/Estuary byte routes](chatgpt-private-attachment-byte-transports.md) and [reservation allocation/claim](chatgpt-private-upload-reservations.md) now implemented in source | Confirm integrated private-route provenance, image/PDF/Office reading, runtime access, new byte/reservation routes, read-only file placement and temporary/project library behavior; finish native picker-open prewarm, temporary reservation persistence, remaining MIME categories and direct-library reuse. Production file delivery is not a private-route pass. See [upload contract](chatgpt-private-attachment-upload.md), [PDF extension](chatgpt-private-pdf-upload.md) and [scope contract](chatgpt-private-attachment-scopes.md). |
...
| Share/delete/conversation files | Native pin/rename/archive/move; private file index plus ordinary/project/library-linked and [simple image-pointer download candidates](chatgpt-private-image-download.md); guarded flag-selected private delete and current-chat reset; [ordinary full-conversation private share candidate](chatgpt-private-conversation-share.md) | Verify saved-file downloads, live flag binding, current/noncurrent deletion and share creation/update. Complete shared-library/connector/parameterized-image downloads and remaining sharing variants. System distribution alone is not official share creation. |
