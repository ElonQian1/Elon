...
该 capability 仅证明“扫描所得目录表与已验 Manifest 形成精确、可移植且有界的计划”，不读取归档正文、不创建目录、不写入文件，也不授权 promotion。后续受控解包入口现会在受管根下创建随机 create-new staging run，二次解析同一已验证 ZIP 句柄，按计划逐项落盘，并对每个输出文件复核字节数和 SHA-256、拒绝尾随数据、执行文件 flush/fsync 和同句柄身份复验。它尚未形成目录耐久回执、Store staged 事务、跨重启恢复或 promotion 权限，精确边界见 `node-plugin-archive-extraction.md`。
...
以上新增代码仍为 `implementation_uncompiled`：未编译、未测试、未执行 DDL，也未在 NodeAgent 启动或 Host 路径接线，因此没有真实数据库、网络下载、插件安装或 Sidecar 运行。代码层已形成 fence-owned cancellation、分段耐久写入、commit 恢复边界，以及由精确文件 damage evidence 驱动的 claim/download 原子损坏终结；authority v3 还形成 candidate verification journal、abort/revoke/resolution fence、promotion 禁入门卫，以及受管独占句柄从 byte 0 有界 SHA-256、每次 read 取消检查、exact EOF 和 hash 后身份复验底座。verification 现已形成 S1 exact closure、已验 root 下全文件 pin、两类规范摘要、post-pin S2、prepared begin、精确 absence/outcome reader、canonical terminal result validator、prepared recovery abort、受一次性本机字节/单调时间预算约束的完整 ordinal hash、首个 mismatch、observed artifact-set JCS digest、线性 Hashed/S3 TrustedHashed capability，以及事务内 current-admission S4、exact target resolution、跨 verification epoch 撤销、slot `verifying/failed` 投影和三类 fence 原子推进；可信时间已有本机 epoch、一次性挑战和签名证明验证入口；整库回滚防护已有规范本机检查点、固定见证的外部锚/不存在证明验签、严格启动比较合同、首次注册回执，以及前一锚 CAS 绑定的更新回执；已验 Manifest 还可产生拒绝危险条目、额外条目和 portable path 冲突的有界规范解包计划，已验证 ZIP 还可在随机 create-new staging run 中二次解析、逐文件受管写入、核对精确长度/Manifest SHA-256、flush/fsync 并保留全部输出句柄及 JCS 证据。所有 Store mutation error 与 commit error 仍只能恢复查询，不能盲重试；raw verified、解包计划和尚未登记 Store 的解包证据都不开放 promotion。生产取时/key resolver、认证传输与真实回滚见证查询/注册/发布、启动门卫接线、socket-read 取消、真实 HTTPS downloader、解包证据 Store 登记与跨重启恢复、失败 staging 治理、candidate health/promotion/GC 和 Host/Sidecar/IPC/沙箱仍待实现；本节不代表下载器或插件系统已经可用。解包边界见 `node-plugin-archive-extraction.md`。
