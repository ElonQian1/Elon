...
这一步刻意没有伪造签名计划：仓库只有节点侧 Ed25519 验签与严格 admission，没有生产 Control 私钥、Signer/KMS/HSM，也没有云端 Manifest catalog、signed Manifests、coherent inventory/node-profile/双 keyring 事实或生产 Bootstrap root/trusted-time resolver。本机 authority 初始化固定为 sharing disabled，而 InstallPlan admission 又要求计划授权与 live authority 精确相等；源码现已用独立 v4 policy-binding Store 打破这一循环依赖的本机子条件。它只消费 Bootstrap 在状态锁内产生的不可复制本机意图、同一 `PinnedComputePluginRoot` 锁 lease、当前 process fence 与 authenticated trusted-time observation，以追加式 receipt 和 exact authority/inventory CAS 写入当前 policy/authorization；同 revision 只允许 exact replay，commit-uncertain 可区分 current durable、historical committed、not-created 与 superseded not-created。每个非 replay 修订都会先使旧意图与 fetch source 失效，并把既有插件的 desired activation/admission 投影为 disabled/revoked，避免旧安装被新授权自动继承。

这只是 PlanApply live-binding 的必要前置，不是完整 planning context。当前 Store 没有生产 Host 调用点，v4 DDL/迁移也未实际运行；NodeRuntime 账号切换尚未接入 Bootstrap 失效代次，SQLite authority open 仍未与 pinned root 句柄闭合，prepared fetch/verification 仍采用明确阻断而非专用 policy 撤销，已发 Ready/Attempt 还缺 authority epoch/receipt 绑定，cleanup 也需要独立删除 custody。正确后续顺序是先补齐这些本机门卫与 planning snapshot，再由默认 unavailable 的真实 signer 接口产生计划，节点验签展示并取得本机 exact confirmation；在这些门卫全部闭合前继续保持零下载、零安装、零 Sidecar。
