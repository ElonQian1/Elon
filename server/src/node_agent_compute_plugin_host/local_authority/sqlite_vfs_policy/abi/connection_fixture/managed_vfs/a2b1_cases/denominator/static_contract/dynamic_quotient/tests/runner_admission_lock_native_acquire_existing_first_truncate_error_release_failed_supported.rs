//! Windows isolated controlled-fault admission for every exact q17 member.
...
use super::lock_native_acquire_existing_first_truncate_error_release_failed_cases::{
    ...
    lock_existing_first_truncate_error_release_failed_descriptor_v1,
    FrozenExistingFirstTruncateErrorReleaseFailedCompletionV1,
...
const EXACT_TEST: &str = "node_agent_compute_plugin_host::local_authority::sqlite_vfs_policy::abi::connection_fixture::managed_vfs::a2b1_cases::denominator::static_contract::dynamic_quotient::tests::runner_admission_lock_native_acquire_existing_first_truncate_error_release_failed_supported::isolated_q17_receipts_are_exact_controlled_fault_actual";
...
fn supported_key_and_member(
    ...
    let leaf = &frozen_lock_existing_first_truncate_error_release_failed_leaves_v1()[&case];
...
#[test]
fn isolated_q17_receipts_are_exact_controlled_fault_actual() -> anyhow::Result<()> {
    let selected_child =
        selected_lock_native_acquire_existing_first_truncate_error_release_failed_selector_for_test(
        );
    let mut parent_receipts = 0;
    for (&case, leaf) in frozen_lock_existing_first_truncate_error_release_failed_leaves_v1() {
        let candidate =
            lock_native_acquire_existing_first_truncate_error_release_failed_selector_for_test(
        ...
        let descriptor = lock_existing_first_truncate_error_release_failed_descriptor_v1(
            case,
        ...
        .map_err(|error| anyhow::anyhow!("supported q17 member {case:?} failed: {error:?}"))?;
    ...
    match selected_child {
        Some(selected) => Err(anyhow::anyhow!(
            "parent-selected q17 member was not found: {selected}"
        )),
        None => {
            assert_eq!(
                parent_receipts,
...
const fn runner_completion_v1(
    completion: FrozenExistingFirstTruncateErrorReleaseFailedCompletionV1,
