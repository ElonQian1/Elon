...
最新 schema-only 增量又把 cleanup execution plan、expected objects、plan seal 与 append-only step events 纳入 v3 必需对象。plan 绑定 authorization、installation、staging root、candidate-parent anchor identity 与完整对象父子拓扑；每个对象只有完成 durable intent、exact disposition/受约束的恢复缺失、exact parent 下的 namespace absence 和独立 namespace-durability fence，completion 才能同时引用 plan digest、既有物理 execution evidence digest 与独立 terminal journal digest。该增量未编译、未运行 DDL 或 schema 测试；物理 evidence v1 还没有不可变记录及 plan/object digest 绑定。JCS/路径/对象/物理证据摘要重算的 typed Store、目录 identity 与 namespace durability 原语、既有物理执行器对 durable-intent/journal 的适配及跨重启 recovery 均未实现。
...
当前合并后的供应链、候选健康、cleanup cancellation/exact-chain 加固与物理执行器已通过 `elon-pc-node` 测试目标编译；既有健康状态机、观察校验与 schema 定向测试共 12 项通过，清理链另有 3 项 Windows 同句柄删除测试和 1 项目录顺序测试。私有 cleanup authorization Store 可 fresh 对账 failed slot、quarantine、owner 与 fences，原子写入授权并推进 `cleanup_pending`；私有执行器随后只消费固定句柄，失败保留已完成证据和剩余 custody，成功也仅产生等待 completion Store 的物理证据。本批未重跑完整 Store/schema 事务测试，当前仍未接入生产 Host 或生产本地库，也不释放 owner、不修改 candidate pointer/inventory、不授予重试、安装、promotion、ReadyCapability 或商业 Verification。completion Store、目录耐久闭包、跨重启恢复、真实下载/Sidecar/探针、installed/promotion/GC 与 Attempt 接线仍待实现；精确边界见 `node-ready-capability.md` 与 `node-plugin-candidate-cleanup.md`。
