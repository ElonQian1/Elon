//! Exact frozen-member authority for the 88-member q15 initialization truncate-release tranche.
...
use super::{
    range_mask_v1, LockExistingFirstTruncateErrorReleaseSucceededCompletionV1,
    EXISTING_FIRST_TRUNCATE_ERROR_RELEASE_SUCCEEDED_MEMBER_COUNT,
};
...
pub(super) fn exact_member_v1(
    ...
    if observed_rows != EXISTING_FIRST_TRUNCATE_ERROR_RELEASE_SUCCEEDED_MEMBER_COUNT {
        return Err(LockRunnerExecutionViolationV1::MemberCatalogInvalid);
    }
...
fn expected_row_v1(row: usize) -> Option<ExpectedRowV1> {
    if row == 0 || row > EXISTING_FIRST_TRUNCATE_ERROR_RELEASE_SUCCEEDED_MEMBER_COUNT {
        return None;
    }
    let ordinal = row - 1;
    let request_ordinal = ordinal / 2;
    let completion = if ordinal % 2 == 0 {
        LockExistingFirstTruncateErrorReleaseSucceededCompletionV1::RetentionSucceeded
    } else {
        LockExistingFirstTruncateErrorReleaseSucceededCompletionV1::RetentionRouteUnknown
    };
