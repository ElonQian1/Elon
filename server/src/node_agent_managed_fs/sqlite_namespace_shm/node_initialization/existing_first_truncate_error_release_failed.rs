//! Q17 production initialization branch with unread truncate and cleanup-release receipts.
...
impl ManagedSqliteShmCoordinator {
    pub(super) fn execute_q17_truncate_release_failed_test_v1(
        &self,
        ...
        if !self.begin_test_initialization_existing_first_truncate_error_release_failed_v1(
            state,
            connection_id,
        )? {
            return Ok(file);
        ...
        let truncate_receipt = match file.truncate_outcome_unavailable_for_initialization_test_v1(0)
        {
            ...
            Ok(_) | Err(_) => {
                return Err(self.retain_after_q17_controller_rejection(
                    state,
                    connection_id,
                    file,
                    ...
                    false,
                    "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q17_TRUNCATE_SEAM_FAILED",
                ));
            }
        };
        ...
        if let Err(failure) = self
            .record_test_initialization_existing_first_truncate_error_release_failed_truncate_receipt_v1(
                state,
                connection_id,
                truncate_native,
            )
        ...
        if let Err(failure) = self
            .begin_test_initialization_existing_first_truncate_error_release_failed_cleanup_unlock_v1(
                state,
                connection_id,
            )
        {
        ...
        if let Err(failure) = self
            .record_test_initialization_existing_first_truncate_error_release_failed_cleanup_receipt_v1(
                state,
                connection_id,
                release_native,
            )
        ...
        self.record_test_initialization_existing_first_truncate_error_release_failed_poisoned_v1(
            state,
            connection_id,
        )?;
    ...
    fn retain_after_q17_controller_rejection(
        &self,
        state: &mut ManagedSqliteShmCoordinatorState,
        connection_id: u64,
