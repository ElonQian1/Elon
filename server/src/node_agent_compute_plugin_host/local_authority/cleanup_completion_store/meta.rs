...
use super::{
    ...
    projection::CandidateCleanupCompletionProjection,
};

pub(super) fn update_cleanup_completion_authority_meta(
    ...
    let updated = transaction
        .execute(
            ...
            WHERE singleton = 1
              ...
              AND trusted_time_high_water_ms = :completed_at
              AND clock_status = 'trusted' AND updated_at_ms = :completed_at"#,
            named_params! {
        ...
        .context("COMPUTE_PLUGIN_CANDIDATE_CLEANUP_COMPLETION_AUTHORITY_UPDATE")?;
    if updated != 1 {
        bail!("COMPUTE_PLUGIN_CANDIDATE_CLEANUP_COMPLETION_AUTHORITY_CAS");
    }
    Ok(())
}
