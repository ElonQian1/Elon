...
如果本轮修改或修复一龙 `pc-frontend/`、`/pc`、`/pc-next` 或用户可见 PC 工作台 UI，CodePushed 只代表代码同步，不代表用户已看到修复；除非用户明确说只同步代码或暂不发布，否则最终回复前必须完成：`pc-frontend` 构建验证 → commit/push → `scripts/publish-server.ps1` 或 `.sh` 上传 `$DATA_DIR/pc-next-dist` → `scripts/check-task-complete.ps1 -Kind PcFrontend` 校验 `/pc` 返回正常且 `/api/server/version.gitSha` 等于本次提交。截图、遮挡、错位、层级、弹窗或按图修复类问题，必须先把截图中的区域定位到真实组件/样式文件，再用本地预览、浏览器截图、DOM/坐标/层级检查之一做视觉验收；无法截图时最终回复必须说明替代证据或未完成视觉验收，不能只凭构建通过宣称解决。\n\
...
mod tests {
    ...
    fn pc_project_execution_prompt_requires_continuing_after_rules() {
        ...
        assert!(prompt.contains("CodePushed 只代表代码同步"));
