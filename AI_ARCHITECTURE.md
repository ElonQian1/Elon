...
当前 `server/src/compute_federation/` 提供未编译、未完整接线的云端领域合同；`server/src/store/compute_capacity_*.rs` 与 Provider、Offer、Price Snapshot、Job、Reservation Registry 已形成隔离的 CapacityPool、供给、Claim、Store-canonical 请求摘要、可组合事务内 Claim kernel、账本审计、到期恢复、生命周期、不可变历史与锁价快照。本人 Provider HTTP/MCP 控制面只允许创建和查询服务端固定为 `registering/self_declared` 的 `user_node` 或 `managed_cluster`；它不接受外部矿池、路由、凭据、适配器或 verified 证据，并返回脱敏视图。本人 CapacityPool 控制面只在自有 Provider 下创建和查询 `registering` Pool，服务端固定 epoch/revision 并生成摘要，响应不返回原始资源范围密钥或档案；它不激活或发行容量。Hold V2 固定完整 causal binding，Reservation Claim 强制绑定 Offer/Job/Reservation，Finish 继承并审计原始 held 绑定；Job、Reservation 登记和余额预授权、退款均已有不提交的事务内入口。v175 `compute_broker_reservation` 已将这些构件组合为单一事务的第一版 Reserve，并强制预算结果为 `reserved` 且含余额结果；不可变回执绑定原 quoted Job、reserved Job、Capacity Claim、active Reservation 和平台人民币余额预授权，可在合同到期后继续重放且不依赖余额表的可变到期字段。v176 在 Attempt 尚未激活时，以另一单一事务完成严格退款、held Claim Release/Expire、Job 与 Reservation 终态，并保存可审计的不可变终态回执。通用余额释放与到期器排除 Broker 管理的预算，v176 通过精确预授权 ID 的严格入口终结，避免单腿退款。登录用户 HTTP 与项目范围 MCP 已提供本人 Job/Reservation 列表、详情和 Reserve、Release、Expire；MCP 的财务冻结与取消要求显式确认。整条路径仍是 `implementation_uncompiled`，只支持 `platform_balance_cny`，尚未执行迁移、HTTP/MCP 运行验证、激活 Attempt 或实现运行中任务与实际用量结算。`server/src/node_agent_compute_plugin_host/` 已增加未编译的 Manifest、InstallPlan、双槽生命周期、ReadyCapability、Attempt 命令和 typed event 合同，但尚无下载器、Sidecar 或通用协议接线。现有节点模型白名单、Token 预留和流租约继续作为 `user_node + llm_chat` 兼容路径。权威架构与阶段见 `docs/distributed-compute/README.md`。
...
Provider 本人接口与信任边界见 `docs/distributed-compute/provider-api.md`，共享资源边界见 `docs/distributed-compute/capacity-pool-api.md`；Job、报价和预留接口见 `docs/distributed-compute/broker-api.md`。
...
| `server/src/store/compute_capacity_*.rs`、`compute_{provider,offer,job,reservation}_registry*`、`compute_quote_candidates.rs`、`compute_broker_reservation/`、`compute_federation_{provider,capacity_pool,broker}_{service,api,mcp}.rs` | 共享容量池、版本化合同、本人 self-declared Provider 与 registering Pool 控制面、项目级 Job 创建/既有候选发现与锁价、本人 Job/Reservation 查询、本地原子 Reserve、未执行任务原子 Release/Expire；当前未编译、未迁移、未运行验证，未接 Provider 路由/激活、Bucket/容量发行、Offer 发布、新报价生成、自动撮合、Attempt 或运行中结算 |
