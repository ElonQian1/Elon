(function () {
  ...
  if (previous && Number(previous.version) >= 1) return;
  ...
  function releaseLocal(nextPhase) {
    generation += 1;
    clearTimers();
  ...
  async function prepare() {
    ...
    try {
      ...
      peer.addEventListener('connectionstatechange', function () {
        if (token !== generation || !peer) return;
        if (peer.connectionState === 'connected') phase = 'active';
  ...
  const api = Object.freeze({
    ...
    dispose: end,
    status() {
      return Object.freeze({ version: 1, phase, active: phase === 'active', routeBound: Boolean(route) });
    },
  });
  ...
  root.__elonWinChatGptManagedVoicePeerLifecycle = Object.freeze({
    ...
    commit(target) {
      ...
      if (!base || typeof base.command !== 'function' || base.__elonWinManagedVoice === true) return false;
