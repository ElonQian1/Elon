...
| Offer 草稿、发布与生命周期控制面 | HTTP/MCP 已可管理本人 draft Offer；管理员 HTTP 可原子发布 active、转为 draining，并在无 pending/active Reservation 时终结为 expired/revoked。v182-v184 保存追加式回执和依赖索引，所有写入口均不移动资金，状态为 `implementation_uncompiled` |
