...
| Fresh independent text dispatch | ChatGPT | Partial source candidate, disabled: fresh preparation/security, lower HTTP/SSE and guarded history hydration covered offline; server stop, handoff recovery and device acceptance pending | Accepted sender only before dispatch; uncertain writes are never replayed. [Implementation boundary](chatgpt-fresh-text-dispatch.md) |
