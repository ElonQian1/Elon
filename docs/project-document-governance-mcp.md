...
Rust 单元测试覆盖：元数据目录不泄露正文、分页与字符预算、路径越界、虚构建议路径、审核门禁、revision 冲突、幂等应用、短期会话鉴权、`tools/list` 和直接 `tools/call`。

发布前至少运行：

```powershell
cargo test --manifest-path server/Cargo.toml --bin elon-pc-node project_document_governance_tests
cargo test --manifest-path server/Cargo.toml --bin elon-pc-node node_agent_project_docs_mcp_tests
cd pc-frontend
npm run build
```
