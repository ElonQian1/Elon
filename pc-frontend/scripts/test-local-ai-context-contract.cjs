...
const googleUrlPolicy = read('desktop-shell/src-tauri/src/local_ai_browser/google_url_policy.rs')
...
const realtimeVoiceDock = read('pc-frontend/src/features/user-browser/AiWebRealtimeVoiceDock.tsx')
...
assert.match(snapshotCache, /google_url_policy::sanitize_conversation_url/)
assert.match(googleUrlPolicy, /url\.path\(\) != "\/search"/)
assert.match(googleUrlPolicy, /values\.get\("csuir"\)/)
assert.match(googleUrlPolicy, /values\.get\("udm"\)/)
assert.match(googleUrlPolicy, /values\.get\("aep"\)/)
...
assert.match(controls, /localAiStableInteractionNeedsRefresh/)
assert.match(controls, /localAiComposerOptionsOrPreset/)
assert.match(controls, /localAiComposerSnapshotFromState/)
...
assert.match(controls, /AiWebRealtimeVoiceDock/)
assert.match(controls, /managedVoiceControllable/)
assert.match(realtimeVoiceDock, /Win 托管实时语音/)
assert.match(realtimeVoiceDock, /私有转写/)
...
assert.match(realtimeVoiceControl, /managedControlAvailable/)
assert.match(realtimeVoiceControl, /control_managed_realtime_voice/)
...
assert.equal(realtimeVoice.localAiManagedRealtimeVoiceControllable('requesting_microphone'), true)
assert.equal(realtimeVoice.localAiManagedRealtimeVoiceControllable('connecting'), true)
assert.equal(realtimeVoice.localAiManagedRealtimeVoiceControllable('active'), true)
assert.equal(realtimeVoice.localAiManagedRealtimeVoiceControllable('failed'), false)
assert.equal(realtimeVoice.localAiManagedRealtimeVoiceControllable('closed'), false)
