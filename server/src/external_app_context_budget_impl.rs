...
pub(super) fn max_context_chars() -> usize {
...
pub(super) fn trim_heavy_field(context: &mut Value, field: &str, keep: usize) {
...
pub(super) fn trim_context_pack(context: &mut Value, max_chars: usize) {
...
pub(super) fn json_char_len(value: &Value) -> usize {
    serde_json::to_string(value)
        .map(|text| text.chars().count())
        .unwrap_or(0)
}

pub(super) fn value_as_prompt_string(value: &Value) -> Option<String> {
...
pub(super) fn context_fact_summary(context: &Value) -> Value {
    json!({
...
pub(super) fn context_gap_summary(context: &Value) -> Value {
...
pub(super) fn context_warning_codes(context: &Value) -> Vec<String> {
    context
        .get("context_quality")
...
pub(super) fn truncated_fields(context: &Value) -> Vec<String> {
    [
...
pub(super) fn preflight_readiness_summary(value: Option<&Value>) -> Value {
...
pub(super) fn array_len(context: &Value, field: &str) -> usize {
    context
        .get(field)
        .and_then(Value::as_array)
        .map(Vec::len)
...
pub(super) fn id_samples(value: Option<&Value>, fields: &[&str], limit: usize) -> Vec<String> {
    value
        .and_then(Value::as_array)
        .map(|items| {
            items
...
pub(super) fn first_string_field(value: &Value, fields: &[&str]) -> Option<String> {
...
pub(super) fn citation_ids_by_kind(value: Option<&Value>, kind: &str, limit: usize) -> Vec<String> {
    value
        .and_then(Value::as_array)
        .map(|items| {
            items
...
pub(super) fn citation_source_samples(value: Option<&Value>, limit: usize) -> Vec<Value> {
    value
        .and_then(Value::as_array)
        .map(|sources| {
...
pub(super) fn order_samples(value: Option<&Value>, limit: usize) -> Vec<Value> {
    value
        .and_then(Value::as_array)
        .map(|orders| {
            orders
...
pub(super) fn compact_slip_value(slip: &Value) -> Value {
    json!({
...
pub(super) fn first_prompt_value(value: &Value, fields: &[&str]) -> Value {
    fields
        .iter()
