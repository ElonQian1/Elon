//! Q16 controller path for unread truncate and cleanup-release receipts.
...
impl ManagedSqliteShmTestInitializationControllerV1 {
    pub(super) fn reject_q16_truncate_success_if_selected(
        &mut self,
        target: ExactTarget,
    ) -> Result<(), &'static str> {
        ...
        if is_selected(active) {
            return fail(
                active,
                "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q16_TRUNCATE_SUCCESS_FORBIDDEN",
            );
        }
        Ok(())
    }

    pub(super) fn reject_q16_release_path_if_selected(
        &mut self,
        target: ExactTarget,
    ) -> Result<(), &'static str> {
        let Some(active) = self.active_for_event(target)? else {
        ...
        if is_selected(active) {
            return fail(
                active,
                "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q16_RELEASE_PATH_INVALID",
            );
        }
        Ok(())
    }

    pub(super) fn begin_created_first_truncate_error_release_failed(
        &mut self,
        target: ExactTarget,
            ...
            || active.consumed
        {
            return fail(
                active,
                "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q16_TRUNCATE_SEQUENCE_INVALID",
            );
        }
        Ok(true)
    }

    pub(super) fn record_created_first_truncate_error_release_failed_truncate_receipt(
        &mut self,
        target: ExactTarget,
            ...
            || active.cleanup_native.is_some()
        {
            return fail(
                active,
                "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q16_TRUNCATE_RECEIPT_INVALID",
            );
        }
        advance(
            active,
            ...
            "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q16_TRUNCATE_RECEIPT_SEQUENCE_INVALID",
        )?;
        active.native = Some(native);
        Ok(())
    }

    pub(super) fn begin_created_first_truncate_error_release_failed_cleanup_unlock(
        &mut self,
        target: ExactTarget,
    ) -> Result<(), &'static str> {
        let active = self.require_active_for_event(target)?;
            ...
            || active.consumed
        {
            return fail(
                active,
                "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q16_CLEANUP_SEQUENCE_INVALID",
            );
        }
        advance(
            active,
            ...
            "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q16_CLEANUP_ATTEMPT_SEQUENCE_INVALID",
        )
    }

    pub(super) fn record_created_first_truncate_error_release_failed_cleanup_receipt(
        &mut self,
        target: ExactTarget,
        native: ManagedSqliteShmTestInitializationNativeReceiptV1,
    ) -> Result<(), &'static str> {
            ...
            || active.consumed
        {
            return fail(
                active,
                "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q16_CLEANUP_RECEIPT_INVALID",
            );
        }
        advance(
            active,
            ...
            "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q16_CLEANUP_RECEIPT_SEQUENCE_INVALID",
        )?;
...
pub(super) fn reject_release_receipt_if_selected(
    ...
    if is_selected(active) {
        return fail(
            active,
            "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q16_LEGACY_RELEASE_RECEIPT_INVALID",
        );
    }
    Ok(())
}
...
pub(super) fn record_poisoned_if_selected(
    ...
    advance(
        ...
        "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q16_POISON_SEQUENCE_INVALID",
    )?;
    Ok(true)
}
...
pub(super) fn validate_finish_if_selected(
    ...
    let cleanup = active
        .cleanup_native
        .ok_or("NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q16_CLEANUP_RECEIPT_MISSING")?;
    ...
    {
        return Err("NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q16_CLEANUP_RECEIPT_INVALID");
    }
    Ok(true)
}
...
fn validate_exact_counts(counts: &EventCounts) -> Result<(), &'static str> {
    if counts.request != 1
        ...
        || counts.poisoned != 1
    {
        return Err("NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q16_EVENT_COUNTS_INVALID");
    }
    Ok(())
}
...
fn validate_terminal(terminal: TerminalStateV1) -> Result<(), &'static str> {
    ...
    {
        return Err("NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q16_TERMINAL_STATE_INVALID");
    }
    Ok(())
}
