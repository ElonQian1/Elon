...
- 2026-06-24 子项目最新协作状态：fb2 `origin/main` 已推进到 `d0967bb5 feat(ai-context): add p4 source enumerator`。最新关键 checkpoint 是 `165e50f2 feat(ai-context): compress context pack first pass`、`d4efde21 fix(ai-context): clarify p4 optional gate status`、`d93287e9 feat(ai-context): add p4 vector readiness plan`、`e6caccac feat(ai-context): define p4 vector contract`、`b2493cbc feat(ai-context): add p4 embedding dry run`、`d0967bb5 feat(ai-context): add p4 source enumerator`，以及之前的 `01972c2d` 公开 `123qwe` 复盘质量 drill、`fbe2c857` match index dry-run 自测、`1b0cdc2b` final gate topic 对齐。主项目本阶段只消费 Context Pack、tool manifest、read-only tools、retrieval trace、P4-lite/report、P4 vector readiness plan、`fb2_p4_vector_contract_v1` 契约设计、`fb2_p4_source_enumerator_v1` source-specific no-write sample、`fb2_p4_embedding_build_dry_run_v1` no-write 估算报告和复盘质量 summary；`answer_time_refresh_allowed=false`、`maintenance_rest` refresh 工具不得放进普通回答链路。
...
- 主项目对 fb2 最新 P4 向量计划的处理规则：`fb2_p4_vector_readiness_plan_v1` 是只读计划报告，`fb2_p4_vector_contract_v1` 是已提交的契约设计，`fb2_p4_source_enumerator_v1` 只是 source-specific no-write endpoint sample，`fb2_p4_embedding_build_dry_run_v1` 只是 offline embedding build 之前的 no-write 估算报告；四者都不是生产 grounding。它们必须同时保持 `contract_design_committed_embedding_not_started`、`source_specific_no_write_sample_available`、`dry_run_available_no_writes`、`writes_embedding_rows=false`、`writes_vector_store=false`、`writes_public_group_messages=false`、`ready_to_write_embeddings=false`、`refresh_operations_used=false`、`candidate_rows_require_live_hydration=true`、`vector_rows_are_model_input=false`、`does_not_enable_vector=true`、`ready_to_build_embedding_store=false`、`ready_to_enable_answer_time_vector_candidates=false`、`answer_time_vector_candidates_enabled=false`、`production_grounding=false`。本人订单或平台聚合 scope 缺失时必须保留 `not_checked_missing_current_user_scope` / `not_checked_missing_platform_scope`，不能越权读取，也不能误报为空。`fb2.search_vector_candidates` 只是 reserved planned query 且 `implemented=false`；在 fb2 完成 full partitioned no-write chunk manifest、权限过滤 SQL、idempotent embedding write transaction、hydration、shadow eval 和 answer-time readthrough 门禁前，主项目不得把 P4 vector/RAG 当成生产 grounding、不得把 vector candidates 注入用户回答。
