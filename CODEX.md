...
- Per-machine build cache paths must be local configuration, not committed script defaults. `scripts/publish-server.ps1` and `scripts/publish-server.sh` read `ELON_BUILD_TARGET_DIR` from the process environment or root `.env.local`; the scripts always create a **fixed-name** subdirectory `elon-server-musl/` under that root — never a SHA-specific directory. If `ELON_BUILD_TARGET_DIR` is unset, Windows uses `%LOCALAPPDATA%\Elon\build-target\elon-server-musl\` and Linux/macOS uses `~/.cache/elon/build/elon-server-musl/` (XDG standard). Both paths survive across sessions on the same machine, enabling **incremental compilation**: first build on a new machine takes ~10 minutes (full); subsequent builds where only `elon-server` changed take ~30 seconds. Never manually set `CARGO_TARGET_DIR` to a SHA-specific path — that defeats the cache.
