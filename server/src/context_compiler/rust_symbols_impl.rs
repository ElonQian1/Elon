...
pub(super) fn scan_dir(base: &Path, _dir: &Path, max_files: usize, index: &mut RustIndex) {
...
pub(super) fn extract_file_symbols(base: &Path, path: &Path, content: &str) -> Vec<RustSymbol> {
    let relative = relative_path(base, path);
...
pub(super) fn parse_symbol_line(line: &str) -> Option<ParsedSymbol> {
...
pub(super) fn visibility(line: &str) -> (SymbolVisibility, &str) {
    let trimmed = line.trim_start();
...
pub(super) fn after_keyword<'a>(line: &'a str, keyword: &str) -> Option<&'a str> {
...
pub(super) fn parse_identifier(rest: &str) -> Option<String> {
...
pub(super) fn find_block_end(lines: &[&str], start_idx: usize) -> Option<usize> {
...
pub(super) fn collect_signature(lines: &[&str], start_idx: usize, line_end: usize) -> String {
...
pub(super) fn rust_safety_notes(
    parsed: &ParsedSymbol,
    signature: &str,
    lines: &[&str],
    line_start: usize,
...
pub(super) fn assign_impl_parents(symbols: &mut [RustSymbol]) {
    let impls = symbols
        .iter()
