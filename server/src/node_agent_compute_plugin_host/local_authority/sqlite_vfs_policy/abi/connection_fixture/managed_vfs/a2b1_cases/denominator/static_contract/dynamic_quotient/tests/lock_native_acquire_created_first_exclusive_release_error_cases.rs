//! Independent typed fixtures for the 88 q12 created-first exclusive-release terminals.
...
pub(super) const LOCK_CREATED_FIRST_EXCLUSIVE_RELEASE_ERROR_MEMBER_COUNT: usize = 88;

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub(super) enum FrozenCreatedFirstExclusiveReleaseCompletionV1 {
    RetentionSucceeded,
    RetentionRouteUnknown,
}

impl FrozenCreatedFirstExclusiveReleaseCompletionV1 {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub(super) struct FrozenLockCreatedFirstExclusiveReleaseErrorCaseV1 {
    pub(super) action: LockActionV1,
    pub(super) first: u8,
    pub(super) count: u8,
    pub(super) mask: u8,
    pub(super) completion: FrozenCreatedFirstExclusiveReleaseCompletionV1,
}

#[derive(Clone)]
pub(super) struct FrozenLockCreatedFirstExclusiveReleaseErrorLeafV1 {
    ...
    LEAVES.get_or_init(|| {
        ...
        super::super::super::source_leaf_authority::validate_lock_graph_with_records(
            ...
            |leaf| {
                ...
                let Some(case) = created_first_exclusive_release_error_v1(record, descriptor)
                else {
                    return Ok(());
                };
                ...
                if previous.is_some() {
                    return Err(format!("duplicate frozen q12 member {case:?}"));
                }
                Ok(())
            },
        )
        .expect("the frozen Lock authority must validate before q12 source tests");
        assert_eq!(
            leaves.len(),
...
pub(super) fn lock_created_first_exclusive_release_error_descriptor_v1(
    case: FrozenLockCreatedFirstExclusiveReleaseErrorCaseV1,
    capability: RunnerCapabilityV1,
) -> TerminalDescriptorV1 {
    let mut descriptor =
        frozen_lock_created_first_exclusive_release_error_leaves_v1()[&case].descriptor;
    let TerminalDescriptorV1::Lock(value) = &mut descriptor else {
        unreachable!("a q12 fixture must have a Lock descriptor")
    };
    value.recipe.capability = capability;
    descriptor
}

pub(super) fn lock_created_first_exclusive_release_error_expected_groups_v1(
        ...
        .map(|leaf| {
            (
                prepare_dynamic_terminal_v1(&leaf.record, &leaf.descriptor)
                    .expect("a frozen q12 descriptor must prepare")
                    .key,
                leaf.member,
            )
        })
...
fn created_first_exclusive_release_error_v1(
    record: &LeafRecordV1,
    descriptor: &TerminalDescriptorV1,
) -> Option<FrozenLockCreatedFirstExclusiveReleaseErrorCaseV1> {
    ...
    let completion = match completion {
        LockCompletionV1::UnsafeRetentionSucceededThenRouteUnknown => {
            FrozenCreatedFirstExclusiveReleaseCompletionV1::RetentionSucceeded
        }
        LockCompletionV1::UnsafeRetentionRouteUnknownThenRouteUnknown => {
            FrozenCreatedFirstExclusiveReleaseCompletionV1::RetentionRouteUnknown
        }
        _ => return None,
    };
    let case = FrozenLockCreatedFirstExclusiveReleaseErrorCaseV1 {
        action,
        first,
        count,
        mask,
...
const fn expected_axes_v1(case: FrozenLockCreatedFirstExclusiveReleaseErrorCaseV1) -> LockAxesV1 {
    LockAxesV1 {
...
#[test]
fn frozen_q12_family_is_exact_unique_and_only_contains_acquire_requests() {
    let leaves = frozen_lock_created_first_exclusive_release_error_leaves_v1();
    assert_eq!(
        leaves.len(),
        LOCK_CREATED_FIRST_EXCLUSIVE_RELEASE_ERROR_MEMBER_COUNT
    );
