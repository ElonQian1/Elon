...
如果本轮影响一龙 `pc-frontend/`、`/pc`、`/pc-next` 或用户可见 PC 工作台 UI，不能只用 CodePushed 收尾；除非用户明确说暂不发布，否则最终回复前必须构建 PC 前端、运行 publish-server 上传 pc-next-dist、校验 /pc 和 /api/server/version。截图/遮挡/错位类 UI 修复还必须先定位真实组件并做视觉验收或说明替代证据。\n\n\
{preflight}\
用户请求：\n\
{user_message}",
...
如果本轮修改或修复一龙 `pc-frontend/`、`/pc`、`/pc-next` 或用户可见 PC 工作台 UI，CodePushed 只代表代码同步，不代表用户已看到修复；除非用户明确说只同步代码或暂不发布，否则最终回复前必须完成：`pc-frontend` 构建验证 → commit/push → `scripts/publish-server.ps1` 或 `.sh` 上传 `$DATA_DIR/pc-next-dist` → 校验 `/pc` 返回正常且 `/api/server/version.gitSha` 等于本次提交。截图、遮挡、错位、层级、弹窗或按图修复类问题，必须先把截图中的区域定位到真实组件/样式文件，再用本地预览、浏览器截图、DOM/坐标/层级检查之一做视觉验收；无法截图时说明替代证据，不能只凭构建通过宣称解决。\n\
