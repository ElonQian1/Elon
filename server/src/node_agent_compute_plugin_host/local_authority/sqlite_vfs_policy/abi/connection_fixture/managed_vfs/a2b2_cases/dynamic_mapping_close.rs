//! Physical-SHM-only actual bridge for final Keep/MappingClose/AfterSuccessKnown.
//!
...
pub(in super::super) struct MappingClosePhysicalSubsetActual {
...
pub(in super::super) fn validate_mapping_close_after_success_physical_subset(
    actual: MappingClosePhysicalSubsetActual,
) -> Result<(), &'static str> {
...
fn select_frozen_case(inventory: &[Case]) -> Result<&Case, &'static str> {
    ...
    let case = selected.next().ok_or("frozen MappingClose case missing")?;
    if selected.next().is_some() {
        return Err("frozen MappingClose case is not unique");
    }
    Ok(case)
}
