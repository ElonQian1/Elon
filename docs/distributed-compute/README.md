...
| UserNode Windows Runner loader/进程监管前置草案 | 未登记且不可达：loader 定义单一 successor/by-value seam、FileId leases/reopen receipts 与五类 failure custody；exact PE/launch/live-OS、launch-security/currentness/release-recovery producers 均缺，unconfirmed termination parks whole graph。extraction-share、pre-resume/dynamic-load blocked；严格 `source_written/source_review_only/implementation_uncompiled/implementation_unrun`、`passed=0/failed=0`、`migration/table/writer=none/none/none`、四 gap missing、loader 18 effects none。见 [loader authority](user-node-windows-runner-loader-load-set-authority.md)、[loader acceptance](user-node-windows-runner-loader-load-set-acceptance.md)、[process authority](user-node-windows-runner-process-custody-authority.md) 与 [process acceptance](user-node-windows-runner-process-custody-acceptance.md) |
