//! Exact frozen-member authority for completed own-overlap and not-held rejections.
...
use super::{
    range_mask_v1, LockLocalProtocolRejectionPathSpecV1, LOCAL_PROTOCOL_REJECTION_MEMBER_COUNT,
};

const HEADER: &str =
    "action\tfirst\tcount\tmask\trejection\tcompletion\tcase_key_sha256\tfull_record_sha256";
const MEMBER_CATALOG: &str =
    include_str!("local_protocol_own_overlap_or_not_held_completed_members.v1.tsv");
...
pub(super) fn exact_member_v1(
    ...
    for line in lines {
        ...
        let Some(rejection_text) = fields.next() else {
            return Err(LockRunnerExecutionViolationV1::MemberCatalogInvalid);
        };
        ...
        if fields.next().is_some()
            ...
            || rejection_text != rejection_tag_v1(expected.action)
    ...
    if observed_rows != LOCAL_PROTOCOL_REJECTION_MEMBER_COUNT {
        return Err(LockRunnerExecutionViolationV1::MemberCatalogInvalid);
    }
...
fn expected_row_v1(row: usize) -> Option<ExpectedRowV1> {
    if row == 0 || row > LOCAL_PROTOCOL_REJECTION_MEMBER_COUNT {
        return None;
    }
    let ordinal = row - 1;
    let (path, action, first, count) = if ordinal < 8 {
        (
...
const fn rejection_tag_v1(action: LockActionV1) -> &'static str {
    match action {
        LockActionV1::LockShared | LockActionV1::LockExclusive => "own-overlap",
        LockActionV1::UnlockShared => "shared-not-held",
        LockActionV1::UnlockExclusive => "exclusive-not-held",
    }
}
...
pub(super) const fn catalog_row_count_for_test() -> usize {
    LOCAL_PROTOCOL_REJECTION_MEMBER_COUNT
}
