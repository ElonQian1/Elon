...
for (const kind of ['scoped selector', 'semantic composer', 'scoped label', 'scoped instant label']) {
  test(kind + ' precedes the page header and admits the private picker', async () => {
    ...
    const instant = kind === 'scoped instant label';
    f.node.textContent = instant ? '\u5373\u65f6' : 'High';
    const unrelated = { ...f.node, textContent: 'Settings', __reactFiber$fixture: null };
    const scope = {
      querySelector: selector => kind === 'scoped selector' && selector === '[aria-label*="选择模型"]' ? f.node : null,
      querySelectorAll: () => instant ? [unrelated, f.node] : kind === 'scoped label' ? [f.node] : []
    };
    ...
    page.__elonChatGptActionTargetPolicy = {
      actionPoint: node => node === f.node || instant && node === unrelated ? { x: 40, y: 40 } : null
    };
    page.__elonChatGptModelLabelPolicy = instant
      ? require('../android/app/src/main/assets/chatgpt_web_adapter_model_label_policy.js')
      : { isModelLabel: () => true };
