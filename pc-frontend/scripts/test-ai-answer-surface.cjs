...
const messageRows = read('pc-frontend/src/features/ai/AiChatMessageRows.tsx')
...
assert.match(page, /<AiChatMessageRows/)
assert.match(messageRows, /messages\.filter\(\(message\) => message\.role !== 'system'\)/)
assert.match(messageRows, /visibleMessages\.map\(\(message, index\) =>/)
assert.match(messageRows, /<AiChatMessageRow/)
