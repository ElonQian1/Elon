...
| 外部算力池适配器与统一报价 | v221-v264 前置门为 `implementation_partially_verified`。V263 已向受限 child 交付短时 config/credential；V264 新增 server-owned fresh DNS、公网地址门卫、直连 TCP、TLS 1.3/WebPKI/exact SPKI pin 及 V258 Store 前后复验，本地动态与源码合同 `11 passed / 0 failed`。当前仍未发送 upstream application bytes 或生成 probe/route/activation；V254 18 deny 不变。见 [`V264 Broker TLS 权威`](external-pool-adapter-broker-tls-transport-authority.md)、[`V264 验收`](external-pool-adapter-broker-tls-transport-acceptance.md)、[`V263 Secret delivery`](external-pool-adapter-ephemeral-secret-delivery-authority.md) 与 [`当前状态`](current-implementation-status.md) |
...
Provider Adapter 的 v227-v264 前置门已形成分层权威。V260 已验证 authenticated session，V261 验证 Linux supervisor，V262 组合真实 exec 后 runtime，V263 向同一受限 child 交付 V256 短时 config/credential。V264 由 server broker 使用 V258 exact target 完成 fresh DNS、公网地址全答案门卫、直连 TCP、TLS 1.3/WebPKI/exact SPKI 认证和 Store 前后复验，但只在本地 TLS fixture 动态验收且没有应用层 I/O。下一硬门是 Adapter-generated bounded upstream no-work request/response observation。V254 18 deny 逐字保留，完整 admission gate 只能与 atomic activation 同批替换。见 [`V264 Broker TLS`](external-pool-adapter-broker-tls-transport-authority.md)、[`V263 Secret delivery`](external-pool-adapter-ephemeral-secret-delivery-authority.md)、[`V262 authenticated runtime`](external-pool-adapter-authenticated-runtime-authority.md) 与 [`current-implementation-status.md`](current-implementation-status.md)。
