...
impl Store {
    pub(crate) fn record_task_sui_preflight_report(
        ...
        let report = self
            .task_sui_preflight_report_by_key(input.adapter_id, input.idempotency_key)?
    ...
    fn task_sui_preflight_report_by_key(
        &self,
        adapter_id: &str,
        idempotency_key: &str,
    ) -> Result<Option<SuiPreflightReport>> {
        self.conn()?
            .query_row(
                &format!("{REPORT_SELECT} WHERE adapter_id=?1 AND idempotency_key=?2"),
                params![adapter_id.trim(), idempotency_key.trim()],
                report_from_row,
            )
            .optional()
...
fn report_from_row(row: &Row<'_>) -> rusqlite::Result<SuiPreflightReport> {
...
const REPORT_SELECT: &str = "SELECT id, project_id, adapter_id, credential_version, package_kind,
            projection_package_id, target_network, handoff_digest,
            projection_digest, outcome, summary, tool_version,
            idempotency_key, report_digest, created_at
