...
fn admission_gate_limits_per_user_concurrency() {
    ...
    drop(first);
    let after_release = try_acquire_runtime_admission_for_request(&user_id, limits, None).unwrap();
    drop(after_release);
}
...
fn admission_gate_limits_global_concurrency_across_users() {
    ...
    let first = try_acquire_runtime_admission_for_request(&first_user, open_limits, None).unwrap();
    let second =
        try_acquire_runtime_admission_for_request(&second_user, capped_limits, None).unwrap_err();
    assert_eq!(
        second,
...
fn admission_gate_debounces_duplicate_request_fingerprints() {
    ...
    let first =
        try_acquire_runtime_admission_for_request(&user_id, limits, Some("fingerprint-a")).unwrap();
    let duplicate =
    ...
    let distinct =
        try_acquire_runtime_admission_for_request(&user_id, limits, Some("fingerprint-b")).unwrap();
    drop(distinct);
    drop(first);
}
