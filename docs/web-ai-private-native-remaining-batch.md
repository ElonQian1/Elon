...
| Independent native file library | Root/folder/search/cursor browsing, cache, downloads, rename/soft-delete and existing-file composer association | 1579 live GET succeeds but native snapshot is null; v5 object-event bridge fix passes real-adapter tests, grouped APK pending | [Browser/receipt](chatgpt-private-library-browser.md), [mutations](chatgpt-private-library-mutations.md), [attachment](chatgpt-private-library-attachment.md); signed in, no new login prerequisite; folders/special sources remain open |
