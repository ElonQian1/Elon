...
const restartStorage = { getItem: key => savedDrafts.get(key) ?? null,
  setItem: (key, value) => savedDrafts.set(key, value), removeItem: key => savedDrafts.delete(key) }
const beforeRestart = new LocalAiProviderDraftCache(2, restartStorage)
const boundIdentity = localAiProviderDraftIdentity('chatgpt', 'fixture-owner')
beforeRestart.remember(boundIdentity, 'unsent fixture')
assert.equal(savedDrafts.size, 0)
beforeRestart.checkpointForRestart()
beforeRestart.remember(boundIdentity, 'last edit before exit')
const afterRestart = new LocalAiProviderDraftCache(2, restartStorage)
assert.equal(afterRestart.read(boundIdentity), 'last edit before exit')
assert.equal(afterRestart.read(localAiProviderDraftIdentity('chatgpt', 'other-owner')), '')
assert.equal(savedDrafts.size, 0)
afterRestart.checkpointForRestart()
afterRestart.cancelRestartCheckpoint()
assert.equal(savedDrafts.size, 0)
const unknownOwnerDraft = new LocalAiProviderDraftCache(2, restartStorage)
unknownOwnerDraft.remember(localAiProviderDraftIdentity('chatgpt', ''), 'pending owner')
assert.throws(() => unknownOwnerDraft.checkpointForRestart(), /restart_draft_owner_unresolved/)
...
assert.match(controller, /resumeLocalAiWebSession\(providerId, ownerKey, cachedState, \(cached\) =>/)
