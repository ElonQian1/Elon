...
const documentMock = {
  querySelectorAll(selector) {
    if (selector === '*') return [root, legacy, modern, modernOptions, create, hidden];
    return [legacy, modern, modernOptions, create];
  }
};
...
assert.deepEqual(projects.map((project) => project.id), ['g-p-legacy_1', 'g-p-modern_2']);
