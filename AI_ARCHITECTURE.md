...
| `server/src/node_agent_compute_plugin_host/` | Windows 节点 legacy Host seam、按需插件、签名准入、authority/PlanApply/fetch/verification、可信时间/回滚锚、ZIP 安全物化、staging Store、稳定结果读取、同句柄 adoption 与进程内候选健康评估合同；`PinnedComputePluginRoot` 已内嵌父句柄相对、share-none 的工件根独占锁，取数/验证 custody 以非 Clone lease 继续保活同一锁句柄。默认关闭的 Bootstrap 已挂入 NodeRuntime，当前只有旧 LLM 执行接入；节点状态目录 witness 不授权工件根副作用，sharing-on 获取根锁、生产时间源/回滚见证、真实下载、目录耐久/跨重启恢复、真实 Sidecar 探针、health Store/恢复、installed/promotion 与正式 Attempt 路径未接线 |
