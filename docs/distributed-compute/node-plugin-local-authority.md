...
以上供应链与候选健康代码已通过 `elon-pc-node` 编译，候选健康状态机、观察校验与 schema 定向测试共 9 项通过；它们仍未在 NodeAgent 启动或 Host 路径接线，也未对生产本地库执行 DDL。authority v3 已覆盖 PlanApply、可恢复分段取数、损坏终结、候选原子验证、schema 定义指纹、可信时间、回滚锚合同、ZIP 安全物化、staging Store，以及候选健康回执 Store/恢复。健康写入前会 fresh read 当前 staged 候选、精确 staging receipt、state/inventory/authority/process fence 与 trusted-time high-water；同一 `BEGIN IMMEDIATE` 只插入不可变健康回执并推进可信时间高水位，不改变槽位、inventory 或激活代次。不确定结果可按进程内 recovery key 稳定读出 `NotCreated` 或 exact `Recorded`；后者只有在 fresh read 与 retained-handle 全量重哈希仍精确匹配时才恢复 typed durable custody，前者不恢复写许可。这仍不是 installed/promotion permit、ReadyCapability 或商业 Verification。目录耐久闭包、跨重启恢复、失败 staging/quarantine 治理、真实 Sidecar 探针、installed/promotion/GC、生产取时/key resolver、认证传输与回滚见证、真实 HTTPS downloader、Host/Sidecar/IPC/沙箱仍待实现；`staged` 或健康回执都不等于 installed、promotable、running 或可产生收益。解包边界见 `node-plugin-archive-extraction.md`，健康边界见 `node-ready-capability.md`。
