//! Exact frozen-member authority for the 88-member q14 initialization truncate-release tranche.
...
use super::{
    range_mask_v1, LockCreatedFirstTruncateErrorReleaseSucceededCompletionV1,
    CREATED_FIRST_TRUNCATE_ERROR_RELEASE_SUCCEEDED_MEMBER_COUNT,
};
...
pub(super) fn exact_member_v1(
    ...
    if observed_rows != CREATED_FIRST_TRUNCATE_ERROR_RELEASE_SUCCEEDED_MEMBER_COUNT {
        return Err(LockRunnerExecutionViolationV1::MemberCatalogInvalid);
    }
...
fn expected_row_v1(row: usize) -> Option<ExpectedRowV1> {
    if row == 0 || row > CREATED_FIRST_TRUNCATE_ERROR_RELEASE_SUCCEEDED_MEMBER_COUNT {
        return None;
    }
    let ordinal = row - 1;
