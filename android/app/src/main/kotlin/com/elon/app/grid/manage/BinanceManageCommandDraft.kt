...
internal data class BinanceManageCommandDraft(val id: String, val action: String, val cps: Boolean,
    val amount: String, val range: BinanceRangeDraft?) {
    companion object {
        fun parse(raw: String): BinanceManageCommandDraft {
            ...
            return BinanceManageCommandDraft(id,action,if(action in setOf("settings","close")) flag("cps") else false,amount,range)
        }
    }
}
...
internal object BinanceManageCommandView {
    ...
    fun action(value: String) = when(value) {
        ...
        "investment" -> "追加策略投入"; "range" -> "修改区间和格数"; else -> "尚未选择"
    }
    fun detail(s: BinanceManageSnapshot): Map<String,Any?> = linkedMapOf(
    ...
    fun digest(state: BinanceManageState) = BinanceHostState.digest(StrictJson.encode(linkedMapOf(
        ...
        "range" to state.rangeDraft?.payload(),"summary" to summary(state))))
}
