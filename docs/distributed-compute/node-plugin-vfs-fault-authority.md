...
A2c runner foundation 不读写 A2b2 expected record，也不产生 `WindowsDynamic` evidence。四条源码路径只接通 route-exact callback-before 与 registration unregister 的进程隔离执行形状；`ShmLock` observation 不含具体 shared/exclusive 动作，registration after-success retained parts 也不能外推为 SHM/file 的平台 after-success custody。现阶段仍不得从该源码推断 native failure、barrier 无返回码、unmap、joint close、route lifecycle 或 117-key 覆盖。
