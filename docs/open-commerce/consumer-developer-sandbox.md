...
用户决定停止时，AI 只有在获得明确指令并提交固定短语后，才能调用 `open_commerce_cancel_my_action_confirmation`。该工具可幂等终止尚未创建 Invocation 的 pending 或 confirmed 确认；自然过期和已消费确认不会被改写。取消确认不是取消订单、退款或回滚已经发生的商户动作。
...
消费者还可针对本人关系发起关联数据删除请求。创建请求会原子撤销该关系；商户只能看到匿名关系别名，可接单、拒绝或声明完成。消费者可在接单前撤回请求，但关系不会恢复。跟进按 7 天内部运营目标，在 24 小时后允许首次催办、每 24 小时一次且最多三次，超时并至少催办一次后可升级关注；Rust/SQLite、本地 Axum 路由、PC 静态契约和正式构建已验证，但不会自动通知第三方、仲裁或处罚。`completed` 只表示商户提交了可审计声明。完成后，商户可按外部系统追加回执编号、原始回执 SHA-256 和摘要，消费者可查看；该证明账本的迁移、权限、幂等、匿名读取和 V5 投影已通过本地验证，但记录仍固定标记为商户提供且平台未核验，不能证明美团、ERP、CRM 或会员系统真实完成删除。
...
当前已形成商户主动发布目录、候选范围摘要、服务端发现输入规范化、商户声明来源与新鲜度、能力与内部同步回执来源绑定、消费者主动声明期筛选、内部回执来源筛选、厂商数据域与最大回执年龄过滤、非穷尽来源筛选建议、币种安全价格过滤、能力类型与访问级别筛选、偏好硬约束、五种用户可选透明非付费排序器、可复核未签名单次排序凭证、限时授权、消费者可撤销关系、低敏偏好字段披露、客户端加密数据保险箱、匿名删除请求、有界催办与升级关注、商户未核验证明、含证明与调用凭证的本人 V5 可验证导出、隔离导入、Schema 驱动填写、短时动作确认、持久化调用配额、活动证据、商户级手动 App 封禁和沙盒 App 生命周期闭环。输入规范化、能力来源绑定、内部回执来源要求、厂商/数据域/年龄筛选及非穷尽建议、价格与币种、能力类型与访问级别、偏好硬约束、声明期筛选、五种排序器、候选范围、服务端排序凭证、来源联合条件及删除请求有界跟进已通过定向 Rust/SQLite 回归，删除请求跟进另通过本地 Axum 路由和 PC 正式构建，消费者发现 MCP 也通过同源回归；V5 零条/一条删除证明导出及主要失败关闭边界已有服务端专项，完整 V5 导入和保险箱等批次仍有未统一回归项。携带筛选条件的绑定端口 HTTP 请求、PC 浏览器、Web Crypto 与下载文件尚未实测；外部平台签名与官方回读证明、运营方签名凭证、第三方排序器 SDK、生产应用审核、完整订单迁移、自动外部通知、平台仲裁、商户或 AI 授权解密、真实删除适配器、支付和真实平台适配器仍是后续模块。
