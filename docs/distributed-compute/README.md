...
| 节点插件治理合同 | Signed Manifest、InstallPlan、双槽 lifecycle 与短期 ReadyCapability 合同已写；独立 SQLite authority v3、root-signed 双 keyring、库存/计划/候选原子应用、三段式取数、跨代撤销与不确定结果恢复已形成代码。Windows Volume GUID 卷根、父句柄相对打开、同句柄游标协调和耐久分段写入也已形成。v3 预留候选级 artifact verification run、撤销 fence 和 promotion 门卫；节点现可从 sealed plan 重建完整 candidate closure，经已验 root 一次固定共享父句柄链并 share-none 打开全部文件，生成跨节点内容摘要和本机 file-set binding，再用 pin 后新可信时间完成第二次权威对账。begin/recovery、真实 hash 编排、post-hash binder 和终态事务仍未实现；全部未编译、建库或接线 |
