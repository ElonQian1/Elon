...
| fb2 场景矩阵进入工具计划审计 | Rust 单测验证 `external_app.tool_plan.v1` metadata 带 `domain_scenario_selection schema=fb2.domain_scenario_selection.v1`，并记录 `permission_scope`、`primary_tools`、`required_citations`、`forbidden_outputs`；默认 smoke 检查 `tool_execution_contract.main_project_execution_result.plan.domain_scenario_selection` | 已补 planner metadata 并发布到 `v0.3.645 / 1c55e3e0`；默认 `smoke-fb2-ai-center.ps1` 已把该字段升级为 contract 门槛，防止后续漂移 |
...
| live 预检交接 | `status-current.json` 输出 `live_preflight_request`，受控 token bridge 或显式 `FB2_AI_CENTER_TOKEN` 跑 no-write direct read 和 `-DataOnlyAcceptance -PreflightOnly` | 最新 token bridge 已满足 protected live preflight：`protected_live_preflight_satisfied=true`、`token_passed_as_argument=false`、`fb2_password_passed_to_child_argv=false`、`writes_visible_group_messages=false`、`project_network_proxy_policy=direct_no_proxy`；群聊证据必须继续使用 direct API read，包含 `message_id/text_len/text_sha256`，截图不作为验收证据 |
...
| 最终验收 wrapper 离线回归 | `smoke-fb2-final-acceptance.ps1 -SelfTest` 检查三类 feedback coverage、直接群聊回读完整性、summary post fallback 分层、子脚本 exit code、voice/quality/permission evidence 摘录、no-write preflight 群聊直读和 summary success 门槛 | 已补本地自测；`604b2b88` 后 `success=true` 还必须满足 `visible_direct_read_complete=true`，证明 baseline、`@EL` seed/回复、selected-message seed/回复和 summary-post 都有接口回读正文长度与 sha256。本轮又加入 `read_only_direct_read_complete` 到 `-PreflightOnly` summary：写真实群前必须先 no-write 读取 `/api/me/groups/{group_id}/messages`，且 summary 不能是写入证据、不能缺正文 hash。full final 不接受 summary fallback。它只证明 wrapper 逻辑未退化，不替代 live token、真实群聊写入或真机语音最终证据 |
...
- 真实 `FB2_AI_CENTER_TOKEN` 或受控 token bridge 已用于最新 data-only 验收，并完成 authenticated readiness/direct manifest、live Context Pack、平台匿名摘要、权限、质量汇总和三类 feedback 样本验证；token 仍不应写入仓库或文档，fb2 密码也不得进入子进程 argv。
