...
function Library({ groupId, groups, onClose }: { groupId: string; groups: Group[]; onClose: () => void }) {
  ...
  return <ArticleDialog label="文章中心" onCancel={() => { if (!editing) onClose() }}><section className={styles.panel}>
    {editing ? <ArticleEditor initial={editing} groups={groups} groupId={groupId} onClose={() => { setEditing(undefined); setReload(v => v + 1) }} /> : <>
      ...
      <nav className={styles.tabs} aria-label="文章范围"><button disabled={busy} aria-pressed={!mine} onClick={() => setMine(false)}>群文章</button><button disabled={busy} aria-pressed={mine} onClick={() => setMine(true)}>我的文章</button><button onClick={() => setSquareOpen(true)}>币安广场</button></nav>
    ...
    {squareOpen && <SquareCenter onClose={() => setSquareOpen(false)} />}
