...
structured reading, native editing/export and ordinary-message save-back are
...
| Text submit/stream/stop/follow-up | [Independent page-owned HTTP](chatgpt-fresh-text-dispatch.md) completed/default-enabled for existing personal plain text on 1696. [1699 stopped-turn follow-up](reports/chatgpt-command-recovery-1698.md#stopped-turn-follow-up-on-1699) accepted native Stop and subsequent send after a partial answer, with unique user messages and history reconciliation. Runtime send retained for other scopes; native retry accepted on 1644 | Reuse [native retry](reports/chatgpt-regeneration-store-20260911.md#normal-1644-acceptance). Stop before any assistant message, new/tool/file/project/temporary scopes, initial composer-free ownership and real network-loss recovery remain |
