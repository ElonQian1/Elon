...
| 算力结算管理页 | 🟡 中 | `src/features/compute-settlement/`、`server/src/compute_federation_settlement_*` | 🟡 已写入管理员平台账户、到期候选、显式逐笔释放、提款队列、拒绝退款和外部已付款证明登记源码并注册 `/compute-settlement`；终态操作复用 v201 且不执行真实付款，页面尚未构建、视觉验收或发布 |
