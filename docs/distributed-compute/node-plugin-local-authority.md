...
该 capability 仅证明“扫描所得目录表与已验 Manifest 形成精确、可移植且有界的计划”，不读取归档正文、不创建目录、不写入文件，也不授权 promotion。真实归档解析器仍须在受管根下执行句柄相对的逐项落盘，防止解析与使用之间的替换，并按 Manifest 对每个输出文件复核字节数和 SHA-256、拒绝尾随条目、完成文件与目录 fsync 后再形成 staged 证据；这些执行能力尚未实现。
