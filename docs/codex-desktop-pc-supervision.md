...
运维时必须同时显式传入 `-StateRoot <Desktop 身份专属状态目录>` 与 `-InstallRoot <NodeAgent 安装目录>`，两条路径会冻结进 Desktop review 状态，后续 Validate、Commit、Rollback 和 ticket 签名路径不一致时一律 fail-closed。先以 `desktop-review-credential.ps1 -Action Prepare` 在 Desktop 身份的 `Cert:\CurrentUser\My` 生成暂存密钥并收紧状态目录和 CNG 私钥 ACL，再 `-Action Validate` 真实复核证书、owner、ACL 与身份，经 Desktop 独立审查后执行 `-Action Commit`；随后先重启 NodeAgent 读取 InstallRoot 中的公钥，再重启 Desktop/helper 以相同 StateRoot/InstallRoot 取得私钥签名能力。更新包只需保留 NodeAgent 的 `node-agent.env` 公钥行；Desktop 专属 StateRoot 不属于节点更新包。节点与 helper 必须共同宣告 `desktop_review_ticket_v3` 才允许 Review；仅宣告 v2/v1 的旧节点保持业务任务与 journal，但 helper 不发送 v3 review。服务端只在显式设置 `ELON_DESKTOP_REVIEW_ALLOW_V2=1` 时接受 v2 迁移票据，默认拒绝；v1 共享凭据路径不属于公钥模式的回退能力。
