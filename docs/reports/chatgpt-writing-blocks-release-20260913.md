...
- **代码块本机编辑/导出已实测，不重复生成代码块样本。** 1688 当时未覆盖的 Writing Block 变体，现已由本报告顶部的 1692 验收补齐。
...
- 早期本机导出脚本只修改带固定测试标记且摘要匹配的副本，不触发官网保存；后续独立保存脚本才显式确认写回。临时测试工具自动移除，测试会话与导出文件保留，不删除账号数据。
