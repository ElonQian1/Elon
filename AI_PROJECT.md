...
调试包约定：每个稳定 Windows 节点在真机上只有一个 Launcher 调试包：`com.elon.app.uituner_<8位节点指纹>`，指纹只从持久 `install_id` 派生并跨客户端更新、重启保持不变。PC 工作台、HTTP、MCP 和旧调用即使传入 `.uituner`、`.uitest` 或 `.uitest_anim`，真机也必须归一到该固定包，禁止以新后缀作为失败兜底；显式隔离后缀只允许模拟器。正式包 `com.elon.app` 和名称“一龙ai”不变。构建输出保留在代次 worktree 中，协调器只把校验过包名、标签、签名、版本和 SHA-256 的 APK 原子暂存到内容寻址目录，再覆盖安装；冲突、离线或签名不一致时保留最后可用 APK 和手机当前版本。历史杂包只报告，不自动卸载用户手机应用。
