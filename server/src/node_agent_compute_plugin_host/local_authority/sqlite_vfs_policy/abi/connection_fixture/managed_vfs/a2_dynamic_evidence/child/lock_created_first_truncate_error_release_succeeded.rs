//! Canonical q14 child header for created-first truncate uncertainty with known release.

pub(in super::super) const REPORT_VERSION: &str = "a2lockq14";
pub(in super::super) const REPORT_VALUE_COUNT: usize = 160;
...
pub(in super::super) fn selector(
    ...
    Ok(format!(
        "initialization-{action}-first-{first}-count-{count}-created-first-truncate-error-release-succeeded-{completion}-terminal-route-unknown"
    ))
}
...
mod tests {
    ...
    #[test]
    fn eighty_eight_q14_selectors_are_unique_and_fail_closed() {
