...
> v218 合同修正：历史“v11 Planning Snapshot V2”现以协议阈值 12 解释；work-admission `None` 允许新晋升槽首次重授权，`Some` 才承诺 current head。当前 A1 只有未编译的同事务 projector，A2 只有未编译、未运行的 test-only VFS static contract/source；无 producer，v14 永久 blocked-only，ready 另建 v15，Runtime/Ready/派发仍不可达。
...
| 节点插件治理合同 | Signed Manifest/InstallPlan、双 keyring、authority、受管取数/验证/staging/cleanup 与 v216 install/promotion 双回执已形成；v216 已随节点编译并通过 11 项版本链及既有 69 项 SQLite 回归，其中 3 项为单 Connection 成功路径。v217/v218 work-admission、Planning A1 尚未编译；A2 只形成未编译、未运行的 test-only SHM/联合 close/同 namespace 多 Connection static contract/source，既有回归不证明该增量。生产 VFS/register/live `sqlite3_file`/process owner/open、A1 producer、root/keyring/time/rollback、Signer、Host、downloader、Sidecar、Runtime、Ready 与 Attempt 接线仍不可达；v14 永久 blocked-only。失败清理 sequence 1–4 的 32 项既有证据继续有效，但完整 cleanup/跨重启/驱动链仍缺。精确边界见本页阅读顺序第 4 项 |
...
F1 的下一硬门槛是以后实际执行 [`node-plugin-vfs-fault-authority.md`](node-plugin-vfs-fault-authority.md) 的 Windows SHM、联合关闭与多 Connection 矩阵。本批 static contract/source 未编译、未运行，不能据此进入生产 VFS/open、Planning producer、v15、Runtime 或 Ready。
