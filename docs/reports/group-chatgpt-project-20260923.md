...
- 源 `90bda5f5d`，APK `1.1.1807 / 1807` 完整 Release 构建和发布成功，`CHATGPT_PRIVATE_RESEARCH_ENABLED=false`。SHA-256 `4d97b3c707c32b8c33bf6bd77f47f51ab48bc9a4087f59e660e40b936f645b4a`。
- 主项目登记的小米通过无线 ADB `install -r` 覆盖安装，版本回读 1807 成功；荣耀离线。没有清应用数据、Cookie 或登录状态。
- 该安装回执只证明正式包交付；项目创建、群回答和会话复用须继续单独验收，不能由编译或安装成功推导通过。
- 后端第一次 1776 构建未部署：日志监控在 Release 编译静默 900 秒后终止，退出 125 / `stalled_output`，并非通过；线上仍为 1775。重试需明确延长静默等待，不能使用 SkipBuild 绕过。
- 1807 原生多选正式回归：`project_identity_ready`、服务器 `acquire` 均已通过，随后项目恢复返回通用 `project_unavailable`，发送尚未授权。未标为群项目完成。
- 下一候选 adapter 437 直接复用已存在的项目目录分页路径（20 条），并区分接口拒绝、响应格式、超时、缺依赖及网络错误；不改变未知写禁止自动重放。Node 16 项通过，待正式装机验证，不把分页参数差异未经验证就认作根因。
