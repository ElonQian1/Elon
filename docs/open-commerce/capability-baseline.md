...
| 消费者关联数据删除请求与外部证明 | 请求已实现；证明代码已形成、未编译、范围受限 | 消费者可针对本人关系发起删除请求并原子撤销关系；商户只见匿名别名，可接单、拒绝或声明完成。完成后商户可按外部系统追加回执编号、SHA-256 与摘要，消费者可见；证明固定标为商户提供且平台未核验，不代表外部删除真实发生 | `server/src/open_commerce_data_request_service.rs`、`server/src/open_commerce_data_erasure_evidence_service.rs`、`pc-frontend/src/features/open-commerce/ConsumerDataRequestManager.tsx`、`docs/open-commerce-consumer-data-erasure-evidence-v1-acceptance.md` |
...
| 商户数据自主控制和跨应用授权 | 部分实现 | 跨项目脱敏目录、消费者沙盒、限时 App 授权、匿名关系、低敏偏好披露、客户端加密消费者保险箱、删除请求、商户未核验证明、V5 可验证导出、隔离导入、调用配额、终态结果流、商户业务证据、人工和受限机器 ERP/CRM 衔接回执、活动证据、手动 App 封禁和首个商户自有运行时已形成；保险箱授权解密、完整订单迁移、外部主动推送、真实删除执行与回执核验、生产 ERP/CRM 写入适配器、外部回读、全网签名信任、自动风控与公共互操作治理尚未完成 |
