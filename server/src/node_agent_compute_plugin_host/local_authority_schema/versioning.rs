...
use super::{
    ...
    create_schema_objects_v7_additions, schema_integrity, work_admission,
};

/// The singleton row shape remains V3. Database `user_version` advances independently because V4
/// through V8 add append-only journals and exact transition triggers without rebuilding that row.
...
pub(super) const COMPUTE_PLUGIN_LOCAL_AUTHORITY_DATABASE_VERSION_V8: i64 = 8;
...
pub(in crate::node_agent_compute_plugin_host) fn ensure_schema(
    ...
    match version {
        ...
        COMPUTE_PLUGIN_LOCAL_AUTHORITY_DATABASE_VERSION_V8
            if application_id == COMPUTE_PLUGIN_LOCAL_AUTHORITY_APPLICATION_ID =>
        {
            verify_required_objects_v8(connection)
        }
        COMPUTE_PLUGIN_LOCAL_AUTHORITY_SCHEMA_VERSION
        | COMPUTE_PLUGIN_LOCAL_AUTHORITY_DATABASE_VERSION_V4
        | COMPUTE_PLUGIN_LOCAL_AUTHORITY_DATABASE_VERSION
        | COMPUTE_PLUGIN_LOCAL_AUTHORITY_DATABASE_VERSION_V6
        | COMPUTE_PLUGIN_LOCAL_AUTHORITY_DATABASE_VERSION_V7
        | COMPUTE_PLUGIN_LOCAL_AUTHORITY_DATABASE_VERSION_V8 => bail!(
...
fn install_schema_v8(connection: &mut Connection) -> Result<()> {
    ...
    work_admission::create_schema_objects_v8(&transaction)?;
    transaction
        .pragma_update(
            None,
            "user_version",
...
fn migrate_schema_v3_to_v8(connection: &mut Connection) -> Result<()> {
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .context("COMPUTE_PLUGIN_AUTHORITY_SCHEMA_V8_MIGRATION_BEGIN")?;
    ...
    transaction
        ...
        .context("COMPUTE_PLUGIN_AUTHORITY_SCHEMA_V8_VERSION_WRITE")?;
    transaction
        .commit()
        .context("COMPUTE_PLUGIN_AUTHORITY_SCHEMA_V8_MIGRATION_COMMIT")?;
    verify_required_objects_v8(connection)
}

fn migrate_schema_v4_to_v8(connection: &mut Connection) -> Result<()> {
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .context("COMPUTE_PLUGIN_AUTHORITY_SCHEMA_V8_MIGRATION_BEGIN")?;
...
fn migrate_schema_v5_to_v8(connection: &mut Connection) -> Result<()> {
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .context("COMPUTE_PLUGIN_AUTHORITY_SCHEMA_V8_MIGRATION_BEGIN")?;
...
fn migrate_schema_v6_to_v8(connection: &mut Connection) -> Result<()> {
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .context("COMPUTE_PLUGIN_AUTHORITY_SCHEMA_V8_MIGRATION_BEGIN")?;
...
fn migrate_schema_v7_to_v8(connection: &mut Connection) -> Result<()> {
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .context("COMPUTE_PLUGIN_AUTHORITY_SCHEMA_V8_MIGRATION_BEGIN")?;
    verify_required_objects_v7(&transaction)?;
    work_admission::create_schema_objects_v8(&transaction)?;
    transaction
        .pragma_update(
            None,
...
fn verify_required_objects_v8(connection: &Connection) -> Result<()> {
    schema_integrity::verify_schema_objects_v8(connection)?;
    verify_foreign_keys(connection)
}
