//! Independent typed fixtures for the 88 q16 created-first truncate-error release-failed terminals.
...
pub(super) const LOCK_CREATED_FIRST_TRUNCATE_ERROR_RELEASE_FAILED_MEMBER_COUNT: usize = 88;

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub(super) enum FrozenCreatedFirstTruncateErrorReleaseFailedCompletionV1 {
    RetentionSucceeded,
    RetentionRouteUnknown,
}

impl FrozenCreatedFirstTruncateErrorReleaseFailedCompletionV1 {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub(super) struct FrozenLockCreatedFirstTruncateErrorReleaseFailedCaseV1 {
    pub(super) action: LockActionV1,
    pub(super) first: u8,
    pub(super) count: u8,
    pub(super) mask: u8,
    pub(super) completion: FrozenCreatedFirstTruncateErrorReleaseFailedCompletionV1,
}

#[derive(Clone)]
pub(super) struct FrozenLockCreatedFirstTruncateErrorReleaseFailedLeafV1 {
    pub(super) record: LeafRecordV1,
    pub(super) descriptor: TerminalDescriptorV1,
    pub(super) member: StaticMemberSealV1,
}

pub(super) fn frozen_lock_created_first_truncate_error_release_failed_leaves_v1(
) -> &'static BTreeMap<
    FrozenLockCreatedFirstTruncateErrorReleaseFailedCaseV1,
    FrozenLockCreatedFirstTruncateErrorReleaseFailedLeafV1,
> {
    ...
    LEAVES.get_or_init(|| {
        ...
        super::super::super::source_leaf_authority::validate_lock_graph_with_records(
            ...
            |leaf| {
                ...
                let Some(case) = created_first_truncate_error_release_failed_v1(record, descriptor)
                else {
                    return Ok(());
                };
                ...
                if previous.is_some() {
                    return Err(format!("duplicate frozen q16 member {case:?}"));
                }
                Ok(())
            },
        )
        .expect("the frozen Lock authority must validate before q16 source tests");
        assert_eq!(
            leaves.len(),
...
pub(super) fn lock_created_first_truncate_error_release_failed_descriptor_v1(
    case: FrozenLockCreatedFirstTruncateErrorReleaseFailedCaseV1,
    capability: RunnerCapabilityV1,
) -> TerminalDescriptorV1 {
    let mut descriptor =
        frozen_lock_created_first_truncate_error_release_failed_leaves_v1()[&case].descriptor;
    let TerminalDescriptorV1::Lock(value) = &mut descriptor else {
        unreachable!("a q16 fixture must have a Lock descriptor")
    };
    value.recipe.capability = capability;
    descriptor
}

pub(super) fn lock_created_first_truncate_error_release_failed_expected_groups_v1(
        ...
        .map(|leaf| {
            (
                prepare_dynamic_terminal_v1(&leaf.record, &leaf.descriptor)
                    .expect("a frozen q16 descriptor must prepare")
                    .key,
                leaf.member,
            )
        })
...
fn created_first_truncate_error_release_failed_v1(
    record: &LeafRecordV1,
    descriptor: &TerminalDescriptorV1,
) -> Option<FrozenLockCreatedFirstTruncateErrorReleaseFailedCaseV1> {
    ...
    let completion = match completion {
        LockCompletionV1::UnsafeRetentionSucceededThenRouteUnknown => {
            FrozenCreatedFirstTruncateErrorReleaseFailedCompletionV1::RetentionSucceeded
        }
        LockCompletionV1::UnsafeRetentionRouteUnknownThenRouteUnknown => {
            FrozenCreatedFirstTruncateErrorReleaseFailedCompletionV1::RetentionRouteUnknown
        }
        _ => return None,
    };
    let case = FrozenLockCreatedFirstTruncateErrorReleaseFailedCaseV1 {
        action,
        first,
        count,
        mask,
...
#[test]
fn frozen_q16_family_is_exact_unique_and_only_contains_acquire_requests() {
