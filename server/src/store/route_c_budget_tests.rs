...
fn route_c_budget_blocks_per_user_daily_limit_without_blocking_other_users() {
    ...
    recorded_event_id(
        store
            .route_c_budget_try_record_call(&user_a.id, "fp-a1", "2026-06-23", Some(10), Some(1))
            .unwrap(),
        1,
        1,
    );
    assert_eq!(
        store
            .route_c_budget_try_record_call(&user_a.id, "fp-a2", "2026-06-23", Some(10), Some(1))
            .unwrap(),
    ...
    recorded_event_id(
        store
            .route_c_budget_try_record_call(&user_b.id, "fp-b1", "2026-06-23", Some(10), Some(1))
            .unwrap(),
        2,
        1,
    );
    ...
    store
        .route_c_budget_mark_completed(
            ...
            RouteCBudgetCompletion {
                ...
                error_summary: Some("rate_limit fingerprint=abc secret prompt text".to_string()),
            },
        )
        .unwrap();
