...
交互式云电脑扩展遵循“一套联邦控制面、两个执行平面”：Provider/Offer/Capacity/Price/分层计量原则可复用，`interactive_desktop` 另建 Session、HostLease、ViewerGrant、MediaEpoch 与 ControlEpoch，WebRTC 媒体和输入不进入现有批任务 Attempt 或控制 WSS。产品模式分为同账号远程访问、邀请式好友共玩和有明确授权的付费云座位；只有 `licensed_cloud_seat` 可进入陌生用户市场。当前仅有未编译、未运行的 C0 文档与内部源合同，未新增 migration、route、媒体、输入、计费或生产效果，精确边界见 `docs/distributed-compute/interactive-desktop-architecture.md`。
