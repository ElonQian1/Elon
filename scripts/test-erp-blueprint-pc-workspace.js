...
const commerceReadiness = read(`${feature}/ErpOpenCommerceReadinessPanel.tsx`)
...
assert.ok(instance.includes('ErpMaterializationPanel'), 'merchant workspace must show derived materialization status')
assert.ok(instance.includes('ErpOpenCommerceReadinessPanel'), 'merchant workspace must show open commerce readiness')
assert.ok(materialization.includes('plan_contract_matches'), 'materialization UI must expose contract drift')
assert.ok(materialization.includes('failed_assignment_ids'), 'materialization UI contract must keep retry evidence')
assert.ok(commerceReadiness.includes('consumer_invocation_ready'), 'readiness UI must distinguish consumer invocation')
assert.ok(commerceReadiness.includes('consumer_discovery_ready'), 'readiness UI must distinguish directory discovery')
assert.ok(commerceReadiness.includes('merchant_selection.candidates'), 'readiness UI must require merchant selection when ambiguous')
...
assert.ok(api.includes('/open-commerce-readiness'), 'PC API must expose read-only open commerce readiness')
...
assert.ok(mcp.includes('erp_get_materialization_status'), 'AI agents may inspect materialization without starting it')
assert.ok(mcp.includes('erp_get_open_commerce_readiness'), 'AI agents may inspect open commerce readiness without mutating it')
