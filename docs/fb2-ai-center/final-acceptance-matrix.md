...
| readiness 运行时使用 | 主项目拉 Context Pack 前读取 fb2 `/context/readiness`，把结果写入 `preflight_readiness`，并把非 ready 状态提升为 `context_quality.warnings`；`blocked` 时工具执行记录 skipped | 本轮已补主项目运行时代码和单测；还需 commit/push、发布服务端，并带真实 token 做 live 复核 |
