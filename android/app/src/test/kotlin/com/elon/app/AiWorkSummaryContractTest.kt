...
class AiWorkSummaryContractTest {
    ...
    fun androidSummaryKeepsRequiredInformationAndActions() {
        ...
        assertTrue("日期选择组必须按整屏居中，日历独立贴右", source.contains("FrameLayout.LayoutParams(dp(96), MATCH, Gravity.CENTER)"))
        assertTrue("问候文字必须使用固定设计对齐线，禁止负边距", source.contains("marginStart = dp(94)") && !source.contains("marginStart = dp(-7)"))
        assertTrue("进展行项目图标必须使用设计稿 29dp 起点", source.contains("setPadding(dp(29), 0, dp(4), 0)"))
        assertTrue("折叠标题与查看全部必须使用设计稿边界", source.contains("setPadding(dp(13), 0, dp(9), 0)") && source.contains("setPadding(0, 0, dp(28), 0)"))
