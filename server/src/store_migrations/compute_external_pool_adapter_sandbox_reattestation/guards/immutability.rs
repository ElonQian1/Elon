...
pub(super) fn install(conn: &Connection) -> Result<()> {
    for (prefix, table, identity) in [
        (
            "external_pool_adapter_sandbox_reattestation_challenge",
        ...
        (
            "external_pool_adapter_sandbox_reattestation_receipt",
            "compute_external_pool_adapter_sandbox_reattestation_receipts",
            "old.reattestation_receipt_id=NEW.reattestation_receipt_id OR old.reattestation_receipt_digest=NEW.reattestation_receipt_digest OR old.challenge_id=NEW.challenge_id OR old.verifier_report_id=NEW.verifier_report_id OR (old.registry_release_id=NEW.registry_release_id AND old.sequence=NEW.sequence) OR old.predecessor_receipt_id=NEW.predecessor_receipt_id OR (old.idempotency_scope=NEW.idempotency_scope AND old.idempotency_key=NEW.idempotency_key)",
        ),
        (
            "external_pool_adapter_sandbox_reattestation_revocation",
        ...
        conn.execute_batch(&format!(
            "CREATE TRIGGER IF NOT EXISTS {prefix}_no_update BEFORE UPDATE ON {table}
             BEGIN SELECT RAISE(ABORT,'V252 authority is immutable'); END;
             CREATE TRIGGER IF NOT EXISTS {prefix}_no_delete BEFORE DELETE ON {table}
             BEGIN SELECT RAISE(ABORT,'V252 authority is append-only'); END;
             CREATE TRIGGER IF NOT EXISTS {prefix}_no_replace BEFORE INSERT ON {table}
             WHEN EXISTS(SELECT 1 FROM {table} old WHERE {identity})
             BEGIN SELECT RAISE(ABORT,'V252 authority cannot replace immutable history'); END;"
        ))?;
    }
    Ok(())
}
