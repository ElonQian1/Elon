//! Independent typed fixtures for the 88 q17 existing-first truncate-error release-failed terminals.
...
pub(super) const LOCK_EXISTING_FIRST_TRUNCATE_ERROR_RELEASE_FAILED_MEMBER_COUNT: usize = 88;

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub(super) enum FrozenExistingFirstTruncateErrorReleaseFailedCompletionV1 {
    RetentionSucceeded,
    RetentionRouteUnknown,
}

impl FrozenExistingFirstTruncateErrorReleaseFailedCompletionV1 {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub(super) struct FrozenLockExistingFirstTruncateErrorReleaseFailedCaseV1 {
    pub(super) action: LockActionV1,
    pub(super) first: u8,
    pub(super) count: u8,
    pub(super) mask: u8,
    pub(super) completion: FrozenExistingFirstTruncateErrorReleaseFailedCompletionV1,
}

#[derive(Clone)]
pub(super) struct FrozenLockExistingFirstTruncateErrorReleaseFailedLeafV1 {
    pub(super) record: LeafRecordV1,
    pub(super) descriptor: TerminalDescriptorV1,
    pub(super) member: StaticMemberSealV1,
}

pub(super) fn frozen_lock_existing_first_truncate_error_release_failed_leaves_v1(
) -> &'static BTreeMap<
    FrozenLockExistingFirstTruncateErrorReleaseFailedCaseV1,
    FrozenLockExistingFirstTruncateErrorReleaseFailedLeafV1,
> {
    ...
    LEAVES.get_or_init(|| {
        ...
        super::super::super::source_leaf_authority::validate_lock_graph_with_records(
            ...
            |leaf| {
                ...
                if previous.is_some() {
                    return Err(format!("duplicate frozen q17 member {case:?}"));
                }
                Ok(())
            },
        )
        .expect("the frozen Lock authority must validate before q17 source tests");
        assert_eq!(
            leaves.len(),
...
pub(super) fn lock_existing_first_truncate_error_release_failed_descriptor_v1(
    case: FrozenLockExistingFirstTruncateErrorReleaseFailedCaseV1,
    capability: RunnerCapabilityV1,
) -> TerminalDescriptorV1 {
    let mut descriptor =
        frozen_lock_existing_first_truncate_error_release_failed_leaves_v1()[&case].descriptor;
    let TerminalDescriptorV1::Lock(value) = &mut descriptor else {
        unreachable!("a q17 fixture must have a Lock descriptor")
    };
    value.recipe.capability = capability;
    descriptor
}

pub(super) fn lock_existing_first_truncate_error_release_failed_expected_groups_v1(
        ...
        .map(|leaf| {
            (
                prepare_dynamic_terminal_v1(&leaf.record, &leaf.descriptor)
                    .expect("a frozen q17 descriptor must prepare")
                    .key,
                leaf.member,
            )
        })
...
fn existing_first_truncate_error_release_failed_v1(
    record: &LeafRecordV1,
    descriptor: &TerminalDescriptorV1,
) -> Option<FrozenLockExistingFirstTruncateErrorReleaseFailedCaseV1> {
    ...
    let completion = match completion {
        LockCompletionV1::UnsafeRetentionSucceededThenRouteUnknown => {
            FrozenExistingFirstTruncateErrorReleaseFailedCompletionV1::RetentionSucceeded
        }
        LockCompletionV1::UnsafeRetentionRouteUnknownThenRouteUnknown => {
            FrozenExistingFirstTruncateErrorReleaseFailedCompletionV1::RetentionRouteUnknown
        }
        _ => return None,
    };
    let case = FrozenLockExistingFirstTruncateErrorReleaseFailedCaseV1 {
        action,
        first,
        count,
        mask,
...
fn expected_v1() -> ExpectedV1 {
    ExpectedV1 {
        ...
        disposition: TerminalDispositionV1::CleanupRewritten,
        phase: "DmsExclusiveRelease".to_owned(),
        ...
        dms_lock: DmsLockCustodyV1::ExclusiveOutcomeUncertain,
...
#[test]
fn frozen_q17_family_is_exact_unique_and_only_contains_acquire_requests() {
    let leaves = frozen_lock_existing_first_truncate_error_release_failed_leaves_v1();
    assert_eq!(
        leaves.len(),
        LOCK_EXISTING_FIRST_TRUNCATE_ERROR_RELEASE_FAILED_MEMBER_COUNT
    );
