...
fn classifies_codex_websocket_failure_as_retryable_capacity() {
    let classified = classify_ai_error("Codex CLI network unhealthy: Responses WebSocket failed");
...
#[test]
fn classifies_deprecated_model_as_operator_config_error() {
    let classified = classify_ai_error(
        "当前 AI 模型已下线，请管理员迁移到 TokenHub 的可用模型或切换其他可用模型通道",
    );

    assert_eq!(classified.code, "ai_model_config_unavailable");
    assert_eq!(classified.category, AiErrorCategory::AuthConfig);
    assert!(classified.message.contains("模型已下线"));
    assert!(classified.message.contains("TokenHub"));
    assert!(!classified.retryable);
}
...
#[test]
fn classifies_tokenhub_postpaid_disabled_as_quota() {
    let classified = classify_ai_error(
        "当前 AI 模型额度已用尽或未开启后付费，请切换可用模型，或联系管理员补充额度后重试",
    );
