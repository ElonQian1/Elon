//! Process-isolated q14 receipts for truncate uncertainty followed by a known release.
//!
//! The child invokes installed production `xShmLock`, the production truncate call point, and
//! production `UnlockFileEx`. Only the truncate return receipt is deliberately unread, so emitted
//! evidence remains `controlled_fault_actual`.
...
const SELECTOR_ENV: &str =
    "ELON_SQLITE_A2_LOCK_CREATED_FIRST_TRUNCATE_ERROR_RELEASE_SUCCEEDED_SELECTOR";

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in super::super::super) enum LockRunnerCreatedFirstTruncateErrorReleaseSucceededCompletionV1 {
    RetentionSucceeded,
    RetentionRouteUnknown,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in super::super::super) struct LockRunnerNativeAcquireCreatedFirstTruncateErrorReleaseSucceededBindingV1
{
...
pub(in super::super::super) fn run_lock_native_acquire_created_first_truncate_error_release_succeeded_program_isolated(
    exact_test: &str,
    binding: LockRunnerNativeAcquireCreatedFirstTruncateErrorReleaseSucceededBindingV1,
    ...
    if let Some(root) = super::selected_child_root()? {
        let selected = std::env::var(SELECTOR_ENV)
            .context("read parent-selected q14 Lock initialization program")?;
...
fn run_parent(
    ...
    if exact_test.is_empty() {
        return Err(anyhow!("q14 Lock initialization exact test name is empty"));
    }
    let executable =
        std::env::current_exe().context("resolve q14 Lock initialization test executable")?;
...
fn validate_parent_receipt(
    ...
    if !child.matches_family(SanitizedPayloadFamily::LockQuotient) {
        return Err(anyhow!(
            "q14 Lock initialization child payload family mismatch"
        ));
    }
    ...
    if !child.matches_registration_id(payload.registration_id) {
        return Err(anyhow!(
            "q14 Lock initialization child registration binding mismatch"
        ));
    }
    let environment =
    ...
    {
        return Err(anyhow!(
            "q14 Lock initialization parent cleanup binding mismatch"
        ));
    }
...
pub(super) fn validate_binding(
    ...
    let end = binding
        ...
        .ok_or_else(|| anyhow!("q14 Lock initialization range overflow"))?;
    if binding.count == 0 || binding.first >= 8 || end > 8 {
        return Err(anyhow!(
            "q14 Lock initialization range is outside eight slots"
        ));
    }
    if binding.action == LockRunnerActionV1::LockShared && binding.count != 1 {
        return Err(anyhow!(
            "q14 shared Lock initialization range is not one slot"
        ));
    }
    if !matches!(
        binding.action,
        ...
        return Err(anyhow!("q14 Lock initialization action is not an acquire"));
    }
    let mask = (((1_u16 << binding.count) - 1) << binding.first) as u8;
    if binding.mask != mask {
        return Err(anyhow!("q14 Lock initialization range mask mismatch"));
    }
    super::super::child::lock_created_first_truncate_error_release_succeeded::selector(
...
pub(super) fn exact_selector(
    ...
    .expect("validated q14 Lock initialization selector")
}

pub(super) const fn completion_tag(
    completion: LockRunnerCreatedFirstTruncateErrorReleaseSucceededCompletionV1,
) -> u64 {
    match completion {
        LockRunnerCreatedFirstTruncateErrorReleaseSucceededCompletionV1::RetentionSucceeded => 1,
        LockRunnerCreatedFirstTruncateErrorReleaseSucceededCompletionV1::RetentionRouteUnknown => 2,
    }
}

#[cfg(all(test, windows))]
pub(in super::super::super) fn selected_lock_native_acquire_created_first_truncate_error_release_succeeded_selector_for_test(
) -> Option<String> {
...
#[cfg(all(test, windows))]
pub(in super::super::super) fn lock_native_acquire_created_first_truncate_error_release_succeeded_selector_for_test(
    ...
    let binding = LockRunnerNativeAcquireCreatedFirstTruncateErrorReleaseSucceededBindingV1 {
        action,
        first,
        count,
        mask,
    ...
    validate_binding(binding)
        ...
        .map_err(|_| "q14 Lock initialization selector binding invalid")
}
