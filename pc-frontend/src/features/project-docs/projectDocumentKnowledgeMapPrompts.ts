...
export function discussionSourceInstruction(path: string) {
  return `把已保存的原始聊天“${path}”编译为可继续工作的讨论知识图。` +
    ...
    '调用 project_discussions_save_proposal 保存增量图；只对已确认且值得长期维护的结论提出 promotions，避免一节点一文档和重复权威文档。' +
    '按当前权限允许时调用 project_discussions_apply，并用 project_discussions_get_graph 检查结果。'
}
...
export function discussionNodeInstruction(
  ...
  return `围绕讨论节点“${node.title}”（node_id=${node.id}）继续工作：${action}。` +
    ...
    '通过 project_discussions_save_proposal 保存增量变更；按当前权限允许时调用 project_discussions_apply，并回读节点确认。'
}

export function discussionApplyInstruction() {
  return '用户已在讨论推理页面明确批准待处理的讨论图建议。调用 project_discussions_get_graph 和 project_discussions_get_suggestions 核对 revision 与摘要，再调用 project_discussions_apply；review_all 模式传 reviewed=true。应用后回读图谱并报告新增节点、分支和晋升文档。'
}
