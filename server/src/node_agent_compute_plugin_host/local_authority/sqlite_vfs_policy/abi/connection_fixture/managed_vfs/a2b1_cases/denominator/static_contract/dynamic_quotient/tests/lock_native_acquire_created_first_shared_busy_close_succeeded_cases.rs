//! Independent typed fixtures for the 88 q18 created-first shared-busy close-ok terminals.
...
pub(super) const LOCK_CREATED_FIRST_SHARED_BUSY_CLOSE_SUCCEEDED_MEMBER_COUNT: usize = 88;

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub(super) enum FrozenCreatedFirstSharedBusyCloseSucceededCompletionV1 {
    RetentionSucceeded,
    RetentionRouteUnknown,
}

impl FrozenCreatedFirstSharedBusyCloseSucceededCompletionV1 {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub(super) struct FrozenLockCreatedFirstSharedBusyCloseSucceededCaseV1 {
    pub(super) action: LockActionV1,
    pub(super) first: u8,
    pub(super) count: u8,
    pub(super) mask: u8,
    pub(super) completion: FrozenCreatedFirstSharedBusyCloseSucceededCompletionV1,
}

#[derive(Clone)]
pub(super) struct FrozenLockCreatedFirstSharedBusyCloseSucceededLeafV1 {
    ...
    LEAVES.get_or_init(|| {
        ...
        super::super::super::source_leaf_authority::validate_lock_graph_with_records(
            ...
            |leaf| {
                ...
                let Some(case) = created_first_shared_busy_close_succeeded_v1(record, descriptor)
                else {
                    return Ok(());
                };
                ...
                if previous.is_some() {
                    return Err(format!("duplicate frozen q18 member {case:?}"));
                }
                Ok(())
            },
        )
        .expect("the frozen Lock authority must validate before q18 source tests");
        assert_eq!(
            leaves.len(),
...
pub(super) fn lock_created_first_shared_busy_close_succeeded_descriptor_v1(
    case: FrozenLockCreatedFirstSharedBusyCloseSucceededCaseV1,
    capability: RunnerCapabilityV1,
) -> TerminalDescriptorV1 {
    let mut descriptor =
        frozen_lock_created_first_shared_busy_close_succeeded_leaves_v1()[&case].descriptor;
    let TerminalDescriptorV1::Lock(value) = &mut descriptor else {
        unreachable!("a q18 fixture must have a Lock descriptor")
    };
    value.recipe.capability = capability;
    descriptor
}

pub(super) fn lock_created_first_shared_busy_close_succeeded_expected_groups_v1(
        ...
        .map(|leaf| {
            (
                prepare_dynamic_terminal_v1(&leaf.record, &leaf.descriptor)
                    .expect("a frozen q18 descriptor must prepare")
                    .key,
                leaf.member,
            )
        })
...
fn created_first_shared_busy_close_succeeded_v1(
    record: &LeafRecordV1,
    descriptor: &TerminalDescriptorV1,
) -> Option<FrozenLockCreatedFirstSharedBusyCloseSucceededCaseV1> {
    ...
    let completion = match completion {
        LockCompletionV1::UnsafeRetentionSucceededThenRouteUnknown => {
            FrozenCreatedFirstSharedBusyCloseSucceededCompletionV1::RetentionSucceeded
        }
        LockCompletionV1::UnsafeRetentionRouteUnknownThenRouteUnknown => {
            FrozenCreatedFirstSharedBusyCloseSucceededCompletionV1::RetentionRouteUnknown
        }
        _ => return None,
    };
    let case = FrozenLockCreatedFirstSharedBusyCloseSucceededCaseV1 {
        action,
        first,
        count,
        mask,
    ...
    if range_mask_v1(action, first, count) != Some(mask)
        ...
        || value.phase != PhaseV1::DmsSharedAcquire
...
fn expected_v1() -> ExpectedV1 {
    ExpectedV1 {
        ...
        phase: "DmsSharedAcquire".to_owned(),
...
#[test]
fn frozen_q18_family_is_exact_unique_and_only_contains_acquire_requests() {
    let leaves = frozen_lock_created_first_shared_busy_close_succeeded_leaves_v1();
    assert_eq!(
        leaves.len(),
        LOCK_CREATED_FIRST_SHARED_BUSY_CLOSE_SUCCEEDED_MEMBER_COUNT
    );
    ...
    let retention_succeeded = leaves
        .keys()
        .filter(|case| {
            case.completion
                == FrozenCreatedFirstSharedBusyCloseSucceededCompletionV1::RetentionSucceeded
        ...
        .count();
    assert_eq!(retention_succeeded, 44);
    assert_eq!(leaves.len() - retention_succeeded, 44);
