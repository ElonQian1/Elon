use super::{protocol_start_requested, target_from_args, target_path, MaintenanceProtocolTarget};
use std::path::Path;
...
fn target_paths_stay_under_known_roots() {
    ...
    assert!(
        target_path(MaintenanceProtocolTarget::LauncherLogs, install).ends_with(r"_internal\logs")
    );
    assert_eq!(
