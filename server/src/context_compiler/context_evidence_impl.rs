...
pub(super) fn snippet_for_symbol(
    workspace: &Path,
    symbol: &RustSymbol,
    ranked: &RankedSymbol,
    task: &TaskProfile,
...
pub(super) fn snippet_for_relevant_file(
    workspace: &Path,
    file: &RelevantFile,
    task: &TaskProfile,
) -> Option<EvidenceSnippet> {
...
pub(super) fn collect_neighbor_summaries(
    index: &RepoContextIndex,
...
pub(super) fn collect_test_targets(
    index: &RepoContextIndex,
    snippet_paths: &HashSet<String>,
) -> Vec<TestTarget> {
...
pub(super) fn collect_build_commands(
    index: &RepoContextIndex,
    snippet_paths: &HashSet<String>,
) -> Vec<BuildCommand> {
    let manifest = index
...
pub(super) fn collect_invariants(index: &RepoContextIndex) -> Vec<ContextFact> {
    let mut facts = vec![
        ContextFact {
...
pub(super) fn collect_public_api_contracts(index: &RepoContextIndex) -> Vec<ContextFact> {
...
pub(super) fn collect_unsafe_boundaries(index: &RepoContextIndex) -> Vec<ContextFact> {
    index
        .rust
        .symbols
        .iter()
...
pub(super) fn collect_feature_flags(index: &RepoContextIndex) -> Vec<FeatureFlagFact> {
...
pub(super) fn collect_missing_context(
    index: &RepoContextIndex,
...
pub(super) fn recommended_actions(task: &TaskProfile) -> Vec<String> {
    let mut actions = vec![
...
pub(super) fn evidence_role(symbol: &RustSymbol, task: &TaskProfile) -> &'static str {
    if task
        .suspected_files
        .iter()
...
pub(super) fn needed_if(kind: RelationshipKind) -> &'static str {
    match kind {
...
pub(super) fn extract_lines(content: &str, line_start: usize, line_end: usize) -> String {
    content
        .lines()
        .enumerate()
        .filter_map(|(idx, line)| {
...
pub(super) fn snippet_key(snippet: &EvidenceSnippet) -> String {
    format!(
        "{}:{}:{}",
...
pub(super) fn role_priority(role: &str) -> usize {
    match role {
        "edit-target" => 5,
        "interface" => 4,
        "caller" => 3,
