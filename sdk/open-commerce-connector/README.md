...
`createMerchantRuntime` 抽取了咖啡店参考节点中所有商户共同需要的协议层：HMAC 签名、5 分钟重放窗口、商户和凭据来源、Grant 检查、平台动作确认、幂等占位与重放、能力分发、不可变 Manifest 摘要和标准结果/错误信封。平台把已经服务端确认并一次性消费的 `action_confirmation_id` 写入签名信封；能力定义使用 `action: true` 声明经营动作，所有动作都会在处理器前检查确认。历史 `order.commit` 即使未声明也继续按动作保护，不能用业务输入中的确认布尔值降级。商户只实现自己的商品、库存、报价和订单处理器。

## 消费者可携带数据包签名

`signConsumerPortabilityPackage` 使用运营方 2048 至 8192 位 RSA 私钥签署固定协议消息，绑定来源运营方、公钥摘要、导出包标识、来源项目、幂等键、负载 SHA-256 和创建时间。接收方可用 `consumerPortabilityPublicKeyId` 登记公钥，并用 `verifyConsumerPortabilityPackageSignature` 在上传前复核；验证会拒绝字段替换、错误密钥、算法和非规范 Base64。私钥只由来源运营方持有，不上传平台；签名证明某个已信任密钥签过该包，不会自动恢复关系、Grant、ERP、订单或资金状态。

`encryptConsumerPortabilityArchive` 默认写入 V2：PBKDF2-SHA256（310000 次）与 AES-256-GCM 离线归档的算法参数、随机盐、随机 Nonce、明文 SHA-256 和创建时间全部进入固定顺序 AAD，PC 与 SDK 已通过双向互操作。`decryptConsumerPortabilityArchive` 继续只读兼容 V1。口令只存在于调用方进程；当前不负责口令找回、同步或云端密钥托管，明文上限为 6 MiB。
