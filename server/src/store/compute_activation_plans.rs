...
use crate::{
    compute_federation::provider::{ComputeProvider, PROVIDER_STATUS_ACTIVE},
...
use super::{
    compute_activation_plan_dependencies::validate_prepare_dependencies_on,
    compute_provider_registry::validate_compute_provider_contract, new_id, now, Store,
};
...
impl Store {
    pub(crate) fn prepare_compute_activation_plan(
        ...
        validate_prepare_dependencies_on(&tx, &input)?;
...
pub(super) fn plan_by_request_on(
    conn: &Connection,
    request_id: &str,
) -> Result<Option<ComputeActivationPlan>> {
    let plan_id = conn
