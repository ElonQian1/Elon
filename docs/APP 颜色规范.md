...
- 图标只使用用户确认的明暗两套 PNG 素材，不显示文字标签；触控区域仍不得小于 `48dp x 48dp`，并保留 `contentDescription`。
- 当前页使用浅灰选中胶囊和深色图标；其余入口使用浅色图标。主菜单打开时临时使用同一选中态，关闭后恢复。
- 选中胶囊尺寸为 `58dp x 46dp`；位于最左入口时，距离长胶囊左、上、下边均为 `5dp`；位于最右入口时，距离长胶囊右、上、下边均为 `5dp`。端点三边内缩必须一致且不得随屏幕宽度变化。
