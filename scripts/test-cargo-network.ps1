...
if($scenario -eq 'test_failure'){Write-Error 'proxy configuration was discovered' -ErrorAction Continue;Write-Error 'test result: FAILED. 1 passed; 1 failed' -ErrorAction Continue;Write-Error 'error: test failed, to rerun pass `--bin elon-pc-node`' -ErrorAction Continue;exit 101}
...
'@|Set-Content -LiteralPath $fake -Encoding UTF8
    ...
    Assert-Equal 'CARGO_TEST_FAILURE' $result.diagnostic.code 'test failure must not be misclassified as proxy failure';Assert-True ($capturedText -match 'CARGO_ATTEMPT_LOG .*test result: FAILED') 'test failure tail must reach outer validation logs'
