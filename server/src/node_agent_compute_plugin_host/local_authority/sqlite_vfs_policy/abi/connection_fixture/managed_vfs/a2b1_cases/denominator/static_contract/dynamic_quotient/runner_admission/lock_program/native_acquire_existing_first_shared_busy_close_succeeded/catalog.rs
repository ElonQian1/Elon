//! Exact frozen-member authority for the 88-member q19 existing-first shared-busy tranche.
...
use super::{
    range_mask_v1, LockExistingFirstSharedBusyCloseSucceededCompletionV1,
    EXISTING_FIRST_SHARED_BUSY_CLOSE_SUCCEEDED_MEMBER_COUNT,
};
...
pub(super) fn exact_member_v1(
    ...
    for line in lines {
        ...
        if fields.next().is_some()
            ...
            || fault_site_text != "dms-shared-acquire"
    ...
    if observed_rows != EXISTING_FIRST_SHARED_BUSY_CLOSE_SUCCEEDED_MEMBER_COUNT {
        return Err(LockRunnerExecutionViolationV1::MemberCatalogInvalid);
    }
...
fn expected_row_v1(row: usize) -> Option<ExpectedRowV1> {
    if row == 0 || row > EXISTING_FIRST_SHARED_BUSY_CLOSE_SUCCEEDED_MEMBER_COUNT {
        return None;
    }
    let ordinal = row - 1;
    let request_ordinal = ordinal / 2;
    let completion = if ordinal % 2 == 0 {
        LockExistingFirstSharedBusyCloseSucceededCompletionV1::RetentionSucceeded
    } else {
        LockExistingFirstSharedBusyCloseSucceededCompletionV1::RetentionRouteUnknown
    };
...
const fn completion_tag_v1(
    ...
    match completion {
        LockExistingFirstSharedBusyCloseSucceededCompletionV1::RetentionSucceeded => "succeeded",
        LockExistingFirstSharedBusyCloseSucceededCompletionV1::RetentionRouteUnknown => {
