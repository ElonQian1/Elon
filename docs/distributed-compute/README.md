...
| 节点插件治理合同 | Signed Manifest/InstallPlan、双 keyring、authority v3/PlanApply、可恢复受管取数、候选 exact closure/hash 与原子 resolution、schema 定义指纹、可信时间和回滚锚查询/首次注册/更新合同均已写。已验证 ZIP 还可生成有界规范计划，在随机 create-new run 中逐文件安全物化并封存 seal；retained-handle 重验与 fresh authority binding 后，Store 在一个 `BEGIN IMMEDIATE` 中保存不可变 staging receipt、推进三类 fence，并把槽从 `verifying` 原子推进到 `staged`。结果不确定时可按 recovery key 稳定读取 `NotCreated` 或 exact `Staged`，碰撞或权威漂移失败关闭；exact `Staged` adoption 还会二次读取同一结果并从原句柄重哈希文件/seal 后恢复 typed custody，`NotCreated` 不授予盲重试。默认关闭的 Bootstrap 已挂入 NodeRuntime，但根级独占锁、生产时间源/回滚见证、root pin、authority open、process fence 和 sharing-on 入口仍未建立。全部为 `implementation_uncompiled`；真实 HTTPS downloader、目录耐久/跨重启恢复、健康 promotion、Sidecar 和正式 Attempt 接线仍缺，`staged` 不等于 installed、ready 或商业 Verification |
