...
use super::{
    ...
    native_acquire_created_first_shared_busy_close_succeeded::{
        program_spec_v1 as native_acquire_created_first_shared_busy_close_succeeded_program_spec_v1,
        LockNativeAcquireCreatedFirstSharedBusyCloseSucceededProgramSpecV1,
    },
    native_acquire_created_first_truncate_error_release_failed::{
        program_spec_v1 as native_acquire_created_first_truncate_error_release_failed_program_spec_v1,
        LockNativeAcquireCreatedFirstTruncateErrorReleaseFailedProgramSpecV1,
    },
    native_acquire_created_first_truncate_error_release_succeeded::{
        program_spec_v1 as native_acquire_created_first_truncate_error_release_succeeded_program_spec_v1,
        LockNativeAcquireCreatedFirstTruncateErrorReleaseSucceededProgramSpecV1,
    },
    native_acquire_existing_first_exclusive_release_error::{
    ...
    native_acquire_existing_first_shared_busy_close_succeeded::{
        program_spec_v1 as native_acquire_existing_first_shared_busy_close_succeeded_program_spec_v1,
        LockNativeAcquireExistingFirstSharedBusyCloseSucceededProgramSpecV1,
    },
                                                                    ...
                                                                    match pre_managed_callback_rejection_program_spec_v1(key, plan) {
                                                                        ...
                                                                        Err(LockRunnerExecutionViolationV1::UnsupportedProgram) => {
                                                                            match raw_state_rejection_program_spec_v1(key, plan) {
                                                                                ...
                                                                                Err(LockRunnerExecutionViolationV1::UnsupportedProgram) => {
                                                                                    match native_acquire_created_first_exclusive_release_error_program_spec_v1(key, plan) {
                                                                                        Ok(program) => Ok(from_native_acquire_created_first_exclusive_release_error_v1(program)),
                                                                                        Err(LockRunnerExecutionViolationV1::UnsupportedProgram) => {
                                                                                            match native_acquire_existing_first_exclusive_release_error_program_spec_v1(key, plan) {
                                                                                                Ok(program) => Ok(from_native_acquire_existing_first_exclusive_release_error_v1(program)),
                                                                                                Err(LockRunnerExecutionViolationV1::UnsupportedProgram) => {
                                                                                                    match native_acquire_created_first_truncate_error_release_succeeded_program_spec_v1(key, plan) {
                                                                                                        Ok(program) => Ok(from_native_acquire_created_first_truncate_error_release_succeeded_v1(program)),
                                                                                                        Err(LockRunnerExecutionViolationV1::UnsupportedProgram) => {
                                                                                                            match native_acquire_existing_first_truncate_error_release_succeeded_program_spec_v1(key, plan) {
                                                                                                                ...
                                                                                                                Err(LockRunnerExecutionViolationV1::UnsupportedProgram) => {
                                                                                                                    match native_acquire_created_first_truncate_error_release_failed_program_spec_v1(key, plan) {
                                                                                                                        ...
                                                                                                                        Err(LockRunnerExecutionViolationV1::UnsupportedProgram) => {
                                                                                                                            match native_acquire_existing_first_truncate_error_release_failed_program_spec_v1(key, plan) {
                                                                                                                                Ok(program) => Ok(from_native_acquire_existing_first_truncate_error_release_failed_v1(program)),
                                                                                                                                Err(LockRunnerExecutionViolationV1::UnsupportedProgram) => {
                                                                                                                                    match native_acquire_created_first_shared_busy_close_succeeded_program_spec_v1(key, plan) {
                                                                                                                                        Ok(program) => Ok(from_native_acquire_created_first_shared_busy_close_succeeded_v1(program)),
                                                                                                                                        Err(LockRunnerExecutionViolationV1::UnsupportedProgram) => {
                                                                                                                                            native_acquire_existing_first_shared_busy_close_succeeded_program_spec_v1(key, plan)
                                                                                                                                                .map(from_native_acquire_existing_first_shared_busy_close_succeeded_v1)
                                                                                                                                        }
                                                                                                                                        Err(error) => Err(error),
                                                                                                                                    }
                                                                                                                                }
...
fn from_native_acquire_existing_first_exclusive_release_error_v1(
    program: LockNativeAcquireExistingFirstExclusiveReleaseErrorProgramSpecV1,
) -> SourceLockProgramSpecV1 {
    SourceLockProgramSpecV1 {
        #[cfg(windows)]
        case: LockProgramCaseV1::NativeAcquireExistingFirstExclusiveReleaseError(program),
        normalized_descriptor_sha256: program.normalized_descriptor_sha256,
        expected_member: Some(program.member),
        plan_sha256: program.plan_sha256,
        implementation_sha256: program.implementation_sha256,
...
fn from_native_acquire_created_first_truncate_error_release_succeeded_v1(
    program: LockNativeAcquireCreatedFirstTruncateErrorReleaseSucceededProgramSpecV1,
) -> SourceLockProgramSpecV1 {
    SourceLockProgramSpecV1 {
        #[cfg(windows)]
        case: LockProgramCaseV1::NativeAcquireCreatedFirstTruncateErrorReleaseSucceeded(program),
        normalized_descriptor_sha256: program.normalized_descriptor_sha256,
        expected_member: Some(program.member),
        plan_sha256: program.plan_sha256,
        implementation_sha256: program.implementation_sha256,
...
fn from_native_acquire_existing_first_truncate_error_release_succeeded_v1(
    program: LockNativeAcquireExistingFirstTruncateErrorReleaseSucceededProgramSpecV1,
) -> SourceLockProgramSpecV1 {
    SourceLockProgramSpecV1 {
        #[cfg(windows)]
        case: LockProgramCaseV1::NativeAcquireExistingFirstTruncateErrorReleaseSucceeded(program),
        normalized_descriptor_sha256: program.normalized_descriptor_sha256,
        expected_member: Some(program.member),
        plan_sha256: program.plan_sha256,
        implementation_sha256: program.implementation_sha256,
...
fn from_native_acquire_created_first_truncate_error_release_failed_v1(
...
fn from_native_acquire_created_first_shared_busy_close_succeeded_v1(
    program: LockNativeAcquireCreatedFirstSharedBusyCloseSucceededProgramSpecV1,
) -> SourceLockProgramSpecV1 {
    SourceLockProgramSpecV1 {
        #[cfg(windows)]
        case: LockProgramCaseV1::NativeAcquireCreatedFirstSharedBusyCloseSucceeded(program),
        normalized_descriptor_sha256: program.normalized_descriptor_sha256,
        expected_member: Some(program.member),
        plan_sha256: program.plan_sha256,
        implementation_sha256: program.implementation_sha256,
...
fn from_native_acquire_existing_first_shared_busy_close_succeeded_v1(
    program: LockNativeAcquireExistingFirstSharedBusyCloseSucceededProgramSpecV1,
) -> SourceLockProgramSpecV1 {
    SourceLockProgramSpecV1 {
        #[cfg(windows)]
        case: LockProgramCaseV1::NativeAcquireExistingFirstSharedBusyCloseSucceeded(program),
        normalized_descriptor_sha256: program.normalized_descriptor_sha256,
        expected_member: Some(program.member),
        plan_sha256: program.plan_sha256,
        implementation_sha256: program.implementation_sha256,
