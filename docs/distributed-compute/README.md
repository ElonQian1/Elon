...
| 外部算力池适配器与统一报价 | v221-v254 前置门为 `implementation_partially_verified`。V250 scanner 链已通过 migration 4 项与进程内 HTTP 5 项，V252 sandbox 链已通过 migration 7 项与进程内 HTTP 2 项，V253 Provider-specific credential 链已通过 migration/源码合同 5 项与进程内 HTTP 3 项；三者不执行扫描、真实沙箱或 secret 解析。V254 仅追加 owner delegation 与 inert static compatibility candidate，并以 dynamic preflight 同次消费 V249 Prepared + current V250/V252/V253；closure 固定未实现、Provider 保持 `registering`、`activation_ready=false`，源码未编译或运行，`passed=0`。candidate 不等于 activation、route、market 或 runtime readiness。见 [`V254 候选权威`](external-pool-provider-activation-candidate-authority.md)、[`V254 验收边界`](external-pool-provider-activation-candidate-acceptance.md)、[`V253 凭据续签权威`](external-pool-adapter-credential-reattestation-authority.md) 与 [`当前状态`](current-implementation-status.md) |
