...
历史上曾批准的授权申请不代表当前仍可调用。规划只接受未撤销、未过期且能覆盖下一次调用的当前 Grant；对应 Grant 已撤销或过期时，必须重新申请授权。MCP 显式 App 还必须属于当前用户和当前项目。商户封禁 App 后，即使数据库中仍存在历史批准记录，规划也会在 Grant 选择前失败关闭。
