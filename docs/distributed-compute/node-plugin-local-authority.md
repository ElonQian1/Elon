...
独立 `compute-plugin-state.sqlite3` 的路径型 facade、WAL/FULL/foreign-key/foreign-key-check 配置、私有 `BEGIN IMMEDIATE` seam、authority schema v3 与 inventory payload v2 已形成代码。v3 在原 meta、不可变 keyring/plan journal、candidate owner、download cursor 和 fetch claim 之上加入 candidate verification run、单 candidate prepared/verified 唯一性、跨 authority/process/cancel 的撤销门卫、不可变 `candidate_health_receipts` 及不可变 `candidate_health_quarantine_receipts`；当前必需对象还包括 abort/resolution fence、run authority-state identity、拆分后的 observed/mismatch 结果列、健康回执的写入 fence/重叠有效期阻断、quarantine 写入 fence、两类健康回执的 update/delete 禁止 trigger，以及失败候选清理的 authorization/completion 表和 `owned -> cleanup_pending -> cleaned` 门卫。清理对象的精确边界见 `node-plugin-candidate-cleanup.md`。此前 v1/v2 以及旧形态 v3 都从未接入 NodeAgent、未真实建库；新必需对象会让缺少本批对象的同版本库失败关闭。本批仍采用预生产 fail-close/rebuild 边界，不提供旧 schema 原地迁移。启动校验复用同一组 v3 建库声明生成内存参考库，对全部非 SQLite 内建 table、index 和 trigger 的对象集合、所属表及规范化 SQL 定义 SHA-256 进行精确比较；缺失对象、额外对象或同名异定义均失败关闭。远程 canonical schema 的建库与重开指纹已通过内存 SQLite 定向测试；本批修改 cleanup 时间下界与 owner fence trigger 后未重跑该测试，也尚未对生产本地库执行 DDL 或迁移。

cleanup execution plan、expected objects、plan seal 与 append-only step events 已纳入 v3 必需对象，并已随远程 canonical 的 `elon-pc-node` 编译及 schema 建库与重开指纹定向测试。plan 绑定 authorization、installation、staging root、candidate-parent anchor identity 与完整对象父子拓扑；每个对象只有完成 durable intent、exact disposition/受约束的恢复缺失、exact parent 下的 namespace absence 和独立 namespace-durability fence，completion 才能同时引用 plan digest、既有物理 execution evidence digest 与独立 terminal journal digest。completion Store 内核已按这三类摘要改造，并只接受没有构造入口的终态 journal capability；物理 evidence v1 仍没有不可变记录及 plan/object digest 绑定。本批受管文件系统又形成未编译的对象/直接父级/原始名称 binding、全零 FileId 失败关闭、保留父句柄的强 disposition，以及父句柄相对 `Absent/ExpectedIdentityMatch/IdentityConflict` 观测；两类 name-present 结果都不能回退，namespace durability 入口固定归还 unavailable。JCS/路径/对象/物理证据摘要重算的 topology/journal typed Store、真实 parent namespace flush、既有物理执行器对 durable-intent/journal/强 disposition 的适配及跨重启 recovery 均未实现。
...
远程 canonical 合并后的供应链、候选健康与失败候选清理代码已通过 `elon-pc-node` 编译；清理链通过 3 项 Windows 同句柄删除、1 项目录顺序及 4 项 completion 凭证/库存投影测试，另有 1 项 topology/journal schema 建库与重开测试。前 4 项没有覆盖 retained custody → cleanup authorization → executor prepare，因而没有验证普通目录句柄与 late `DELETE` reopen 的 share-access 冲突。cleanup authorization Store 可原子推进 `cleanup_pending`；旧执行器仍调用不保留父句柄的 `delete_exact()`；completion Store 内核只在 sealed topology 和全部 namespace-durability journal 通过 exact readback 后，才会移除精确 failed slot、清空 candidate pointer、写不可变回执并关闭 owner。由于 topology/journal Store、强执行适配和目录 durability 仍缺，终态 capability 不可构造，完整私有流水线尚不可达。本批身份 binding、强 disposition 和 namespace 观测源码未编译、未运行；当前仍未接入生产 Host 或生产本地库，不能授予生产重试、安装、promotion、ReadyCapability 或商业 Verification。单一 cleanup handle tree、跨重启恢复、真实下载/Sidecar/探针、installed/promotion/GC 与 Attempt 接线仍待实现；精确边界见 `node-ready-capability.md` 与 `node-plugin-candidate-cleanup.md`。
