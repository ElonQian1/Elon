...
- 2026-06-23 当前状态口径已从“等待 FB2_AI_CENTER_TOKEN”升级为“受控 token bridge 已满足 no-write live preflight”。`status-refresh-current.json` 只有在 bridge 结果新鲜、无写群、direct-no-proxy、无 token argv/output 泄漏、preflight/current-state exit code 均为 0 时才置 `protected_live_preflight_satisfied=true`；此时 `blocked_by_external_secret=false`，下一步为 `keep_non_voice_regression_green_resume_ASR_TTS_only_when_user_unpauses`。如果 bridge 证据过期或不满足安全条件，状态会回退到重新跑 bridge / no-write preflight。
...
- 2026-06-23 当前状态门禁新增 fb2 service token bridge：`scripts\run-fb2-ai-center-token-bridge.ps1 -SelfTest` 已接入 `validate-fb2-ai-center-current-state.ps1`，只验证命令安全边界，不读取远程密钥。需要 live 刷新时，具备 fb2 服务器 SSH 权限即可运行 `scripts\run-fb2-ai-center-token-bridge.ps1 -RunDataOnlyPreflight -Fb2Password <FB2_PASSWORD>`，脚本会从 fb2 服务器 `.env` 把 `FB2_MAIN_PROJECT_SHARED_SECRET` 读入当前进程环境变量，执行 no-write `DataOnlyAcceptance -PreflightOnly`，并恢复环境。该路径不把 service token 放进 argv、日志或 summary，并强制项目访问直连：SSH 禁用 ProxyCommand/ProxyJump，子进程设置 `NO_PROXY/no_proxy=*` 并清空常见代理环境变量。
...
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\run-fb2-ai-center-token-bridge.ps1 -RunDataOnlyPreflight -Fb2Password <FB2_PASSWORD>
