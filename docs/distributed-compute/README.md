...
| 外部算力池适配器与统一报价 | Provider onboarding v221 与 Adapter release v222 已编译迁移；10 个 onboarding、6 个 release HTTP 操作及 22 个分角色 MCP 工具已通过治理链和 Store 重开专项。PC `/compute-external-pools` 已完成静态生产构建；真实 TCP、浏览器与生产部署仍未运行。v227 raw bytes source 与 v229 `staged -> withdrawn|revoked|superseded`、exact successor、terminal/current view、两个管理员 HTTP 及三层 currentness 的实现和分组测试源码已写，但测试 passed=0，未编译、未执行 migration 或运行，无 fingerprint/receipt，实际 artifact/terminal 均为 0，状态仍是 `design_frozen/implementation_uncompiled/implementation_unrun`。未执行测试源码现覆盖 HTTP/pre-CAS 失败关闭、`.part` 清理、existing CAS 复用/腐化拒绝、blob missing、路径/权限/reparse 门卫、saga replay、migration/重开、并发及 terminal↔artifact 顺序；仍缺实际执行、精确崩溃/断电注入和目标平台动态 handle 证据。无 MCP/PC；它不解析 candidate ref、不验证 artifact/verifier、不写 v213、不建 Pool/Offer 或派发。见 [`external-pool-adapter-artifact-source-authority.md`](external-pool-adapter-artifact-source-authority.md)、[`external-pool-adapter-release-lifecycle-authority.md`](external-pool-adapter-release-lifecycle-authority.md) 与既有 external-pool 权威/验收 |
...
服务端 Provider Adapter 统一接入公司集群、云 GPU 和其他算力池；每个 Provider 保留自己的内部调度，只向一龙提交标准回执。Provider onboarding 与 Adapter release v221/v222 已编译迁移并通过管理面专项；生产部署仍未运行。v227 [`artifact bytes source`](external-pool-adapter-artifact-source-authority.md) 与 v229 [`release-admission lifecycle`](external-pool-adapter-release-lifecycle-authority.md) 已写入实现和分组测试源码，测试意图覆盖管理员 raw body→quarantine→重开 SHA-256、三终态、exact successor、current view、管理员 HTTP、pre-CAS/Store/trigger currentness、输入失败关闭、文件安全、saga 重放、并发和 terminal↔artifact 顺序；完整进程崩溃/断电 fault injection 与目标平台动态 handle 证据仍缺。新增测试 passed=0，未编译、未执行 migration 或运行，无 fingerprint/receipt，实际 artifact/terminal 均为 0，状态仍是 `design_frozen/implementation_uncompiled/implementation_unrun`。无 MCP/PC，terminal 不产生 Adapter 或 route effect；onboarding、staged admission、terminal 和 bytes receipt 均不证明 candidate ref provenance、verifier、credential、route、容量、派发或结算。
