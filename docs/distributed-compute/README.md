...
| 外部算力池适配器与统一报价 | v221-v263 前置门为 `implementation_partially_verified`。V263 已把 V256 短时 config/credential 通过 V262 authenticated runtime 交付给同一受限 child；两组 source-contract、Linux-musl 产品/fixture 链接和 WSL2 18 项 kernel fixture 通过。当前仍未连接 upstream target 或生成 probe/route/activation；V254 18 deny 不变。见 [`V263 Secret delivery 权威`](external-pool-adapter-ephemeral-secret-delivery-authority.md)、[`V263 验收`](external-pool-adapter-ephemeral-secret-delivery-acceptance.md) 与 [`当前状态`](current-implementation-status.md) |
...
Provider Adapter 的 v227-v263 前置门已形成分层权威。V260 已验证 anonymous seqpacket/seed pipe、HKDF/HMAC mutual bootstrap、双向 frame 与 terminal fail-close；V261 验证真实 clone3/cgroup/namespaces/private root、fd3/fd4/fd5 custody、seccomp、pidfd 与 cleanup；V262 把二者放入同一真实 exec 后生命周期；V263 再从 Store 私有 current authority 借用 V256 config/credential，以 root-bound receipt/commit/ready 交付给同一 child。当前仍未连接 V258 target 或生成 probe/activation。下一硬门是 broker TLS/upstream no-work probe。V254 18 deny 逐字保留，完整 admission gate 只能与 atomic activation 同批替换。见 [`V263 Secret delivery`](external-pool-adapter-ephemeral-secret-delivery-authority.md)、[`V262 authenticated runtime`](external-pool-adapter-authenticated-runtime-authority.md)、[`V261 Linux supervisor`](external-pool-adapter-linux-supervisor-authority.md)、[`V260 authenticated session`](external-pool-adapter-authenticated-child-session-authority.md) 与 [`current-implementation-status.md`](current-implementation-status.md)。
