//! Exact installed-ABI execution for real native-acquire contention.
...
const SELECTOR_ENV: &str = "ELON_SQLITE_A2_LOCK_NATIVE_ACQUIRE_BUSY_SELECTOR";

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in super::super::super) struct LockRunnerNativeAcquireBusyBindingV1 {
...
pub(in super::super::super) fn run_lock_native_acquire_busy_program_isolated(
    exact_test: &str,
    ...
    if let Some(root) = super::selected_child_root()? {
        let selected =
            std::env::var(SELECTOR_ENV).context("read parent-selected Lock native-busy program")?;
...
fn run_parent(
    ...
    if exact_test.is_empty() {
        return Err(anyhow!("Lock native-busy exact test name is empty"));
    }
    let executable =
        std::env::current_exe().context("resolve current Lock native-busy test executable")?;
...
fn validate_parent_receipt(
    ...
    if !child.matches_family(SanitizedPayloadFamily::LockQuotient) {
        return Err(anyhow!("Lock native-busy child payload family mismatch"));
    }
    ...
    if !child.matches_registration_id(payload.registration_id) {
        return Err(anyhow!(
            "Lock native-busy child registration binding mismatch"
        ));
    }
    let environment =
    ...
    {
        return Err(anyhow!("Lock native-busy parent cleanup binding mismatch"));
    }
...
fn exercise_child(
    ...
    let nonce = std::env::var(A2_DYNAMIC_CHILD_NONCE_ENV)
        .context("read parent-created Lock native-busy child nonce")?;
    ...
    fixture::validate_baseline(before)?;
    let lock_observation = ArmedLockObservation::begin(&observer, lock_expectation(binding))?;
    let (callback, holder_receipt) = observer
        .with_native_lock_contention(binding.first, binding.count, || {
            fixture.route(fixture::SELECTED)?.observe_main_shm_lock_raw(
    ...
    let pending_count = witness.pending_count().map_err(anyhow::Error::msg)?;
...
pub(super) fn validate_binding(
    ...
    let end = binding
        ...
        .ok_or_else(|| anyhow!("Lock native-busy range overflow"))?;
    if binding.count == 0 || binding.first >= 8 || end > 8 {
        return Err(anyhow!("Lock native-busy range is outside eight slots"));
    }
    if binding.action == LockRunnerActionV1::LockShared && binding.count != 1 {
        return Err(anyhow!("Lock native-busy shared range is not one slot"));
    }
    if !matches!(
        binding.action,
        LockRunnerActionV1::LockShared | LockRunnerActionV1::LockExclusive
    ) {
        return Err(anyhow!("Lock native-busy action is not an acquire"));
    }
    let mask = (((1_u16 << binding.count) - 1) << binding.first) as u8;
    if binding.mask != mask {
        return Err(anyhow!("Lock native-busy range mask mismatch"));
    }
    Ok(())
}
...
pub(super) fn exact_selector(binding: LockRunnerNativeAcquireBusyBindingV1) -> String {
    super::super::child::lock_native_acquire_busy::selector(
        action_tag(binding.action),
        binding.first,
        binding.count,
    )
    .expect("validated Lock native-busy selector")
}

#[cfg(all(test, windows))]
pub(in super::super::super) fn selected_lock_native_acquire_busy_selector_for_test(
) -> Option<String> {
...
#[cfg(all(test, windows))]
pub(in super::super::super) fn lock_native_acquire_busy_selector_for_test(
    action_tag: u64,
    first: u8,
    count: u8,
) -> Result<String, &'static str> {
    super::super::child::lock_native_acquire_busy::selector(action_tag, first, count)
}
