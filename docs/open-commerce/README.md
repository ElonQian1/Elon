...
| 商户数据来源、健康度和同步回执 | 控制面已实现；能力与合格内部回执的显式绑定已通过定向 Rust/SQLite、集成上下文、管理 HTTP、消费者发现 MCP 和进程内发现 HTTP 回归。真实 TCP、PC 浏览器、历史迁移、具体平台适配器和外部核验仍待逐项验收 | `docs/decisions/open-commerce-integration-control-plane.md`、`docs/decisions/open-commerce-capability-source-link-v1.md`、`docs/open-commerce-integration-control-plane-acceptance.md`、`docs/open-commerce-capability-source-link-v1-acceptance.md` |
