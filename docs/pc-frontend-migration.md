...
| 算力结算管理页 | 🟡 中 | `src/features/compute-settlement/`、`server/src/compute_federation_settlement_*` | 🟡 已写入管理员平台账户、到期候选、显式逐笔释放和提款队列源码并注册 `/compute-settlement`；尚未构建、视觉验收或发布，提款终态操作仍未接入 |
