...
A2a/A2b1 的 exact logical-name routes 共享 WAL/SHM runtime，但每条 Connection 独立 route/authorizer/custody；完整 terminal projection、`SourceBranch`、`Expected`、`CaseKey`、exclusion 与 StaticContract 已闭合为 Map `43476/43476`、Lock `8668/8668`。typed quotient 只有前序验证基线；current runner-admission source 未编译、未运行。Map q4 `MapRegionLoopSuccessV1` 为 511 frozen/509 net-new，未运行 inventory 预期 `521` present、`42,955` missing。Lock q1–q10 current source 中，q10 `LockAbiScalarRejectionV1` 新增 7 个 exact singleton normalized groups，精确覆盖 `offset/count/flags` validity 的七个非全真组合；q9 的 528 个 exact singleton（6×88）保持不变。精确 matcher、production seam、receipt 与 source-only/actual 边界仅见[`Lock dynamic tranches authority`](node-plugin-vfs-lock-dynamic-tranches-authority.md)。q3/q4 catalog SHA-256 为 `4da94c20e91d97a0082116879718b1ccf0271eb235ed785e65a2e36e7a949d85`、`df931ad7725843098f228d07d9798d79e92f2beec4e1c23e83fc89219dfa1396`；q5 catalog SHA-256=`b12bd411f7fa63f822e65a679351dfc103a6368e2887355d5b03c530fc162e2f`；q7 catalog SHA-256=`e9f509d52d294405dd1a7ae528c514a31ba4e0758959374b633bdca2b571d631`。Lock 未运行 inventory（members/groups）=`3657/3657 present, 5011/4483 missing, 8668/8140 total`；3,432 initialization failures 仍 missing 且不属于 q9/q10。以上均仅为 `source_written/source_review_only/implementation_uncompiled/implementation_unrun`，`passed=0 failed=0 actual=not_run`；coverage=`0/8668`，无 actual inventory/receipt/record、reviewed digest/frozen manifest，`Qlock=unknown`、`WindowsDynamic=not_opened`、production closed。q9/q10 source changes 对应的 19 份 global frozen/source-owner artifacts refresh 继续独立 deferred，禁止做半套 owner 重绑。A2b2=`117/117` 不能替代该门或启用生产入口。
...
当前静态验收已闭合为 Map `43476/43476`、Lock `8668/8668`；projector/candidate 只保留前序验证基线。Lock q10 `LockAbiScalarRejectionV1` 为 source-only 7 exact singleton；未运行 inventory（members/groups）=`3657/3657 present, 5011/4483 missing, 8668/8140 total`，其中 3,432 initialization failures 仍 missing 且不属于 q9/q10。current source=`source_written/source_review_only/implementation_uncompiled/implementation_unrun`（uncompiled/unrun），`passed=0 failed=0 actual=not_run`；无 actual inventory/receipt/record，inventory/reviewed digest 缺失且 manifest 未冻结，`Qmap/Qlock=unknown`、coverage=`0/43476 + 0/8668`、`WindowsDynamic=not_opened`、production closed。q9/q10 source changes 对应的 19-artifact global frozen/source-owner refresh 仍独立 deferred。只有补齐全部 program source、冻结 exact quotient manifests 并在同一 clean Windows cohort 逐 class 产生正式记录，才可形成 `Qmap/Qmap` 与 `Qlock/Qlock`；q9/q10 细节只见[`Lock tranches`](node-plugin-vfs-lock-dynamic-tranches-authority.md)。
