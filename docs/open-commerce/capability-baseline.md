...
| 消费者发现请求输入边界 | 代码已形成、未编译 | 第三方 App 的搜索词、能力键、App ID 和返回数量由服务端统一规范化；SQL LIKE 特殊字符按普通文字查询。该边界不改变固定候选窗口、授权、排序或计量规则 | `server/src/open_commerce_consumer.rs`、`server/src/store/open_commerce_directory.rs`、`docs/decisions/open-commerce-consumer-discovery-inputs-v1.md` |
| 消费者 AI 完整发现 MCP | 代码已形成、未编译 | 消费者 AI 可复用 PC 同源的透明排序、来源、价格、能力和偏好约束；默认身份只做公开发现，显式 App 身份继续校验所有权。工具不申请授权、不调用、不下单 | `server/src/open_commerce_consumer_discovery_mcp.rs`、`server/src/open_commerce_mcp.rs`、`docs/decisions/open-commerce-consumer-discovery-mcp-v1.md` |
| 消费者能力执行计划 | 已实现并通过本地 Rust、SQLite 与 MCP 假数据验收 | 用实际输入只读复核能力契约、App、封禁和授权状态，并按下一次调用的次数、价格和币种遍历选择可用 Grant；历史批准申请不能替代当前有效 Grant，全部不可用时返回稳定预算原因和续额下一步。动作明确拆成准备、用户确认、确认、调用。计划不占预算、不创建业务记录 | `server/src/open_commerce_consumer_execution_plan.rs`、`server/src/open_commerce_grant_readiness.rs`、`server/src/open_commerce_consumer_discovery_mcp.rs`、`server/src/open_commerce_consumer_execution_plan_tests.rs`、`docs/decisions/open-commerce-consumer-capability-execution-plan-v1.md` |
| 消费者 AI 单能力授权与续额申请 | 已实现并通过本地 Rust、SQLite 与 MCP 假数据验收 | 用户明确确认后，已注册自有 App 可向商户申请一个受限能力；任一 Grant 仍能覆盖下一次调用时拒绝重复申请，全部预算不可用时允许新申请。相同待审批请求顺序重放复用记录且审计一次，响应不泄露内部身份；旧 Grant 不改写，成功仅为 pending，商户仍决定批准、期限和预算 | `server/src/open_commerce_consumer_discovery_mcp.rs`、`server/src/open_commerce_consumer_authorization_mcp.rs`、`server/src/open_commerce_consumer_authorization_request_mcp_tests.rs`、`server/src/open_commerce_grant_readiness.rs`、`server/src/store/open_commerce_authorization_requests.rs`、`server/src/store/open_commerce_grants.rs`、`docs/decisions/open-commerce-consumer-authorization-mcp-v1.md` |
