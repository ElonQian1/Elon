...
ChatGPT 新会话首条消息不再强制等待官网 DOM 先挂出用户气泡。宿主会在单一发送账本中记录发送前的
`privateStreamRevision`；只有本次发送后修订号前进、状态进入 `streaming` 或 `completed`，且消息携带
共享 APK/Win 适配器生成的 `private-stream:` 稳定 ID 时，才用账本中的用户问题补齐当前轮并接纳私有
回答。旧修订、旧会话延迟帧和普通 DOM 助手节点仍失败关闭；后续流帧按同一私有 ID 更新，官网 DOM
最终挂载后再由既有影子合并器收敛为官方消息并保留富内容。该路径不重放生成请求，也不建立第二套会话。

ChatGPT 金融图表的压缩组件帧可能早于最终正文帧完成解压，而且组件帧与正文帧可以使用不同的
`messageId`。Win 兼容层不能因此把已经解出的图表永久降级为“交互内容”：只要组件已由当前私有流
实际观察到、`conversationId` 一致且没有互相冲突的 `turnId`，就允许在当前发送代次内先按会话结算，
最终正文身份补齐后继续沿用同一条回答。每次新建会话或发送代次重置都会清空已观察组件集合；较早
代次仍在异步解压的结果会被明确拒绝，不能进入下一轮。完整响应捕获恢复和官网 DOM 仍作为只读回退，
不重放生成请求，也不把这项 Win 兼容逻辑复制到 APK。

恢复出的私有金融卡片还要与官网 DOM 的通用占位做标题级对账。官网可能先把同一组件表示成
`interactive: Bitcoin (BTC)`，私有流随后才提供 `finance: Bitcoin (BTC)`；Win 实时合并层在标题一致时
用真实卡片替换该占位，但保留标题不同的其他交互组件。这样既不会同时显示“交互内容”和走势图，
也不会为了清理占位误删同一回答中的独立工具。

Win 会把该恢复链路的安全诊断随消息快照交给宿主：只包含发送代次、是否绑定到会话/回答、已识别的
富内容种类、接纳/拒绝计数、占位是否完成替换，以及 `stale_generation`、`route_mismatch`、
`identity_mismatch` 等原因枚举。诊断不包含问题、回答、会话 ID、原始组件、Cookie 或 Token，并会在
现有网页 AI 状态弹层显示为人类可读说明，便于区分上游结构变化与本地会话隔离保护。
...
- Win ChatGPT 实时语音复用 APK 已验证的同源一次性中继与私有数据通道模型，但媒体 Peer 仍由
  WebView2 托管，不宣称已经移植为 Windows 原生 libwebrtc。结构化生命周期 v2 会把请求麦克风、
  创建 Offer、等待中继、连接、双向音频、静音、失败回退和关闭状态按当前会话投影到原生 UI；
  从空白 `/` 到官网新分配 `/c/{id}` 的正常首轮跳转会迁移绑定，跨既有会话跳转仍立即关闭。
  中继或连接超时会停止本机麦克风并继续使用官网语音，状态中不含 SDP、ICE、凭据、请求头或转写正文。
