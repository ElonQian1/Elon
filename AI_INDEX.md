...
| Web/PWA/Tauri/Android 后台设计会话 / 持久浏览器与安全 fixture 交互 / Tauri 窗口、菜单、对话框和 command trace 分层证据 / 草稿、写回与验证矩阵 / PC 中间画布和右侧聊天 | `docs/headless-ui-design-mcp.md`、`server/src/node_agent_android_live/design_targets.rs`、`server/src/node_agent_android_live/design_browser_runtime.rs`、`server/src/node_agent_android_live/design_verification_matrix.rs`、`server/src/node_agent_android_live/tauri_host_runtime.rs`、`server/src/node_agent_android_live/tauri_behavior.rs`、`server/src/node_agent_android_live/design_drafts.rs`、`server/src/node_agent_pwa_runtime/stateful.rs`、`server/src/node_agent_pwa_runtime/interaction.rs`、`server/src/node_agent_source_preview/writeback_receipt.rs`、`pc-frontend/src/features/ui-tuner/headless-design/` |
