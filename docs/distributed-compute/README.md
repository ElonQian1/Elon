...
| 节点插件治理合同 | Signed Manifest/InstallPlan、双 keyring、authority v3/PlanApply、受管取数、候选验证、ZIP 安全物化、staging 与健康/quarantine Store 均已写。失败候选已有 cleanup authorization Store、只消费固定句柄的物理执行器和原子 completion Store 内核；completion 会移除 failed slot、清空 candidate pointer、关闭 owner 并推进全部 authority fence，但只接受已封存 topology、物理证据和 terminal namespace-durability journal 共同绑定的不透明能力。当前组合通过 `elon-pc-node` 编译、8 项清理定向测试及 1 项包含 topology/journal 对象的 schema 建库与重开测试；topology/journal typed Store、durable-intent 执行适配、父级 identity/absence、真实 namespace durability 和终态能力构造入口尚缺，所以完整清理链仍不可达。完整事务夹具、生产 Host、跨重启恢复、真实 downloader、Sidecar/探针、installed/promotion、ReadyCapability 与 Attempt 接线也仍缺。默认关闭的 Bootstrap 不会建库或启动插件。 |
