...
这只是 PlanApply live-binding 的必要前置，不是完整可用的 planning context。`user_version=6` 现增加独立的目录 binding：只在空库存、无候选 owner/旧 prepared work时，按规范顺序重验完整 Publisher-signed Manifest 来源，并用当前 Control ring 的独立 `ELON-COMPUTE-PLUGIN-MANIFEST-CATALOG-V1` 域验证精确目录头；payload、Control envelope、签名 key/fingerprint、完整来源集合与 immutable receipt 均精确封存。catalog-aware rollback checkpoint/anchor V2再绑定目录摘要与 receipt 摘要，拒绝同修订内容分叉，且不与 V1 permit互转；V2 assessment只接受未来由同一opened authority/root事务铸造的不可复制本机checkpoint custody，裸远端checkpoint不能冒充本机事实。与此同时新增的 opened-authority capability保留 pinned root、root lock、NodeAgent instance lock 与 authority instance，但本批不提供构造入口，未来必须由Bootstrap原子绑定同一NodeDataPaths/root与instance-lock见证；生产 `open()` 固定失败关闭，直到 SQLite main/journal/WAL/SHM 都由 handle-bound VFS拥有。planning 不得调用会建目录、path-open、切 WAL 或迁移的 legacy `with_deferred()`，这些legacy入口也必须在VFS启用前迁移或退役。因此 v4-v6/v210 DDL仍未实际运行，v11 仍不能产出 snapshot。v5 只撤销下载/校验能力，不撤销 Ready/Attempt；cleanup 的 deletion custody仍不跨重启。后续还需真实 VFS与 trust/rollback provider、empty-inventory snapshot producer、work-admission、signed `reauthorize_existing`、Signer/KMS、本机确认与执行门卫；此前继续保持零 PlanApply、零下载、零安装、零 Sidecar。
