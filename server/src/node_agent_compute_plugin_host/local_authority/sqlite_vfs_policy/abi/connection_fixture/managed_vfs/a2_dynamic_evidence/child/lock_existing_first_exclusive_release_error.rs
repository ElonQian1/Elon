//! Canonical q13 child header for one controlled existing-file initialization release uncertainty.

pub(in super::super) const REPORT_VERSION: &str = "a2lockq13";
pub(in super::super) const REPORT_VALUE_COUNT: usize = 168;
...
pub(in super::super) fn selector(
    ...
    Ok(format!(
        "initialization-{action}-first-{first}-count-{count}-existing-first-exclusive-release-error-{completion}-terminal-route-unknown"
    ))
}
...
mod tests {
    ...
    #[test]
    fn eighty_eight_q13_selectors_are_unique_and_fail_closed() {
