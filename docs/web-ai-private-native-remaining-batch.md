...
| Attachment upload | Verified small-text private upload; native byte handoff, store association, ordinary conversation scope and static-image private transaction now connected in source | Accept integrated new/existing-chat text and static images on-device, then cover project/temporary/PDF/multipart variants. The original native-picker smoke failed before byte upload; no fresh integrated-device pass is claimed. See [upload contract](chatgpt-private-attachment-upload.md) and [image extension](chatgpt-private-image-upload.md). |
