...
- `.elon/document-sections.json` 保存所有 AI 供应商共享的项目知识架构：项目类型 `profile`、知识首页 `home`、最多四层主题 `sections`、主题固定项 `assignments`、治理覆盖 `governance_overrides`、文档关系/共享顺序/固定状态 `document_metadata` 和最近 100 条结构操作 `audit_log`；不改变文件实际路径。
