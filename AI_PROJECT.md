...
elon 是一个通过持续讨论把模糊想法变成可交付应用的 AI 开发平台。用户在手机 APK 或 PC 工作台里描述目标，一龙 AI 先帮助用户澄清产品方向和验收标准；方向确认后，总调度 AI 再从平台的 AI-to-AI Skill 能力库中选择和组合合适 Skill，把任务交给 AI CLI/Codex/Copilot 等代理，在真实 Git 工作区修改代码、验证、构建、发布，最后把 APK、项目或其他产物回传给用户。

这里的 Skill 是平台内部、面向 AI 调用的官方工程能力，不要求普通用户手动挑选提示词，也不作为可上架、购买或交易的商品。用户负责表达目标并确认方向和验收标准；总调度 AI 负责需求理解、Skill 路由、Matter 计划、执行编排和验收汇报。
...
短期先验证“官方 Skill + Matter 执行”的闭环。长期把官方 Skill 做成平台内部可版本化、可审核、可组合、可升级的能力仓库，并把生成的应用接入独立的版本分发和二次创作体系。产品不建设 Skill 市场，不接收第三方 Skill 上架，也不为 Skill 提供买卖、订阅或创作者结算。详细路线见 `docs/ai-to-ai-skill-roadmap.md`。
