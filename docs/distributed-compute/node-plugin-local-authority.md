...
v10 preparation 只证明节点观察了同一 policy/authorization；v209 保存请求、每会话 delivery 与 observation，不打开 authority，也不返回 planning context。v11/v210 又铺设绑定 cloud session、exact v209 observation 与 Bootstrap 代次的协议及追加式 planning/generation 账本，但尚无云端派发或 authority producer；节点固定 `snapshot_ready=false`，生成结果仅限 `signer_unavailable/rejected`。独立的无下载 policy-binding 内核则把 exact desired policy、installation/root、进程与配置/取消代次封成持实例锁 lease 的一次性意图；disabled 走无 authorization/preparation 的撤销意图。任何非 replay 策略、实例锁或数据根变化都会先推进 generation，使旧意图在 Store 各事务边界失败关闭。
...
这仍不是生产 planning context：缺账号切换失效 hook、pinned-root 句柄型 authority open、可信时间/回滚见证、snapshot producer、Ready/Attempt work-admission 与 signed `reauthorize_existing`。cleanup-only deletion domain 也仅允许已耐久授权、owner=`cleanup_pending` 的线性 custody在同 authority/root/process fence 与 retained handles 连续存活时继续；授权前竞态、进程轮换、重启和跨进程 adoption 均失败关闭。因此 `context_ready`、`snapshot_ready` 及 root/authority/download/Sidecar 标志保持 false，Store 无 Host 调用点，也不允许 PlanApply、下载或任务准入。
