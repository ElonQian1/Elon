...
- “你的工作室”首页只显示一条四入口长胶囊，依次为好友、项目、我的、主菜单；长胶囊落在好友大面板内部，不显示右侧独立新建会话圆钮。
- 项目页、我的页等既有一级页面可继续使用“长胶囊 + 独立圆钮”形态；页面切换时两种形态仍共用同一底边和安全区基准。
...
- 顶栏居中显示“你的工作室”，右侧只保留搜索；新增项目和添加好友分别下沉到对应内容板块。
- 顶栏下方项目区固定高 `153dp`：标题行高 `44dp`，项目横滑项使用 `56dp` 圆形封面和 `72dp` 项宽，圆形封面自然间隔 `16dp`；首项为新增项目，并与首个真实项目额外间隔 `8dp`。
- 好友区使用设计稿提供的大圆角原始背景素材，从项目区下方延伸到页面底部；顶部把手为 `66dp x 5dp`，标题行高 `44dp`。
- 好友、群聊和下拉筛选内容都在好友大面板内呈现；下拉手势必须从该面板内开始，不能在项目横滑区误触发。
- 首页底部四入口长胶囊宽 `300dp`，页面左右各 `30dp`；其底边继续遵守聊天输入胶囊的 `8dp` 基准间距。
