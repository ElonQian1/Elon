...
服务端 Provider Adapter 以独立合同逐步接入外部算力池。v227-v236 已形成制品来源、准入、供应商签名、来源证明、静态包、静态安全、独立 scanner trust root 和 exact-SBOM 漏洞签名报告；v237 建立与供应商/扫描器密钥三方隔离的 sandbox-verifier trust root；v239 再把 current V236、current V237、V222 六项能力和有限动态观察绑定为签名符合性证据。最新边界见 [`V239 动态沙箱符合性证据`](external-pool-adapter-artifact-sandbox-conformance-authority.md)。服务器仍不真实执行制品，也不代表 Adapter、credential verifier、route、派发或结算已完成；真实 TCP、生产升级、部署和 MCP/PC 仍缺。
...
v228 Delivery Allocation 详见 [`delivery-allocation-authority.md`](delivery-allocation-authority.md) 与 [`delivery-allocation-acceptance.md`](delivery-allocation-acceptance.md)。它冻结一份 Commitment→一个消费者 Job 的 whole-only Grant：行权在同一事务内冻结既有 Broker 预算、完整释放父 Commitment Claim、完整建立带 `parent_claim_id` 的标准 Reservation Claim，并写 Reservation、Job、Broker 与 immutable exercised receipt；Commitment current view 派生 `allocated`。Snapshot TTL 例外只属于 sealed Store-private authority，泛用 Reservation 入口继续失败关闭。既有 Grant/Exercise 纵切面为 `design_frozen/implementation_partially_verified`；行权后到期 Reservation 的管理员有界恢复源码仍可手动调用。v234 又写入 server-owned 公平恢复：单行 checkpoint 保存 Store 冻结的 `sweep_cutoff` 与 keyset 游标，worker 每 tick 最多处理 100 项，`blocked/failed` 也推进同一 sweep，空页后才清 checkpoint 并由下一 tick 开始新 sweep，因此不会被固定在最早一批坏项。它不冒充 admin actor，不新增生命周期或经济权威；成功仍只复用 Broker Expire 退回预授权、把 child Claim 置 `expired` 并归还容量、把 Job/Reservation 置 `failed/expired`，v228 仍 `exercised`、Commitment 仍 `allocated`。v234 migration、Store page、worker/main 接线及专项测试仅为 `source_written/implementation_uncompiled/implementation_unrun`、`passed=0`，不产生 verified usage、Provider 收益或 settlement，也不是持仓、撮合、执行或结算完成。
