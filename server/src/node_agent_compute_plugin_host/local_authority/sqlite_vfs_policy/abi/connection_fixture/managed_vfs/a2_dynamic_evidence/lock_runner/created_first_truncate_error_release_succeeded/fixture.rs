//! Retained child fixture for q14 created-first truncate uncertainty with known cleanup release.
...
use super::{
    payload, validate_binding, LockRunnerCreatedFirstTruncateErrorReleaseSucceededCompletionV1,
...
impl Deref for RetainedInitializationFailureFixture {
    ...
    fn deref(&self) -> &Self::Target {
        self.fixture
            .as_ref()
            .expect("retained q14 Lock initialization fixture")
    }
}
...
impl Drop for RetainedInitializationFailureFixture {
    fn drop(&mut self) {
        if let Some(fixture) = self.fixture.take() {
            // The cleanup release is proven, but the poisoned node, file, and terminal domain stay
            // child-owned. Parent cleanup begins only after this isolated child has exited.
...
pub(super) fn exercise_child(
    ...
    let nonce = std::env::var(A2_DYNAMIC_CHILD_NONCE_ENV)
        .context("read parent-created q14 Lock initialization child nonce")?;
    ...
    let observation = fixture
        .route(SELECTED)?
        .observe_main_shm_lock_created_first_truncate_error_release_succeeded_v1(
    ...
    let preemption_values = match binding.completion {
        ...
        LockRunnerCreatedFirstTruncateErrorReleaseSucceededCompletionV1::RetentionRouteUnknown => {
            ...
            if receipt != [1; 5] {
                return Err(anyhow!(
                    "q14 Lock route-unknown preemption receipt mismatch"
                ));
            }
            [
    ...
    {
        return Err(anyhow!(
            "q14 Lock retained registration/root shape mismatch"
        ));
    }
...
fn prepare(root: &Path) -> anyhow::Result<RetainedInitializationFailureFixture> {
    let fixture = ManagedSqliteMultiConnectionFixture::open_single(root, [0xae; 16])?;
    ...
    let mode: String = fixture
        ...
        .context("enable WAL for q14 cold initialization")?;
    if !mode.eq_ignore_ascii_case("wal") || !target_absent_before {
        return Err(anyhow!(
            "q14 Lock fixture was not cold before WAL-main attach"
        ));
    }
    route.into_schema_migration()?;
    route.into_runtime()?;
    // The out-of-budget Map call performs real WAL-main promotion and attach, then stops before
    // coordinator-node creation. Q14 is armed only after this cold prestate is observed.
    let cold_map = route
    ...
    {
        return Err(anyhow!("q14 Lock cold WAL-main attach receipt mismatch"));
    }
...
fn validate_observation(
    ...
    {
        return Err(anyhow!("q14 controlled initialization receipt mismatch"));
    }
    Ok(())
}
...
fn terminal_values(
    ...
    let route = terminal
        .terminal_route()
        .ok_or_else(|| anyhow!("q14 Lock terminal route missing"))?;
    if terminal.joint_close_physical_failure_retention_count() != 0
        || terminal.joint_close_physical_failure().is_some()
    {
        return Err(anyhow!("q14 Lock retained unrelated joint-close custody"));
    }
    let values = [
    ...
    if values != expected {
        return Err(anyhow!("q14 Lock terminal custody ledger mismatch"));
    }
    Ok(values)
}
