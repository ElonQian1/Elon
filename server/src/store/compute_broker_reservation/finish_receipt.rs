...
pub(super) fn replay_broker_finish_on(
    ...
    // occurred_at is generated by the service, so an exact client replay gets a
    // new timestamp and request digest. The audit below still binds every
    // caller-controlled field to the immutable source revision and action.
