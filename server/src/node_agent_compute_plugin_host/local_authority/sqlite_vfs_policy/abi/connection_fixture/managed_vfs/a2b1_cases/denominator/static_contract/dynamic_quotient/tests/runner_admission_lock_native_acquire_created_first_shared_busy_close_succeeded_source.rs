//! Source-only admission contracts for all 88 q18 created-first shared-busy close-ok members.

use super::super::super::terminal_descriptor::{InitializationFaultSiteV1, InitializationPathV1};
use super::super::runner_admission::{
    compile_for_test,
    native_acquire_created_first_shared_busy_close_succeeded_catalog_row_count_for_test,
...
fn assert_rejected(key: DynamicClassKeyV1, member: StaticMemberSealV1, mutation: &str) {
    assert!(
        ...
        "q18 initialization admission accepted {mutation}"
    );
}

#[test]
fn all_88_q18_descriptors_and_exact_catalog_seals_are_source_present() {
    ...
    for &case in leaves.keys() {
        ...
        validate_lock_program_for_test(&key, member, compile_for_test(&key))
            .unwrap_or_else(|error| panic!("exact q18 member {case:?} was rejected: {error:?}"));
    }
}

#[test]
fn q18_is_inventory_present_without_granting_supported() {
...
#[test]
fn q18_rejects_neighboring_seals_and_every_typed_contract_mutation() {
    let leaves = frozen_lock_created_first_shared_busy_close_succeeded_leaves_v1();
    ...
    for (neighbor, label) in [
        ...
        (
            frozen_lock_created_first_truncate_error_release_failed_leaves_v1()
                ...
                .1
                .member,
            "a q16 created-first cleanup seal",
        ),
        (
