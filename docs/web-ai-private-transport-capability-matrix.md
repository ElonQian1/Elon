...
| Fresh independent text dispatch | ChatGPT | Completed/default-enabled for existing personal plain text on 1696: native first/follow-up, handed-off reply, stop and default routing accepted. Lifecycle follow-on released in 1697; device run stopped at seed/navigation timeout, not accepted | Accepted sender only before dispatch; uncertain writes never replay. New/tool/file/project/temporary and initial composer-free scopes remain. [Evidence](chatgpt-fresh-text-dispatch.md) |
