//! Retained q16 child fixture for unread truncate and cleanup-release receipts.
...
use super::{
    payload, validate_binding, LockRunnerCreatedFirstTruncateErrorReleaseFailedCompletionV1,
...
impl Deref for RetainedInitializationFailureFixture {
    ...
    fn deref(&self) -> &Self::Target {
        self.fixture
            .as_ref()
            .expect("retained q16 Lock initialization fixture")
    }
}
...
impl Drop for RetainedInitializationFailureFixture {
    fn drop(&mut self) {
        if let Some(fixture) = self.fixture.take() {
            // Both native outcomes are uncertain, so the poisoned node, file, and terminal domain
            // remain child-owned. Parent cleanup begins only after this isolated child has exited.
...
pub(super) fn exercise_child(
    ...
    let nonce = std::env::var(A2_DYNAMIC_CHILD_NONCE_ENV)
        .context("read parent-created q16 Lock initialization child nonce")?;
    ...
    let observation = fixture
        .route(SELECTED)?
        .observe_main_shm_lock_created_first_truncate_error_release_failed_v1(
    ...
    let preemption_values = match binding.completion {
        ...
        LockRunnerCreatedFirstTruncateErrorReleaseFailedCompletionV1::RetentionRouteUnknown => {
            ...
            if receipt != [1; 5] {
                return Err(anyhow!(
                    "q16 Lock route-unknown preemption receipt mismatch"
                ));
            }
            [
    ...
    {
        return Err(anyhow!(
            "q16 Lock retained registration/root shape mismatch"
        ));
    }
    ...
    let cleanup_native = observation
        ...
        .ok_or_else(|| anyhow!("q16 cleanup native receipt missing"))?;
...
fn prepare(root: &Path) -> anyhow::Result<RetainedInitializationFailureFixture> {
    let fixture = ManagedSqliteMultiConnectionFixture::open_single(root, [0xb0; 16])?;
    ...
    let mode: String = fixture
        ...
        .context("enable WAL for q16 cold initialization")?;
    if !mode.eq_ignore_ascii_case("wal") || !target_absent_before {
        return Err(anyhow!(
            "q16 Lock fixture was not cold before WAL-main attach"
        ));
    }
    ...
    {
        return Err(anyhow!("q16 Lock cold WAL-main attach receipt mismatch"));
    }
...
fn validate_observation(
    ...
    let cleanup = value
        .initialization
        .cleanup_native_receipt()
        .ok_or_else(|| anyhow!("q16 controlled cleanup receipt missing"))?;
    ...
    {
        return Err(anyhow!("q16 controlled initialization receipt mismatch"));
    }
    Ok(())
}
...
fn terminal_values(
    ...
    let route = terminal
        .terminal_route()
        .ok_or_else(|| anyhow!("q16 Lock terminal route missing"))?;
    if terminal.joint_close_physical_failure_retention_count() != 0
        || terminal.joint_close_physical_failure().is_some()
    {
        return Err(anyhow!("q16 Lock retained unrelated joint-close custody"));
    }
    let values = [
    ...
    if values != expected {
        return Err(anyhow!("q16 Lock terminal custody ledger mismatch"));
    }
    Ok(values)
}
