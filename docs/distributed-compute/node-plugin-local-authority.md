...
cleanup execution plan、expected objects、plan seal 与 append-only step events 已纳入 v3 必需对象，并已随当前 `elon-pc-node` 编译及 schema 建库与重开指纹定向测试。plan 绑定 authorization、installation、staging root、candidate-parent anchor identity 与完整对象父子拓扑；每个对象只有完成 durable intent、exact disposition/受约束的恢复缺失、exact parent 下的 namespace absence 和独立 namespace-durability fence，completion 才能同时引用 plan digest、既有物理 execution evidence digest 与独立 terminal journal digest。completion Store 内核已按这三类摘要改造，并只接受没有构造入口的终态 journal capability；物理 evidence v1 仍没有不可变记录及 plan/object digest 绑定。本批把对象/直接父级/原始名称 binding 接入确定性 topology builder，并形成只消费 `AuthorizedCandidateCleanup` 的 topology typed Store：它原子写入 plan、全部 expected objects 与 seal，exact readback 后才返回保留原始句柄和根锁的 sealed capability；结果不确定时只能查询 exact `NotCreated/Sealed`。受管文件系统继续提供全零 FileId 失败关闭、保留父句柄的强 disposition，以及父句柄相对 `Absent/ExpectedIdentityMatch/IdentityConflict` 观测；两类 name-present 结果都不能回退，namespace durability 入口固定归还 unavailable。逐步 journal 与物理 evidence v2 typed Store、真实 parent namespace flush、既有物理执行器对 durable-intent/journal/强 disposition 的适配及跨重启物理 recovery 均未实现。
...
当前供应链、候选健康与失败候选清理代码已通过 `elon-pc-node` 编译；清理链通过 3 项 Windows 同句柄删除、1 项目录顺序、4 项 completion 凭证/库存投影及 5 项 topology builder/Store 测试，另有 1 项 topology/journal schema 建库与重开测试。前 4 项没有覆盖 retained custody → cleanup authorization → topology prepare，因而没有验证普通目录句柄与 late `DELETE` reopen 的 share-access 冲突；topology 测试覆盖稳定 child-first 计划、父级顺序/锚点拒绝、Store exact round-trip 和对象列篡改拒绝。cleanup authorization Store 可原子推进 `cleanup_pending`，topology Store 可封存 exact execution plan；旧执行器仍调用不保留父句柄的 `delete_exact()`；completion Store 内核只在 sealed topology 和全部 namespace-durability journal 通过 exact readback 后，才会移除精确 failed slot、清空 candidate pointer、写不可变回执并关闭 owner。由于 journal Store、强执行适配和目录 durability 仍缺，终态 capability 不可构造，完整私有流水线尚不可达。当前仍未接入生产 Host 或生产本地库，不能授予生产重试、安装、promotion、ReadyCapability 或商业 Verification。单一 cleanup handle tree、跨重启物理恢复、真实下载/Sidecar/探针、installed/promotion/GC 与 Attempt 接线仍待实现；精确边界见 `node-ready-capability.md` 与 `node-plugin-candidate-cleanup.md`。
