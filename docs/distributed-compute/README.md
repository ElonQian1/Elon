...
| 外部算力池适配器与统一报价 | Provider onboarding v221 与 Adapter release v222 已编译迁移；10 个 onboarding、6 个 release HTTP 操作及 22 个分角色 MCP 工具已通过治理链和 Store 重开专项。PC `/compute-external-pools` 已完成静态生产构建；真实 TCP、浏览器与生产部署仍未运行。v227 raw bytes source 与 v229 `staged -> withdrawn|revoked|superseded`、exact successor、terminal/current view、两个管理员 HTTP 及三层 currentness 的实现和分组测试源码已写，但测试 passed=0，未编译、未执行 migration 或运行，无 fingerprint/receipt，实际 artifact/terminal 均为 0，状态仍是 `design_frozen/implementation_uncompiled/implementation_unrun`。测试源码拟检查 migration/重开、并发、body 不被 poll 与 terminal↔artifact 顺序；完整 fault injection 和 v227 文件安全矩阵仍缺。无 MCP/PC；它不解析 candidate ref、不验证 artifact/verifier、不写 v213、不建 Pool/Offer 或派发。见 [`external-pool-adapter-artifact-source-authority.md`](external-pool-adapter-artifact-source-authority.md)、[`external-pool-adapter-release-lifecycle-authority.md`](external-pool-adapter-release-lifecycle-authority.md) 与既有 external-pool 权威/验收 |
