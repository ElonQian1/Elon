...
首段平台参考曲线不冒充上述真实价格发现。v223 Store/application 采用 `platform_reference_curve:<curve_id>` 来源身份、`source_kind=fallback_curve` 与 `sample_count=0`：一名管理员提交精确 Offer/窗口/价格批次，另一名独立复核，再在一个事务中直接生成唯一 v171 Snapshot；v224 已补齐管理员 HTTP/MCP 与 TTL/升级门卫。定向 HTTP/MCP、原子 Snapshot、拒绝零副作用、迁移和文件重开已有证据，但它不是 `index`、`mark`、`trade`、YCI、真实市场样本或平台签名价格源，完整边界见 [`platform-reference-price-curve-authority.md`](platform-reference-price-curve-authority.md) 与 [`platform-reference-price-curve-api-acceptance.md`](platform-reference-price-curve-api-acceptance.md)。
...
[`capacity_future` pricing-mode settlement lineage bridge](capacity-future-settlement-lineage-authority.md) 是已登记、当前 `blocked` 的独立 API-free reference-only V1，把 Instrument/activation/adoption、Commitment、exercised Allocation 与既有 execution、verification、settlement/release carrier 相连。v193 decision digest 精确绑定 v192 event；v192 verification-role 与 v195 settlement-role digest 分字段保存并禁止直接判相等。Store resolver source 只以 Lease 为根，在单一 Deferred snapshot 内重审历史 owner、共同 contract multiplier、meter 顺序/granularity、whole-only Claim transfer，并把 Reservation 下 v228 Claim index/cardinality 漂移作为 integrity failure；v195 payee 按 historical Provider settlement-account fallback 审计。它未编译、未运行、无 Service/API/migration/writer，也不创建 ClearingReceipt、短缺或处罚，因此不能把当前市场升级为交付清算已实现。
...
消费者 `accepted` 同样不能直接触发扣款或收益释放。它只能作为 Verification 的一项输入；平台仍需结合 Provider 声明、平台观测、验证策略、重复执行或挑战证据形成独立决定。
...
7. `capacity_future` 交付结算 retained lineage（已登记且 `blocked`；API-free Domain 与 Lease-rooted Store resolver source 已写，编译、运行和动态验收仍缺）；
8. 限价订单簿、成交、持仓和净额；
...
`capacity_future` settlement lineage bridge 同样不继承上述任一运行证据；其 Domain/source equations 与 Store retained resolver 当前为 `registered_blocked/source_written/implementation_uncompiled/implementation_unrun`、`passed=0/failed=0`。源码中的历史 owner audit、共同合约倍数与双摘要域复算均尚未编译或定向运行，不能把 reference bridge 解释为真实计量、交付清算、Provider 收益或资金结算已闭合。
