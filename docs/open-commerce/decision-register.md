...
| 删除证明进入可携带包必须发布 V5 | V5 在同一读事务中加入商户未核验删除证明；V1-V4 缺省空字段且空值不序列化，旧版本显式夹带证明失败关闭 | `docs/decisions/open-commerce-consumer-portability-exports-v5.md` |
...
| 接入器读取待办必须使用显式扩权的短时租约 | 既有凭据默认只写；只有明确加入 `business_handoff.claim` 才能一次领取一条结果；续租不超过首次领取后 1 小时，完成原子绑定回执；拒绝后有界退避，第 6 次拒绝暂停并要求编辑者恢复 | `docs/decisions/open-commerce-adapter-handoff-claims-v1.md` |
...
| 可携带偏好采用必须允许消费者逐字段最小选择 | 只合并预演中真实变化且由用户勾选的五类低敏字段，未选字段保持当前值；继续保存完整前后快照并保留并发安全回滚 | `docs/decisions/open-commerce-consumer-portability-selective-adoption-v1.md` |
...
2. `cofficethinking` 参考节点的真实服务器、真实 PostgreSQL、库存事务、幂等下单和原 ERP 回读子链已完成隔离验收；下一步补齐一龙平台经标准 443 HTTPS、真实消费者身份和生产凭据的公网纵向调用，同时保持支付与外部平台授权边界清晰。
...
| ESK 可消费经济参与权 | 已接受，真实支付/分配未实现 | 单一 ESK 同时用于服务、治理和团队可分配利润快照；订单、资产、利润与量化份额仍分账，见 `docs/decisions/esk-consumable-economic-participation-v1.md` |
| RevenuePosition 特定合同收入权益 | 融资提案 | 仅在 ESK 一般团队利润参与之外确有独立、封顶、到期且可核验的收入来源时评估 |
...
- 在没有版本化政策、独立账本和不可变快照时，模糊地用一个代币冒充法定公司股权、基金份额、服务订单和客户本金。ESK 已接受的服务、治理与团队利润参与组合是明确例外，但不等于法定股权或 `QSHARE`。
