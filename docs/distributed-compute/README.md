...
| 外部算力池适配器与统一报价 | v221-v265 前置门为 `implementation_partially_verified`。V265 在固定 child 与 loopback TLS fixture 中组合 V263 Secret delivery、V264 Broker channel 和一次性 ELNW request/response；Windows `6 passed`、Linux core `2 passed`、WSL2 kernel `1 passed` 且 cgroup 完成清理。当前没有生产 upstream/Secret、durable observation、route/activation；V254 18 deny 不变。见 [`V265 no-work 权威`](external-pool-adapter-authenticated-no-work-probe-authority.md)、[`V265 验收`](external-pool-adapter-authenticated-no-work-probe-acceptance.md)、[`V264 Broker TLS`](external-pool-adapter-broker-tls-transport-authority.md) 与 [`当前状态`](current-implementation-status.md) |
...
Provider Adapter 的 v227-v265 前置门已形成分层权威。V260 已验证 authenticated session，V261 验证 Linux supervisor，V262 组合真实 exec 后 runtime，V263 向同一受限 child 交付 V256 短时 config/credential，V264 由 server broker 认证 V258 exact TLS target。V265 又在固定本地 fixture 中完成 Adapter-generated bounded request、exact response、child semantic validation、post-exchange reproof 和有界回收，但没有生产 upstream、真实 Secret 或 durable readiness。下一硬门是生产 Adapter 兼容性与受控 upstream 验收，并与 atomic activation 同批设计完整 admission gate；V254 18 deny 逐字保留。见 [`V265 no-work`](external-pool-adapter-authenticated-no-work-probe-authority.md)、[`V264 Broker TLS`](external-pool-adapter-broker-tls-transport-authority.md)、[`V263 Secret delivery`](external-pool-adapter-ephemeral-secret-delivery-authority.md) 与 [`current-implementation-status.md`](current-implementation-status.md)。
