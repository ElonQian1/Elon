//! Exact frozen-member authority for the 88-member q18 created-first shared-busy tranche.
...
use super::{
    range_mask_v1, LockCreatedFirstSharedBusyCloseSucceededCompletionV1,
    CREATED_FIRST_SHARED_BUSY_CLOSE_SUCCEEDED_MEMBER_COUNT,
};
...
pub(super) fn exact_member_v1(
    ...
    if observed_rows != CREATED_FIRST_SHARED_BUSY_CLOSE_SUCCEEDED_MEMBER_COUNT {
        return Err(LockRunnerExecutionViolationV1::MemberCatalogInvalid);
    }
...
fn expected_row_v1(row: usize) -> Option<ExpectedRowV1> {
    if row == 0 || row > CREATED_FIRST_SHARED_BUSY_CLOSE_SUCCEEDED_MEMBER_COUNT {
        return None;
    }
    let ordinal = row - 1;
    let request_ordinal = ordinal / 2;
    let completion = if ordinal % 2 == 0 {
        LockCreatedFirstSharedBusyCloseSucceededCompletionV1::RetentionSucceeded
    } else {
        LockCreatedFirstSharedBusyCloseSucceededCompletionV1::RetentionRouteUnknown
    };
...
const fn completion_tag_v1(
    ...
    match completion {
        LockCreatedFirstSharedBusyCloseSucceededCompletionV1::RetentionSucceeded => "succeeded",
        LockCreatedFirstSharedBusyCloseSucceededCompletionV1::RetentionRouteUnknown => {
