...
| 开发者 App 签名 Webhook | 代码已形成、未编译，仅沙箱事件 | App 所有者可配置白名单内标准 443 HTTPS 回调，并在创建时固定选择成功、失败或全部终态事件；新订阅须先完成签名 challenge 精确回显，验证后选中事件才进入耐久队列，工作器按短租约、指数退避、死信和自动停用规则处理。challenge 与每笔实际投递会拒绝任一私网或特殊 DNS 地址、禁用代理和重定向并固定本次公网连接。密钥只在创建或显式轮换时显示一次，轮换会停用订阅并要求重新验证；单条死信可重新排队，也可幂等补发最多 100 条历史通知。自动入队、历史补发、死信重试和工作器读取均拒绝生产调用，生产 Webhook 尚未设计。当前尚未完成编译、迁移、DNS、TLS、并发、网络、跨语言签名和 UI 验证，也不是工商身份、跨运营方总线、支付回调或业务动作重放 | `server/src/open_commerce_webhook_*.rs`、`server/src/open_commerce_outbound_security.rs`、`server/src/store/open_commerce_developer_webhook*.rs`、`docs/open-commerce-invocation-credential-provenance-v1-acceptance.md` |
