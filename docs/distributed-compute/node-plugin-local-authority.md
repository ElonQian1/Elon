...
SQLite 内部 revision、trigger 和 CAS 只能阻止当前数据库中的正常写入回退，不能独立识别“整个数据库文件被替换成一份更旧但内部自洽的副本”。当前 `implementation_uncompiled` 代码已可在同一 deferred snapshot 中重读 `authority_meta` 与库存 JSON，重算 inventory JCS 摘要并核对插件结构、共享授权、keyring、环境摘要、全部 revision/epoch 和可信时间高水位，再生成不含库存正文及原始授权引用的规范回滚检查点与 JCS 摘要；时钟不可信或任一事实不一致都拒绝生成。
...
以上新增代码仍为 `implementation_uncompiled`：未编译、未测试、未执行 DDL，也未在 NodeAgent 启动或 Host 路径接线。authority v3 已覆盖 PlanApply、可恢复分段取数、损坏终结、候选原子验证、schema 定义指纹、可信时间、回滚锚查询/首次注册/更新合同，以及 ZIP 规范解包计划、create-new 安全物化、staging seal、retained-handle 复验和 staging Store。最新 staging 事务会在同一 `BEGIN IMMEDIATE` 中重读 verified/current authority，推进可信时间与 state/inventory/authority 三类 fence，撤销旧 prepared claim/run，插入不可变 receipt，并把槽从 `verifying` 原子推进到 `staged`；结果不确定时只保留绑定完整 before/after 预期的 recovery key 与文件 custody。recovery session 现可重验 provenance、不可变 receipt/evidence、inventory 和 authority，并稳定读出 `NotCreated` 或 exact `Staged`；碰撞、漂移与回滚都失败关闭。exact `Staged` adoption 会消费 recovery、二次读取同一结果并从原句柄重哈希全部文件和 seal 后恢复 staged custody，任一失败继续返还 recovery；`NotCreated` 不会恢复写 permit。进程内候选健康评估合同现可消费该 staged custody，固定 release/Runner/staging receipt、连续成功或失败阈值、追加式探针 transcript 与严格晚于末次探针的可信时间，并输出最长五分钟的 `ValidatedCandidateHealthPublication`。它仍不是 health Store 回执、installed 状态、promotion permit 或 ReadyCapability。目录耐久闭包、跨重启恢复、失败 staging 治理、真实 Sidecar 探针、health Store/恢复、installed/promotion/GC 仍未形成。生产取时/key resolver、认证传输与真实回滚见证、socket-read 取消、真实 HTTPS downloader、Host/Sidecar/IPC/沙箱仍待实现；`staged` 不等于 installed、healthy、promotable、running 或商业 Verification。解包边界见 `node-plugin-archive-extraction.md`，健康边界见 `node-ready-capability.md`。
