...
| 外部算力池适配器与统一报价 | v221-v261 前置门为 `implementation_partially_verified`。V260 已验证 authenticated child-only session；V261 又通过 source-contract、Linux-musl product/test build 和 3 项 WSL2 kernel fixture，验证 clone3/cgroup/namespaces/private root/fd custody/seccomp/pidfd/cleanup。两者尚未形成 exec 后 authenticated runtime，也未读取真实 secret、连接 upstream target 或生成 probe/route/activation；V254 18 deny 不变。见 [`V261 supervisor 权威`](external-pool-adapter-linux-supervisor-authority.md)、[`V261 验收`](external-pool-adapter-linux-supervisor-acceptance.md)、[`V260 session 权威`](external-pool-adapter-authenticated-child-session-authority.md) 与 [`当前状态`](current-implementation-status.md) |
...
Provider Adapter 的 v227-v261 前置门已形成分层权威。V260 已验证 anonymous seqpacket/seed pipe、HKDF/HMAC mutual bootstrap、双向 frame 与 terminal fail-close；V261 又以 3 项 WSL2 kernel fixture 验证真实 clone3/cgroup/namespaces/private root、fd3/fd4/fd5 custody、seccomp、pidfd 与 cleanup，并实际执行 sealed minimal capsule。V260 与 V261 仍是两个独立内核，尚未运行 exec 后 mutual bootstrap，也未读取真实 secret、连接 V258 target 或生成 probe/activation。下一硬门是组合 authenticated runtime，再做 broker TLS/upstream no-work probe。V254 18 deny 逐字保留，完整 admission gate 只能与 atomic activation 同批替换。见 [`V261 Linux supervisor`](external-pool-adapter-linux-supervisor-authority.md)、[`V260 authenticated session`](external-pool-adapter-authenticated-child-session-authority.md)、[`V259 supervisor/session companion`](external-pool-adapter-supervisor-session-policy-companion-authority.md) 与 [`current-implementation-status.md`](current-implementation-status.md)。
