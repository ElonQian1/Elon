...
A2a/A2b1 的 exact logical-name routes 共享 WAL/SHM runtime，但每条 Connection 独立 route/authorizer/custody；完整 terminal projection、`SourceBranch`、`Expected`、`CaseKey`、exclusion 与 StaticContract 已闭合为 Map `43476/43476`、Lock `8668/8668`。typed quotient 只有前序验证基线；current runner-admission source 未编译、未运行。Map q4 `MapRegionLoopSuccessV1` 为 511 frozen/509 net-new，未运行 inventory 预期 `521` present、`42,955` missing。Lock q12–q17 六个 88-singleton slice 事实不变；q18 `LockNativeAcquireCreatedFirstSharedBusyCloseSucceededV1` 再提供 88=`16+72` singleton（44/44 completions），只匹配 `dms.created-first.shared-busy.close-ok`。future controlled-fault seam 必须让同 `FileId` 的 distinct holder/target handles 产生真实争用，分账两侧 attempts 并显式观察 target close success；Expected 为 mutation known、lock certain、file/DMS Released、native lock/unlock=`2/1`、`cleanup_rewrite=false`，协议=`a2lockq18`（186 scalars），catalog=88 rows/18,122 bytes、SHA-256=`4f78ff1678c93b1c06bad92e838423e4202598fd8e0b5b83f79cde0c528a07cd`。精确 matcher、seam 与证据等级只见[`Lock dynamic tranches authority`](node-plugin-vfs-lock-dynamic-tranches-authority.md)。Lock q1–q18 inventory（m/g）=`4284/4284 present,4384/3856 missing,8668/8140 total`；q12–q18=`616/616`，initialization remaining=`2816/2288`。以上均为 `source_written/source_review_only/implementation_uncompiled/implementation_unrun`，`passed=0 failed=0 actual=not_run`；coverage=`0/8668`，无 actual/reviewed digest/frozen manifest，`Qlock=unknown`、`WindowsDynamic=not_opened`、production/economic closed。19-artifact refresh 接受前必须覆盖 q15–q18，禁止半套 owner 重绑；A2b2=`117/117` 不能替代该门。
...
当前静态验收已闭合为 Map `43476/43476`、Lock `8668/8668`；projector/candidate 只保留前序验证基线。Lock q12–q18 七个 initialization slices 各为 source-only 88 singleton；q18 是 CreatedFirst shared-busy/close-ok 切片，要求 same-FileId distinct holder/target 的真实 contention source seam、attempt 分账与 explicit target close success，不能以注入 busy 或 `Drop` 代替。未运行 inventory（m/g）=`4284/4284 present,4384/3856 missing,8668/8140 total`，q12–q18=`616/616`，initialization remaining=`2816/2288`。current source=`source_written/source_review_only/implementation_uncompiled/implementation_unrun`，`passed=0 failed=0 actual=not_run`；无 actual/reviewed digest/frozen manifest，`Qmap/Qlock=unknown`、coverage=`0/43476 + 0/8668`、`WindowsDynamic=not_opened`、production/economic closed。19-artifact refresh 接受前须覆盖 q15–q18。只有补齐全部 program source、冻结 exact quotient manifests 并在同一 clean Windows cohort 逐 class 产生正式记录，才可形成 `Qmap/Qmap` 与 `Qlock/Qlock`；q9–q18 细节只见[`Lock tranches`](node-plugin-vfs-lock-dynamic-tranches-authority.md)。
