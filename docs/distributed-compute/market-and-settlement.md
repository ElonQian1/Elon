...
V1 每份 Reservation 只绑定一个 Pool、一个精确 UTC 半开交付窗口 `[starts_at, ends_at)` 和多个 meter；不在一个 Reservation 内跨 Pool 或跨窗口。领域合同、reducer 与 v165 SQLite schema 骨架已经写入但未编译、未迁移；尚无原子 Store、Broker 接线或真实容量操作。
