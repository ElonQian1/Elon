...
fn load_sources(workspace: &Path, symbols: &[RustSymbol]) -> HashMap<String, Vec<String>> {
...
fn collect_trait_implementations(index: &RepoContextIndex) -> Vec<ImpactFact> {
    let traits = index
        .rust
        .symbols
        .iter()
...
fn collect_public_api_references(index: &RepoContextIndex) -> Vec<ImpactFact> {
    let by_id = index
        .rust
        .symbols
        .iter()
...
fn collect_async_boundaries(index: &RepoContextIndex) -> Vec<ImpactFact> {
    index
        .graph
        .ranked_symbols
        .iter()
...
fn extract_body_lines(symbol: &RustSymbol, sources: &HashMap<String, Vec<String>>) -> Vec<String> {
    sources
        .get(&symbol.path)
        .into_iter()
        .flat_map(|lines| {
...
fn impl_mentions_trait(signature: &str, trait_name: &str) -> bool {
...
fn field_access_kind(line: &str, field: &str) -> Option<ImpactKind> {
