...
| 节点插件治理合同 | Signed Manifest/InstallPlan、双 keyring、authority v3/PlanApply、可恢复受管取数、候选 exact closure/hash 与原子 resolution、schema 定义指纹、可信时间、ZIP 安全物化和 staging Store 均已写。`PinnedComputePluginRoot` 以 existing-only 根句柄与 share-none 持久锁形成跨进程独占，取数、验证和 staging custody 以非 Clone lease 保活同一锁句柄。staging 结果可稳定恢复 `NotCreated` 或 exact `Staged`，exact adoption 会从原句柄重哈希文件和 seal。staged 候选现可进入有界健康状态机；fresh authority read 后，Store 可原子写入不可变健康回执并推进 trusted-time high-water，写入结果不确定时可读出 `NotCreated` 或 exact `Recorded`，exact adoption 还会 fresh read 并重哈希 retained staging 文件/seal。该代码已通过 `elon-pc-node` 编译及 9 项健康/schema 定向测试，schema DDL 仅在内存 SQLite 验证建库与重开。默认关闭的 Bootstrap 已挂入 NodeRuntime，但 sharing-on、生产数据库/时间源/回滚见证、真实 HTTPS downloader、Sidecar/IPC/探针调度、失败候选治理、installed/promotion、ReadyCapability 上报和 Attempt 接线仍缺。staged 或耐久健康回执不等于 installed、ready 或商业 Verification |
...
目标流程仍是：共享关闭时不下载重型组件；开启后按硬件和任务选择签名插件、运行时与模型工件。本机耐久真源已选定为独立 SQLite，根签名双 keyring、原子计划应用、候选所有权、三段式下载认领、候选级全工件验证和可信时间边界见 `docs/distributed-compute/node-plugin-local-authority.md`。raw verified 只把本机槽推进到 `verifying`，不授权解包、健康、promotion 或商业 Offer。staged 候选已有进程内健康状态机和耐久健康回执 Store；Store 只在 fresh authority binding 未漂移时写入不可变回执并推进可信时间高水位，不会推进 installed 或 promotion。不确定结果恢复只允许 `NotCreated` 或 exact `Recorded`，后者经 fresh read 与 retained-handle 重哈希后才恢复 typed custody。代码已通过 `elon-pc-node` 编译与 9 项定向测试，但内存库测试不代表生产本地库已建立。候选观察值仍由 Host 调用方提供，尚未证明真实 Sidecar 已运行；生产时间权威 key resolver、真实网络取时、真实下载器、Sidecar/IPC/探针调度、失败候选治理、installed/promotion、云端 capability gate 和通用 Attempt 协议接线仍未实现。
