//! Canonical q17 child header for existing-first unread truncate and cleanup-release receipts.

pub(in super::super) const REPORT_VERSION: &str = "a2lockq17";
pub(in super::super) const REPORT_VALUE_COUNT: usize = 172;
...
pub(in super::super) fn selector(
    ...
    Ok(format!(
        "initialization-{action}-first-{first}-count-{count}-existing-first-truncate-error-release-failed-{completion}-terminal-route-unknown"
    ))
}
...
mod tests {
    ...
    #[test]
    fn eighty_eight_q17_selectors_are_unique_and_fail_closed() {
