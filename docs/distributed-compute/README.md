...
| 外部算力池适配器与统一报价 | v221-v251 前置门为 `implementation_partially_verified`；V243/V244 保留短时凭据与可撤销采用的局部通过证据，V245-V247 安装链及 V249/V251 registry 链已通过对应本地定向验收。V250 增加可续签/可撤销的 scanner 签名漏洞情报链，但仍为 `implementation_uncompiled`、`passed=0`；V250 不执行扫描或证明情报真实，V252 sandbox re-attestation 仍缺，Provider 未激活且 Adapter 未启动。见 [`当前状态`](current-implementation-status.md)、[`V250 漏洞情报续签权威`](external-pool-adapter-vulnerability-reattestation-authority.md)、[`V249 registry 权威`](external-pool-adapter-registry-authority.md) 与 [`V247 安装/撤销权威`](external-pool-adapter-installation-authority.md) |
...
Provider Adapter 的 v227-v251 前置门已形成分层权威；V244 汇合当前 V239/V243 为可撤销采用授权，V245-V247 增加 legacy 栅栏、可撤销 `installed_inert` 内容树与 sealed seam并通过安装链定向验收；主线 V248 属于 ERP，V249 顺延登记 Provider-neutral release 与 Provider-specific companion，V251 修复合法 JSON 键序兼容并随 registry 链通过定向验收。独立的 V250 为 exact neutral release 建立可续签、可撤销的 scanner 签名漏洞情报链，但尚未编译或运行。最新状态见 [`current-implementation-status.md`](current-implementation-status.md)，漏洞情报续签边界见 [`V250 vulnerability re-attestation`](external-pool-adapter-vulnerability-reattestation-authority.md)，registry 边界见 [`V249 Provider-neutral registry`](external-pool-adapter-registry-authority.md)。服务器不运行扫描、不证明 intelligence 真实性、不删除或执行安装字节、不返回原始凭据，也未激活 Provider；后续 activation 仍须 fresh credential、V252 sandbox re-attestation，并与精确 service actor、Provider-specific route compatibility binding 和 Provider activation 同事务。
