...
| Private full-conversation public sharing | Standard personal-account legacy/redesigned transaction, selected-branch binding, native confirmation and validated Copy/share result implemented; full-URL context checks corrected | Included in 131 focused sharing/delete/mutation Node cases; nine pure Kotlin policy/receipt tests pass; native consumer/MCP tests await grouped execution | Source-only for grouped APK; [contract and other sharing scopes](chatgpt-private-conversation-share.md); no actual link created yet |
...
| Private existing public-link management | Conversation-scoped native list/Copy/confirmed revocation, account-bound 60-second cache, consumed selection ticket and complete readback implemented; no source-conversation deletion | 152 combined Node and 13 pure Kotlin tests passed; native consumer/MCP test and UI compilation pending | Source-only for grouped APK; [contract, browser evidence and remaining scopes](chatgpt-private-shared-links.md) |
| Direct official-runtime text submission | Native input invokes the current website submit transaction without filling the official editor or waiting/clicking its send button; existing send ledger and stream observer retained | 60 focused Node cases pass, including current-context guards, single-writer uncertainty and production asset integration; new native receipt tests await grouped execution | Source-only for grouped APK; [contract and evidence](chatgpt-official-runtime-text-submit.md); not an independent private HTTP sender or live acceptance |
...
| Explicit attachment upload-copy choice | Production ChatGPT pending-file menu retains a per-selection choice; the existing uploader bypasses reuse/reservations without changing privacy intent, and cannot silently fall back after an explicit copy choice | 247 focused Node and 15 Kotlin/JUnit cases pass; menu/preview/model/send-owner sources have SDK partial compilation, not a full consumer build | Source-only, adapter 287; [scope and acceptance](chatgpt-private-attachment-library-reuse.md#explicit-upload-copy-choice); grouped popup/private-byte/reply acceptance pending |
...
| Share/delete/conversation files | Native pin/rename/archive/move; private file index plus ordinary/project/library-linked, [simple image-pointer](chatgpt-private-image-download.md), [imported connector-copy](chatgpt-private-connector-file-download.md) and [shared-library attachment download candidates](chatgpt-private-shared-library-download.md), now with in-app progress, request-bound cancellation and [durable crash cleanup](chatgpt-private-download-recovery.md); guarded flag-selected private delete and current-chat reset; [ordinary full-conversation private share candidate](chatgpt-private-conversation-share.md) | Verify saved-file downloads, in-app progress/cancel, Android crash cleanup, live flag binding, current/noncurrent deletion and share creation/update. Complete standalone/mounted-library, metadata-only shared references, connector-only cloud-reference and parameterized-image downloads, plus remaining sharing variants. Binary process-death resume remains unsupported. System distribution alone is not official share creation. |
...
parameterized pointers and the other explicitly listed scopes remain gaps.
