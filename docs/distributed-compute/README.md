...
| 外部算力池适配器与统一报价 | v221-v253 前置门为 `implementation_partially_verified`；V243/V244 保留短时凭据与可撤销采用的局部通过证据，V245-V247 安装链及 V249/V251 registry 链已通过对应本地定向验收。V250 scanner 签名漏洞情报链已通过 migration 4 项与进程内 HTTP 5 项；V252 sandbox verifier 签名声明链已通过 migration 7 项与进程内 HTTP 2 项。V253 Provider-specific credential re-attestation 源码已写入，但未编译、未执行 migration 或运行测试/服务，`passed=0`。三条链不执行扫描、真实沙箱或 secret 解析，Provider 未激活且 Adapter 未启动。见 [`当前状态`](current-implementation-status.md)、[`V253 凭据续签权威`](external-pool-adapter-credential-reattestation-authority.md)、[`V252 沙箱续签权威`](external-pool-adapter-sandbox-reattestation-authority.md)、[`V250 漏洞情报续签权威`](external-pool-adapter-vulnerability-reattestation-authority.md)、[`V249 registry 权威`](external-pool-adapter-registry-authority.md) 与 [`V247 安装/撤销权威`](external-pool-adapter-installation-authority.md) |
...
Provider Adapter 的 v227-v253 前置门已形成分层权威；V244 汇合当前 V239/V243 为可撤销采用授权，V245-V247 增加 legacy 栅栏、可撤销 `installed_inert` 内容树与 sealed seam并通过安装链定向验收；主线 V248 属于 ERP，V249 顺延登记 Provider-neutral release 与 Provider-specific companion，V251 修复合法 JSON 键序兼容并随 registry 链通过定向验收。独立的 V250 为 exact neutral release 建立可续签、可撤销的 scanner 签名漏洞情报链，已通过 migration 合同 4 项与进程内 Axum HTTP 5 项；V252 再建立 Provider-neutral 六能力 sandbox verifier 签名链，已通过 migration 合同 7 项与进程内 Axum HTTP 2 项，且不接收 Prepared、不证明 live-FS/installed-instance current；V253 建立 exact Provider binding 的凭据签名续签/撤销链、紧邻 active 桥与 active 续签，但源码未编译或运行，`passed=0`。最新状态见 [`current-implementation-status.md`](current-implementation-status.md)，凭据续签边界见 [`V253 credential re-attestation`](external-pool-adapter-credential-reattestation-authority.md)，其余分层见 V249/V250/V252 专题文档。服务器不运行扫描、真实沙箱或 secret 解析，不删除或执行安装字节、不返回原始凭据，也未激活 Provider；后续 activation 仍须将 V249 companion/sealed Prepared、current V250/V252/V253、精确 service actor、Provider-specific route compatibility binding 和 Provider activation 置于同一事务。
