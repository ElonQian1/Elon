...
| `server/src/store/compute_capacity_*.rs`、`compute_{provider,offer,job,reservation}_registry*`、`compute_quote_candidates.rs`、`compute_broker_reservation/`、`compute_federation_broker_{service,api,mcp}.rs` | 共享容量池、版本化合同、项目级 Job 创建/既有候选发现与锁价、本人 Job/Reservation 查询、本地原子 Reserve、未执行任务原子 Release/Expire 与 HTTP/MCP 控制面；当前未编译、未迁移、未运行验证，未接新报价生成、自动撮合、Attempt 或运行中结算 |
