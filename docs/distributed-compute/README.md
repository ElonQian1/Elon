...
| 外部算力池适配器与统一报价 | Provider onboarding v221 与 Adapter release 四眼复核→immutable staged admission v222 的领域、Store-private 来源源码已写并可编译，且随完整临时文件 Store 迁移和两次重开执行；两条业务状态机、trigger 行为与 service/API 仍未运行，状态为 `implementation_unwired`。它们只保存候选来源，不验证 artifact/verifier、不写 v213、不建 Pool/Offer 或派发，也没有 service/HTTP/MCP/PC 入口。见 [`external-pool-adapter-authority.md`](external-pool-adapter-authority.md) 与 [`external-pool-adapter-release-authority.md`](external-pool-adapter-release-authority.md) |
