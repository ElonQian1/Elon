...
| Provider / Offer / Job / Reservation / Lease / Receipt 统一领域合同 | 基础代码、v169-v176 注册表、Broker Reserve/未执行任务终态、v185-v198 Attempt 激活、证据链、可信终态、容量收口、待结算回执、挑战、决议与 pending 释放已写，状态为 `implementation_uncompiled`，尚未迁移、完整接线和运行验证 |
...
| Attempt 待结算原子释放 | v198、追加式 Store、Release Posting/账本腿与消费者/管理员 HTTP 已写；满 72 小时且挑战为 none/rejected/withdrawn 时，管理员可把 Provider/平台对应金额从 pending 原子转入 available。open/accepted 阻断，available 不等于提现或外部付款，尚未编译、执行迁移或运行验证 |
...
28. `docs/distributed-compute/attempt-settlement-release-api.md`：72 小时后 pending 到 available 的原子释放、账本与非提现边界。
29. 现有兼容实现：`docs/decisions/node-compute-sharing-supply-v1.md`。
