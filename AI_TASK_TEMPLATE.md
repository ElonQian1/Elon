...
- 不应改变：
- 需要发布：是 / 否 / 由项目默认规则决定
- 若为 APP 纯视觉微调：是否满足 `docs/app-ui-fast-lane.md`
...
3. 先定位和规划，按 `WF-DEDUP` 固定 `TASK_BASE_SHA` 并查远程已有实现；完整重叠转为复用/审查，同路径不同符号只补缺口，同符号或边界不清时停止重复实现并记录协调项。
4. 再做最小安全改动；不回退或夹带其他任务。按 `WF-FILES` 处置源码、测试和临时产物，并运行风险匹配的验证。
