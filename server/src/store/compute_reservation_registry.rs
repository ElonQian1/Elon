...
impl Store {
    pub(crate) fn register_compute_reservation(
        ...
        if let Some(current) =
            current_reservation_projection_on(&tx, reservation.reservation_id.trim())?
        {
            let stored =
                reservation_version_on(&tx, &current.reservation_id, current.current_revision)?
                    .ok_or_else(|| anyhow!("算力 Reservation 当前历史版本缺失，拒绝继续写入"))?;
            let current_reservation = audited_reservation_on(&tx, Some(&current), &stored)?;
            ...
            let dependencies = registered_dependencies_on(&tx, reservation)?;
            ensure_current_job_and_claim_on(&tx, reservation, &dependencies)?;
        ...
        let dependencies = registered_dependencies_on(&tx, reservation)?;
        ensure_current_job_and_claim_on(&tx, reservation, &dependencies)?;
        ensure_live_creation_dependencies_on(&tx, reservation, &dependencies)?;
        let reservation_digest = validate_with_dependencies(reservation, &dependencies)?;
        ...
        insert_reservation_version(&tx, reservation, 1, &reservation_digest, &reservation_json)?;
        tx.commit()?;
