...
当前 `server/src/compute_federation/` 提供未编译、未完整接线的云端领域合同；`server/src/store/compute_capacity_*.rs` 与 Provider、Offer、Price Snapshot、Job、Reservation Registry 已形成隔离的 CapacityPool、供给、Claim、Store-canonical 请求摘要、可组合事务内 Claim kernel、账本审计、到期恢复、生命周期、不可变历史与锁价快照。本人 Provider HTTP/MCP 控制面只允许创建和查询服务端固定为 `registering/self_declared` 的 `user_node` 或 `managed_cluster`；它不接受外部矿池、路由、凭据、适配器或 verified 证据，并返回脱敏视图。本人 CapacityPool 控制面只在自有 Provider 下创建和查询 `registering` Pool，可从不可变账本重算当前 epoch 的余额一致性，并按稳定序号分页读取省略消费者和业务因果字段的事务与双分录；审计健康和历史存在都不代表硬件 verified。Bucket 控制面只在当前 Pool 版本下创建 open、零发行余额交付窗口并读取账本投影；Supply 控制面在显式确认后向同一窗口原子追加 self-declared `issuance -> available` 双分录，或仅把 available 撤入 retired，且 registering 阶段也可纠正尚未交易的声明。服务端固定版本和首次时间并生成摘要，响应不返回原始资源范围密钥或档案；这些入口不激活、验证或发布 Offer。Hold V2 固定完整 causal binding，Reservation Claim 强制绑定 Offer/Job/Reservation，Finish 继承并审计原始 held 绑定；Job、Reservation 登记和余额预授权、退款均已有不提交的事务内入口。v175 `compute_broker_reservation` 已将这些构件组合为单一事务的第一版 Reserve；v176 在 Attempt 尚未激活时原子完成退款、held Claim Release/Expire、Job 与 Reservation 终态。v185 增加首个 Attempt 已接受登记：Provider 所有者提供外部执行器接受引用并显式确认后，单事务把 Claim `held -> active`、Job `reserved -> running`、Reservation 绑定推进到新精确版本，并保存 staging Lease 与不可变回执。v186 为该 Lease 建立版本化当前状态，Provider 所有者可在精确 revision/digest/fencing 栅栏下提交外部心跳引用、延长软期限；过期 Lease 不可复活。v185/v186 都保持原平台人民币预算预授权和 active 容量不变，不发送节点命令，也不验证外部接受或心跳签名。登录用户 HTTP 与项目范围 MCP 已提供本人 Job/Reservation 列表、详情和 Reserve、Release、Expire；Attempt 激活和续租写入仅为 Provider HTTP，Provider 所有者与消费者可读取激活回执与当前状态。整条路径仍是 `implementation_uncompiled`，只支持 `platform_balance_cny`，尚未执行迁移和接口运行验证，也未实现真实节点派发、Lease 超时归还或运行中实际用量结算。`server/src/node_agent_compute_plugin_host/` 已增加未编译的 Manifest、InstallPlan、双槽生命周期、ReadyCapability、Attempt 命令、RFC 8785 JCS/Ed25519 验证、Manifest 语义校验、本机计划准入与逐段取数再授权 trait，以及 typed event 合同；耐久 inventory store、应用 journal、真实下载器、Sidecar 与通用协议仍未接线。现有节点模型白名单、Token 预留和流租约继续作为 `user_node + llm_chat` 兼容路径。权威架构、协作去重协议与阶段见 `docs/distributed-compute/README.md`。
...
v196 再允许原 Job 消费者在 Settlement Receipt 创建后的固定 72 小时内提交一份不可覆盖挑战。挑战精确绑定 v195 receipt/posting 和完整身份链，只记录原因、摘要与有界证据引用；它不撤销结算、不退款、不移动 pending，只作为后续释放必须失败关闭的门卫。撤回、裁决、纠正、余额释放和提现均未实现，状态为 `implementation_uncompiled`，边界见 `docs/distributed-compute/attempt-settlement-challenge-api.md`。

v197 为每份 v196 挑战增加一份追加式终态决议。原消费者只能撤回，平台管理员只能接受或驳回；`open/accepted` 阻断未来释放，`withdrawn/rejected` 解除挑战门卫，但所有操作都保持余额不变。纠正、退款、pending 释放和提现仍未实现，状态为 `implementation_uncompiled`，边界见 `docs/distributed-compute/attempt-settlement-challenge-resolution-api.md`。
...
| `server/src/node_agent_compute_plugin_host/` | Windows 节点 legacy Host seam、按需插件、签名验证与本机 InstallPlan 准入、Attempt、Runner event 合同；当前只有旧 LLM 接入，耐久 key resolver、库存/取数权威源、下载与正式运行路径未接线 |
