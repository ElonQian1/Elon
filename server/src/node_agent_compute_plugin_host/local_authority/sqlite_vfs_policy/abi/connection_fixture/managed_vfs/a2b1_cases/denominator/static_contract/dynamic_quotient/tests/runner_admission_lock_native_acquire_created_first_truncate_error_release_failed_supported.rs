//! Windows isolated controlled-fault admission for every exact q16 member.
...
use super::lock_native_acquire_created_first_truncate_error_release_failed_cases::{
    ...
    FrozenCreatedFirstTruncateErrorReleaseFailedCompletionV1,
...
const EXACT_TEST: &str = "node_agent_compute_plugin_host::local_authority::sqlite_vfs_policy::abi::connection_fixture::managed_vfs::a2b1_cases::denominator::static_contract::dynamic_quotient::tests::runner_admission_lock_native_acquire_created_first_truncate_error_release_failed_supported::isolated_q16_receipts_are_exact_controlled_fault_actual";
...
fn isolated_q16_receipts_are_exact_controlled_fault_actual() -> anyhow::Result<()> {
    ...
    for (&case, leaf) in frozen_lock_created_first_truncate_error_release_failed_leaves_v1() {
        ...
        let descriptor = lock_created_first_truncate_error_release_failed_descriptor_v1(
            case,
        ...
        .map_err(|error| anyhow::anyhow!("supported q16 member {case:?} failed: {error:?}"))?;
    ...
    match selected_child {
        Some(selected) => Err(anyhow::anyhow!(
            "parent-selected q16 member was not found: {selected}"
        )),
        None => {
            assert_eq!(
                parent_receipts,
...
const fn runner_completion_v1(
    completion: FrozenCreatedFirstTruncateErrorReleaseFailedCompletionV1,
