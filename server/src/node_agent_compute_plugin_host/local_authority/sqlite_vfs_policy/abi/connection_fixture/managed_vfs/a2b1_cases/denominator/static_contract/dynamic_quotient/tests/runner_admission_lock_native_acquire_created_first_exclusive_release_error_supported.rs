//! Windows isolated controlled-fault admission for every exact q12 member.
...
use super::lock_native_acquire_created_first_exclusive_release_error_cases::{
    ...
    lock_created_first_exclusive_release_error_descriptor_v1,
    FrozenCreatedFirstExclusiveReleaseCompletionV1,
...
const EXACT_TEST: &str = "node_agent_compute_plugin_host::local_authority::sqlite_vfs_policy::abi::connection_fixture::managed_vfs::a2b1_cases::denominator::static_contract::dynamic_quotient::tests::runner_admission_lock_native_acquire_created_first_exclusive_release_error_supported::isolated_q12_receipts_are_exact_controlled_fault_actual";

fn supported_key_and_member(
    case: FrozenLockCreatedFirstExclusiveReleaseErrorCaseV1,
) -> (DynamicClassKeyV1, StaticMemberSealV1) {
    let leaf = &frozen_lock_created_first_exclusive_release_error_leaves_v1()[&case];
...
#[test]
fn isolated_q12_receipts_are_exact_controlled_fault_actual() -> anyhow::Result<()> {
    let selected_child =
        selected_lock_native_acquire_created_first_exclusive_release_error_selector_for_test();
    let mut parent_receipts = 0;
    for (&case, leaf) in frozen_lock_created_first_exclusive_release_error_leaves_v1() {
        let candidate =
            lock_native_acquire_created_first_exclusive_release_error_selector_for_test(
        ...
        let descriptor = lock_created_first_exclusive_release_error_descriptor_v1(
            case,
        ...
        .map_err(|error| anyhow::anyhow!("supported q12 member {case:?} failed: {error:?}"))?;
    ...
    match selected_child {
        Some(selected) => Err(anyhow::anyhow!(
            "parent-selected q12 member was not found: {selected}"
        )),
        None => {
            assert_eq!(
                parent_receipts,
...
const fn runner_completion_v1(
    completion: FrozenCreatedFirstExclusiveReleaseCompletionV1,
