...
| 商户数据自主控制和跨应用授权 | 部分实现 | 跨项目脱敏目录、消费者沙盒、限时 App 授权、匿名关系及低敏偏好披露、删除请求、商户未核验证明、V5 可验证导出、隔离导入、调用配额、终态结果流、商户业务证据、人工和受限机器 ERP/CRM 衔接回执、活动证据、手动 App 封禁和首个商户自有运行时已形成；敏感数据保险箱、完整订单迁移、外部主动推送、真实删除执行与回执核验、生产 ERP/CRM 写入适配器、外部回读、全网签名信任、自动风控与公共互操作治理尚未完成 |
