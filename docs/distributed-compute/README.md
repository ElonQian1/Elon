...
| 节点插件治理合同 | Signed Manifest/InstallPlan、双 keyring、authority v3/PlanApply、受管取数、候选验证、ZIP 安全物化及 staging/健康/quarantine Store 均已写。显式共享策略 ACK 后，云端已在同一认证 session 内串行下发 v10 preparation 与 v11 Planning Snapshot V2，并由 v209/v210 追加式账本保存 exact observation；节点凭据/account 变化会先推进不可丢 epoch并撤销旧 Bootstrap custody。该链仍默认阻断：节点固定 `context_ready=false`、`snapshot_ready=false`，generation outcome 仅允许 `signer_unavailable/rejected`，不会伪造 signed Plan。本机 authority v3-v6 已通过 8 项版本链、4 项 policy-binding、5+6 项 v5 Store/恢复判定、3+7 项 v6 binding Store/恢复判定、5 项真实 Ed25519 目录候选及69项SQLite专项测试。SQLite测试包括15项纯策略、42项one-shot registry、9项raw-state/受控假文件callback ABI和3项真实Connection；同一exact route现承载authorizer判断和线性阶段转换，失败不释放custody。原有`cfg(test)`命名VFS委托SQLite默认VFS验证WAL、连接加固和危险SQL拒绝；Windows独占受管VFS又验证rollback与WAL两条真实Connection路径，把main、Journal/WAL sidecar的`xOpen`、I/O、锁、access/delete、SHM与`xClose`接入managed namespace、registry/file-custody和同一路由authorizer。首次`xShmMap`消费普通main custody、领取唯一SHM lease并原位提升为WAL-main，关闭后联合消费main/SHM证明、退休路由、注销VFS并删除根目录，仅将随机数、睡眠和时间委托默认VFS；仅测试可达的终端中毒注入又证明提升幂等、解绑后拒绝及失败绑定先永久保管main句柄、main lease和SHM lease再隔离路由。该受管夹具仍不支持动态加载、临时文件、并发多Connection或生产注册；生产`xOpen`仍不安装state或I/O表，也没有生产process owner、live `sqlite3_file`构造、VFS注册或Connection。prepared work及v5/v6内层恢复证据已验证，但外层bind/adopt生产接管、生产磁盘迁移及Host接线仍未验证。这不撤销 Ready/Attempt，也不让旧安装在新授权下自动恢复；SHM map/lock/unmap与联合close故障注入矩阵、句柄型 authority open、真实 planning snapshot producer、signed reauthorization、独立 work-admission epoch、生产 root/keyring、Signer/KMS 与可信时间仍缺。失败候选已有 cleanup authorization Store，其后 sealed topology 与首对象 intent/disposition/absence/namespace-durability 共五个独立 typed Store；sequence 1–4 已通过 32 项清理测试，minifilter wire 与首方特权组件 shape 分别通过 9 项和 5 项测试。真实 WDK 驱动、首方/Windows Catalog 验签、安装/transport、安全 fence 构造器、evidence v2、后续 ordinal、terminal journal、跨重启物理恢复、完整事务夹具与 Host 均缺，因此当前仍不产生生产可达的 completion/retry、Ready 或商业 Verification。v216 源码新增本机 v7 install/promotion 双回执与线性恢复，仍保持 Runtime stopped；该增量已随节点编译，v7 authority 全新安装、重开及 v3-v6 原子迁移等 11 项测试和 69 项 SQLite VFS 回归通过，但生产磁盘迁移、install/promotion 完整事务夹具与 Host 接线仍未验证。HTTPS downloader、Sidecar/IPC/探针、work-admission、ReadyCapability 和 Attempt 接线仍缺 |
| 节点端点凭据与认证会话权威 | 服务端 v216 已铺 credential/version/revocation/session receipt/session head 五表，v217/v218 闭合目标绑定的五分钟 owner 重认证与单次 consumption。默认关闭的 direct-TLS-only listener 在 credential gate 下提供 issue/rotate/recover/revoke，独立 bootstrap gate再提供 secure login/register；真实 DB bearer、当前密码、TLS request/response permit 与目标 currentness在同一 IMMEDIATE 事务闭合，新 secret仅首回出现。Windows NodeAgent 已能从唯一 HTTPS origin，以账号+密码、独立 DPAPI状态、先写 no-downgrade tombstone和 pending request的顺序执行 register→issue/recover；注册响应不含 legacy secret或 `ws://`。endpoint root会阻断旧注册续约、DB/env WS和Codex Vault hash proof，旧 process session/Registry/waiter按 exact key收口。secure `/agent/ws` 仍固定503、不upgrade、不写session Store；proto credential triple、耐久session、Ready、route、outbox、command、Lease与派发仍未接。源码尚未编译、测试、运行或执行迁移。见 `node-endpoint-session-authority.md` |
...
| Broker 原子 Reserve 与未执行任务终态 | v175/v176 schema、不可变回执、严格请求重放与历史绑定审计已写；Reserve 单事务完成预算、容量、Reservation 和 Job，Finish 单事务完成退款、held Claim Release/Expire 与 Job/Reservation 终态。存在 Start 时只接受 exact `local_never_sent/prepare_rejected/remote_never_committed` proof；v214 可在 rejected 或 final reconcile Store 闭包产出后两类 proof，但不自动 Finish。ACK 缺失、accepted/quarantined、cancel ACK 或 timeout 均继续失败关闭。HTTP/MCP 与 PC `/compute-market` 源码已接线；状态为 `implementation_uncompiled`，尚未迁移、构建或运行验证 |
