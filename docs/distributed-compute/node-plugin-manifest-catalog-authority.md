...
新增 [`handle-bound open attempt V1`](node-plugin-handle-bound-open-attempt-authority.md) 未登记草案只在 registry child 内复用上述 generic owner：registration 失败返还完整 open intent，pending 放弃走 exact `retire_pending`，成功 begin 后但 Connection 尚未成立的放弃只走 `FailureCustodyRetained` quarantine。registration-owner seal没有生产 constructor，Opening没有成功 consumer；`elon-pc-node` 已实际编译，新 guard `4/4` 与 registry lifecycle `45/45` 通过，但 typestate runtime 未运行，生产`open()`继续固定 unavailable。A2 Barrier 与 RegistrationShutdown 均有 WindowsDynamic `8/8` 正式记录，RegistryLifecycle 有 `16/16`，A2b2 当前为 `32/117`；剩余 85 无 record，clean wide regression `142/142` 已通过，但 Map/Lock、Unmap 与 JointClose 仍未闭合。
