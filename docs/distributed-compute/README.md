...
| `capacity_future` pricing-mode 交付结算谱系 bridge | 未登记 API-free Domain 与 Lease-rooted Store retained resolver source 已写入：单一 Deferred snapshot 历史重建 Instrument/activation/adoption/publication、Commitment、exercised Allocation、immutable Snapshot 与 F0 execution/verification/settlement/release owners，并封入 crate-visible、private-field、non-Serde seal。v192/v195 两套 role-specific usage digest 分别复用原 owner 公式、禁止判相等。当前 `source_review_only/implementation_uncompiled/implementation_unrun`、`passed=0/failed=0`，无 Service/API/migration/writer/ClearingReceipt 或 effect 接线 |
...
55. `docs/distributed-compute/capacity-future-settlement-lineage-authority.md` 与对应 acceptance：记录未登记的 v238→v225→v228→F0→v195/v198 reference-only `capacity_future` bridge、v192/v195 双摘要域边界、封闭经济历史分支与 Lease-rooted Store retained resolver source；当前未编译/运行，无 Service/API/migration/writer 或 effect 接线。
