...
| 激活证据申请与计划控制面 | v177-v181、v203-v205 的 Store/Service 已通过临时 SQLite 全量迁移和“登记→批准→双人复核→激活→隔离→恢复计划废止重做→恢复”定向测试；PC `/compute-supply`、`/compute-activation` 已通过严格类型、lint 和生产构建。HTTP/MCP、权限、并发、真实 TCP、浏览器和生产磁盘迁移仍未验证，状态为 `implementation_partially_verified`；流程不发节点命令、不退款、不付款或移动资金，详见 `activation-control-plane-acceptance.md` |
| Offer 草稿、发布与生命周期控制面 | v170、v182-v184 Store/Service 已通过临时 SQLite 全量迁移和“草稿→幂等重放→修订→发布→排空→提前过期拒绝→撤销”定向测试；PC `/compute-supply`、`/compute-offers` 已通过严格类型、lint、生产构建和 bundle budget。HTTP/MCP、权限、并发、真实 TCP、浏览器和生产磁盘迁移仍未验证，状态为 `implementation_partially_verified`；所有写入口均不移动资金，详见 `offer-control-plane-acceptance.md` |
...
| 外部算力池适配器与统一报价 | 首段 owner request→独立管理员复核→immutable apply 的领域、Store-private 与 v221 来源源码已写但未编译/迁移/接 API；apply 只登记 `external_pool/registering` Provider 与专用 application source，不写 v213 route rows、不建 Pool/Offer 或派发。credential verifier、Adapter 和网络仍未实现，见 [`external-pool-adapter-authority.md`](external-pool-adapter-authority.md) |
...
节点内部已经形成 Plugin Host 兼容 seam、Signed Manifest、InstallPlan、双槽安装/切换/回滚 lifecycle 和 ReadyCapability 合同骨架，并写入真实 Ed25519/JCS 验证、Manifest 语义校验及本机 InstallPlan 准入内核；节点供应链源码已随 `elon-pc-node` 编译通过，但未启动或生产接线。云端还形成 v169 版本化 Provider Registry、v170 追加式 Offer Registry及 v182-v184 发布/生命周期控制；本人可创建规范化 draft Offer，平台管理员可发布 active、转为 draining，并在无活动预留时终结为 expired/revoked。Offer Store/Service 已通过临时 SQLite 全量迁移和定向状态链测试，但 HTTP/MCP 与生产磁盘仍未验证。ReadyCapability 只是有明确过期时间的本机技术就绪证据，不包含市场价格、可预留容量或账户授权，**不等于 Compute Offer**。
...
Provider 本人控制面由 `docs/distributed-compute/provider-api.md` 维护，Pool、Bucket 和 Supply 控制面分别由 `docs/distributed-compute/capacity-pool-api.md`、`docs/distributed-compute/capacity-bucket-api.md`、`docs/distributed-compute/capacity-supply-api.md` 维护；激活证据申请、计划、第二人复核与“内部激活不等于市场可交易”边界由 `docs/distributed-compute/activation-evidence-api.md` 维护，隔离后的追加式恢复由 `docs/distributed-compute/activation-recovery-api.md` 维护，定向验证证据由 `docs/distributed-compute/activation-control-plane-acceptance.md` 维护。PC `/compute-supply` 承载本人供给、证据和 Offer 草稿入口，仅管理员可见的 `/compute-activation` 承载审核、计划准备、第二人复核、应用、废止、隔离和恢复，`/compute-offers` 承载 Offer 发布和安全退场，所有登录用户可见的 `/compute-market` 承载本人 Job、锁价和预留，`/compute-execution` 承载本人 Provider 的 Attempt 激活、Lease、用量与终态候选，`/compute-reviews` 承载消费者待验收队列和第一份审核证据，仅管理员可见的 `/compute-observations` 承载待观测队列和第一份平台证据，`/compute-verification` 承载待验证证据链、保守计量预览和第一份人工决定，`/compute-receipts` 承载 Execution Receipt 确认，`/compute-finalization` 承载可信终态确认，`/compute-settlement-issuance` 承载首份待结算回执确认；`/compute-supply`、`/compute-activation` 与 `/compute-offers` 已完成静态生产构建，其余入口仍为 `implementation_uncompiled`。Offer 规范化草稿与无市场效果边界由 `docs/distributed-compute/offer-api.md` 维护，定向证据见 `docs/distributed-compute/offer-control-plane-acceptance.md`；Job、报价和预留控制面由 `docs/distributed-compute/broker-api.md` 维护；Attempt 激活、续租、中止、声明用量、Provider 候选、消费者审核、平台观测、Verification、Execution Receipt、可信终态、待结算、挑战、决议、纠正与释放边界见本页“阅读顺序”第 16 至 30 项。
...
服务端 Provider Adapter 统一接入公司集群、云 GPU 和其他算力池；每个 Provider 保留自己的内部调度，只向一龙提交标准回执。首段来源权威见 [`external-pool-adapter-authority.md`](external-pool-adapter-authority.md)：领域、Store-private 与 v221 源码已写但未编译、未迁移、未接 API，也不产生 credential、route、容量、派发或结算效果。
