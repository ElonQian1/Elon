...
进入 A1 依赖顺序的生产 process owner/VFS 注册/open 阶段之前，仍必须按[`动态验收`](node-plugin-vfs-fault-acceptance.md)另批完成 Windows SHM map/lock 独立 denominator，并把每条动态观察与静态 case key 一一对应。A2b2 五族已全部完成：Barrier、RegistrationShutdown、RegistryLifecycle、Unmap 与 JointClose 分别为 `8/8`、`8/8`、`16/16`、`49/49`、`36/36`，合计 `117/117`；map/lock denominator 尚未 source-review clean，因此 dynamic 尚未开放。宽范围回归已有 clean runtime-source baseline `266/266`，但每次运行时源码改动后必须重证；Map/Lock 静态和 Windows 动态证据任一缺失，都不得把 A2 标记完成或推进生产入口。
