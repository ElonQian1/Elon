...
- 官网从新会话首页导航到回答 URL 时，后台 WebView 必须在页面开始加载和完成加载两个边界都重新停放；上游导航不得在回答期间把子 WebView 恢复到创建坐标并覆盖原生聊天。
...
6. 回答 URL 开始导航、流式生成和导航完成后，官网 WebView 均保持后台，只有用户主动点击“官网完整内容/显示官方页”才能进入前台。
