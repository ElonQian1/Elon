...
| 激活证据申请与计划控制面 | v177-v181、本人 HTTP/MCP、管理员审核/废止、申请/计划预检、不可变计划、原子应用与紧急隔离回执已写；v203 增加 prepared 计划第二人复核回执，并在预检和应用事务内强制准备人与复核人分离。PC `/compute-supply` 已写入本人申请、历史、预检和取消源码，`/compute-activation` 已写入复核操作。复核无激活效果；应用单事务激活内部 Provider/Pool，隔离单事务把其当前 active 状态转为 quarantined；均不发送节点命令、不直接改写 Offer、不移动资金，恢复尚未实现，状态为 `implementation_uncompiled` |
