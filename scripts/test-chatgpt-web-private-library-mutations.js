...
test('directory dispatcher upgrades an older registered version and preserves same-version instances', () => {
  ...
  const version = window.__elonChatGptConversationDirectoryRequests.version;
  assert.ok(Number.isInteger(version) && version > 1);
  const outdated = { version: version - 1 };
  window.__elonChatGptConversationDirectoryRequests = outdated;
  vm.runInNewContext(source, { window });
  assert.notEqual(window.__elonChatGptConversationDirectoryRequests, outdated);
  assert.equal(window.__elonChatGptConversationDirectoryRequests.version, version);
