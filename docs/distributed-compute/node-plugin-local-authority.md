...
共享策略 ACK 之后现有独立 v10 preparation 握手只证明节点观察了同一 policy snapshot/authorization，并通过 v209 云端追加式账本保存请求、每会话 delivery 与 observation；它明确不打开本机 authority，当前也固定不返回 planning context。必须先补一条独立、无下载、精确 CAS 的本机 policy-binding transition：把已接受的 node/installation/policy revision+digest/authorization ref+revision+digest 写成 live authority 事实，关闭或代次变化时撤销 prepared work。否则初始化仍固定 `sharing_enabled=false` 且 authorization 为 NULL，而 InstallPlan admission 又要求计划与 live authority exact match，形成首份 enabled Plan 的循环依赖。该 transition 完成前，即使外部出现合法签名也不得调用 PlanApply；更不得用 Bootstrap 内存意图、云端 ACK 或伪造 signer 绕过。

当前供应链、候选健康与失败候选清理截至 sequence 2 的远程基线已通过 `elon-pc-node` 编译；清理链通过 3 项 Windows 同句柄删除、1 项目录顺序、4 项 completion、5 项 topology builder/Store、4 项首 intent builder/Store、3 项强 disposition 及 4 项 disposition event builder/Store 测试，合计 24 项，另有 1 项 topology/journal schema 建库与重开测试。新增 sequence 3 parent-observation/absence Store 与恢复代码按本轮策略只做 Rust 格式化、静态差异和文档门禁，未运行 Cargo 编译、单元测试或真实 Windows namespace 观察，不能计入上述验证。仍没有 retained custody → cleanup authorization → topology → intent → physical disposition → disposition Store → parent absence Store 的完整事务夹具。cleanup authorization Store 可推进 `cleanup_pending`，topology Store 可封存 exact execution plan，intent Store 可独立提交第一个物理动作前置授权，私有单对象 executor 可消费该 capability 并保留物理结果 custody，disposition 与 absence Store 可分别独立持久化 sequence 2/3 并分类不确定提交；这些入口均没有 Host 调用方。由于 namespace durability Store、后续 ordinal 和目录 durability 仍缺，终态 capability 不可构造，完整私有流水线尚不可达。当前仍未接入生产 Host 或生产本地库，不能授予生产重试、安装、promotion、ReadyCapability 或商业 Verification。多对象 handle-tree 收口、跨重启物理恢复、真实下载/Sidecar/探针、installed/promotion/GC 与 Attempt 接线仍待实现；精确边界见 `node-ready-capability.md` 与 `node-plugin-candidate-cleanup.md`。
