    ...
    conn.execute_batch(
        ...
        SELECT installation.installation_receipt_id,
               ...
               CASE WHEN terminal.terminal_receipt_id IS NULL
                          AND adoption.current_status='adopted_current'
               ...
               CASE WHEN terminal.terminal_receipt_id IS NULL THEN 'none'
                    ELSE 'revoked' END AS terminal_status
          FROM compute_external_pool_adapter_installation_receipts installation
          LEFT JOIN compute_external_pool_adapter_installation_terminal_receipts terminal
            ON terminal.installation_receipt_id=installation.installation_receipt_id
           AND terminal.installation_receipt_digest=installation.installation_receipt_digest
