//! Canonical q15 child header for existing-first truncate uncertainty with known release.

pub(in super::super) const REPORT_VERSION: &str = "a2lockq15";
...
pub(in super::super) fn selector(
    ...
    Ok(format!(
        "initialization-{action}-first-{first}-count-{count}-existing-first-truncate-error-release-succeeded-{completion}-terminal-route-unknown"
    ))
}
...
mod tests {
    ...
    #[test]
    fn eighty_eight_q15_selectors_are_unique_and_fail_closed() {
