//! Exact source-bound programs for completed local sibling-contention outcomes.
//!
...
pub(super) const LOCAL_SIBLING_CONTENTION_MEMBER_COUNT: usize = 44;

#[derive(Clone, Copy)]
pub(super) struct LockLocalSiblingContentionProgramSpecV1 {
    #[cfg(windows)]
...
pub(super) fn program_spec_v1(
...
) -> Result<LockLocalSiblingContentionProgramSpecV1, LockRunnerExecutionViolationV1> {
    ...
    if key.schema_version != DYNAMIC_PROJECTOR_SCHEMA_V1
        ...
        || key.prestate != PrestateV1::Lock(expected_prestate_v1(action))
        || key.operation != DynamicOperationV1::Lock(LockOperationV1::LocalAcquire)
        || key.phase != PhaseV1::LockAcquire
        || key.timing != TimingV1::Natural
        || key.occurrence != OccurrenceV1::Natural
        || key.recipe.fixture != FixtureV1::ManagedWalMainTwoConnections
    ...
    Ok(LockLocalSiblingContentionProgramSpecV1 {
        #[cfg(windows)]
        action,
        #[cfg(windows)]
        first,
