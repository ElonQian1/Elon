...
| Text submit/stream/stop/follow-up | Official runtime send path verified, reused | 1640 passed private model admission (6/6 handles in two documents), but native retry stopped before dispatch at `semantic_control_missing`. Mapper/command DOM gates are fixed in source using shared admission; 41 Android and 79 runtime tests pass. [Evidence](chatgpt-official-runtime-regeneration.md). Grouped delivery/native retry acceptance remain pending; earlier post-dispatch `timeout_owner_changed` is unresolved. Independent Android POST and other contexts remain separate |
