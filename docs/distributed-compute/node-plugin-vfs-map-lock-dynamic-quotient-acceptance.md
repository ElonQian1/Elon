...
双 unavailable receipt 各增加 88 个，q18/q19 `CreatedFirst/ExistingFirstSharedBusyCloseSucceeded` 再各增加 88 个，
故 q1–q19 未运行 inventory（members/groups）=`4372/4372 present,4296/3768 missing,8668/8140 total`。
q12–q19=`704/704`，完整 initialization umbrella 仍缺 `2728 members/2200 groups`。q9–q19 细节只见
...
- [x] root-specific producer coherence 对全部真实 typed tuple 失败关闭：source/stimulus/prestate/operation/
      ...
      truncate-error/release-failed，以及 q18/q19 各 88-member CreatedFirst/ExistingFirst shared-busy/close-succeeded
      exact-singleton source shape，
...
- [x] projector provenance 覆盖 `producer_coherence/{map,map_axes,lock,lock_axes}.rs`、
      ...
      cleanup-release-outcome 与 q18/q19 shared-contention/close-success source scope（精确文件清单只见
...
- [x] Test source：q9 `LockPreManagedCallbackRejectionV1` 精确增加 528 个 singleton member/group（6×88）；
      ...
      完整 initialization umbrella=`3,432 members/2,904 groups` 且不属于 q9；q12–q19 各承接 88/88，
      remaining=`2,728 members/2,200 groups`。
...
- [x] Test source：q11 `LockRawStateRejectionV1` 在两个 pointer safety-premise exclusion 后精确增加 11 个
      singleton member/group；完整 typed key、Expected、completion 与两个 seals 必须逐 member 精确相等。
      受控 memory-safe premise 必须进入真实 installed `xShmLock → run_code → raw admission/abandon/drop`，
      但其未来证据等级只能是 `controlled_fault_actual`，不得称为自然生产可达或正常 runtime coverage。
- [x] Test source：q12 `LockNativeAcquireCreatedFirstExclusiveReleaseErrorV1` 精确增加 88 个 singleton
      member/group；catalog 88 rows / 18,386 bytes、SHA-256=
      `51d675ee9b2fe990b71a924a6f7cf016c6738e7f88872449f91a20ba6d2566df`，与 frozen 目标 88/88 精确相等。
      dedicated controller 只在真实 CreatedFirst DMS release 点调用一次 `UnlockFileEx` 并故意不读取 BOOL；
      requested Lock range 保持 native/local no-entry，证据只能是未运行的 `controlled_fault_actual` source seam。
- [x] Test source：q13 `LockNativeAcquireExistingFirstExclusiveReleaseErrorV1` 精确增加 88 个 singleton
      member/group；catalog 88 rows / 18,474 bytes、SHA-256=
      `03b883842b3fd1886779dcb07573521d14ca3125894b38c4d341a462137424f4`。typed precreation receipt 必须先证明
      SHM 物理预创建并关闭，随后 cold attach 必须观察 ExistingFirst；q12 CreatedFirst 和任何 receipt/case swap 失败关闭。
      one-shot `UnlockFileEx` BOOL 仍故意不读，因此未来证据也只能是 `controlled_fault_actual`，不得写为 natural actual。
- [x] Test source：q14 `LockNativeAcquireCreatedFirstTruncateErrorReleaseSucceededV1` 精确增加 88 个
      singleton member/group；catalog 88 rows / 17,594 bytes、SHA-256=
      `95f1c8e40da35ac23cc46e20310178c2aef09adacf777155d9b67e5802d69abd`。dedicated controller 只在同一
      production truncate validation 点调用一次真实 `File::set_len(0)` 并故意不读取其 `Result`/return receipt，
      形成 typed `ReturnReceiptUnavailable`；随后 normal production `UnlockFileEx` success 必须被观察，DMS
      到达 `Released`，mutation uncertain 但 lock certain、file retained。未来证据只能是
      `controlled_fault_actual`，不得写为 natural actual。
- [x] Test source：q15 `LockNativeAcquireExistingFirstTruncateErrorReleaseSucceededV1` 精确增加 88 个
      singleton member/group；catalog 88 rows / 17,682 bytes、SHA-256=
      `d8ac9985f433b9fbd089a8804136f107cf37bd21566b5852611e98674f52911b`。typed precreation receipt 必须先证明
...
- [x] Test source：q18/q19 CreatedFirst/ExistingFirst shared-busy/close-ok 各增加 88 singleton（16+72，44/44）；
      catalog 分别为 18,122/18,210 bytes，q19 SHA-256=`eb318d91edbd0bbcd7e68ff626504a007a3f3c96d5eb60b965c9e362a421eee8`。
      两批均绑定同 `FileId` distinct holder/target 真争用、分账与 explicit target close；q19 另绑定 physical
      precreation、cold `was_created=false`，`a2lockq19`=194。未来证据只能是 `controlled_fault_actual`。
- [x] Unrun inventory expectation：q1–q19 Lock 为 `4,372` source-present members/groups，静态总数为
      `8,668 members/8,140 groups`，仍缺 `4,296 members/3,768 groups`；initialization q12–q19=`704/704`，
      remaining=`2,728 members/2,200 groups`。
...
- [ ] Current full admission：source test 预期 Map `42,955` members、Lock `4,296 members/3,768 groups` 仍
...
q11 的 11 个 raw-state singleton 只建立受控 fault source receipt 要求。未来 runner 必须只构造 memory-safe
synthetic premise，并观察真实 production raw validation、abandon、slot clear/retain、envelope/payload Drop、adapter
fallback 与 callback return；其结果只能标为 `controlled_fault_actual`。两个 invalid pointer premise 永远 excluded，
不得为了制造“覆盖”进入 runner。

q12 的 88 个 CreatedFirst singleton 是另一条 initialization vertical slice：当前源码已具备 matcher/catalog、
production DMS lock/truncate/release seam、typed initialization receipt、requested-range no-entry ledger、child/
runner/payload 与 parent cleanup。它仍未编译、未运行；真实 `UnlockFileEx` 返回值被故意丢弃的未来回执也只能
标为 `controlled_fault_actual`，不能成为 natural actual、coverage 或生产开放证据。

q13 的 88 个 ExistingFirst singleton 将同一 release-outcome seam 绑定到物理 SHM 预创建/关闭回执与
cold attach `was_created=false` observation。当前 matcher/catalog、case-aware controller、receipt、child/runner/payload 与
cleanup 仍只是 uncompiled/unrun source；它不复用 q12 receipt，也不产生 natural actual、coverage 或生产许可。

q14 的 88 个 CreatedFirst singleton 将 DMS truncate outcome uncertainty 与随后可观察的 normal production
release success 严格分开：同一 production truncate validation 点真实调用一次 `File::set_len(0)`，故意不读
`Result`/return receipt 并形成 typed `ReturnReceiptUnavailable`；随后真实 `UnlockFileEx` success 必须被 receipt
观察，最终 DMS=`Released`、native lock/unlock=`1/1`、mutation uncertain、lock certain、file retained。
当前 matcher/catalog/controller/receipt/child/runner/payload 与 cleanup 仍只是 uncompiled/unrun source；它只能支撑
未来 `controlled_fault_actual`，不产生 natural actual、coverage 或生产许可。

q15 的 88 个 ExistingFirst singleton 复用同一 truncate/release 语义但不复用 case/receipt：typed precreation
receipt 必须先证明物理 SHM 已创建并关闭，cold attach 再观察 `was_created=false`；随后真实 `File::set_len(0)`
Result 仍故意不读，normal release success 必须被观察。当前 matcher/catalog、case-aware controller、receipt、
child/runner/payload 与 cleanup 同样只是 uncompiled/unrun source；只允许未来 `controlled_fault_actual`。

q16 的 88 个 CreatedFirst singleton 在 q14 的 truncate-outcome seam 后继续冻结 cleanup release outcome
uncertainty：第一份 typed receipt 来自真实 `File::set_len(0)` 的 unread `Result`，第二份来自 cleanup 点真实
`UnlockFileEx` 的 unread BOOL；两者必须同 case/stage 且严格有序。typed stimulus 保持
`cleanup_rewrite=false`，只有 terminal disposition 为 `CleanupRewritten`；最终 DMS=`ExclusiveOutcomeUncertain`、
native lock/unlock=`1/1`、mutation/lock 均 uncertain、file retained。当前 matcher/catalog/two-stage controller/
receipt/child/runner/payload 与 cleanup 仍只是 uncompiled/unrun source；只允许未来 `controlled_fault_actual`。

q17 的 88 个 ExistingFirst singleton 是 q16 对切而不复用其 case/receipt：typed helper 必须先物理创建并关闭
SHM，cold attach 再观察 `was_created=false`；同一 tuple 内 truncate Result unavailable 必须先于 cleanup
`UnlockFileEx` BOOL unavailable。stimulus=`cleanup_rewrite=false`、terminal=`CleanupRewritten`、最终
DMS=`ExclusiveOutcomeUncertain`、mutation/lock uncertain、file retained。current matcher/catalog、case-aware
...
q19 的 88 个 ExistingFirst 对切先物理预建并关闭 SHM，cold attach 观察 `was_created=false`，再复用上述真实
contention/close 顺序而不复用 case/receipt。`a2lockq19`=194 scalars；current seam/payload 同样 uncompiled/unrun。
...
| sealed runner admission | `source written; exact receipt-only 6 request-budget + 6 q3 lifecycle + q4 511-member semantic scope/509 net-new support; default gap retained; not run` | `source written through q19; q11=11 exact singleton after 2 exclusions, q12–q19=88 each; default gap retained; not run` | raw `Supported` 不能成为 permit；current actual 仍缺 |
| narrow executable program | `MapRegionLoopSuccessV1 q4: two exact families, exact N ordered mapping ledger, typed matcher, per-member seal/source digest; source-only; uncompiled/unrun` | `q1–q19 source-only; q9–q19 exact scope and receipt contract only in Lock tranches; uncompiled/unrun` | 源码形状不是动态执行证据 |
...
| pre-manifest program inventory | `full-root two-pass source written; unrun expectation: 43476 total, 521 source-present members/groups, 42955 planned-missing members` | `unrun members/groups: 4372/4372 present, 4296/3768 missing, 8668/8140 total` | 两个 digest 均未生成/冻结；不是 quotient、coverage 或 Windows evidence |
...
| full candidate gate | `prior 43476 checked; current source expected to fail at 42955 missing` | `prior 8668 checked; current source expected to fail at 4296 members/3768 groups missing` | current source 未运行；prior baseline 只证明当时失败关闭，不产生 quotient denominator |
| current blocker | `42955 planned-missing source expectation + no reviewed inventory digest + actual not run` | `4296 members/3768 groups missing + compile/runtime/actual receipts/reviewed digest absent` | q3–q19 只有 source；两个 root 的全局 blocker 均未解除 |
...
| global source-leaf authority scope | `shared blocker` | `q9–q14 checkpoint 的 19 frozen/source-owner artifacts 已逾期并 deferred；重生成/复核须含 q15–q19` | 禁止只改 owner/needle 的半套权威；未来编译或真实验收前须单独续绑，不构成新运行证据 |
...
当前 verdict：`design_frozen / Lock_q18_q19=CreatedFirst_ExistingFirst_shared_busy_close_succeeded_each_88 / q19=physical_precreation+cold_existing_first+a2lockq19_194 / same_FileId_distinct_holder_target_real_contention_separate_ledgers_explicit_close / source_written_review_only_uncompiled_unrun / Lock inventory=4372/4372 present,4296/3768 missing,8668/8140 total / initialization remaining=2728/2200 / passed=0 failed=0 actual=not_run / natural_actual=none / Qmap_Qlock=unknown / coverage=0/43476+0/8668 / WindowsDynamic=not_opened / refresh_must_include_q15_q19 / production_economic=closed`。q9–q19 细节只见[`Lock tranches`](node-plugin-vfs-lock-dynamic-tranches-authority.md)。
