...
v204 隔离恢复计划、第二人复核、只读预检、原子应用回执、管理员 HTTP 路由和 PC `/compute-activation` 管理界面已写入源码。当前尚未执行 Cargo/TypeScript 编译、v204 迁移、HTTP 调用、并发验证、浏览器验收或发布，状态固定为 `implementation_uncompiled`。
...
5. 管理员读取 `compute_federation.activation_recovery_preflight.v1`，查看隔离摘要、Provider/Pool 版本、目标合同、第二人复核及旧 active Offer 是否已退场。
6. 只有预检无阻断项时 PC 才开放应用确认；服务端不信任该快照，写事务仍会重新检查全部条件。
7. 首次成功应用在一个事务中登记计划锁定的 active Provider 下一版本、追加 Pool `quarantined -> active` 事件、把恢复计划改为 applied，并写入不可变恢复应用回执。
...
应用按钮仅在 `ready_for_apply=true` 时可用，并显示当前 active Offer 数量。应用成功后展示恢复应用摘要和目标 Provider revision。前端快照不代替服务端事务检查。

## 9. 并发与历史边界

- 恢复计划、复核和应用均使用稳定幂等键；相同键不能改变已绑定输入。
...
- Cargo/TypeScript 编译、v204 迁移、HTTP、权限、并发、浏览器和发布验证；
- prepared 恢复计划的废止、替换或过期机制；
