...
- 动作确认定向 Rust 套件已通过，覆盖四种读取状态、不可枚举错误、最小安全投影、只读无状态副作用、主动取消、幂等重试、用户与 App 隔离、自然过期、已消费拒绝、同幂等请求重新准备、审计单写以及取消与 Invocation 创建并发二选一。
- v166 迁移已通过带既有 `pending`、`confirmed`、`consumed` 和 `expired` 记录的旧表升级及重复执行测试；多个独立数据库连接之间的竞争仍需统一回归。
