...
- AI 可以在结构化建议中提出 Markdown 重命名或移动，但不得自行执行。用户必须逐项核对源路径和目标路径，并只对本次选中操作授予 `rename`/`move` 权限。
- 实体整理只允许项目内 Markdown，必须校验源文件哈希，禁止覆盖、删除、越界、改写正文或自动 commit/push。正文修改、引用重写、归档和删除需要单独授权。
