...
internal object BinanceReportFields {
    ...
    val positionExtras = setOf("leverage", "notional", "pnl", "positionMargin", "initialMargin", "maintenanceMargin", "mark", "liquidation",
        "accountMarginBalance", "accountMaintenanceMargin")
    ...
    fun decode(kind: String, value: Any?): Map<String, Any?> {
        ...
        val optional = if(kind=="positions")positionExtras else emptySet()
        require(row.keys.containsAll(fields.keys + extra) && row.keys.all {it in fields.keys + extra + optional})
        optional.forEach { key -> require(row[key]==null || row[key] is String && Regex(DECIMAL).matches(row[key] as String)) }
