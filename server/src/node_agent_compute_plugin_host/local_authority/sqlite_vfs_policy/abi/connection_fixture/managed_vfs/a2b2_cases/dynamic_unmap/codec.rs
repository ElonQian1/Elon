...
const REPORT_VERSION: &str = "a2b2un1";
...
impl UnmapActual {
    ...
    pub(super) fn from_report_payload(input: &str) -> Result<Self, &'static str> {
        if input.len() > MAX_REPORT_PAYLOAD_BYTES || !input.is_ascii() {
            return Err("Unmap report payload is not bounded ASCII");
        }
        let mut parts = input.split(',');
        if parts.next() != Some(REPORT_VERSION) {
            return Err("Unmap report payload version is unsupported");
        }
        let selector = UnmapSelector::from_report_name(
            parts.next().ok_or("Unmap report payload has no selector")?,
        )
        .ok_or("Unmap report payload selector is unsupported")?;
        let mut fields = Fields { parts };
        let identity = UnmapActualIdentity {
            path: UnmapPath::try_from(fields.u64()?)?,
            topology: UnmapTopology::try_from(fields.u64()?)?,
            mode: UnmapMode::try_from(fields.u64()?)?,
            node: UnmapNode::try_from(fields.u64()?)?,
            variant: fields.u8()?,
            pre_shared_mask: fields.u8()?,
            pre_exclusive_mask: fields.u8()?,
            phase: UnmapPhase::try_from(fields.u64()?)?,
            cause: UnmapCause::try_from(fields.u64()?)?,
            timing: UnmapTiming::try_from(fields.u64()?)?,
            class: UnmapFailureClass::try_from(fields.u64()?)?,
            target: UnmapActualTarget {
                scope: UnmapTargetScope::try_from(fields.u64()?)?,
                registration_id: fields.u64()?,
                route_ordinal: fields.u64()?,
                runtime_generation: fields.u64()?,
                shm_connection_id: fields.u64()?,
                role: UnmapRole::try_from(fields.u64()?)?,
                callback: UnmapCallback::try_from(fields.u64()?)?,
                occurrence: fields.u32()?,
            },
            sqlite_outcome: UnmapSqliteOutcome::try_from(fields.u64()?)?,
        };
        let actual = Self {
            selector,
            identity,
            ...
            registry_route_phase: UnmapRegistryRoutePhase::try_from(fields.u64()?)?,
            logical_route_phase: UnmapLogicalRoutePhase::try_from(fields.u64()?)?,
            registration_phase: UnmapRegistrationPhase::try_from(fields.u64()?)?,
        ...
        if fields.parts.next().is_some() || actual.to_report_payload() != input {
            return Err("Unmap report payload is not canonical");
        }
        Ok(actual)
    }
}
...
impl Fields<'_> {
    fn u64(&mut self) -> Result<u64, &'static str> {
        self.parts
            .next()
            .ok_or("Unmap report payload is truncated")?
            .parse()
            .map_err(|_| "Unmap report field is not an unsigned integer")
    }

    fn u32(&mut self) -> Result<u32, &'static str> {
        self.u64()?
            .try_into()
            .map_err(|_| "Unmap report field exceeds u32")
    }

    fn u8(&mut self) -> Result<u8, &'static str> {
        self.u64()?
            .try_into()
            .map_err(|_| "Unmap report field exceeds u8")
    }
    ...
    fn boolean(&mut self) -> Result<bool, &'static str> {
        match self.u8()? {
            0 => Ok(false),
            1 => Ok(true),
            _ => Err("Unmap report boolean is not zero or one"),
        }
    }

    fn topology(&mut self) -> Result<UnmapActualTopology, &'static str> {
    ...
    fn custody(&mut self) -> Result<UnmapActualCustody, &'static str> {
        Ok(UnmapActualCustody {
            node: self.boolean()?,
            views: self.u8()?,
            mappings: self.u8()?,
            dms: UnmapDmsCustody::try_from(self.u64()?)?,
    ...
    fn counts(&mut self) -> Result<UnmapActualCounts, &'static str> {
