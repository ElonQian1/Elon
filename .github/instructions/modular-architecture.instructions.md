...
1. **永远不允许制造新的巨型文件**：新建 `.rs` / `.kt` / `.ts` / `.py` 文件超过 800 行立刻警觉；不允许一次性写出 1500 行以上的文件。
2. **永远不允许在已有巨型文件里继续叠加**：超过 1500 行的文件，除小修外，必须先把本次触碰到的成块职责抽到新模块再加代码。
3. **永远按职责模块化 + 长期主义**：先想"这块逻辑属于哪个领域、应该独立成什么模块"，再动手；不要因为"快"就塞回入口文件。
4. **永远先 `git fetch origin main` 再开干**：本项目长期多 AI 并发拆分，开始前 / 提交前都要同步远端，避免重复抽同一块代码。
5. **永远只 stage 本次任务的文件**：拆分提交 = `refactor(...)`，新功能 = `feat(...)`/`fix(...)`，不允许混合。
