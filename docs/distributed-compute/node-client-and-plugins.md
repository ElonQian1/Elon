...
第一批内部兼容 seam 仍直接调用现有 Rust 本地推理函数。节点供应链已形成独立 authority、PlanApply、可恢复 fetch/verification、Windows pinned-root/同句柄耐久写入、ZIP staging、候选健康与失败 cleanup custody。v216 又铺设本机 installed/promotion 双回执：线性入口消费 retained handles、staging 与未过期健康回执，单一事务写互相引用的 install/promotion receipt，并原子完成 `staged -> installed`、active pointer/代次/fence 推进和 owner `owned -> promoted`；不确定结果只允许 exact recovery 与 fresh retained-handle 重哈希。v216 增量已随 `elon-pc-node` 编译，v7 authority 全新安装、重开及 v3-v6 原子迁移等 11 项测试和 69 项 SQLite VFS 回归通过；生产磁盘迁移与 install/promotion 完整事务夹具仍未验证。v217 源码又加入只面向当前 installed release 的签名 `reauthorize_existing`、authenticated applied-only PlanApply barrier，以及消费 retained-handle fresh rehash、严格后发可信时间和 fresh authority 的本机 work-admission source/receipt/head/recovery。该账本封存签名 Plan/Manifest、精确 grant、launch profile 与授权资源上限，但保持 runtime stopped，不声称 Host enforcement、活动健康或 Ready；v217 尚未编译、迁移或运行事务夹具。真实 downloader、Sidecar/IPC/沙箱/探针调度、完整清理 completion、Runtime 启动与健康、`ReadyCapability` 上报及 Attempt 接线仍未实现。精确边界由 `node-plugin-local-authority.md`、`node-plugin-candidate-cleanup.md` 与 `node-ready-capability.md` 维护。
...
当前策略 ACK 已接上第二段、独立能力协商的 v10 InstallPlan 准备握手。云端只有在当前 sharing observation 已精确验证并耐久写入后，才创建或复用绑定同一 immutable consent、policy snapshot 与 authorization 的准备请求，再为本次认证会话追加 fresh delivery。节点只对照当前 Bootstrap 意图，返回随机进程实例、配置/取消代次和 blocker；当前 `context_ready=false`、`context=None`、`local_confirmation_available=false`，root lock、authority、process fence、下载与任务准入均为 false。v209 仅保存不可变请求、delivery events 及规范化 observation/context 摘要；10 秒 ACK 只表示请求已观察，不是用户确认，也不是 signed Plan 已生成。

协议阈值 12 的 Planning Snapshot V2 仍是独立、默认阻断的能力。云端只在同一认证 session 已耐久接收 V1 policy 与 v10 preparation ACK 后串行派发，请求精确引用该 session、v209 preparation delivery 与 observation digest。installed record 的 work-admission 现为可选 current-head commitment：`None` 明确表示新晋升、candidate-only 或历史 head 已与当前 active release/authority 失配，允许 planner 生成首次 `reauthorize_existing`；`Some` 才承诺正 generation 与 receipt digest，未来 producer 还必须在同一只读 authority snapshot 中复核 exact current head。这样不会用伪造 generation 1 填平 v216 promotion 与首次 v217 admission 之间的自举环。当前 Bootstrap 仍没有 authority snapshot producer，始终返回 `snapshot_ready=false`、`snapshot=None` 和全部副作用标志为 false；云端 v210 账本与 V2 schema/capability 名保持不变，proto 11 节点则由版本阈值失败关闭。它不是签名器、计划投递、本机确认或 PlanApply。

endpoint 路径另以 v14 `planning_snapshot_bootstrap_only` profile重放完整 sharing→preparation→Planning 链，而不是把 legacy `cloud_session_id` 升级成权威。六条独立消息固定 exact session binding、单一 capability、sequence 与前序摘要；云端每段 observation 和下一段 intent 在同一 IMMEDIATE 事务重验 v216 session head/receipt/current credential并追加 v219 provenance。节点只在 Accepted V2 后建立 typed endpoint witness，credential epoch或session变化即撤销旧链。accepted disabled sharing 返回 `phase=disabled`；disabled 或 rejected sharing 均在 seq2 终止。enabled sharing 后的 preparation/Planning 仍只会得到诚实的 blocked observation。生产 handle-bound SQLite VFS、可信时间、rollback、policy/catalog/keyring/inventory/install/work-admission 同快照 projector齐备前，禁止输出 `snapshot_ready=true`。
...
这只是 PlanApply live-binding 的必要前置，不是可用 planning context。v6 目录 binding、rollback V2 和 opened-authority 合同均保持失败关闭：仅空库存可绑定，V1/V2 不互转，裸远端 checkpoint 不能冒充本机事实，open intent 与 snapshot producer 都不可构造。v3-v6 版本链已通过 8 项内存 SQLite 测试，v4 policy-binding 通过 4 项投影/回执测试；v5 companion receipt 通过 5 项 Store 和 6 项恢复判定测试；v6 空目录 binding 通过 3 项事务和 7 项恢复判定测试，候选层另有 5 项真实 Ed25519 测试。恢复内核已覆盖前态、历史/后继、prepared work、碰撞、回滚、缺头和错误epoch失败关闭，但未覆盖v5/v6外层`bind/adopt`完整会话与生产接管、生产root/keyring、生产磁盘库或Host联调。精确合同与启用前置见 [`node-plugin-manifest-catalog-authority.md`](node-plugin-manifest-catalog-authority.md)。v5 当前也只撤销下载/校验能力；继续保持零 PlanApply、零下载、零安装、零 Sidecar。
...
本文是已接受的客户端目标设计。NodeAgent legacy Host seam、Manifest/InstallPlan、本机 authority/PlanApply、受管 fetch/verification/staging、健康与失败 cleanup 合同已形成代码；v216 新增的 v7 installed/promotion 双回执、线性 Store/recovery 与 retained-handle custody 已随节点编译，v7 authority 全新安装、重开及 v3-v6 原子迁移等 11 项测试通过。v217 新增的 v8 work-admission、authenticated PlanApply barrier、线性 Store/recovery 与 retained-handle fresh rehash 目前仅形成源码，尚未编译、迁移或运行验证，也未接入生产 Bootstrap/Host。生产磁盘迁移、完整 promotion/work-admission 事务夹具、生产时间源、真实 downloader、跨重启 custody、Sidecar/IPC/沙箱、Host enforcement、Runtime 健康、Ready 上报和调度仍未实现；节点不直接发布商业 Offer。精确物化、健康和清理边界见 `node-plugin-archive-extraction.md`、`node-ready-capability.md` 与 `node-plugin-candidate-cleanup.md`。
