...
| 外部算力池适配器与统一报价 | v221-v266 仅保留历史证据。V267 已通过 Windows/WSL2 编译、WSL2 `v267_` 12/12、Yama 2 内核 12/12、transport 4/4 与 lifecycle 目标故障测试，状态为 `implementation_compiled/local_windows_and_wsl2_kernel_verified`；V268-V271 已随完整 Windows server 与 WSL2 test target 编译，专属动态仍为 `passed=0`。V272 只冻结独立 startup gate、14-root/8-exchange、2 表 1 诊断 view、短 TTL HMAC authority 与 3 条 admin route，严格为 `source_review_only/implementation_uncompiled/implementation_unrun`、`passed=0/failed=0`。Provider=`registering`、V254 18 deny 和打开 fence=`0` 不变；producer/worker、stable executor、active refresh 与 atomic activation 仍缺。见 [`V272 权威`](external-pool-adapter-task-protocol-conformance-authority.md)、[`V272 验收`](external-pool-adapter-task-protocol-conformance-acceptance.md) 与 [`当前状态`](current-implementation-status.md) |
...
Provider Adapter v227-v266 只保留历史结果。V267 current hardening 已完成 Windows/WSL2 编译、Yama 2 内核、ancillary 与 cleanup 故障验证；V268-V271 已随完整 Windows server 与 WSL2 test target 编译，但专属动态均为 `passed=0`。V272 仅冻结独立默认关闭的 ELTP v1 controlled conformance，当前严格为 `source_review_only/implementation_uncompiled/implementation_unrun`、`passed=0/failed=0`，不能打开 fence。Atomic activation 仍须按 V273 producer/worker+authenticated ingress → V274 active refresh/successor → V275 stable executor+atomic activation 闭合；V254 18 deny 在此前逐字保留。见 [`V272 conformance`](external-pool-adapter-task-protocol-conformance-authority.md)、[`V272 acceptance`](external-pool-adapter-task-protocol-conformance-acceptance.md) 与 [`当前状态`](current-implementation-status.md)。
