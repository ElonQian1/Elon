...
V1 不包含消费者偏好数据保险箱、字段级数据内容、外部 CRM 自动删除、平台级删除证明、法定时限判断、自动通知或处罚、跨运营方工单迁移、真实支付和链上对象。后续形成的有限手动催办与升级关注代码由 `docs/decisions/open-commerce-consumer-data-request-followups-v1.md` 单独约束。
