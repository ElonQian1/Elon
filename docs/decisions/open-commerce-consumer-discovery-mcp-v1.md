...
状态：已接受、已实现，并通过本地 Rust、SQLite、MCP 与领域服务同源假数据验收。
...
- 显式设置 App ID 时，服务端要求该开发者 App 处于启用状态，并同时属于当前用户和当前 MCP 项目；同一用户在其他项目的 App 失败关闭。
- 工具参数不能覆盖调用身份。

同一项目身份门禁也统一作用于该模块承载的执行计划和授权申请工具，避免通过相邻消费者工具绕过项目边界。
