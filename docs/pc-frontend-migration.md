...
| OneNote 式项目文档工作区（按需读写、Markdown 编辑/预览、低 token 项目图谱） | 🔴 高 | `src/features/project-docs/` | ✅ 已实现知识首页、最多四层主/副主题树、四维治理总览和可执行健康中心；项目图谱以 Rust 后端机器契约分别展示产品功能、技术架构和文档主题，文档覆盖与 `file:/test:/route:/symbol:` 实现证据分开，支持缩放、展开、搜索、局部详情、图级 AI 评审与节点级 AI 讨论；网页和供应商无关 MCP 共用同一事实来源，目录/图谱阶段零模型 token、零正文读取，并用工作区、manifest revision 和 knowledge-map revision 阻止旧模板快照冒充正式图谱。右键、`⋯`、`Shift+F10` 与触摸长按共用分区命令，个人排序留在浏览器，共享结构和审计写清单；健康问题支持筛选、分派、期限、原因化忽略/延期、选中后 AI 建议、趋势与评分解释，普通 Git 和托管知识库都可查看差异并安全恢复；AI 建议可包含 `proposed_knowledge_graph`，默认整理前/后双提交，可安全 rename/move Markdown，不夹带代码、不自动 push |
...
项目文档工作台现已包含“知识首页 / 项目图谱 / 主题分区 / 治理总览 / 文档健康 / AI 整理建议”。主题树支持一个主主题和多个副主题，治理总览把检索、生命周期、权威性和文档类型分开筛选编辑。“文档健康”直接展示目录 API 的服务端 `analysis`，包括可处理的问题状态、负责人和期限、趋势、评分解释、持续维护、联邦模块、Git 差异与安全恢复；前端本地分析只作为旧节点兼容回退，不能形成第二套健康度真源。
