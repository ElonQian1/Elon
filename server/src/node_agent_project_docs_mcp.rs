...
pub(crate) async fn handle_request(workspace: &Path, request: McpRequest) -> Option<Value> {
    ...
    let result = match request.method.as_str() {
        "initialize" => Ok(json!({
            ...
            "instructions": "先调用 project_docs_analyze；再按任务局部查询图谱、节点和 token 阅读计划。大型项目按 federation scope_id 分页。目录与图谱预分类不读正文且 classification_model_tokens=0；用 project_docs_get_issues 按状态、严重度、负责人取得证据，update_issue 分派/设期限/原因化忽略延期，get_health_history 看趋势；只用 project_docs_read 按需读歧义文档。建议可写主/副主题、四维 governance_facets、typed relations 和 proposed_knowledge_graph。authorization_mode 默认 git_backed_full：apply_suggestions 创建整理前基线，apply_file_operations 完成安全路径操作和整理后提交。普通 Git 与托管知识库均可 get_history/get_version_diff；restore 只创建新提交并拒绝混合代码版本。review_all 必须用户审核；suggestions_only 只保存建议。禁止覆盖、越界、修改代码或自动 push。"
        })),
