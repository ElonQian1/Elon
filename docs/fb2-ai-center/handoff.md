...
- 2026-06-22 20:36 最新严格直接群聊接口验收证明“不是截图问题”：`target\fb2-ai-center\data-only-acceptance-20260622T123604Z.json` 返回 `visible_chat_exit_code=0`、`feedback_coverage.complete=true`、`observed_count=3/3`，并通过群聊/summary-post 接口直接回读 `@EL` 回复 `gai_9c23c6474b5a454d9b8cbbbdfdaf79de`、selected-message `AI回复` `gai_d324da35daba4302b8ca1636f24f2e8b`、summary post `gsp_4e1a6b0042da4dbfb61b62c5563ecec7 status=ready`。三类 feedback 均已写回且 matched=7/unmatched=0；当前唯一失败是短窗口 `quality non-synthetic adoption count value=0 min=1`。主项目本轮已把 data-only visible 的默认 `MinOpinionAdoptionCount=1` 保留下来，并在 `social_ai` 里补 fb2 群观点记忆 source id 兜底：当回复使用群友观点且 `opinion_memories` 工具结果 grounded 时，自动补 `opinion_memory_id` / `source_message_id` 来源行，促使 `record_opinion_adoption` 能落库。下一步是提交、推送、发布主项目服务端，然后用同一条 `-DataOnlyAcceptance -AllowVisibleMessages -GroupId official` 复测；ASR/TTS 继续暂停。
