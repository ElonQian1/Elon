...
| 节点插件治理合同 | Signed Manifest/InstallPlan、双 keyring、authority v3/PlanApply、受管取数、候选验证、ZIP 安全物化及 staging/健康/quarantine Store 均已写。显式共享策略 ACK 后的独立 v10 InstallPlan 准备握手与 v209 云端追加式账本已铺设，但节点诚实返回 `context_ready=false`：生产 Signer/KMS、Manifest catalog、coherent inventory/profile/双 keyring、可信时间以及首次 enabled policy 写入 authority 的无下载 CAS 都缺，禁止伪造 signed Plan。失败候选已有 cleanup authorization Store；其后 sealed topology 与首对象 intent/disposition/absence/namespace-durability 共五个独立 typed Store，并具备线性 outcome-uncertain recovery 和原子 completion 内核。序号 4 的同句柄 pre-absence、Windows native barrier、post-absence、evidence 与 Store/recovery 形状已写；mutation fence 已改为绑定 exact object/name/parent、plan/authorization/object/ordinal、installation 与 authority/process generation，且随 retry、terminal、physical/Store custody 唯一移动的线性租约。签名 Windows minifilter v1 现已有固定 LE codec/descriptor、完整 grant/query/release shape 和独立首方 PrivilegedComponent 供应链合同，但 WDK 驱动、首方/Windows catalog 验签、安装/transport、安全构造器及 evidence v2 均缺且 gate 固定失败。远程 sequence 2 基线通过 `elon-pc-node` 编译、24 项清理定向测试及 1 项 schema 建库重开测试；sequence 3/4、共享 v9/v10 与 v209 未编译、未测试。后续 ordinal、显式 fence release、terminal journal、跨重启物理恢复、完整事务夹具与 Host 均缺，因此当前仍不产生生产可达的 completion/retry、installed、ready 或商业 Verification；生产数据库/时间源/回滚见证、HTTPS downloader、Sidecar/IPC/探针、installed/promotion、ReadyCapability 和 Attempt 接线也仍缺 |
...
节点失败候选清理的 schema v3 已包含 expected-object topology、candidate-parent identity anchor、plan seal 与四阶段 hash-chain step journal，并要求 completion 同时绑定 plan digest 和 terminal namespace-durability digest。当前代码已形成 topology 及首对象四阶段 typed Store、handle-derived identity/parent/name binding、Windows authority-domain parent namespace barrier 和进程内 exact recovery；mutation fence 已具备 exact scope/authority binding、线性 custody、固定 v1 wire codec/descriptor 与独立首方驱动供应链 shape，但真实 WDK driver/transport/trust/install 尚未实现，安全构造器仍不存在。远程仅验证到 sequence 2，本批 sequence 3/4 与 minifilter 合同未编译或测试。evidence v2、显式 release、后续对象、terminal journal、跨重启物理恢复、旧执行器适配与 Host 仍未实现，不能理解为节点已经会自动清理生产失败候选。
...
目标流程仍是：共享关闭时不下载重型组件；开启后按硬件和任务选择签名插件、运行时与模型工件；只有用户开启并确需 cleanup hard-fence 时，才按独立首方特权组件合同下载并经 UAC 安装驱动，普通插件签名无权授权内核代码。本机耐久真源已选定为独立 SQLite，根签名双 keyring、原子计划应用、候选所有权、三段式下载认领、候选级全工件验证和可信时间边界见 `docs/distributed-compute/node-plugin-local-authority.md`。raw verified 只把本机槽推进到 `verifying`；staged 候选可经健康失败 quarantine 进入 `failed`，再进入 cleanup authorization。固定句柄旧执行器和 completion Store 内核的远程基线已编译；中间的新路线已能封存 topology 并写入首对象 intent、disposition 与 absence，sequence 4 的 native barrier/Store/recovery 形状也已写，mutation fence 也已成为 exact scope/authority-bound 的线性租约。签名 minifilter 的 wire/供应链 shape 已写，但真实驱动、信任/安装/transport、safe constructor、evidence v2 与显式 release 未实现。sequence 3/4 未编译测试，后续对象/terminal producer 也缺。只有完整 journal exact readback 后返回的不透明终态能力才能进入 completion，并恢复为 `NotCreated` 或 exact `Completed`。因此当前完整清理链仍不可达，不会因内存物理证据或首对象完成直接释放 owner。候选观察值仍由 Host 调用方提供，尚未证明真实 Sidecar 已运行；生产时间权威、真实下载器、Sidecar/IPC/探针调度、完整清理事务夹具、跨重启恢复、Host 接线、installed/promotion、云端 capability gate 和通用 Attempt 协议仍未实现。
