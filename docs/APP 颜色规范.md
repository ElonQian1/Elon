...
本表按 1272px 宽参考图量测，按 3x 截图密度折算为 Android `sp`。`参考图可见字高` 是截图里的字形包围盒高度，用于视觉验收；实现时以 `textSize` 和 `lineHeight` 为准。

| 场景 | textSize | 字重 | lineHeight | 参考图可见字高 | 来源 |
|---|---:|---:|---:|---:|---|
| 一级页面大 Tab | `26sp` | `700` | `34sp` | `71px` | 图 1“我的项目”、图 2“项目广场” |
| 详情页导航标题 | `20sp` | `500` | `28sp` | `51px` | 图 3 顶部“魔王” |
| 项目广场卡片标题 | `22sp` | `700` | `30sp` | `46px` | 图 2 卡片“魔王” |
| 项目广场状态文字 | `17sp` | `400` | `24sp` | `39px` | 图 2“无需审批”“可安装” |
| 搜索框占位 | `22sp` | `400` | `30sp` | `54px` | 图 2“搜索应用” |
| 筛选 / 分段文字 | `18sp` | `400` | `26sp` | `43-44px` | 图 1“独立”、图 2“全部” |
| 我的项目列表标题 | `17sp` | `500` | `24sp` | `38px` | 图 1“项目名称” |
| 我的项目列表描述 | `17sp` | `400` | `24sp` | `37px` | 图 1“亲子、旅行、购物一体” |
| 我的项目列表元信息 | `14sp` | `400` | `22sp` | `31px` | 图 1“创建者：叶云 成员：1” |
| 项目广场元信息 | `16sp` | `400` | `24sp` | `34-36px` | 图 2“创建者：叶云”“成员：1” |
| 项目广场简介 | `16sp` | `400` | `24sp` | `34-36px` | 图 2“简介：一款自动化...” |
| 主按钮文字 | `16sp` | `700` | `22sp` | `33-34px` | 图 2“进入空间”“下载APK” |
| 底部导航文字 | `14sp` | `400` | `20sp` | `38px` | 图 1/2“好友”“项目”“我的” |
| 项目空间公告标题 / 正文 | `22sp` | `400` | `30sp` | `49-50px` | 图 3“公告”“不得发布...” |
| 项目空间空状态 | `16sp` | `400` | `24sp` | `40px` | 图 3“还没有帖子...” |
| 弹出菜单文字 | `20sp` | `400` | `28sp` | `45-46px` | 图 3“项目文档”“下载APK” |
| AI 会话按钮文字 | `22sp` | `400` | `30sp` | `52px` | 图 3“AI 会话” |

文字颜色规则：

- 一级 Tab、项目广场卡片标题、状态文字使用 `color.text.primary`。
- 我的项目列表项标题使用 `color.text.list.title`。
- 项目空间详情页居中标题使用 `color.text.detail.title`。
- 元信息、简介使用 `color.text.secondary`；搜索占位、空状态、公告正文使用 `color.text.placeholder`。
- 白色按钮内文字使用 `color.text.inverse`，字重必须加粗。
...
- 底部导航可视背景高度：约 `76dp`；系统手势安全区保持 `#000000`，不要和底部导航灰色背景连成一整块。
...
- 标题居中，字号 `20sp`，不加粗到页面大 Tab 的级别。

### 底部导航

- 底部导航固定三项：好友、项目、我的。
- 背景 `color.nav.bg`，可视高度约 `76dp`；系统导航栏 / 手势安全区使用 `color.bg.mine`，避免灰色背景板块过高。
- 图标使用线性轮廓，约 `24dp` 到 `28dp`；文字按设计图视觉高度锁定，约 `12sp` 到 `14sp`。
...
- 搜索图标 `24dp`，左内边距 `20dp`。
- 占位文字 `22sp`，颜色 `color.text.placeholder`。
...
- 菜单文字使用 `color.text.menu`，字号固定 `20sp`。
