...
外部锚合同现已形成独立代码边界：节点为每次“查询最新锚”生成最多存活 60 秒的一次性随机挑战；见证回执必须用专用 Ed25519 域精确绑定挑战、安装身份、锚 ID/序列和完整规范检查点，经发布内置的专用 key resolver 验证后才成为不可 Clone、不可序列化的已验锚。启动比较只在本机与外部检查点高水位及摘要完全一致时产生线性启动许可；本机落后、同高水位摘要分叉、关键 revision/epoch/keyring 回退均失败关闭。本机领先时不授予启动，只产生携带完整本机检查点的一次性“必须发布新锚”能力，避免后续发布器根据摘要重建权威事实。
...
该合同仍不是可用的数据库外锚服务：仓库尚无生产见证服务器、经身份认证的传输会话、真实网络查询与发布协议、生产见证根公钥，也未接入 NodeAgent 启动门卫。客户端 publication challenge 只绑定新鲜度与内容，不能替代见证服务对发布节点身份和安装身份的认证。无法取得可信最新锚时仍必须失败关闭；不得把 DTO、验签、比较与回执能力描述成已经解决整库回滚。
