...
| 开发者 App 签名 Webhook | 代码已形成、未编译 | App 所有者可配置白名单内 HTTPS 回调；新订阅须先完成签名 challenge 精确回显，验证后终态事件才进入耐久队列，工作器按短租约、指数退避、死信和自动停用规则处理。密钥只在创建或显式轮换时显示一次，轮换会停用订阅并要求重新验证，版本 1 保持既有派生兼容；Node SDK 可验签。当前尚未完成编译、迁移、并发、网络、跨语言签名和 UI 验证，也不是工商身份、跨运营方总线或支付回调 | `server/src/open_commerce_webhook_*.rs`、`server/src/store/open_commerce_developer_webhook*.rs`、`pc-frontend/src/features/open-commerce/DeveloperWebhookPanel.tsx`、`docs/open-commerce-developer-webhook-secret-rotation-v1-acceptance.md` |
