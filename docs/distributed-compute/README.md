...
| 节点端点凭据与认证会话权威 | 服务端 v216 已铺默认空的 credential/version/revocation/session receipt/session head 五表及 sealed Store currentness 内核；零 legacy backfill，未接 owner HTTP、WS 认证、NodeAgent 或协议。后续源码为 legacy WS 增加 process-local exact key、Manager→Registry 固定锁序及三条插件 ACK read fence，并新增默认关闭的 direct-TLS-only listener：只有同一 rustls TLS 1.3 握手能形成一次性、boot-bound transport proof；plain/legacy listener、URL 与代理头均不能构造。secure `/agent/ws` 当前取 proof 后固定 503，不 upgrade、不写 v216 Store，故仍不等于在线 socket、Ready、route、outbox、command、Lease 或派发权威。源码尚未编译、测试、运行或执行迁移。见 `node-endpoint-session-authority.md` |
