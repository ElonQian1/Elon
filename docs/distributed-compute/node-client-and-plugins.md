...
这一步刻意没有伪造签名计划：仓库只有节点侧 Ed25519 验签与严格 admission，没有生产 Control 私钥、Signer/KMS/HSM，也没有云端 Manifest catalog、signed Manifests、coherent inventory/node-profile/双 keyring 事实或生产 Bootstrap root/trusted-time resolver。更前置的循环依赖是本机 authority 初始化固定为 sharing disabled，而 InstallPlan admission 又要求计划授权与 live authority 精确相等；当前没有一条无下载的 policy/authorization-binding CAS，因此首份 enabled Plan 不可达。正确后续顺序是先补该 CAS 与 planning snapshot，再由默认 unavailable 的真实 signer 接口产生计划，节点验签展示并取得本机 exact confirmation；在这些门卫全部闭合前继续保持零下载、零安装、零 Sidecar。
