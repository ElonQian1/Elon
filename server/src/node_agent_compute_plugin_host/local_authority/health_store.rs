...
use crate::node_agent_compute_plugin_host::{
    candidate_health_contract::{
        ValidatedCandidateHealthPublication, ValidatedCandidateHealthStorePermit,
    },
...
pub(in crate::node_agent_compute_plugin_host) struct ComputePluginCandidateHealthAuthoritySession<
    'authority,
> {
...
pub(in crate::node_agent_compute_plugin_host) struct ComputePluginCandidateHealthAuthorityFacts {
...
impl ComputePluginLocalAuthority {
    pub(in crate::node_agent_compute_plugin_host) fn bind_candidate_health_authority_session<
        'authority,
    >(
        &'authority self,
        process_fence: &'authority ComputePluginFetchProcessFence,
        observation: &ComputePluginTrustedTimeObservation,
    ) -> Result<ComputePluginCandidateHealthAuthoritySession<'authority>> {
        ...
        {
            bail!("COMPUTE_PLUGIN_CANDIDATE_HEALTH_AUTHORITY_SESSION_INVALID");
        }
        Ok(ComputePluginCandidateHealthAuthoritySession {
            authority: self,
            process_fence,
            trusted_now,
            observed_at,
...
impl ComputePluginCandidateHealthAuthoritySession<'_> {
    ...
    pub(in crate::node_agent_compute_plugin_host) fn read_candidate_health_binding(
        &self,
        ...
        self.authority.with_deferred(|transaction| {
            binding::read_candidate_health_binding(transaction, self, publication)
        })
    }

    pub(in crate::node_agent_compute_plugin_host) fn persist_candidate_health(
        &self,
        ...
        self.authority.with_immediate(|transaction| {
            write::persist_candidate_health(transaction, self, permit)
        })
    }
}

impl ComputePluginCandidateHealthAuthorityFacts {
