...
| 外部算力池适配器与统一报价 | v221-v244 前置门为 `implementation_partially_verified`；V243 已形成短时凭据验证回执，V244 已形成可撤销采用授权，但仍未安装或激活生产 Adapter。见 [`当前状态`](current-implementation-status.md) 与 [`V244 采用权威`](external-pool-adapter-adoption-authority.md) |
...
Provider Adapter 的 v227-v244 前置门已形成分层权威；最新状态见 [`current-implementation-status.md`](current-implementation-status.md)，签名沙箱边界见 [`V239 动态沙箱符合性证据`](external-pool-adapter-artifact-sandbox-conformance-authority.md)，采用边界见 [`V244 采用授权`](external-pool-adapter-adoption-authority.md)。服务器仍不真实安装或执行制品，不返回原始凭据，也未启用生产 Adapter。
