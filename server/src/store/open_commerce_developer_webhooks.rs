...
use crate::{
    ...
    open_commerce_webhook_model::{
        ...
        DEVELOPER_WEBHOOK_DELIVERY_SCHEMA,
    },
};
        ...
        let claim = tx.query_row(
            &format!(
                "SELECT delivery.id, delivery.subscription_id, delivery.invocation_id,
                        delivery.event_sequence, delivery.event_type, delivery.enqueue_source,
                        ...
                        delivery.delivered_at, delivery.dead_letter_acknowledged_at,
                        delivery.dead_letter_acknowledged_by_user_id,
                        delivery.dead_letter_acknowledgement_reason,
            ...
            |row| {
                Ok(DeveloperWebhookDeliveryClaim {
                    ...
                    owner_user_id: row.get(20)?,
                    app_id: row.get(21)?,
                    environment: row.get(22)?,
                    callback_url: row.get(23)?,
                    signing_key_id: row.get(24)?,
                    signing_secret_version: row.get(25)?,
