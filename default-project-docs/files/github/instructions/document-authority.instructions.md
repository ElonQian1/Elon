...
- 主题知识树只回答“讲什么”；`retrieval`、`lifecycle`、`authority`、`document_type` 分别回答“是否读取、是否当前、能否作为事实、是什么类型”。不得把决策、工作区、草稿等状态作为业务主题，主题位置不能提高路径权威性。
...
- 大型仓库可用 `.elon/knowledge-federation.json` 声明项目根、子项目和模块节点，并用 `scope_path`、`include_globs`、`exclude_globs` 组合范围；先选择命中任务的 `scope_id`，再在该节点内分页。
...
普通 Git 项目和个人笔记都可查看文档历史与差异。普通项目只以新提交回滚仅文档、非合并的祖先提交；个人笔记由平台用 `vaultId` 创建隐藏托管 Git 库，用户无需理解 Git。
