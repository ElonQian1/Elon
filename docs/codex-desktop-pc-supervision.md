...
最后更新：2026-07-26
状态：paused（自 2026-07-26 起暂停使用）

> 暂停期间不再新建、等待、验收、改进、续跑或变更监督任务。已有任务不自动继续，也不自动取消；保留任务记录、journal、worktree lease 与审计证据，等待用户明确处置。普通项目任务改由当前代理直接按照目标仓库规则完成。本文其余内容保留为历史设计、故障恢复与未来恢复启用时的参考。

## 目标

在暂停前，当用户在 Codex 桌面端讨论项目改动时，默认由一龙 PC 本机节点启动 Codex CLI 完成真实修改，桌面端负责需求拆解、过程监督、独立验收和能力改进决策。这个机制同时长期实测 PC 端，但不把一次任务变成无限自改，也不把执行者的自述当成验收。
