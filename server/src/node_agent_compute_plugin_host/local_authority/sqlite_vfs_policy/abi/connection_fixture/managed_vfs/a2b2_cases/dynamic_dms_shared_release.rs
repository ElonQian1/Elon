//! Physical-SHM-only actual bridge for final Keep/DmsSharedRelease/AfterSuccessKnown.
//!
...
//! `Released` is coordinator state synchronized only after the platform unlock succeeds; it is not
//! an independent observation of the Windows kernel lock table.
...
pub(in super::super) struct DmsSharedReleasePhysicalSubsetActual {
...
pub(in super::super) fn validate_dms_shared_release_after_success_physical_subset(
    actual: DmsSharedReleasePhysicalSubsetActual,
) -> Result<(), &'static str> {
...
fn select_frozen_case(inventory: &[Case]) -> Result<&Case, &'static str> {
    let mut selected = inventory.iter().filter(|case| {
        case.path == Path::Unmap
            ...
            && case.phase == Phase::DmsSharedRelease
    ...
    let case = selected
        .next()
        .ok_or("frozen DmsSharedRelease case missing")?;
    if selected.next().is_some() {
        return Err("frozen DmsSharedRelease case is not unique");
    }
    Ok(case)
}
...
fn validate_static_boundary(expected: &Case) -> Result<(), &'static str> {
    ...
    check(expected.retained.node, true, "frozen node custody")?;
    check(expected.retained.shm_file, true, "frozen SHM-file custody")?;
    check(
...
fn validate_post(
    ...
    check(
        actual.topology.dms,
        ManagedSqliteShmTestDmsCustody::Released,
        "post DMS custody",
    )?;
    check(
        actual.topology.shm_file_present,
