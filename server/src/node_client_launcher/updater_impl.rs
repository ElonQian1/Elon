...
pub(super) fn auto_update_disabled(env_values: &std::collections::HashMap<String, String>) -> bool {
    env_values
...
pub(super) fn try_update_from_client_package(
    install_dir: &Path,
    package_url: &str,
    expected_sha256: &str,
    remote_text: &str,
...
pub(super) fn update_uses_system_proxy(env_values: &HashMap<String, String>) -> bool {
    env_values
...
pub(super) fn matches_truthy(value: &str) -> bool {
    matches!(
        value.trim().to_ascii_lowercase().as_str(),
        "1" | "true" | "yes" | "on"
    )
...
pub(super) fn download_bytes_with_retries(
...
pub(super) fn download_bytes_once(
    client: &reqwest::blocking::Client,
    url: &str,
    read_context: &'static str,
) -> Result<Vec<u8>> {
...
pub(super) fn env_u64(name: &str, default: u64, min: u64, max: u64) -> u64 {
    std::env::var(name)
        .ok()
...
pub(super) fn replace_client_package(
    tmp_zip: &Path,
    install_dir: &Path,
    tmp_version: &Path,
    version_file: &Path,
...
pub(super) fn schedule_self_replace_package(
    tmp_zip: &Path,
    install_dir: &Path,
    tmp_version: &Path,
    version_file: &Path,
...
#[cfg(windows)]
pub(super) fn package_self_update_via_repair_script(
    pid_to_wait: u32,
    tmp_zip: &Path,
    install_dir: &Path,
    tmp_version: &Path,
...
#[cfg(windows)]
pub(super) fn package_replace_script(
...
pub(super) fn replace_client_files(
    tmp_exe: &Path,
    client: &Path,
    uninstall: &Path,
    tmp_version: &Path,
...
pub(super) fn running_from_path(path: &Path) -> bool {
    std::env::current_exe()
        .ok()
...
pub(super) fn schedule_self_replace(
    tmp_exe: &Path,
    client: &Path,
    uninstall: &Path,
    tmp_version: &Path,
...
#[cfg(windows)]
pub(super) fn self_replace_script(
    tmp_exe: &Path,
    client: &Path,
    uninstall: &Path,
    tmp_version: &Path,
...
pub(super) fn update_base_url(env_values: &std::collections::HashMap<String, String>) -> String {
    env_values
