...
长期上，能力路由不是“普通聊天优先 vs 开发任务”的二分，而是多条并行路线的选择：项目 CLI 透传、项目执行、产品讨论、Skill 规划、验收发布、图片/资产旁路、非执行类项目问答等都可以是合法路线。`intent_router` 只是在未显式指定路线时提供确定性辅助；总调度 AI 在其后负责需求整理和 Skill 选择，不能让低成本分类模型直接获得写代码或发布权限。
...
| 规划 / Skill 路线 | 目标阶段枚举 | 一龙会话主 AI、总调度 AI、Skill Router | 产品讨论、Matter、预算、风险、验收标准 | 一等路线；用于正式开发前的决策和确认 |
...
这些是产品目标阶段，不表示当前代码枚举已经全部实现。MVP 应先在现有 `ChatAgent -> CodeAgent` 之间增加可选的 `SkillPlanning`，并保持明确的小改动可以直接进入正式开发。
...
- 文档、UI、诊断文案优先使用具体路线名：项目 CLI 透传、Route A 直连、项目执行、规划 / Skill、非执行类项目问答。少用“普通聊天/闲聊”作为项目会话的总称。
