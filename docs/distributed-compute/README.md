...
| Attempt 挑战终态决议与历史 | v197、追加式 Store、角色 open/历史队列、HTTP 与 PC 控制面已写；消费者在 `/compute-challenges` 可撤回本人 open 申诉并查看本人证据链，管理员在 `/compute-challenge-resolution` 可裁决并查看全局历史。历史逐项重审计 v195-v199 引用和状态组合；open/accepted 阻断，withdrawn/rejected 解除挑战门卫，available 仅指内部余额且不证明外部付款，尚未编译、执行迁移、接口联调、页面验收或发布 |
