...
pub(crate) fn list_withdrawal_queue_for_provider_owner(
    store: &Store,
    user_id: &str,
    provider_id: &str,
    status: &str,
    ...
    let account = store.compute_settlement_account_view(provider_id)?;
    if account.owner_user_id != user_id {
        bail!("算力 Provider 结算账户不属于当前登录用户");
    }
    store.list_compute_settlement_withdrawal_queue_for_provider(provider_id, status, limit)
}
