...
| 算力执行工作区 | 🔴 高 | `src/features/compute-execution/`、`server/src/compute_federation_attempt_*` | 🟡 所有登录用户可见的 `/compute-execution` 已写入本人 Provider 选择、待激活 Reservation 候选、首次 Attempt 激活、Lease 查询/续租及 revision 1 无心跳 staging 中止源码。激活只登记外部执行器已接受声明，续租只登记外部心跳，中止才原子退款并归还容量；不发送节点命令、不验证外部证明、不登记用量或结算，尚未构建、接口联调、视觉验收或发布 |
