...
| 通用 ERP 蓝图 / 独立商户实例 / AI 通用提案 / 兼容升级与回滚 | `docs/erp/README.md`、`docs/erp/api-and-agent-tools-v1.md`、`server/src/erp_blueprint/`、`server/src/erp_blueprint_api.rs`、`server/src/erp_blueprint_mcp*.rs`、`server/src/store/erp_*.rs`、`contracts/erp/` |
| 消费者发现 / 第三方应用沙盒 / 一次性测试 Token / 授权审批 | `docs/open-commerce/consumer-developer-sandbox.md`、`server/src/open_commerce_client_*.rs`、`server/src/open_commerce_consumer*.rs`、`pc-frontend/src/features/open-commerce/ConsumerCommerceSandbox.tsx`、`DeveloperCommercePortal.tsx` |
