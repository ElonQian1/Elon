...
| 外部算力池适配器与统一报价 | v221/v222 管理面、MCP、Store 重开与 PC 静态构建已有专项。v227 raw bytes source 和 v229 三终态/currentness 已通过完整服务端编译及 51 项 Windows 临时 DATA_DIR/SQLite 测试，覆盖 HTTP、迁移/重开、双连接竞争、terminal↔artifact 顺序、CAS、失败清理、恢复和目录 reparse，状态为 `implementation_partially_verified`。仍缺真实崩溃/断电、Windows leaf symlink、Unix 权限/symlink、真实 TCP、生产升级和部署；v227/v229 无 MCP/PC，也不证明 candidate ref、provenance、verifier、Adapter、v213 route、Pool/Offer 或派发。见 [`external-pool-adapter-release-api-acceptance.md`](external-pool-adapter-release-api-acceptance.md) 及两份权威文档 |
...
服务端 Provider Adapter 统一接入公司集群、云 GPU 和其他算力池；每个 Provider 保留自己的内部调度，只向一龙提交标准回执。v221/v222 已通过管理面专项；v227 [`artifact bytes source`](external-pool-adapter-artifact-source-authority.md) 与 v229 [`release-admission lifecycle`](external-pool-adapter-release-lifecycle-authority.md) 又通过 51 项 Windows 本地专项，覆盖 raw body→quarantine→重开 SHA-256、三终态、successor、current view、HTTP、三层 currentness、migration/重开、并发、CAS/DB saga 与目录 reparse，当前仅为 `implementation_partially_verified`。真实崩溃/断电、Windows leaf symlink、Unix 权限/symlink、真实 TCP、生产升级和部署仍缺；无 MCP/PC，也不证明 provenance、verifier、credential、Adapter、route、容量、派发或结算。证据见 [`external-pool-adapter-release-api-acceptance.md`](external-pool-adapter-release-api-acceptance.md)。
