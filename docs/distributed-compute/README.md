...
| UserNode Windows Runner loader/进程监管前置草案 | 未登记且不可达：loader 定义单一 successor/by-value seam、FileId leases/reopen receipts 与五类 failure custody；extraction directory share custody 已写 DELETE owner + narrow share-delete probe，其上新增 Runner/package-root/全部 plan-directory retained handle-chain 候选发现，但不选择 CWD、不授予 grant，两层 Windows 动态验证均缺。exact PE/launch authority/live-OS、launch-security/currentness/release-recovery producers 均缺，pre-resume/dynamic-load blocked；严格 `source_written/source_review_only/implementation_uncompiled/implementation_unrun`、`passed=0/failed=0`、`migration/table/writer=none/none/none`、四 gap missing、loader 18 effects none。见 [directory-share authority](user-node-windows-runner-extraction-directory-share-custody-authority.md)、[directory-share acceptance](user-node-windows-runner-extraction-directory-share-custody-acceptance.md)、[launch-path discovery authority](user-node-windows-runner-launch-path-discovery-authority.md)、[launch-path discovery acceptance](user-node-windows-runner-launch-path-discovery-acceptance.md)、[loader authority](user-node-windows-runner-loader-load-set-authority.md)、[loader acceptance](user-node-windows-runner-loader-load-set-acceptance.md)、[process authority](user-node-windows-runner-process-custody-authority.md) 与 [process acceptance](user-node-windows-runner-process-custody-acceptance.md) |
