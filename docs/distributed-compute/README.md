...
| 节点插件治理合同 | Signed Manifest/InstallPlan、双 keyring、authority v3/PlanApply、受管取数、候选验证、ZIP 安全物化及 staging/健康/quarantine Store 均已写。失败候选已有 cleanup authorization Store、旧式固定句柄执行器和原子 completion Store 内核；completion 只接受 sealed topology、物理证据与 terminal namespace-durability journal 共同绑定的不透明能力。远程 canonical 最新组合通过 `elon-pc-node` 编译、8 项清理定向测试及 1 项 topology/journal schema 建库与重开测试，但完整 authorization → prepare 链未覆盖且 terminal capability 仍无构造入口。本批形成未编译的 handle-derived 对象/父级/原始名称 binding、拒绝全零 FileId 的门卫、保留 exact parent custody 的强 disposition，以及父句柄相对 `Absent/ExpectedIdentityMatch/IdentityConflict` 观测；两类 name-present 结果均不可降级，`namespace_durable` 明确不可用。旧执行器尚未接入强路径，并存在 retained 普通目录句柄与 late `DELETE` reopen 的 Windows share-access 冲突。当前仍不产生生产可达的 durable completion/retry、installed、ready 或商业 Verification；sharing-on、生产数据库/时间源/回滚见证、topology/journal typed Store、单一 cleanup handle tree、durable intent/journal 执行器、真实 parent namespace flush、完整事务夹具、跨重启恢复、HTTPS downloader、Sidecar/IPC/探针、installed/promotion、ReadyCapability 和 Attempt 接线仍缺 |
...
目标流程仍是：共享关闭时不下载重型组件；开启后按硬件和任务选择签名插件、运行时与模型工件。本机耐久真源已选定为独立 SQLite，根签名双 keyring、原子计划应用、候选所有权、三段式下载认领、候选级全工件验证和可信时间边界见 `docs/distributed-compute/node-plugin-local-authority.md`。raw verified 只把本机槽推进到 `verifying`；staged 候选可经健康失败 quarantine 进入 `failed`，再进入 cleanup authorization。固定句柄物理执行器和 completion Store 内核均已编译，但二者之间必须先由尚未实现的 topology/journal Store 逐步持久化 durable intent、exact disposition、父级相对 absence 与 namespace durability；只有该 Store exact readback 后返回的不透明终态能力才能进入 completion，并恢复为 `NotCreated` 或 exact `Completed`。因此当前完整清理链仍不可达，不会因内存物理证据直接释放 owner。候选观察值仍由 Host 调用方提供，尚未证明真实 Sidecar 已运行；生产时间权威、真实下载器、Sidecar/IPC/探针调度、完整清理事务夹具、父级 identity/absence、目录 durability、跨重启恢复、Host 接线、installed/promotion、云端 capability gate 和通用 Attempt 协议仍未实现。
