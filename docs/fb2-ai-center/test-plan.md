...
- fb2 最新 `fb2_p4_vector_readiness_plan_v1`、`fb2_p4_vector_contract_v1` 和 `fb2_p4_embedding_build_dry_run_v1` 只能作为只读计划/契约设计/no-write dry-run 验收，不是生产 grounding，也不是当前 completion blocker。主项目验收时必须确认该计划能力保持 `contract_design_committed_embedding_not_started`、`dry_run_available_no_writes`、`production_grounding=false`、`blocks_data_goal=false`、`writes_embedding_rows=false`、`writes_vector_store=false`、`refresh_operations_used=false`、`candidate_rows_require_live_hydration=true`、`vector_rows_are_model_input=false`、`does_not_enable_vector=true`、`ready_to_build_embedding_store=false`、`ready_to_enable_answer_time_vector_candidates=false`、`answer_time_vector_candidates_enabled=false`，并确认 `fb2.search_vector_candidates` 仍是 `implemented=false` 的 reserved planned query；只有 fb2 后续完成 source-specific no-write chunk enumerators、permission filter SQL、idempotent embedding write transaction、live hydration、shadow eval 和 answer-time readthrough 后，才允许新增单独的 P4/vector 生产验收。
