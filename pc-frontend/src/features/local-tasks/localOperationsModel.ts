...
export function normalizeGlobalPublishStatus(payload: unknown): GlobalPublishStatus {
  ...
  return {
    ...
    coalescingKey: textValue(global.coalescingKey) || 'batchId+stage+kind+sha',
...
function normalizeBatch(payload: unknown): ReleaseBatchLedger {
  ...
  return {
    ...
    expectedStages: arrayValue(batch.expectedStages).map(textValue).filter(Boolean),
...
function normalizeBatchStage(payload: unknown): ReleaseBatchStage {
  ...
  return {
    ...
    phase: textValue(stage.phase) || undefined,
    phaseStatus: textValue(stage.phaseStatus) || undefined,
