...
pub(crate) fn tool_definitions() -> Vec<Value> {
        ...
        tool(
            ...
            json!({
                ...
                "properties":{
                    ...
                    "preferEmulator":{"type":"boolean","default":false,"description":"纯视觉任务可显式优先申请空闲模拟器 slot；不会因为真机在线而复用真机"},
                    "fastFailPhysicalVisualProbe":{"type":"boolean","default":false,"description":"物理设备在线但锁屏、AOD、通知栏遮挡或窗口不可读时立即回退模拟器"},
                    "taskId":{"type":"string","description":"可选业务任务身份；写入 rendererLease.owner 便于并发审计"},
    ...
    definitions.extend(super::design_tools::tool_definitions());
