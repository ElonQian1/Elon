...
A2a/A2b1 的 exact logical-name routes 共享 WAL/SHM runtime，但每条 Connection 独立 route/authorizer/custody；完整 terminal projection、`SourceBranch`、`Expected`、`CaseKey`、exclusion 与 StaticContract 已闭合为 Map `43476/43476`、Lock `8668/8668`。typed quotient 只有前序验证基线；current runner-admission source 未编译、未运行。Map q4 `MapRegionLoopSuccessV1` 为 511 frozen/509 net-new，未运行 inventory 预期 `521` present、`42,955` missing。Lock q1–q15 current source 中，q12/q13 分别提供 CreatedFirst/ExistingFirst DMS release outcome uncertain 的 88 个 exact singleton；q14/q15 `LockNativeAcquire{CreatedFirst,ExistingFirst}TruncateErrorReleaseSucceededV1` 又分别提供 DMS truncate outcome uncertain、release success observed 的 88 个 exact singleton；q11 的 11 个 raw-state、q10 的 7 个 ABI scalar 与 q9 的 528 个 pre-managed singleton 保持不变。精确 matcher、production seam、`controlled_fault_actual`/natural actual 边界与 receipt 仅见[`Lock dynamic tranches authority`](node-plugin-vfs-lock-dynamic-tranches-authority.md)。q15 catalog 为 88 rows / 17,682 bytes，SHA-256=`d8ac9985f433b9fbd089a8804136f107cf37bd21566b5852611e98674f52911b`。Lock 未运行 inventory（members/groups）=`4020/4020 present, 4648/4120 missing, 8668/8140 total`；完整 initialization umbrella=`3432 members/2904 groups`，q12–q15 各取 88/88 后 remaining=`3080/2552`。以上均仅为 `source_written/source_review_only/implementation_uncompiled/implementation_unrun`，`passed=0 failed=0 actual=not_run`、`controlled_fault_actual=source_only_unrun`；coverage=`0/8668`，无 actual inventory/receipt/record、reviewed digest/frozen manifest，`Qlock=unknown`、`WindowsDynamic=not_opened`、production closed。q9–q14 checkpoint 的 19 份 global frozen/source-owner artifacts refresh 已逾期并继续独立 deferred，未来接受前必须覆盖 q15，禁止做半套 owner 重绑。A2b2=`117/117` 不能替代该门或启用生产入口。
...
当前静态验收已闭合为 Map `43476/43476`、Lock `8668/8668`；projector/candidate 只保留前序验证基线。Lock q12/q13 release-error slices 与 q14/q15 CreatedFirst/ExistingFirst truncate-error/release-succeeded slices 各为 source-only 88 exact singleton；未运行 inventory（members/groups）=`4020/4020 present, 4648/4120 missing, 8668/8140 total`，完整 initialization umbrella=`3432/2904`，q12–q15 各取 88/88 后 remaining=`3080/2552`。current source=`source_written/source_review_only/implementation_uncompiled/implementation_unrun`（uncompiled/unrun），`passed=0 failed=0 actual=not_run`、`controlled_fault_actual=source_only_unrun`；无 actual inventory/receipt/record，inventory/reviewed digest 缺失且 manifest 未冻结，`Qmap/Qlock=unknown`、coverage=`0/43476 + 0/8668`、`WindowsDynamic=not_opened`、production closed。q9–q14 checkpoint 的 19-artifact global frozen/source-owner refresh 已逾期并独立 deferred，未来接受前的重生成/复核必须覆盖 q15。只有补齐全部 program source、冻结 exact quotient manifests 并在同一 clean Windows cohort 逐 class 产生正式记录，才可形成 `Qmap/Qmap` 与 `Qlock/Qlock`；q9–q15 细节只见[`Lock tranches`](node-plugin-vfs-lock-dynamic-tranches-authority.md)。
