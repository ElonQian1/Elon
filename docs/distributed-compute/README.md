...
| `capacity_future` pricing-mode 交付结算谱系 bridge 草案 | 未登记 API-free source draft 已写入 Instrument/activation/adoption、Commitment、exercised Allocation、immutable Snapshot 与 F0 execution/verification/settlement/release refs。v192/v195 两套 role-specific usage digest 分字段保留、禁止判相等；同一 v193 readings 的双公式 owner proof 后移 Store。当前 `source_review_only/implementation_uncompiled/implementation_unrun`、`passed=0/failed=0`，无 Store resolver、API、migration、writer、ClearingReceipt 或 effect 接线 |
...
55. `docs/distributed-compute/capacity-future-settlement-lineage-authority.md` 与对应 acceptance：记录未登记的 v238→v225→v228→F0→v195/v198 reference-only `capacity_future` bridge 草案、v192/v195 双摘要域边界、封闭经济历史分支与未来 Store owner resolver 义务；当前无 Store/API/migration/writer 或 effect 接线。
