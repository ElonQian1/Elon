...
use crate::node_agent_compute_plugin_host::{
    candidate_cleanup_contract::{
        build_initial_delete_intent, build_namespace_durability_evidence_digest,
...
pub(in crate::node_agent_compute_plugin_host) struct ComputePluginCandidateCleanupNamespaceDurabilityRecoveryAuthoritySession<
    'authority,
> {
...
pub(in crate::node_agent_compute_plugin_host) enum ComputePluginCandidateCleanupNamespaceDurabilityRecoveryOutcome
{
    NotCreated,
...
impl ComputePluginLocalAuthority {
    pub(in crate::node_agent_compute_plugin_host) fn bind_candidate_cleanup_namespace_durability_recovery_session<
        'authority,
    >(
        &'authority self,
        process_fence: &'authority ComputePluginFetchProcessFence,
        observation: ComputePluginTrustedTimeObservation,
    ) -> Result<ComputePluginCandidateCleanupNamespaceDurabilityRecoveryAuthoritySession<'authority>>
    {
        ...
        {
            bail!("COMPUTE_PLUGIN_CANDIDATE_CLEANUP_NAMESPACE_DURABILITY_RECOVERY_SESSION_INVALID");
        }
        Ok(
...
impl ComputePluginCandidateCleanupNamespaceDurabilityRecoveryAuthoritySession<'_> {
    ...
    pub(in crate::node_agent_compute_plugin_host) fn read_candidate_cleanup_namespace_durability_outcome(
        &self,
        key: &CandidateCleanupNamespaceDurabilityRecoveryKey,
    ) -> Result<ComputePluginCandidateCleanupNamespaceDurabilityRecoveryOutcome> {
...
impl ComputePluginCandidateCleanupNamespaceDurabilityRecoveryOutcome {
...
fn validate_recovery_provenance(
    ...
    if !key
        ...
        || key.namespace_event().event().recorded_at_ms() > session.trusted_now_ms()
        || key.disposition_set_at() >= key.parent_absence_observed_at()
        || key.parent_absence_observed_at() >= key.barrier_completed_at()
        || key.barrier_completed_at() >= key.post_absence_observed_at()
        || key.post_absence_observed_at() > key.namespace_completed_at()
        || key.namespace_completed_at() >= key.prepared_at()
    ...
    {
        bail!("COMPUTE_PLUGIN_CANDIDATE_CLEANUP_NAMESPACE_DURABILITY_RECOVERY_PROVENANCE_CHANGED");
    }
    ...
    validate_hashed_cleanup_step_event(key.namespace_event())?;
    ...
    let object = key.plan().objects().first().ok_or_else(|| {
        anyhow::anyhow!("COMPUTE_PLUGIN_CANDIDATE_CLEANUP_NAMESPACE_DURABILITY_OBJECT_MISSING")
    })?;
    let disposition = key.disposition_event().event();
    let absence = key.absence_event().event();
    let namespace = key.namespace_event().event();
    let expected_evidence_digest = build_namespace_durability_evidence_digest(
        key.plan(),
        key.absence_event(),
        key.namespace_durability_kind(),
        key.filesystem_kind(),
        namespace.recorded_at_ms(),
    )?;
    if expected_intent != *key.intent_event()
        ...
        || namespace.cleanup_id() != key.plan().plan().cleanup_id()
        || namespace.plan_digest() != key.plan().plan_digest()
        || namespace.event_sequence() != 4
        || namespace.step_ordinal() != 0
        || namespace.event_kind() != "namespace_durable"
        || namespace.object_digest() != object.object_digest()
        || namespace.observed_identity_digest().is_some()
        || namespace.observed_parent_identity_digest()
            != object.object().expected_parent_identity_digest()
        || namespace.namespace_durability_kind() != Some(key.namespace_durability_kind())
        || namespace.namespace_durability_evidence_digest()
            != Some(expected_evidence_digest.as_str())
        || namespace.previous_event_digest() != key.absence_event().event_digest()
        || namespace.process_owner_epoch() != key.plan().plan().process_owner_epoch()
        || namespace.recorded_at_ms() <= absence.recorded_at_ms()
    {
        bail!("COMPUTE_PLUGIN_CANDIDATE_CLEANUP_NAMESPACE_DURABILITY_RECOVERY_BINDING_CHANGED");
    }
    Ok(())
}

fn validate_authorization_receipt(
    key: &CandidateCleanupNamespaceDurabilityRecoveryKey,
) -> Result<()> {
    ...
    {
        bail!(
            "COMPUTE_PLUGIN_CANDIDATE_CLEANUP_NAMESPACE_DURABILITY_RECOVERY_AUTHORIZATION_CHANGED"
        );
    }
    Ok(())
}
...
fn read_outcome(
    ...
    if stored_plan.as_ref() != Some(key.plan())
        ...
        || read_exact_step_event(transaction, key.absence_event())?.as_ref()
            != Some(key.absence_event())
        || count_event_identity_matches(transaction, key.absence_event())? != 1
    {
        bail!("COMPUTE_PLUGIN_CANDIDATE_CLEANUP_NAMESPACE_DURABILITY_RECOVERY_AUTHORITY_CHANGED");
    }
    let stored = read_exact_step_event(transaction, key.namespace_event())?;
    let identity_matches = count_event_identity_matches(transaction, key.namespace_event())?;
    match stored {
        Some(event) => {
            if identity_matches != 1 || count_events(transaction, event.event().cleanup_id())? != 4
            {
                bail!(
                    "COMPUTE_PLUGIN_CANDIDATE_CLEANUP_NAMESPACE_DURABILITY_RECOVERY_RESULT_AMBIGUOUS"
                );
            }
            validate_authority_snapshot(transaction, session, key, event.event().recorded_at_ms())?;
            Ok(ComputePluginCandidateCleanupNamespaceDurabilityRecoveryOutcome::Durable(event))
        }
        None => {
            if identity_matches != 0
                || count_events(transaction, key.plan().plan().cleanup_id())? != 3
            {
                bail!(
                    "COMPUTE_PLUGIN_CANDIDATE_CLEANUP_NAMESPACE_DURABILITY_RECOVERY_ABSENCE_AMBIGUOUS"
                );
            }
            ...
            Ok(ComputePluginCandidateCleanupNamespaceDurabilityRecoveryOutcome::NotCreated)
        }
    }
}
...
fn count_exact_authorization(
    ...
    let receipt_json = serde_json::to_string(receipt).context(
        "COMPUTE_PLUGIN_CANDIDATE_CLEANUP_NAMESPACE_DURABILITY_RECOVERY_AUTHORIZATION_SERIALIZE",
    )?;
    transaction
        .query_row(
        ...
        .context(
            "COMPUTE_PLUGIN_CANDIDATE_CLEANUP_NAMESPACE_DURABILITY_RECOVERY_AUTHORIZATION_READ",
        )
}
...
fn count_exact_pending_owner(
    ...
    let release_json = serde_json::to_string(key.owner_release()).context(
        "COMPUTE_PLUGIN_CANDIDATE_CLEANUP_NAMESPACE_DURABILITY_RECOVERY_OWNER_RELEASE_SERIALIZE",
    )?;
    transaction
        .query_row(
        ...
        .context("COMPUTE_PLUGIN_CANDIDATE_CLEANUP_NAMESPACE_DURABILITY_RECOVERY_OWNER_READ")
}
...
fn count_authority_time(
    ...
    transaction
        ...
        .context("COMPUTE_PLUGIN_CANDIDATE_CLEANUP_NAMESPACE_DURABILITY_RECOVERY_TIME_READ")
}
