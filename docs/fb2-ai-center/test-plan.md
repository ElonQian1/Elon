...
- `context-contract.answer_policy_contract.eval_scenarios` 返回七个机器可读评测场景：今日比赛、我的票、平台匿名订单摘要、群友观点、长按消息复核、群聊总结帖、来源审计。
...
- `context-contract.domain_context_projection_contract.domain_scenario_matrix` 返回同七类真实用户问题的域数据矩阵，并声明每类问题需要的 Context Pack 小节、可自动工具、权限请求、source kinds、feedback 路由和验收信号。
...
- `scripts\validate-fb2-context-format-route.ps1` 必须通过并接入 current-state：它把 `repo map格式建议.md`、`项目理解与讨论.md`、主项目 `contracts/data-tools/test-plan`、`external_app_context_projection/pack_template/index_contract/query_intent` 和 public smoke/status 脚本串成同一条路线门禁，防止后续把 AI-facing 输入从 XML-wrapped Markdown Context Pack + JSON metadata 退化成原始 HTML、大 JSON、完整 MCP/RAG 事实源或 fb2 数据复制。
- fb2 最新 `bfed1cc8` 对应的 `fb2_p4_vector_readiness_plan_v1`、`fb2_p4_vector_contract_v1`、`fb2_p4_source_enumerator_v1`、`fb2_p4_chunk_manifest_v1`、`fb2_p4_embedding_build_dry_run_v1`、`fb2_p4_embedding_transaction_preflight_v1`、`fb2_p4_embedding_text_fixture_v1`、`fb2_p4_embedding_provider_migration_plan_v1`、`fb2_p4_disabled_embedding_skeleton_v1`、`fb2_p4_provider_write_implementation_shape_v1`、`fb2_p4_disabled_adapter_write_store_v1`、`fb2_p4_runtime_approval_checklist_v1`、`fb2_p4_migration_approval_packet_v1`、`fb2_p4_post_migration_verification_plan_v1`、`fb2_ai_context_requirement_traceability_v1`、`main_project_answer_handoff_markdown` 和 `p4_blocker_phase_matrix` 只能作为只读计划/契约设计/no-write source enumeration/ID-only no-write manifest/no-write dry-run/no-write transaction preflight/sanitized no-write text fixture/provider migration no-write plan/disabled runtime skeleton/approval handoff 验收，不是生产 grounding，也不是当前 completion blocker。主项目验收时必须确认该计划能力保持 `contract_design_committed_embedding_not_started`、`source_specific_no_write_sample_available`、`id_only_no_write_manifest_available`、`dry_run_available_no_writes`、`transaction_preflight_available_no_writes`、`embedding_text_fixture_materialized_no_writes`、`provider_migration_plan_available_no_writes`、`fb2_p4_embedding_provider_policy_v1`、`not_selected_no_provider_calls`、`fb2_p4_vector_migration_plan_v1`、`fb2_p4_embedding_write_execution_plan_v1`、`business_context_requirements_traceable_final_goal_still_open`、`business_data_requirements_traceable=true`、`full_original_goal_complete=false`、`final_completion_blocker=p4_vector_rag_planned_not_approved_or_enabled`、`p4_phase_approvals_all_false`、`approval_granted=false`、`executable_now=false`、`production_grounding=false`、`blocks_data_goal=false`、`writes_chunk_manifest_file=false`、`persists_manifest_rows=false`、`persists_transaction_rows=false`、`persists_fixture_rows=false`、`creates_or_migrates_tables=false`、`executes_migration_ddl=false`、`calls_embedding_provider=false`、`provider_secret_required_now=false`、`provider_secret_printed=false`、`provider_model_id_selected=false`、`vector_dimension_locked=false`、`ready_to_call_embedding_provider=false`、`source_payload_included=false`、`embedding_text_included=true`、`embedding_text_is_sanitized=true`、`content_hash_computed=true`、`writes_embedding_rows=false`、`writes_vector_store=false`、`writes_public_group_messages=false`、`ready_to_execute_embedding_transaction=false`、`ready_to_execute_migrations=false`、`ready_to_write_embeddings=false`、`ready_for_shadow_eval=false`、`refresh_operations_used=false`、`candidate_rows_require_live_hydration=true`、`vector_rows_are_model_input=false`、`does_not_enable_vector=true`、`ready_to_build_embedding_store=false`、`ready_to_enable_answer_time_vector_candidates=false`、`answer_time_vector_candidates_enabled=false`，并确认本人订单或平台聚合 scope 缺失时必须是 `not_checked_missing_current_user_scope` / `not_checked_missing_platform_scope`，不能越权读或误报空。`fb2.search_vector_candidates` 仍是 `implemented=false` 的 reserved planned query；只有 fb2 后续完成 phase-specific approval、provider/model batch policy、migration DDL、live hydration、shadow eval 和 answer-time readthrough 后，才允许新增单独的 P4/vector 生产验收。
...
- Prompt metadata 必须区分“阻断型缺口”和“部分/已裁剪上下文”：`metrics.budget_status=empty`、readiness `blocked/unavailable/not_configured` 或缺 Context Pack 时 `fact_answer_allowed=false` 且 `required_user_notice=fb2_context_gap_or_unverified_data_present`；`metrics.budget_status=too_large`、`_context_budget.trimmed=true` 或 readiness `degraded/partial` 时必须保留 `fact_answer_allowed=true`，同时输出 `partial_answer_only=true` 和 `required_user_notice=fb2_context_partial_or_truncated_context_present`。
...
- `handoff-prompt-current.md` 的 `fb2 导出样本` 表必须使用 `business` / `quality_history` 分类列，不得只展示混合 `kinds`。如果某行出现 `opinion_result_review_summary`，它必须位于 `quality_history` 列，不能位于 `business` 列；`scripts\validate-fb2-ai-center-handoff-prompt.ps1` 必须解析并验证该表。
...
- 样本集通过后，status 必须继续输出 `latest_context_answer_readiness.complete=true`、`passed_count=4`，并在 `refresh_gaps` 中包含 `context_answer_readiness_validated_offline`。该字段只检查四类核心问题的必需 source kinds 与回答边界，不替代 live 模型回复、feedback 或 quality summary。若任一场景的 `present_source_kinds` 包含 `opinion_result_review_summary`，answer-readiness 必须把它放进 `quality_history_source_kinds`，并且不得出现在 `business_source_kinds`；四类核心问题的缺失来源判断只允许使用业务事实来源。
...
- Prompt 投影必须通过 `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\validate-fb2-main-prompt-projection.ps1`。该门禁证明模型实际 prompt 包含 Context Pack XML 边界、事实摘要、缺口摘要、预算摘要、场景 guidance、已执行工具规则、工具事实/缺口摘要，并且生成后仍有数据缺口 notice 与 answer source validation 防护。
