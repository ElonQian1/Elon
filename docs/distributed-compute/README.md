...
| 外部算力池适配器与统一报价 | v221-v254 前置门为 `implementation_partially_verified`。V254 仅追加 owner delegation 与 inert static compatibility candidate，并以 dynamic preflight 同次消费 V249 Prepared + current V250/V252/V253；closure 固定未实现、Provider 保持 `registering`、`activation_ready=false`，源码未编译或运行，`passed=0`。candidate 不等于 activation、route、market 或 runtime readiness。见 [`V254 候选权威`](external-pool-provider-activation-candidate-authority.md)、[`V254 验收边界`](external-pool-provider-activation-candidate-acceptance.md)、[`当前状态`](current-implementation-status.md) 与 V249-V253 专题文档 |
...
Provider Adapter 的 v227-v254 前置门已形成分层权威；V249-V253 保留 registry、live-FS seam 与三条可续签声明链的既有边界。V254 新增 owner delegation 和静态兼容候选，dynamic preflight 才在同一次检查中消费 V249 Prepared + current V250/V252/V253；即使输入 current，closure 仍为 `activation_closure_not_implemented`、`activation_ready=false`、Provider 仍为 `registering`，且源码未编译或运行，`passed=0`。最新状态见 [`current-implementation-status.md`](current-implementation-status.md) 与 [`V254 activation candidate`](external-pool-provider-activation-candidate-authority.md)。candidate 不创建 v213 route/service actor authority，不运行扫描、沙箱或 secret resolver，也不授权 Pool/Offer/Job/Attempt/Start。V254 已安装覆盖 direct SQL 与 versions 的 `external_pool` CapacityPool active、Offer draft/active temporary absolute deny；后续 atomic activation 必须显式把它替换为完整 readiness/currentness admission gate。
