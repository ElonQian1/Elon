...
| 开发者 App 签名 Webhook | 代码已形成、未编译 | App 所有者可配置白名单内 HTTPS 回调；终态事件进入耐久队列，工作器签名投递并按短租约、指数退避、死信和自动停用规则处理。密钥只显示一次，Node SDK 可验签。当前尚未完成编译、迁移、并发、网络、跨语言签名和 UI 验证，也不是跨运营方总线或支付回调 | `server/src/open_commerce_webhook_*.rs`、`server/src/store/open_commerce_developer_webhooks.rs`、`pc-frontend/src/features/open-commerce/DeveloperWebhookPanel.tsx`、`docs/open-commerce-developer-webhooks-v1-acceptance.md` |
