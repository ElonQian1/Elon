...
如果当前机器没有 `FB2_AI_CENTER_TOKEN`，但具备 fb2 服务器 SSH 权限，可用主项目受控桥接脚本运行无写群 live 预检：`pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\run-fb2-ai-center-token-bridge.ps1 -RunDataOnlyPreflight -Fb2Password <FB2_PASSWORD>`。脚本只从 fb2 服务器 `.env` 读取 `FB2_MAIN_PROJECT_SHARED_SECRET` 到当前进程内存，把它作为环境变量传给子进程，不把 token 放进命令参数、日志或 summary；它只跑 `-DataOnlyAcceptance -PreflightOnly`，不会发送可见群消息。bridge 会强制项目访问直连：SSH 禁用代理跳板，子进程设置 `NO_PROXY/no_proxy=*` 并清空常见代理环境变量。普通 `validate_current_state` 只运行该脚本的 `-SelfTest`，不会自动读取远程密钥；bridge 跑过后，`status-refresh-current.json.token_bridge_live_preflight` 会记录本次 no-write live 预检结果。
