...
pub(super) fn build_semantic(index: &RepoContextIndex) -> ContextQualitySemantic {
...
pub(super) fn coverage_penalty(covered: usize, total: usize, max_penalty: i32) -> i32 {
    if total == 0 {
        return 0;
    }
...
pub(super) fn recommended_actions(
    index: &RepoContextIndex,
    coverage: &ContextQualityCoverage,
    semantic: &ContextQualitySemantic,
    gaps: &[ContextQualityGap],
...
pub(super) fn symbol_has_snippet(index: &RepoContextIndex, symbol_id: &str) -> bool {
    index
        .evidence
        .snippets
        .iter()
...
pub(super) fn count_impact_facts(index: &RepoContextIndex) -> usize {
