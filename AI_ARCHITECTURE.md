...
v198 再为 v195 Settlement Receipt 增加独立的追加式 Release Receipt。结算满 72 小时且挑战状态为 `none/rejected/withdrawn` 时，管理员可在单一事务中把 Provider 与平台对应金额从 pending 转入 available；原结算、挑战和决议不被改写，释放后也不能再创建新挑战。`open/accepted` 失败关闭；available 只表示内部账本可用，不执行提现、外部支付或链上结算。状态为 `implementation_uncompiled`，边界见 `docs/distributed-compute/attempt-settlement-release-api.md`。
