...
| 节点端点凭据与认证会话权威 | 服务端 v216 已铺默认空的 credential/version/revocation/session receipt/session head 五表及 sealed Store currentness 内核；零 legacy backfill，未接 HTTP、WS、AgentManager、NodeRegistry、NodeAgent 或协议，也没有 secure transport proof producer。源码尚未编译、测试或执行迁移；它不等于在线 socket、Ready、route、outbox、command、Lease 或派发权威。见 `node-endpoint-session-authority.md` |
