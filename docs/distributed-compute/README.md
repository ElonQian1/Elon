...
| UserNode Windows Runner 进程监管前置草案 | 未登记、默认不可达的 private source draft：写入无 producer 的 sealed loader load-set/launch-security 边界、restricted-token `CreateProcessAsUserW(CREATE_SUSPENDED)`、atomic Job-list 配置/回读/membership、PID/TID/creation-time 与 terminate/wait custody；owned loader transition、launch-security producer 与 `ResumeThread` 均不存在。严格 `source_review_only/implementation_uncompiled/implementation_unrun`、`passed=0/failed=0`、`migration/table/writer=none/none/none`；不是 start、完整 sandbox、runtime transition、health、Ready 或市场权威。见 [authority](user-node-windows-runner-process-custody-authority.md) 与 [acceptance](user-node-windows-runner-process-custody-acceptance.md) |
