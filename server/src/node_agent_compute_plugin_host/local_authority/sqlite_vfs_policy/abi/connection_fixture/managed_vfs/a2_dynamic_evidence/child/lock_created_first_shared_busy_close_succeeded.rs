//! Canonical q18 child header for CreatedFirst DMS shared-busy close-success receipts.

pub(in super::super) const REPORT_VERSION: &str = "a2lockq18";
pub(in super::super) const REPORT_VALUE_COUNT: usize = 186;
...
pub(in super::super) fn selector(
    ...
    Ok(format!(
        "initialization-{action}-first-{first}-count-{count}-created-first-shared-busy-close-succeeded-{completion}-terminal-route-unknown"
    ))
}
...
mod tests {
    ...
    #[test]
    fn eighty_eight_q18_selectors_are_unique_and_fail_closed() {
