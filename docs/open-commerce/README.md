...
117. `docs/open-commerce-consumer-portability-selective-adoption-v1-acceptance.md`：查看字段级采用的 Rust、HTTP、PC 构建验证证据。
...
129. `docs/open-commerce-consumer-data-request-followups-v1-acceptance.md`：查看 V163 跟进的 Rust/SQLite 验证证据与剩余边界。
...
137. `docs/open-commerce-capability-source-link-v1-acceptance.md`：查看 V164 来源绑定的 Rust/SQLite 验证证据及剩余 HTTP、兼容性和 PC 回归。
...
160. `docs/decisions/open-commerce-consumer-authorization-mcp-v1.md`：查看消费者 AI 单能力授权申请和商户决定权边界。
...
175. `docs/requirements/open-commerce-native-edge-v1.md`：查看 Rust 原生 HTTPS 商户入口的目标、安全边界和非目标。
176. `docs/open-commerce/native-rust-edge.md`：查看多商户回环路由、配置、systemd、热更新和回滚方法。
177. `docs/open-commerce/native-rust-edge-v1-acceptance.md`：查看原生入口的 Rust 测试、真实 TLS 纵向检查和生产未完成边界。
...
| Rust 原生商户 HTTPS 入口 | V1 代码和本机 TLS 纵向已验证；生产证书、DNS、Linux systemd 安装和 ACME 尚未完成 | `docs/open-commerce/native-rust-edge.md`、`docs/open-commerce/native-rust-edge-v1-acceptance.md` |
...
| 商户数据来源、健康度和同步回执 | 控制面已实现；能力与合格内部回执的显式绑定已通过定向 Rust/SQLite、集成上下文、管理 HTTP、消费者发现 MCP 和进程内发现 HTTP 回归。本机真实 TCP 纵向测试已通过生产 Router 验证来源绑定、消费者按厂商/数据域发现和解绑后不再命中；PC 浏览器、历史迁移、生产部署、具体平台适配器和外部核验仍待逐项验收 | `docs/decisions/open-commerce-integration-control-plane.md`、`docs/decisions/open-commerce-capability-source-link-v1.md`、`docs/open-commerce-integration-control-plane-acceptance.md`、`docs/open-commerce-capability-source-link-v1-acceptance.md` |
| 连接器 SDK 与兼容性门禁 | 已实现 V1，并验证通用 ERP 写入后回读处理器；尚不包含具体大厂或生产 ERP 适配器 | `sdk/open-commerce-connector/`、`docs/open-commerce/connector-sdk.md`、`docs/open-commerce-verified-erp-handoff-handler-v1-acceptance.md` |
| 消费者发现、关系授权与第三方应用接入 | 跨项目目录、商户声明来源与新鲜度、透明非付费排序、匿名关系、低敏偏好、客户端加密数据保险箱、删除请求、可验证导出与隔离导入、动作确认、授权预算、调用事件和封禁主干已形成，多项子集已通过 Rust/SQLite、进程内 Axum 或 PC 构建验收。开发者资料、域名本地状态、准入、一次性生产凭据哈希、轮换、撤销生产 Webhook 联动和生产就绪八步投影已通过全新 SQLite 专项测试；生产能力仍默认关闭，测试不代表真实域名验证、环境开关启用、生产鉴权或网络调用。外部平台官方回读、真实 DNS/TLS、历史升级、第三方排序器 SDK、自动删除仲裁、多设备密钥同步、完整订单迁移、跨运营方身份互认、真实资金和生产公共网络仍未完成 | `docs/open-commerce/capability-baseline.md`、`docs/decisions/open-commerce-developer-production-credentials-v1.md`、`docs/decisions/open-commerce-production-webhooks-v1.md` |
