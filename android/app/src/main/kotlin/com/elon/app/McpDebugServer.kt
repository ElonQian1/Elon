...
object McpDebugServer {
    ...
    private fun chatProbe(args: JSONObject): JSONObject {
        ...
        val latency = latencyReportJson(appContext, JSONObject().put("trace_id", traceId).put("timeline_limit", timelineLimit))
        val taskStatus = taskStatusJson(appContext, JSONObject().put("trace_id", traceId))
        val serverTrace = if (includeServerTrace) {
            JSONObject()
