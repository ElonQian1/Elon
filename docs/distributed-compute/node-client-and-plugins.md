...
其后新增的 v11 Planning Snapshot V2 仍是独立、默认阻断且尚未接入云端会话派发的能力。请求合同精确引用本次 cloud session、v209 preparation delivery 与 observation digest；节点合同预留有界 installed records、policy binding/revocation receipt、authority/inventory/process/clock、rollback、catalog 与双 keyring 等事实，但当前 Bootstrap 没有 authority snapshot producer，始终返回 `snapshot_ready=false`、`snapshot=None` 和全部副作用标志为 false。云端 v210 只以追加式 delivery、snapshot、generation request 与 generation outcome 保存这条因果链，并把 outcome 限定为 `signer_unavailable` 或 `rejected`；过期快照和当前 consent/policy 漂移不得进入 generation request。它不是签名器、计划投递、本机确认或 PlanApply。
...
这只是 PlanApply live-binding 的必要前置，不是完整可用的 planning context。当前 Store 没有生产 Host 调用点，v4/v5/v210 DDL 或迁移也未实际运行；NodeRuntime 账号切换尚未接入 Bootstrap 失效代次，SQLite authority open 仍未与 pinned root 句柄闭合，v11 也没有从同一只读 authority snapshot 产生事实的 capability。v5 companion 只撤销尚未终结的下载/校验能力，不撤销已发布 Ready 或运行中 Attempt；cleanup 删除链现在有独立的同进程 deletion custody，但跨重启 adoption、后续 ordinal 和 terminal producer仍缺。Ready/Attempt 仍需要独立 work-admission generation、signed `reauthorize_existing` 和 exact receipt 绑定。正确后续顺序是补齐本机 snapshot producer与这些运行门卫，再接默认 unavailable 的真实 Signer/KMS、节点验签展示和本机 exact confirmation；在全部闭合前继续保持零下载、零安装、零 Sidecar。
