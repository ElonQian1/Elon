...
4. Codex CLI 不能依赖跨任务记忆；未知项目必须先读 `AGENTS.md`、`CODEX.md`、`README.md`、`.github/instructions` 和相关 `docs/`。
5. Codex CLI 完成后，后端负责验收和产品化状态：任务记录、进度展示、下载链接、版本信息、合并/发布/部署锁。
6. 并发安全、版本顺序、APK 发布、服务器部署不能只靠提示词，必须由后端代码和发布脚本强制执行。
