...
| 外部算力池适配器与统一报价 | 首段 owner request→独立管理员复核→immutable apply 已冻结为 docs-only 权威；未来 apply 只登记 `external_pool/registering` Provider 与专用 application source，不写 v213 route rows、不建 Pool/Offer 或派发。Rust、v221、API、credential verifier、Adapter 和网络均未实现，见 [`external-pool-adapter-authority.md`](external-pool-adapter-authority.md) |
...
服务端 Provider Adapter 统一接入公司集群、云 GPU 和其他算力池；每个 Provider 保留自己的内部调度，只向一龙提交标准回执。首段来源权威见 [`external-pool-adapter-authority.md`](external-pool-adapter-authority.md)：当前只有文档冻结，不产生 credential、route、容量、派发或结算效果。
