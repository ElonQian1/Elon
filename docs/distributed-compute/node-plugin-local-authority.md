...
cleanup execution plan、expected objects、plan seal 与 append-only step events 已纳入 v3 必需对象，并已随当前 `elon-pc-node` 编译及 schema 建库与重开指纹定向测试。plan 绑定 authorization、installation、staging root、candidate-parent anchor identity 与完整对象父子拓扑；每个对象只有完成 durable intent、exact disposition/受约束的恢复缺失、exact parent 下的 namespace absence 和独立 namespace-durability fence，completion 才能同时引用 plan digest、既有物理 execution evidence digest 与独立 terminal journal digest。completion Store 内核已按这三类摘要改造，并只接受没有构造入口的终态 journal capability；物理 evidence v1 仍没有不可变记录及 plan/object digest 绑定。当前 topology typed Store 只消费 `AuthorizedCandidateCleanup`，原子写入 plan、全部 expected objects 与 seal；新增的首对象 intent Store 只消费 sealed topology，在独立事务推进可信时间、写入序号 1 的 `delete_intent`，并 exact readback 事件列、JSON 与 JCS 摘要。两类 Store 的不确定结果只能分别查询 exact `NotCreated/Sealed` 与 `NotCreated/Durable`，所有能力继续保留原始句柄和根锁。旧批量 executor 已收回 cleanup 合同内部，Host 不能绕过 intent。受管文件系统继续提供强 disposition 和父句柄相对观测，但单步执行、后三阶段 journal、物理 evidence v2、真实 parent namespace flush 及跨重启物理 recovery 均未实现。
...
当前供应链、候选健康与失败候选清理代码已通过 `elon-pc-node` 编译；清理链通过 3 项 Windows 同句柄删除、1 项目录顺序、4 项 completion、5 项 topology builder/Store 及 4 项首 intent builder/Store 测试，另有 1 项 topology/journal schema 建库与重开测试。前 4 项没有覆盖 retained custody → cleanup authorization → topology → intent，因而没有验证普通目录句柄与 late `DELETE` reopen 的 share-access 冲突；intent 测试覆盖稳定 plan/event hash、时间下界、Store exact round-trip 和列篡改拒绝。cleanup authorization Store 可推进 `cleanup_pending`，topology Store 可封存 exact execution plan，intent Store 可独立提交第一个物理动作前置授权；对外已无 sealed topology 直达旧 executor 的入口。由于单步强执行、后三阶段 journal Store 和目录 durability 仍缺，终态 capability 不可构造，完整私有流水线尚不可达。当前仍未接入生产 Host 或生产本地库，不能授予生产重试、安装、promotion、ReadyCapability 或商业 Verification。单一 cleanup handle tree、跨重启物理恢复、真实下载/Sidecar/探针、installed/promotion/GC 与 Attempt 接线仍待实现；精确边界见 `node-ready-capability.md` 与 `node-plugin-candidate-cleanup.md`。
