...
- 已实现：通用 ERP 蓝图 V1 可登记官方蓝图和不可变版本、创建独立商户项目实例、检索能力并解析需求、聚合经授权的脱敏通用提案、创建正式 Matter，以及执行兼容检查和人工确认的采用/回滚；AI 不得自动接受提案、合并代码、发布公共内核或批量升级。权威入口为 `docs/erp/README.md`。
...
- 通用 ERP V1 管理继承、提案和升级治理，但不会自动复制商户源码、执行数据库迁移或部署生产系统；真实商户项目仍走现有项目开发和发布流程。
