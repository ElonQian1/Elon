...
use crate::{
    ...
    open_commerce_model::{
        ...
        InvokeCapabilityRequest, OpenCommerceCapability, OpenCommerceMerchant,
        OpenCommerceMerchantDetail, OpenCommerceOverview, OpenCommerceTotals,
        UpdateCapabilityRequest, UpdateMerchantRequest, ACCESS_AUTHORIZED, ACCESS_OWNER_ONLY,
        HANDLER_MERCHANT_PROFILE, HANDLER_STATIC_JSON, OPEN_COMMERCE_SCHEMA,
    },
    ...
    store::{RecordOpenCommerceSyncReceipt, Store},
};
...
pub(crate) async fn invoke(
    ...
    crate::open_commerce_invocation_service::invoke(store, actor, request).await
}
...
pub(crate) async fn invoke_with_action_confirmation(
    ...
    crate::open_commerce_invocation_service::invoke_with_action_confirmation(
        store,
        actor,
        request,
...
pub(crate) fn execute_first_party_handler(
    merchant: &OpenCommerceMerchant,
    capability: &OpenCommerceCapability,
    _input: &Value,
) -> Result<Value> {
