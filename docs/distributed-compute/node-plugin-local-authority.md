...
v4 是第一条显式、窄范围的本机 authority 迁移：旧 `user_version=3` 库必须先用未修改的纯 v3 reference 对全部非 SQLite 内建 table、index 和 trigger 的对象集合、所属表及规范化 SQL 定义 SHA-256 与 foreign key 完整性做精确验证；通过后才追加 `sharing_policy_binding_receipts`、把 PlanApply 的可信时间门从 legacy `>=` 收紧为严格 `>`。v5 保持纯 v3/v4 reference 不变，追加 `sharing_policy_binding_revocation_receipts` 和精确终结 trigger；v6 再追加 Control-signed Manifest catalog binding 对象。v3、v4 或 v5 只有在同一 `BEGIN IMMEDIATE` 内通过对应旧指纹与 foreign-key 检查后才能升级，fresh 库则依次构造 exact v3、v4、v5、v6。缺失对象、额外对象、同名异定义或迁移中断均失败关闭。8 项内存 SQLite 测试验证 fresh v6 精确对象与重开、纯 v3/v4/v5 原子迁移、PlanApply 时间门收紧、各代漂移不残留后续半套 DDL以及漂移 v6 重开失败；另有 4 项 policy-binding 纯合同测试验证 fence 投影、enabled/disabled 授权形状、既有插件撤销及回执篡改拒绝。上述版本尚未接入生产 NodeAgent、未对生产磁盘库执行 DDL 或迁移；v5 撤销 Store、v6 catalog Store、完整 SQLite CAS/recovery 与 Host 联调仍未专项验证。
...
这仍不是生产planning context：opened-authority与rollback本机custody均无构造入口，legacy path Store尚未退役，生产VFS、trust/rollback provider和snapshot producer仍缺；详细失败关闭边界见上页。cleanup deletion domain也只允许已耐久授权、owner=`cleanup_pending`的线性custody在同authority/root/process fence与retained handles连续存活。`context_ready`、`snapshot_ready`及root/authority/download/Sidecar标志保持false，Store无Host调用点，也不允许PlanApply、下载或任务准入。
