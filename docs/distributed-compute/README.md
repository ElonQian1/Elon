...
| 外部算力池适配器与统一报价 | v221-v259 前置门为 `implementation_partially_verified`。V255 inert profile 为 `12 passed`；V256 为 `14 passed`，但生产 mount/secret 未验收；V257 已在 WSL2 Linux 通过 4 项真实 kernel fixture 与 7 项 source-contract，验证 memfd/mode/CLOEXEC/seals/失败关闭/Drop，但没有执行 capsule。V258 inert target 为 11 项，V259 durable supervisor/session policy companion 为 13 项；二者仍不 spawn/IPC/secret/network/probe/route/activation，readiness 全 false。V254 18 deny 逐字保留。见 [`V259 companion 权威`](external-pool-adapter-supervisor-session-policy-companion-authority.md)、[`V259 验收边界`](external-pool-adapter-supervisor-session-policy-companion-acceptance.md)、[`V258 target 权威`](external-pool-adapter-upstream-transport-target-authority.md)、[`V257 capsule 权威`](external-pool-adapter-entrypoint-capsule-authority.md)、[`V257 capsule 验收`](external-pool-adapter-entrypoint-capsule-acceptance.md)、[`V256 resolver 验收`](external-pool-adapter-runtime-bundle-resolver-acceptance.md) 与 [`当前状态`](current-implementation-status.md) |
...
Provider Adapter 的 v227-v259 前置门已形成分层权威。V255 durable inert profile 已通过 `12 passed`，V256 strict bundle resolver 为 `14 passed`。V257 从 fresh V249 retained entrypoint 形成 Linux-only exact `0500` sealed memfd，并用独立 policy 审计 static ELF64；现已在 WSL2 Linux 通过 4 项 kernel fixture 与 7 项 source-contract，真实验证 memfd、CLOEXEC、四项 seals、失败关闭和 Drop，但没有执行 capsule或读取真实 secret。V258 exact target 通过 11 项，V259 server-fixed supervisor/session policy companion 通过 13 项；V259 仍只封存 framing、stream auth/KDF/transcript 与 Linux confinement/resource/egress/shutdown 合同，不 spawn/IPC/network/probe/activation。下一批仍须实现 authenticated child-only IPC 和 supervisor 级 Linux enforcement，之后才可绑定 V258 target 做 broker TLS/upstream probe。V254 18 deny 逐字保留，完整 admission gate 只能与 atomic activation 同批替换。见 [`V259 supervisor/session companion`](external-pool-adapter-supervisor-session-policy-companion-authority.md)、[`V258 upstream target`](external-pool-adapter-upstream-transport-target-authority.md)、[`V257 entrypoint capsule`](external-pool-adapter-entrypoint-capsule-authority.md)、[`V255 runtime launch profile`](external-pool-adapter-runtime-launch-profile-authority.md)、[`V256 runtime bundle resolver`](external-pool-adapter-runtime-bundle-resolver-authority.md) 与 [`current-implementation-status.md`](current-implementation-status.md)。
