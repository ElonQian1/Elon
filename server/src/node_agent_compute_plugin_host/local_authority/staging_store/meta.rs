...
use super::{
    super::plan_application::AuthorityPlanApplicationState, projection::CandidateStagingProjection,
};

pub(super) fn update_staging_authority_meta(
    ...
    let updated = transaction
        .execute(
            ...
            WHERE singleton = 1
              ...
              AND trusted_time_high_water_ms = :staged_at
              AND clock_status = 'trusted' AND updated_at_ms = :staged_at"#,
            named_params! {
                ...
                ":staged_at": staged_at_ms,
            },
        )
        .context("COMPUTE_PLUGIN_STAGING_AUTHORITY_UPDATE")?;
    if updated != 1 {
        bail!("COMPUTE_PLUGIN_STAGING_AUTHORITY_CAS");
    }
    Ok(())
}
