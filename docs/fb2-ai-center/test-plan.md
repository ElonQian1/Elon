...
- fb2 最新 `fb2_p4_vector_readiness_plan_v1` 只能作为只读计划报告验收，不是生产 grounding，也不是当前 completion blocker。主项目验收时必须确认该报告保持 `writes_embedding_rows=false`、`does_not_enable_vector=true`、`ready_to_build_embedding_store=false`、`ready_to_enable_answer_time_vector_candidates=false`；只有 fb2 后续提交 source allowlist、chunk schema、permission filter SQL、offline embedding build、shadow eval 和 answer-time readthrough 后，才允许新增单独的 P4/vector 生产验收。
