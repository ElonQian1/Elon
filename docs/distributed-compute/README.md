...
8. `docs/distributed-compute/broker-api.md`：Job、报价与预留 HTTP/MCP 控制面。
9. 现有兼容实现：`docs/decisions/node-compute-sharing-supply-v1.md`。
...
Provider 本人控制面由 `docs/distributed-compute/provider-api.md` 维护，Pool 控制面由 `docs/distributed-compute/capacity-pool-api.md` 维护；Job、报价、预留与未执行任务终态控制面由 `docs/distributed-compute/broker-api.md` 维护。
