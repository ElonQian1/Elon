...
输入当前任务和最多 2400 token、8 份文档的后续阅读规划预算；`task_paths` 可提供最多 16 个任务相对路径，`scope_id` 可限定知识联邦分片，`release` 可限定发布线。服务端另绑定当前 Git branch 与 clean/dirty 状态。`max_response_tokens` 把本工具结构化输出限制在约 800–2000 token，默认 1200。投影只保留少量图节点、每节点最多 4 个文档路径和 4 个实现引用、每文档最多 4 个标题，并在超预算时继续删减或退化为路径摘要。输出不含 Markdown 或源码正文。

响应的 `plan_receipt.plan_id` 绑定查询、任务范围、Git HEAD、catalog revision、dirty worktree 指纹和实际返回的选择，并记录 `estimated_full_plan_tokens`。任务路径、联邦 scope、branch、release 或 clean/dirty 变化都会进入缓存键并触发重规划。代理在同一任务再次调用时必须把它传为 `previous_plan_id`；revision 和选择未变时只返回 `status=not_modified` 小回执，不重复发送阅读计划。`performance_receipt` 同时报告本次规划耗时、是否复用缓存、完整/小回执估算 token 和本次预计避免量，供调用方量化收益。
...
### 普通 Codex 任务后的 receipt Hook

节点对未被精确任务门控跳过的普通 Codex 任务，以会话级配置接入官方 `PostToolUse`、`Stop` 和 `SessionEnd` 命令 Hook。Hook 子进程只从 `tool_input` 派生规范化仓库相对路径及 `read/write/access` 类型，明确忽略并不持久化 `tool_response`、命令正文、prompt、聊天、assistant message、transcript、源码正文和 Codex 私有 memories。账本位于系统临时目录，以 workspace/session/turn 的不可逆短指纹隔离，SessionEnd 删除，异常残留最多保留 24 小时，不提交 Git。`SessionStart`、`PreCompact`、`PostCompact`、`SubagentStart`、`SubagentStop` 只保留有界 no-op 适配缝，真实运行证明收益前不注入，避免把未验证生命周期扩展变成新的上下文或稳定性成本。

Stop 只在当前 turn 至少观察两个有效路径、其中至少一个为读证据，且相对本会话旧 turn 出现新路径时继续一次；提示最多列六个路径、路径总长不超过 360 字符，单会话最多三次。继续提示要求模型只在形成可复用、非重复、非猜测、非任务局部且不冲突的结论时调用 receipt；没有新结论直接结束。`ELON_CODEX_PROJECT_MEMORY_HOOKS=0` 或 `<elon-project-memory-receipt-skip version="1">` 可关闭；精确单文件任务默认不注入。

这些是非托管命令 Hook，遵守 Codex 官方信任模型：首次出现或定义变化后，用户仍需用 `/hooks` 审核信任；未信任时 Codex 跳过。节点与仓库插件包绝不自动传 `--dangerously-bypass-hook-trust`，也不写用户级或项目级信任记录。因此“配置已生成”不等于“Hook 已真实执行”；真实运行验收必须单独证明。
...
### `project_docs_record_native_context_candidate` / `project_docs_record_native_context_receipt` / `project_docs_list_native_context_candidates` / `project_docs_check_native_context_memory`
