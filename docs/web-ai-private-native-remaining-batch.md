...
| Text submit/stream/stop/follow-up | Official runtime send path verified, reused | Not independent Android HTTP POST. Retry v8 is installed on 1639; changed reply used the legacy DOM receipt, so runtime acceptance remains failed. Earlier 1638 returned `timeout_owner_changed`. A fresh-document acceptance guard now passes 18 cases; it does not prove runtime admission. [Current evidence](reports/chatgpt-regeneration-observation-1637.md). Other contexts remain separate |
