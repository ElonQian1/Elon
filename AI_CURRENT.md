...
- 已实现：通用 ERP 蓝图 V1.1 可按修订号追加蓝图定义、发布不可变版本、创建和配置独立商户项目实例、按固定版本检索能力、聚合经授权的脱敏通用提案、原子创建提案或实例初始化 Matter，以及保存升级前后配置快照、商户执行证据和完整回滚；AI 不得自动接受提案、演进蓝图、合并代码、发布公共内核或采用升级。权威入口为 `docs/erp/README.md`。
...
- 通用 ERP V1.1 用初始化 Matter 衔接真实项目开发，但不会自动复制商户源码、启动 Assignment、执行数据库迁移或部署生产系统；Matter 仍需商户批准和启动，真实商户项目继续走现有项目开发和发布流程。
