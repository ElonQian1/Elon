...
v177-v180 另行保存 Provider/Pool 激活证据申请、过期批准废止审计、不可变激活计划和原子应用回执。本人 HTTP/MCP 可显式提交、查询和取消；平台 `admin/owner` 通过 HTTP 审核、废止失效 `approved`、准备计划或应用精确计划，双方可只读预检申请，管理员还可预检计划。准备阶段固定下一 Provider revision、路由引用、verified 硬件摘要和规范计划摘要，仍无激活效果。应用阶段不信任预检快照，而是在一个 `BEGIN IMMEDIATE` 内重新审计依赖，组合 Provider 版本登记与 Pool 生命周期事务内 kernel，把申请/计划推进为 activated/applied，并写入追加式 v180 回执；任一步失败整笔回滚。历史回执审计不可变 Provider 版本和 Pool 激活事件，不依赖其未来仍为 active。该效果只改变平台内部 Provider/Pool 状态，不连接节点、不读取凭据正文、不发布 Offer、不开放预留、不派发任务或移动资金；控制面仍为 `implementation_uncompiled`，详细边界见 `docs/distributed-compute/activation-evidence-api.md`。
...
| `server/src/store/compute_activation_{requests,plans,applications}.rs`、`compute_federation_activation_{service,api,mcp}.rs`、`compute_federation_activation_{plan,application}_{model,service}.rs` | v177-v180 激活证据申请、管理员审核/废止、申请/计划预检、不可变计划和显式原子应用回执；应用只激活内部 Provider/Pool，不连接节点、不发布 Offer、不移动资金，当前未编译、未迁移、未运行验证 |
