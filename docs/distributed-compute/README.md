...
| 外部算力池适配器与统一报价 | v221-v266 保留各层历史证据。V267 current hardening 已通过 Windows 受管编译/定向测试、WSL2 `v267_` 12/12、Yama 2 内核矩阵 12/12、transport 4/4 与 lifecycle 目标故障测试，状态为 `implementation_compiled/local_windows_and_wsl2_kernel_verified`；V268-V271 已随完整 Windows server target 与 WSL2 test target 编译，但各自专属动态仍为 `passed=0`。V271 只做 migration-only source bridge，不打开 fence。私钥托管、signer transport、生产 upstream/Secret、六能力派发、stable executor、active refresh、atomic activation 与 market admission 仍缺；Provider=`registering`、V254 18 deny 不变。见 [`V267 权威`](external-pool-adapter-post-exec-supervisor-hardening-authority.md)、[`V270 权威`](external-pool-adapter-provider-runtime-readiness-authority.md)、[`V271 权威`](external-pool-route-source-projection-bridge-authority.md) 与 [`当前状态`](current-implementation-status.md) |
...
Provider Adapter v227-v266 的证据只在各自历史版本内有效。V267 current hardening 已在 production materializer fixture 上完成 Windows/WSL2 编译、migration/source 定向测试、Yama 2 内核矩阵、ancillary 与 cleanup 故障测试；Yama 3/完整 host-gate 负值、第三方 binary、生产 upstream/Secret、长时/并发故障仍缺。V268 是唯一 Profile V2/signed compatibility authority，V269 再接默认关闭、同步、管理员 courier 的 signing handoff；V268-V271 已随完整 Windows server target 与 WSL2 test target 编译，但专属动态均为 `passed=0`。V270 Provider-specific post-cleanup readiness 与 V271 migration-only source bridge 仍未运行，不能据此打开 activation fence。Atomic activation 仍须闭合六能力 producer/worker、authenticated ACK/event、stable executor root、active refresh 和 V254 replacement 动态矩阵；18 deny 在此之前逐字保留。见 [`V271 bridge`](external-pool-route-source-projection-bridge-authority.md)、[`V270 readiness`](external-pool-adapter-provider-runtime-readiness-authority.md)、[`V269 signing handoff`](external-pool-adapter-runtime-compatibility-signing-handoff-authority.md)、[`V268 signed compatibility`](external-pool-adapter-signed-runtime-compatibility-authority.md)、[`V267 hardening`](external-pool-adapter-post-exec-supervisor-hardening-authority.md) 与 [`当前状态`](current-implementation-status.md)。
