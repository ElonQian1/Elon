...
| 开发者 App 限权生产凭据 | 代码已形成、未编译，默认关闭 | 当前资料、域名证明和准入均有效后，平台管理员可显式签发一次性 `oc_live_` 凭据；明文不入库，范围不得超出已审资料，并受风险层级期限、轮换和紧急撤销约束。生产调用复核能力范围；资料变化、域名重验、App 停用或准入暂停原子撤销。它不代表外部身份核验、真实资金或生产部署 | `server/src/open_commerce_developer_credential_*.rs`、`server/src/store/open_commerce_developer_credentials.rs`、`pc-frontend/src/features/open-commerce/DeveloperProductionCredentialPanel.tsx`、`docs/open-commerce-developer-production-credentials-v1-acceptance.md` |
...
| 开发者 App 签名 Webhook | 代码已形成、未编译 | App 所有者可配置白名单内标准 443 HTTPS 回调，并在创建时固定选择成功、失败或全部终态事件；新订阅须先完成签名 challenge 精确回显，验证后选中事件才进入耐久队列，工作器按短租约、指数退避、死信和自动停用规则处理。challenge 与每笔实际投递会拒绝任一私网或特殊 DNS 地址、禁用代理和重定向并固定本次公网连接。密钥只在创建或显式轮换时显示一次，轮换会停用订阅并要求重新验证，版本 1 保持既有派生兼容；已验证且启用订阅下的单条死信可原地重新排队，也可从指定序号后按筛选幂等补发最多 100 条历史通知，并记录补发来源。Node SDK 可验签。当前尚未完成编译、迁移、DNS、TLS、并发、网络、跨语言签名和 UI 验证，也不是工商身份、跨运营方总线、支付回调或业务动作重放 | `server/src/open_commerce_webhook_*.rs`、`server/src/open_commerce_outbound_security.rs`、`server/src/store/open_commerce_developer_webhook*.rs`、`pc-frontend/src/features/open-commerce/DeveloperWebhookPanel.tsx`、`docs/open-commerce-developer-webhook-history-replay-v1-acceptance.md` |
