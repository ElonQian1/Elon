...
export default function CapacityOfferPanel({ provider, pool, buckets }: Props) {
  ...
  return <section className={styles.panel}>
    ...
    {offers.length > 0 ? <div className={styles.workspace}><nav>{offers.map((item) => <button type="button" key={item.offer.offer_id} data-active={item.offer.offer_id === selected?.offer.offer_id} onClick={() => setSelectedId(item.offer.offer_id)}><strong>{item.offer.sku.sku_id}</strong><span>v{item.offer.offer_version} · {statusLabel(item.offer.status)}</span></button>)}</nav>{selected && <article><header><div><span>Offer ID</span><strong>{selected.offer.offer_id}</strong></div><b>{statusLabel(selected.offer.status)}</b></header><div className={styles.facts}><div><span>任务</span><strong>{selected.offer.sku.task_kind}</strong></div><div><span>区域</span><strong>{selected.offer.sku.region_or_data_zone}</strong></div><div><span>运行时</span><strong>{selected.offer.runtime.runtime_family}</strong></div><div><span>有效期</span><strong>{formatTime(selected.offer.valid_until)}</strong></div></div><div className={styles.capacity}>{selected.offer.capacity.map((line) => <div key={line.bucket.bucket_id}><span>{line.bucket.meter}</span><strong>{line.reservable_units} / {line.total_units}</strong></div>)}</div><code>{selected.offer.offer_digest}</code><footer><span>当前响应 `market_effect=none`；发布、报价和预留均为独立步骤。</span>{selected.offer.status === 'draft' && <div><button type="button" onClick={() => { setError(''); setReviseOpen(true) }}><PencilLine size={13} />修订</button><button type="button" data-tone="danger" onClick={() => { setError(''); setRevokeOpen(true) }}><Trash2 size={13} />撤销</button></div>}</footer><PriceSnapshotPanel view={selected} /></article>}</div> : !loading && <div className={styles.empty}><CircleDollarSign size={21} /><span>{canCreate ? '尚未创建 Offer 草稿' : 'Provider、Pool 和可用 Bucket 激活后才能创建草稿'}</span></div>}
