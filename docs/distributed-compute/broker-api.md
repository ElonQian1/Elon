...
本控制面的 Job、候选、锁价、原子 Reserve 与未执行 Release Store/Service 已通过临时 SQLite 全量迁移和 2 项组合事务测试，进程内 HTTP/MCP 又通过 1 项工具、确认、Bearer 与项目/消费者隔离测试，状态为 `implementation_partially_verified`；真实 TCP、并发和生产磁盘仍未验证。它开放项目级 Job 创建、既有 Offer/Price Snapshot 候选发现与锁价、平台人民币余额的原子 Reserve，以及 Attempt 尚未激活时的 Release/Expire。这些能力仍不能描述为完整算力交易或结算系统，证据见 `broker-control-plane-acceptance.md`。
...
- HTTP/MCP 真实 TCP、并发验证、完整路由联调和生产磁盘迁移；
