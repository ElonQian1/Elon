//! Canonical q15 controlled-fault payload and independent parent-side validation.

use anyhow::anyhow;
use rusqlite::ffi;
use sha2::{Digest, Sha256};

use super::super::super::child::lock_existing_first_truncate_error_release_succeeded::{
...
pub(in super::super) struct ValidatedExistingFirstTruncateErrorReleaseSucceededPayloadV1 {
...
pub(in super::super) fn validate_payload(
    payload: &str,
    binding: LockRunnerNativeAcquireExistingFirstTruncateErrorReleaseSucceededBindingV1,
) -> anyhow::Result<ValidatedExistingFirstTruncateErrorReleaseSucceededPayloadV1> {
    validate_binding(binding)?;
    let selector = exact_selector(binding);
    let mut fields = payload.split(',');
    if fields.next() != Some(REPORT_VERSION) || fields.next() != Some(selector.as_str()) {
        return Err(anyhow!("q15 Lock initialization payload identity mismatch"));
    }
    let values = fields
        .map(parse_canonical_u64)
        .collect::<anyhow::Result<Vec<_>>>()?;
    if values.len() != REPORT_VALUE_COUNT || values[..25] != binding_values(binding) {
        return Err(anyhow!("q15 Lock initialization payload binding mismatch"));
    }
    ...
    let expected_after = [1, 0, 0, 1, 1, 0, 0, 5, 1, 1, 1, 0, 1, 0];
    ...
    {
        return Err(anyhow!(
            "q15 controlled initialization receipt/custody mismatch"
        ));
    }
    Ok(
        ValidatedExistingFirstTruncateErrorReleaseSucceededPayloadV1 {
...
fn digest_receipt(values: &[u64]) -> [u8; 32] {
    ...
    hasher.update(
        b"elon-a2-lock-existing-first-truncate-error-release-succeeded-controlled-fault-v1\0",
    );
...
fn parse_canonical_u64(value: &str) -> anyhow::Result<u64> {
    ...
    if parsed.to_string() != value {
        return Err(anyhow!(
            "q15 Lock initialization payload scalar is not canonical"
        ));
    }
    Ok(parsed)
}
