...
assert.match(hookSource, /useState<Record<string, LocalAiProviderActivity>>\(\(\) => \(/)
assert.match(hookSource, /initializeProviderActivities\(providerIds, ownerKey\)/)
...
assert.match(sidebarSource, /const activity = web\.providerActivities\[provider\.id\]/)
assert.match(sidebarSource, /activity && activity\.phase !== 'idle'/)
assert.doesNotMatch(sidebarSource, /providerActivities\[provider\.id\]\.phase/)
