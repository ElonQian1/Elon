...
export function buildTaskTimeline(
  ...
  messages.forEach((message, index) => {
    ...
    if (!parsedEvent && ((coverage.command && isShellCommandEcho(text)) || (coverage.toolResult && isShellCommandCompletionEcho(text)))) return
...
function parseHeartbeat(text: string, message: ChatMessage, index: number): TimelineItem | null {
  ...
  return {
    ...
    meta: wait ? `累计前面全部流程总耗时 ${wait}s` : undefined,
    message,
    compact: true,
  }
}
