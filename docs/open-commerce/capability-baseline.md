...
| 消费者本人授权状态与撤回 | 代码已形成、未编译 | 按当前用户和当前项目本人 App 隔离列出四种申请状态及批准后的 Grant 条件；项目编辑者经用户确认可撤回本人 pending 请求，非 pending 不变且不撤销 Grant。响应省略内部身份，历史批准仍须复核当前 Grant | `server/src/open_commerce_consumer_authorization_mcp.rs`、`server/src/store/open_commerce_authorization_requests.rs`、`docs/decisions/open-commerce-consumer-authorization-status-mcp-v1.md` |
