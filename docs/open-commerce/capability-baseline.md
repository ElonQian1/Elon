...
| 开发者 App 签名 Webhook | 代码已形成、未编译，生产默认关闭 | App 所有者可配置白名单内标准 443 HTTPS 回调，并固定沙箱或生产环境以及成功、失败事件范围；新订阅须完成签名 challenge 才能启用。耐久队列、短租约、指数退避、死信、自动停用、公网地址固定、密钥轮换、单条重试和最多 100 条历史补发均保留。沙箱只接收旧调用和沙箱调用；生产只接收生产调用，并要求双开关和当前有效 App、资料、域名、准入及凭据，资格失效会停用订阅并终止未完成投递。当前尚未完成编译、迁移、DNS、TLS、并发、网络、跨语言签名和 UI 验证，也不是工商身份、跨运营方总线、支付回调或业务动作重放 | `server/src/open_commerce_webhook_*.rs`、`server/src/open_commerce_production_webhook*.rs`、`server/src/store/open_commerce_developer_webhook*.rs`、`server/src/store/open_commerce_production_webhooks.rs`、`docs/open-commerce-production-webhooks-v1-acceptance.md` |
