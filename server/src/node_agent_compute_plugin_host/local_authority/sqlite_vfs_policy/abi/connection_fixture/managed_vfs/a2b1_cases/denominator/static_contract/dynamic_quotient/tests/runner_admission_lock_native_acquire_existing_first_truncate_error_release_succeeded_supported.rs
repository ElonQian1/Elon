//! Windows isolated controlled-fault admission for every exact q15 member.
...
use super::lock_native_acquire_existing_first_truncate_error_release_succeeded_cases::{
    ...
    FrozenExistingFirstTruncateErrorReleaseSucceededCompletionV1,
...
use crate::node_agent_compute_plugin_host::local_authority::sqlite_vfs_policy::abi::connection_fixture::managed_vfs::a2_dynamic_evidence::{
    ...
    LockRunnerActionV1, LockRunnerExistingFirstTruncateErrorReleaseSucceededCompletionV1,
};

const EXACT_TEST: &str = "node_agent_compute_plugin_host::local_authority::sqlite_vfs_policy::abi::connection_fixture::managed_vfs::a2b1_cases::denominator::static_contract::dynamic_quotient::tests::runner_admission_lock_native_acquire_existing_first_truncate_error_release_succeeded_supported::isolated_q15_receipts_are_exact_controlled_fault_actual";
...
#[test]
fn isolated_q15_receipts_are_exact_controlled_fault_actual() -> anyhow::Result<()> {
    let selected_child =
        selected_lock_native_acquire_existing_first_truncate_error_release_succeeded_selector_for_test();
    ...
    for (&case, leaf) in frozen_lock_existing_first_truncate_error_release_succeeded_leaves_v1() {
        ...
        let descriptor = lock_existing_first_truncate_error_release_succeeded_descriptor_v1(
            case,
        ...
        .map_err(|error| anyhow::anyhow!("supported q15 member {case:?} failed: {error:?}"))?;
    ...
    match selected_child {
        Some(selected) => Err(anyhow::anyhow!(
            "parent-selected q15 member was not found: {selected}"
        )),
        None => {
            assert_eq!(
                parent_receipts,
...
const fn runner_completion_v1(
    completion: FrozenExistingFirstTruncateErrorReleaseSucceededCompletionV1,
