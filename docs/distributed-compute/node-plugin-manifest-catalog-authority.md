...
当前仍没有可注册或可成功打开的handle-bound SQLite VFS、生产process-owner实例、从SQLite ABI进入registry的live route、持有私有Rust file-custody的生产live `sqlite3_file`或生产非空`pMethods`、authorizer/PRAGMA安装与持续门卫、生产trusted-time/rollback provider、opened snapshot producer或consumer。真实managed-fs句柄与registry租约的不可拆托管状态、受callback lease约束的安全操作门面、raw-state所有权协议、具体ABI回调桥和测试专用真实file-custody回调矩阵均已存在，但尚无生产路径构造适配器、安装state或把SQLite `xOpen`接到registry。临时文件仍不实现；URI、ATTACH/DETACH等只在纯策略中定义拒绝，尚未接入SQLite执行面。现有locking/SHM/close、关闭receipt适配、请求投影、owner/状态核、file-custody和ABI表仍不可生产到达，controller与namespace仍只接到不可打开的sealed intent，生产`open()`继续固定unavailable。

下一批应增加隔离且仅测试可达的Connection生命周期夹具：使用独立非默认测试VFS和测试process-owner走SQLite真实open/close，并在同一Connection安装authorizer、关闭extension loading及设置/回读DEFENSIVE、TRUSTED_SCHEMA、DQS、ATTACHED和worker限制；任何设置或回读失败都必须关闭并隔离，不得把当前私有测试表直接变成生产注册入口。该夹具通过后才能另批设计生产process-owner、`xOpen` route producer与注册/注销所有权。v11仍固定`context_ready=false`、`snapshot_ready=false`，并保持root/authority、PlanApply、work-admission、下载、安装和Sidecar标志为false。非空库存还缺installed/promotion/signed-manifest provenance、work-admission generation、Ready/Attempt撤销及signed `reauthorize_existing`，不得由本合同推断为已完成。
