...
2. **确定当前真实产品。** 核对 origin、入口、账户/模型、页面代次和当前官方变更公告。区分新建、已有对象读取/修改与已退役能力；资源包仍有旧代码不等于当前模型可创建。不要把 Google AI Mode 与 Gemini 混为一项。
...
|---|---|---|
| 官网资源包仍有 Canvas 导出，实际模型却拒绝创建；继续扩展旧接口也得不到验收样本 | 先核对当前产品公告和一次真实操作，再分开旧对象兼容、新建能力与替代产品。已有函数/mock 通过不证明当前账户可用；没有新证据不重复创建、不改动私人原件充当测试样本 | [1682 产品边界](reports/chatgpt-canvas-provider-boundary-1682.md) |
...
- 输入框定位必须限定原生输入栏，不能全局匹配占位文字或测试消息；Accessibility 的 hint 也不等于草稿。原生发送验收包含实际输入、收起键盘后按钮状态和实际 Send，不以 MCP 发送代替。见[输入栏回归](reports/chatgpt-collapsed-composer-20260912.md)。
...
- 原生页面正在等待自己的请求时，不再用独立 MCP 刷新替换它，否则会把页面忽略异主请求快照误判成 UI 故障。用页面自己的刷新按钮和对应回执验收。
- 文件带产物类型不等于所有操作都不支持；也不能先猜它属于某个生成类型。先观察来源分类，再按每项操作的格式、归属和真实协议准入。[1666 图片下载](reports/chatgpt-library-source-audit-1662.md#normal-1666-acceptance)复用原下载模块完成，不应重做。
- 接口分页消息不等于官网内存里的消息树。按当前官方转换函数读取消息 ID，再与实际选中分支核对；不能因为旧 `mapping` 缺失而拒绝，也不能直接改用服务器默认分支。[1670 项目引用](reports/chatgpt-project-library-reference-20260911.md#normal-1670-acceptance)已验证此差异。
- 引用已有库文件与上传新文件分开准入。引用不能被新上传的索引开关拦住，也不能反过来授权上传；追加本地文件须保留输入租约并重新取得上传条件。会话、分支、权限、索引失败应有各自闭集错误码，不统一吞成“上下文变化”。

- MCP 正文读取遵守 `message_window_start`，不要固定 offset 0。`context_history_unavailable` 先查窗口元数据，使用当前 offset/游标；这是读取范围错误，不允许据此重发已成功的消息。Markdown 转义后的显示文本也不等于原始结构化 block 协议，见 [1682 对账](reports/chatgpt-canvas-provider-boundary-1682.md)。
