//! Child-only Unmap actual capture and sanitized stdout publication.
...
use super::super::{
    ...
    a2b2_cases::UnmapSelector,
    unmap_harness::exercise_unmap,
};

pub(super) const CHILD_ROOT_ENV: &str = "ELON_SQLITE_A2_UNMAP_CHILD_ROOT";
...
pub(super) fn selected_child_root() -> anyhow::Result<Option<PathBuf>> {
    ...
    if !root.is_absolute() {
        return Err(anyhow!("Unmap child root is not absolute"));
    }
    Ok(Some(root))
}

pub(super) fn exercise_child(root: &Path, selector: UnmapSelector) -> anyhow::Result<()> {
    let nonce =
    ...
    let actual = exercise_unmap(root, selector)?;
    ...
    if !root.is_dir() {
        return Err(anyhow!(
            "Unmap child root disappeared before parent observation"
        ));
    }
    Ok(())
}
