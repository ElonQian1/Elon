...
impl Store {
    ...
    pub(crate) fn prepare_node_compute_plugin_install_plan_preparation_delivery(
        ...
        let intent = prepare_node_compute_plugin_install_plan_preparation_delivery_on(
            &tx,
            sharing,
...
pub(in crate::store) fn prepare_node_compute_plugin_install_plan_preparation_delivery_on(
    tx: &Transaction<'_>,
    sharing: &NodeComputePluginSharingDispatchIntent,
    policy_snapshot_digest: &str,
) -> Result<Option<NodeComputePluginInstallPlanPreparationDispatchIntent>> {
    ...
    if !current_sharing_head_matches(tx, sharing, authorization)?
        || !accepted_sharing_ack_matches(tx, sharing, authorization, policy_snapshot_digest)?
    {
        return Ok(None);
    }
    let existing =
...
pub(in crate::store) fn record_node_compute_plugin_install_plan_preparation_delivery_on(
    tx: &Transaction<'_>,
    intent: &NodeComputePluginInstallPlanPreparationDispatchIntent,
    event_kind: &str,
    detail_code: Option<&str>,
    ...
    tx.execute(
        "INSERT INTO node_compute_plugin_install_plan_preparation_delivery_events (
           ...
           policy_revision, policy_digest, event_sequence, event_kind, detail_code, created_at
...
#[allow(clippy::too_many_arguments)]
pub(in crate::store) fn record_node_compute_plugin_install_plan_preparation_observation_on(
    tx: &Transaction<'_>,
    intent: &NodeComputePluginInstallPlanPreparationDispatchIntent,
    accepted: bool,
    replayed: bool,
    ...
    let observation_id = new_id("cpio");
    tx.execute(
    ...
    Ok((observation_id, observed_digest))
}
