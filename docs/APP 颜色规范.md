...
颜色部分必须按截图采样值实现，不允许用“相近色”“更亮一点”“差不多的灰”替代。比如图中底部导航是 `#212121`，实现时就必须使用 `#212121`，不能改成 `#202020` 或 `#222222`。
...
- 原生工具感：图标使用线性轮廓，菜单使用白色浮层，底部导航固定、稳定、低干扰。
...
| 底部导航背景 | `color.nav.bg` | `elon_nav_bg` | `--nav-bg` | `#212121` | 图 1/2 底部导航 |
...
| `font.bottom.nav` | 底部导航文字：好友、项目、我的 | `14sp` | `400` | `MainTabText` |
...
- 底部导航固定三项：好友、项目、我的。
- 背景 `color.nav.bg`，高度 `84dp` 到 `92dp`，系统导航栏 / 手势安全区也使用同一灰底，形成完整底部菜单板块。
- 图标使用线性轮廓，文字使用 `font.bottom.nav`。
- 当前页不使用彩色选中；图标和文字统一使用 `color.text.nav`，当前页状态由页面内容和顶部导航表达。
- 底部导航上方不要加高亮线；保持沉稳的深灰块。
...
- 不使用彩色图标表达普通导航状态。
- 图标颜色只能从 `color.text.primary`、`color.text.secondary`、`color.text.nav`、`color.icon.member`、`color.icon.primary`、`color.icon.add.top` 中选择。
...
- 按下态：保持原始填充 token，不新增 HEX；只允许使用系统 ripple 或在原 token 上叠加临时按压反馈。
- 禁用态：沿用原始 token，整体 alpha 降到 `40%`；不得引入新的禁用灰色。
- 加载态：优先在按钮内显示小型进度或禁用按钮，不改变布局高度。
- 选中态：导航用下划线或深灰胶囊，底部导航用亮度，不用彩色。
