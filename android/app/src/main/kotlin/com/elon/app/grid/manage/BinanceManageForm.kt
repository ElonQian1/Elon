...
internal class BinanceManageForm(private val activity: Activity, edited: () -> Unit) {
    ...
    private val operation=select("本次操作", "binance-manage-action", listOf("请选择操作","修改终止时平仓设置","结束网格"),operations)
    private val mode=select("终止时的仓位处理", "binance-manage-mode", listOf("请选择处理方式","按市价平仓","保留仓位，自行处理"),operations)
    init {
