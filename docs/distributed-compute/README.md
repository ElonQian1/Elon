...
| 外部算力池适配器与统一报价 | v221-v245 前置门为 `implementation_partially_verified`；V243 已形成短时凭据验证回执，V244 已形成可撤销采用授权并留下定向迁移与 2 项 HTTP 通过证据；V245 的 V243/V244 legacy 全投影/显式 `checked_at` 加固仍为 `implementation_uncompiled`、`passed=0`。它们仍未安装或激活生产 Adapter。见 [`当前状态`](current-implementation-status.md)、[`V244 采用权威`](external-pool-adapter-adoption-authority.md) 与 [`V243/V245 凭据回执权威`](external-pool-adapter-credential-verification-authority.md) |
...
Provider Adapter 的 v227-v245 前置门已形成分层权威；V244 已把当前 V239/V243 汇合为可撤销采用授权，V245 只加固 V243 凭据与 V244 采用/撤销 legacy 标量投影、历史/当前 authority 类型隔离与显式检查时间，尚未编译或运行。最新状态见 [`current-implementation-status.md`](current-implementation-status.md)，签名沙箱边界见 [`V239 动态沙箱符合性证据`](external-pool-adapter-artifact-sandbox-conformance-authority.md)，采用边界见 [`V244 采用授权`](external-pool-adapter-adoption-authority.md)，凭据报告边界见 [`V243/V245 凭据回执权威`](external-pool-adapter-credential-verification-authority.md)。服务器仍不真实安装或执行制品，不返回原始凭据，也未启用生产 Adapter。
