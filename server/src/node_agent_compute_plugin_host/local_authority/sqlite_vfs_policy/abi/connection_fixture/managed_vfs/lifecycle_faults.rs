...
struct ManagedTestLifecycleFaultState {
    ...
    #[cfg(all(test, windows))]
    pre_managed_lock: ManagedTestPreManagedLockState,
    installed: bool,
    #[cfg(all(test, windows))]
    ...
    pub(super) fn new() -> Arc<Self> {
        Arc::new(Self {
            state: Mutex::new(ManagedTestLifecycleFaultState {
                ...
                #[cfg(all(test, windows))]
                pre_managed_lock: ManagedTestPreManagedLockState::default(),
                installed: false,
                #[cfg(all(test, windows))]
