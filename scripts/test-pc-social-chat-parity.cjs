...
const contextMenus = require('./pc-social-context-menu-cases.cjs')
...
createRoot(document.getElementById('root')).render(React.createElement(React.StrictMode,null,React.createElement(MemoryRouter,null,React.createElement(FriendsPage))));</script></body></html>`)
  ...
  try {
    ...
    groups.push(...contextMenus.mediaMessages(message))
    ...
    await page.route('**/api/**', async route => {
      ...
      if (await contextMenus.routeMedia(route)) return
    ...
    await contextMenus.run({page,row,root})
    ...
    await page.keyboard.press('Escape'); assert.equal(await row('own').evaluate(el=>el===document.activeElement),true)
    await menu('peer'); assert.equal(await page.getByRole('menuitem',{name:'编辑',exact:true}).count(),0)
    await act('修改记录'); await page.getByText('成员最初的原文',{exact:true}).waitFor(); await close()
    await menu('own'); await act('编辑')
    ...
    await menu('article'); assert.equal(await page.getByRole('menuitem',{name:/编辑|引用|转发…|多选/}).count(),0); await page.keyboard.press('Escape')
    await menu('project'); assert.equal(await page.getByRole('menuitem',{name:'编辑',exact:true}).count(),0); await page.keyboard.press('Escape')
    await menu('gone'); assert.equal(await page.getByRole('menuitem',{name:/编辑|复制|引用/}).count(),0); await page.keyboard.press('Escape')
    await menu('old'); assert.equal(await page.getByRole('menuitem',{name:'撤回…',exact:true}).count(),0); await page.keyboard.press('Escape')
    ...
    await menu('own'); await act('撤回…'); await page.getByRole('button',{name:'确认撤回',exact:true}).click()
    ...
    await menu('private'); assert.equal(await page.getByRole('menuitem',{name:'编辑',exact:true}).count(),0); await page.keyboard.press('Escape')
