...
| CapacityPool 本人控制面 | HTTP/MCP 已可在本人 Provider 下登记、读取、列出和审计 `registering` Pool；审计从不可变账本重算当前 epoch 余额，但健康不等于硬件 verified；epoch、revision、状态、时间和摘要由服务端固定，尚未编译和运行验证 |
