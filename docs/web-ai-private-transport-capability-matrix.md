...
| Fresh independent text dispatch | ChatGPT | Completed/default-enabled for existing/new personal plain text, existing personal Search/Image and local TXT/PDF/PNG with prompt through normal 1747. Native first send without usable composer passed on adapter 411; reuses the default sender | No uncertain-write replay. New/tool/file combinations, project/temporary, other MIME, Library/mounted and cold-bootstrap scopes remain separate. [Current evidence](reports/chatgpt-runtime-bindings-20260915.md), [attachments](chatgpt-fresh-attachment-text-dispatch.md) |
