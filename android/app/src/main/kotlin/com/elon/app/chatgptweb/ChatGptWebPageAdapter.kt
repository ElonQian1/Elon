...
internal class ChatGptWebPageAdapter(
    ...
    internal val conversationReader = ChatGptWebConversationRead(context, webView, documentSession::snapshot, onWebExecutionRequested)
    ...
    fun writingBlock(request: JSONObject, confirmed: Boolean, requestId: String) = runCommand(
        action = "writing_block", value = request.toString(), requestId = requestId, selected = confirmed,
    )
    ...
    fun requestNativeAttachmentUpload(files: List<PendingAttachment>, uris: List<Uri>, requestId: String): Boolean {
        ...
        runCommand("request_attachment_upload", value = descriptor, requestId = requestId)
        return true
    }
    ...
    companion object {
internal const val ADAPTER_VERSION = 437
