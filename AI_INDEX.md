...
| 一龙多身份联合登录 / Google OIDC / 密码与恢复码 / 设备会话 / Win、APK、Web 账号安全 / AI CLI Provider 状态机、诊断、保险箱合同与禁用 Web Chat 适配器 | `docs/unified-account-and-ai-provider-architecture.md`、`server/src/{account_security,auth_request_guard,federated_auth*,node_agent_provider_*,node_agent_web_chat_adapter}.rs`、`server/src/store/{account_identities,account_security*}.rs`、`pc-frontend/src/features/{auth,account}/`、`android/app/src/main/kotlin/com/elon/app/{GoogleFederatedAuth,AccountSecurityApi,AccountSecurityViews}.kt`、`server/src/assets/{federated_auth,account_security,ai_provider_accounts}.*` |
