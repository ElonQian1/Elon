...
| 消费者发现请求输入边界 | 已完成编译，部分边界通过 MCP 假数据验收 | 第三方 App 的搜索词、能力键、App ID 和返回数量由服务端统一规范化；MCP 返回数量、未知字段、币种和硬约束缺值已验证失败关闭，SQL LIKE 特殊字符仍按普通文字查询但本批未单独运行。该边界不改变固定候选窗口、授权、排序或计量规则 | `server/src/open_commerce_consumer.rs`、`server/src/open_commerce_consumer_model.rs`、`server/src/store/open_commerce_directory.rs`、`docs/decisions/open-commerce-consumer-discovery-inputs-v1.md` |
