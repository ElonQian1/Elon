...
A2c runner foundation 不读写 A2b2 expected record，也不产生 `WindowsDynamic` evidence。它只把以后可执行的第一条 route-exact callback-before 路径接到真实 Windows test VFS 与 SQLite SQL；现阶段不得从该源码推断 native failure、after-success custody、barrier 无返回码、unmap、joint close、registry lifecycle、registration shutdown 或 117-key 覆盖。
