...
| 外部算力池适配器与统一报价 | v221-v252 前置门为 `implementation_partially_verified`；V243/V244 保留短时凭据与可撤销采用的局部通过证据，V245-V247 安装链及 V249/V251 registry 链已通过对应本地定向验收。V250 scanner 签名漏洞情报链已通过 migration 4 项与进程内 HTTP 5 项；V252 sandbox verifier 签名声明链已通过 migration 7 项与进程内 HTTP 2 项。两者不执行扫描或真实沙箱，Provider 未激活且 Adapter 未启动。见 [`当前状态`](current-implementation-status.md)、[`V252 沙箱续签权威`](external-pool-adapter-sandbox-reattestation-authority.md)、[`V250 漏洞情报续签权威`](external-pool-adapter-vulnerability-reattestation-authority.md)、[`V249 registry 权威`](external-pool-adapter-registry-authority.md) 与 [`V247 安装/撤销权威`](external-pool-adapter-installation-authority.md) |
...
Provider Adapter 的 v227-v252 前置门已形成分层权威；V244 汇合当前 V239/V243 为可撤销采用授权，V245-V247 增加 legacy 栅栏、可撤销 `installed_inert` 内容树与 sealed seam并通过安装链定向验收；主线 V248 属于 ERP，V249 顺延登记 Provider-neutral release 与 Provider-specific companion，V251 修复合法 JSON 键序兼容并随 registry 链通过定向验收。独立的 V250 为 exact neutral release 建立可续签、可撤销的 scanner 签名漏洞情报链，已通过 migration 合同 4 项与进程内 Axum HTTP 5 项；V252 再建立 Provider-neutral 六能力 sandbox verifier 签名链，已通过 migration 合同 7 项与进程内 Axum HTTP 2 项。最新状态见 [`current-implementation-status.md`](current-implementation-status.md)，沙箱续签边界见 [`V252 sandbox re-attestation`](external-pool-adapter-sandbox-reattestation-authority.md)，漏洞情报续签边界见 [`V250 vulnerability re-attestation`](external-pool-adapter-vulnerability-reattestation-authority.md)，registry 边界见 [`V249 Provider-neutral registry`](external-pool-adapter-registry-authority.md)。服务器不运行扫描或真实沙箱、不证明 intelligence/observations 真实性、不删除或执行安装字节、不返回原始凭据，也未激活 Provider；后续 activation 仍须可续签 Provider-specific credential、V249 companion/sealed Prepared、current V250/V252，并与精确 service actor、Provider-specific route compatibility binding 和 Provider activation 同事务。
