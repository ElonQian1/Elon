...
| 激活证据申请与计划控制面 | v177-v179、本人 HTTP/MCP、管理员 HTTP 审核/过期批准废止、双方只读就绪预检及管理员不可变激活计划已写；计划在单一事务内复核依赖并生成下一 Provider revision，申请废止会同步废止 prepared 计划，但所有操作 `activation_effect=none`，状态为 `implementation_uncompiled` |
