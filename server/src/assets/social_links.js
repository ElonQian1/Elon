(function (root) {
  ...
  function links(text) {
    ...
    for (const match of text.slice(0, 50000).matchAll(/https:\/\/[^\s<>"'\]\)]+/g)) {
      ...
      result.push({ schema: 1, url: u.href, site: sites[u.hostname], title: title.slice(0, 160), author: '', image: null, embed: embed(u.href), status: 'unavailable' });
  ...
  function sanitize(value, fallback) {
    ...
    return { ...fallback, title: typeof value.title === 'string' && value.title.trim() ? value.title.slice(0, 160) : fallback.title,
  ...
  function mount(container, text, options) {
    ...
    const host = document.createElement('div'); host.className = 'social-link-cards'; container.append(host);
    ...
    for (const item of items) {
      ...
      function draw(value) {
        current = value; title.textContent = value.title || `${value.site}分享`;
        const time = value.embed?.kind === 'bilibili' ? new URL(value.embed.url).searchParams.get('t') : null;
        source.textContent = `${value.site}${value.author ? ' · ' + value.author : ''}${time ? ' · 从 ' + time + ' 秒开始' : ''}`;
        button.setAttribute('aria-label', `打开${title.textContent}（${value.site}）`);
        if (value.image) { cover.hidden = false; cover.src = value.image; }
      }
      async function load(refresh = false) {
        if (busy || !valid()) return; busy = true; retry.disabled = true;
        const value = await preview(item, options, refresh);
        busy = false;
        if (valid()) { draw(value); retry.disabled = false; retry.hidden = value.status === 'ready'; }
      }
  ...
  root.ElonSocialLinks = { safeUrl, embed, trustedEmbed, links, sanitize, mount };
})(globalThis);
