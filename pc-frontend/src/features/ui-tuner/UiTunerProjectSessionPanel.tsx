...
export function UiTunerProjectSessionPanel({
  ...
  useEffect(() => listenForFitRunCodexRequests((request) => {
    ...
    const taskIntent = request.handoffKind === 'PWA_DRAFT' ? [
      ...
      '为节省 Token，第一步只读取 Context Artifact 的 pwaDesign.aiFitTask；它已经包含 compactHandoff、证据路径、执行策略和源码候选。',
      '按 aiFitTask.compactHandoff.elements[].changedProperties 执行样式写回，优先打开 sourceFilesToInspect 中的候选文件。',
      '只有 aiFitTask 证据不足时，才展开 artifact 中的局部 DOM、compactSourceBundle、相关裁剪与视觉差异；不要默认读取整仓库或整棵 DOM。',
