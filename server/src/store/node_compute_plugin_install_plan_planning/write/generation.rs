...
/// Runs inside the same Immediate transaction that appends the ready observation and snapshot.
/// The currently supported default cannot leave either a snapshot without a generation terminal
/// outcome or an open generation request if the process stops between writes.
pub(super) fn ensure_signer_unavailable_in_transaction(
    tx: &Transaction<'_>,
    snapshot: &DurableComputePluginInstallPlanPlanningSnapshotV2,
) -> Result<DurableComputePluginInstallPlanGenerationOutcomeV1> {
    let request = ensure_generation_request(tx, snapshot)?;
    ensure_signer_unavailable_outcome(tx, &request)
}

fn ensure_generation_request(
    tx: &Transaction<'_>,
    snapshot: &DurableComputePluginInstallPlanPlanningSnapshotV2,
    ...
    if let Some(existing) = read_generation_request(tx, &snapshot.snapshot_id)? {
        ...
        validate_generation_request_readback(tx, &existing)?;
        return Ok(existing);
    }
...
fn ensure_signer_unavailable_outcome(
    tx: &Transaction<'_>,
    request: &DurableComputePluginInstallPlanGenerationRequestV1,
) -> Result<DurableComputePluginInstallPlanGenerationOutcomeV1> {
    const OUTCOME_KIND: &str = "signer_unavailable";
    const DETAIL_CODE: &str = "COMPUTE_PLUGIN_INSTALL_PLAN_SIGNER_UNAVAILABLE";
    const RETRYABLE: bool = true;

    validate_generation_request_readback(tx, request)?;
    if let Some(existing) = read_generation_outcome(tx, &request.request.generation_request_id)? {
        ...
        validate_generation_outcome_readback(tx, &existing)?;
        return Ok(existing);
    }
