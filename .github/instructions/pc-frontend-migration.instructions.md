...
- 修改 `pc-frontend/`、`/pc`、`/pc-next` 或用户可见 PC 工作台 UI 时，`CodePushed` 只代表代码同步，不代表用户页面已经更新。除非用户明确要求只同步代码或暂不发布，否则必须在 push 后运行 `scripts\publish-server.ps1`（Linux/macOS：`bash scripts/publish-server.sh`）上传 `pc-next-dist`，最后用统一收尾的 `PcFrontend` Kind 校验 `/pc`、`/api/server/version`、主基线和 worktree 状态。
