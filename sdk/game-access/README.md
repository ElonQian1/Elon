...
状态：协议实现与离线互操作验证；实际主账号授权服务见 [服务说明](../../docs/esk-game-access.md)。2026-09-11。
...
- 支持七种受限动作：authenticate、inventory、wallet_bind、order、quote、accept_quote、principal_withdraw。wallet_bind 需要独立钱包绑定权限；本金退出也需要独立权限，redeem 不能代替它。
...
- SDK 本身不提供授权码/PKCE、同意 UI、TLS 路由、服务认证、令牌持久化、真实签发或撤销；这些由主项目 game-access 服务和游戏 BFF 实现。调用方不能把 SDK 验签通过称为生产登录完成。
...
scopes 必须去重并按 play、inventory_read、wallet_bind、redeem、principal_withdraw 顺序出现，包含 play。完整字段与动作白名单由 `src/contract.js` 严格验证。新增 wallet_bind 不改变旧签名字节，也不自动升级旧 grant；权限通过仍不代表钱包签名通过或资产已转移。
...
接线：主项目独立 game-access 授权码与观察路由复用真实主会话和 TLS 边界，游戏 BFF 持有受限凭据；不能扩大现有量化只读 token 来代替它。运营配置与真实联合部署验收按服务说明执行。
