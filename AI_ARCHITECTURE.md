...
Offer 控制面允许当前用户为 active Provider/Pool 创建规范化 draft，服务端固定供给身份、容量绑定、窗口、区域、meter 与摘要；所有者可精确撤销 draft。管理员 HTTP 可原子追加 active 与 v182 发布回执，用 v183 转为 draining，并在 v184 索引确认无 pending/active Reservation 后转入 expired/revoked。expired 必须已过 `valid_until`，提前退出只能 revoked；状态转换不取消预留、归还 Claim 或退款，管理员写入口不向 MCP 开放。当前为 `implementation_uncompiled`。
