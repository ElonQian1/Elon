...
write_ai_workflow_guard() {
  ...
  echo "RULE_WINDOWS_BACKGROUND=when automation starts a Windows PowerShell child, include -WindowStyle Hidden; when already in PowerShell, use & <script> instead of nesting another visible host."
  ...
  echo "FINISH_COMMAND_POWERSHELL=powershell -WindowStyle Hidden -NoProfile -ExecutionPolicy Bypass -File scripts\\finish-ai-task.ps1 -Kind CodePushed"
