...
| 受控商户自有运行时 | 已实现、通用 Node 内核完整回归后置 | 平台可通过审核绑定和 HMAC 签名调用商户 ERP；`cofficethinking` 参考节点已实现商品、报价、显式确认下单、订单查询、幂等回执和库存事务。连接器 SDK 新增通用运行时内核，商户只需提供能力处理器与持久化幂等存储 | `docs/open-commerce/merchant-runtime.md`、`sdk/open-commerce-connector/src/merchant-runtime.js`、`docs/open-commerce-merchant-runtime-v1-acceptance.md` |
...
| 开发者 App 资料清单与审核 | 代码已形成、未编译 | App 所有者或项目管理员可维护 HTTPS 主页、隐私政策、服务条款、支持邮箱和申请能力，以修订号提交；平台管理员可批准或要求修改，编辑会清除旧结论。审核不改变沙盒环境、不签发生产凭据，也不证明组织身份或域名所有权 | `server/src/open_commerce_developer_manifest_*.rs`、`server/src/store/open_commerce_developer_app_manifests.rs`、`pc-frontend/src/features/open-commerce/DeveloperAppManifestPanel.tsx`、`docs/open-commerce-developer-app-manifest-review-v1-acceptance.md` |
...
| 开发者 App 签名 Webhook | 代码已形成、未编译 | App 所有者可配置白名单内 HTTPS 回调，并在创建时固定选择成功、失败或全部终态事件；新订阅须先完成签名 challenge 精确回显，验证后选中事件才进入耐久队列，工作器按短租约、指数退避、死信和自动停用规则处理。密钥只在创建或显式轮换时显示一次，轮换会停用订阅并要求重新验证，版本 1 保持既有派生兼容；已验证且启用订阅下的单条死信可原地重新排队，也可从指定序号后按筛选幂等补发最多 100 条历史通知，并记录补发来源。Node SDK 可验签。当前尚未完成编译、迁移、并发、网络、跨语言签名和 UI 验证，也不是工商身份、跨运营方总线、支付回调或业务动作重放 | `server/src/open_commerce_webhook_*.rs`、`server/src/store/open_commerce_developer_webhook*.rs`、`pc-frontend/src/features/open-commerce/DeveloperWebhookPanel.tsx`、`docs/open-commerce-developer-webhook-history-replay-v1-acceptance.md` |
