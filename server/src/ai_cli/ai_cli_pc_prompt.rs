...
如果本轮影响一龙 `pc-frontend/`、`/pc` 或用户可见 PC UI，除非用户明确只同步代码，否则最终回复前必须构建、push、运行 publish-server 上传 pc-next-dist，并用统一收尾的 PcFrontend Kind 校验 /pc 和 /api/server/version；只有 FINALIZABLE=true 才能完整结束。截图/遮挡/错位类修复还必须定位真实组件并做视觉验收。\n\n\
{preflight}\
用户请求：\n\
{user_message}",
...
如果本轮修改一龙 `pc-frontend/`、`/pc` 或用户可见 PC UI，CodePushed 不等于用户已看到修复；除非用户明确只同步代码，否则必须完成：构建 → commit/push → `publish-server.*` 上传 pc-next-dist → 统一收尾 `PcFrontend` 校验 `/api/server/version.gitSha`，并确认 `FINALIZABLE=true`。截图、遮挡、错位或按图修复必须定位真实组件，再用预览、浏览器截图、DOM/坐标/层级检查之一验收；不能只凭构建通过宣称解决，无法验收时明确说明未完成视觉验收。\n\
...
mod tests {
    ...
    fn pc_project_execution_prompt_requires_continuing_after_rules() {
        ...
        assert!(prompt.contains("/api/server/version.gitSha"));
        assert!(prompt.contains("DOM/坐标/层级检查"));
        assert!(prompt.contains("统一收尾 `PcFrontend`"));
        assert!(prompt.contains("FINALIZABLE=true"));
        assert!(prompt.contains("CodePushed 不等于用户已看到修复"));
        assert!(prompt.contains("未完成视觉验收"));
        assert!(prompt.contains("不能只凭构建通过宣称解决"));
