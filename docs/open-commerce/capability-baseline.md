...
| 消费者发现与第三方应用沙盒 | 已实现，排序器批次未编译 | 可注册、轮换、停用和重新启用测试 App，一次性显示测试 Token，跨项目发现主动发布商户，查看或撤回申请并在审批后调用；App 身份绑定所有者。发现代码支持五种用户可选、可解释且非付费的内置排序器；只作用于最多 100 个目录候选，不是全网公平性证明 | `server/src/open_commerce_client_api.rs`、`server/src/open_commerce_client_lifecycle_service.rs`、`server/src/open_commerce_consumer_ranking.rs`、`docs/decisions/open-commerce-pluggable-ranking-v1.md`、`docs/open-commerce/consumer-developer-sandbox.md` |
