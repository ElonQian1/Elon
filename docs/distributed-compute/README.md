...
| 外部算力池适配器与统一报价 | v221-v262 前置门为 `implementation_partially_verified`。V260 authenticated session 与 V261 Linux supervisor 已由 V262 在真实 exec 后组合；Windows source-contract、Linux-musl product/fixture/test build 和 WSL2 12 项 kernel fixture通过。当前仍未读取真实 Secret、连接 upstream target 或生成 probe/route/activation；V254 18 deny 不变。见 [`V262 runtime 权威`](external-pool-adapter-authenticated-runtime-authority.md)、[`V262 验收`](external-pool-adapter-authenticated-runtime-acceptance.md) 与 [`当前状态`](current-implementation-status.md) |
...
Provider Adapter 的 v227-v262 前置门已形成分层权威。V260 已验证 anonymous seqpacket/seed pipe、HKDF/HMAC mutual bootstrap、双向 frame 与 terminal fail-close；V261 验证真实 clone3/cgroup/namespaces/private root、fd3/fd4/fd5 custody、seccomp、pidfd 与 cleanup；V262 再把二者放入同一真实 exec 后生命周期，并以正向/root-drift、network/fcntl/poll negative fixture 验证失败关闭。当前仍未读取 V256 Secret、连接 V258 target 或生成 probe/activation。下一硬门是 ephemeral Secret delivery，再做 broker TLS/upstream no-work probe。V254 18 deny 逐字保留，完整 admission gate 只能与 atomic activation 同批替换。见 [`V262 authenticated runtime`](external-pool-adapter-authenticated-runtime-authority.md)、[`V261 Linux supervisor`](external-pool-adapter-linux-supervisor-authority.md)、[`V260 authenticated session`](external-pool-adapter-authenticated-child-session-authority.md)、[`V259 supervisor/session companion`](external-pool-adapter-supervisor-session-policy-companion-authority.md) 与 [`current-implementation-status.md`](current-implementation-status.md)。
