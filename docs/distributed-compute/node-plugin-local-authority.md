...
cleanup execution plan、expected objects、plan seal 与 append-only step events 已纳入 v3 必需对象，并已随 `elon-pc-node` 编译及 schema 建库与重开指纹定向测试。plan 绑定 authorization、installation、staging root、candidate-parent anchor identity 与完整对象父子拓扑；每个对象只有完成 durable intent、exact disposition/受约束的恢复缺失、exact parent 下的 namespace absence 和独立 namespace-durability fence，completion 才能同时引用 plan digest、既有物理 execution evidence digest 与独立 terminal journal digest。completion Store 内核已经按这三类摘要改造，并只接受无公开构造入口的终态 journal capability；物理 evidence v1 仍没有不可变记录及 plan/object digest 绑定。JCS/路径/对象/物理证据摘要重算的 topology/journal typed Store、目录 identity 与 namespace durability 原语、既有物理执行器对 durable-intent/journal 的适配及跨重启 recovery 均未实现。
...
当前合并后的供应链、候选健康与失败候选清理代码已通过 `elon-pc-node` 编译；清理链通过 3 项 Windows 同句柄删除、1 项目录顺序及 4 项 completion 凭证/库存投影测试。cleanup authorization Store 可原子推进 `cleanup_pending`，旧物理执行器只消费固定句柄并保留失败 custody；completion Store 内核只在 sealed topology 和全部 namespace-durability journal 通过 exact readback 后，才会移除精确 failed slot、清空 candidate pointer、写不可变回执并关闭 owner，任何不确定提交只能恢复为 exact `NotCreated` 或 `Completed`。由于 topology/journal Store、执行适配和目录 durability 仍缺，终态 capability 不可构造，完整私有流水线尚不可达。本批未构造完整目录树+SQLite Store 事务夹具，也未接入生产 Host 或生产本地库，不能授予生产重试、安装、promotion、ReadyCapability 或商业 Verification。跨重启恢复、真实下载/Sidecar/探针、installed/promotion/GC 与 Attempt 接线仍待实现；精确边界见 `node-ready-capability.md` 与 `node-plugin-candidate-cleanup.md`。
