...
| 节点端点凭据与认证会话权威 | 服务端 v216 已铺 credential/version/revocation/session receipt/session head 五表，v217 追加目标绑定的五分钟 owner 重认证回执，v218 追加单次 consumption 账本。默认关闭的 direct-TLS-only listener 现仅在专用 secure Router 暴露 issue/rotate/recover/revoke，且除了启用 TLS listener 还必须独立设置 `NODE_ENDPOINT_OWNER_CREDENTIAL_API_ENABLED=true`：真实 DB bearer、当前密码、TLS request/response permit 与目标 currentness 在同一 IMMEDIATE 事务闭合 v217+v218+v216 CAS；新 secret 只在首次成功响应出现，重放不返明文，rotate 另须旧 endpoint secret。零 legacy backfill；plain HTTP、OWNER_TOKEN、Google/trusted device/恢复码不能进入。legacy WS 的 process-local exact key/read fence仍只防本地 replacement；secure `/agent/ws` 固定 503、不 upgrade、不写 session Store，NodeAgent/proto/耐久 session、Ready、route、outbox、command、Lease 与派发均未接。源码尚未编译、测试、运行或执行迁移。见 `node-endpoint-session-authority.md` |
