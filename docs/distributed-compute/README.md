...
> v218 合同修正：历史“v11 Planning Snapshot V2”现以协议阈值 12 解释；work-admission `None` 允许新晋升槽首次重授权，`Some` 才承诺 current head。当前 A1 只有未编译的同事务 projector；A2 总合同已冻结，源码只到未编译、未运行的 A2a 子集，A2b 与动态证据仍缺。无 producer，v14 永久 blocked-only，Runtime/Ready/派发不可达。
...
F1 的下一硬门槛是先补齐 [`node-plugin-vfs-fault-authority.md`](node-plugin-vfs-fault-authority.md) 的 A2b 内部 phase/terminalization 静态矩阵，再实际执行 Windows SHM、联合关闭与多 Connection 逐 case 验证。A2a 未编译、未运行，不能据此进入生产 VFS/open、Planning producer、v15、Runtime 或 Ready。
