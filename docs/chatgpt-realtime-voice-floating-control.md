...
结束期间挂断按钮禁用，重复点击不会重复发送挂断命令。两轮挂断确认仍未完成时，界面收起为“仍在通话”悬浮球；随后最多两分钟每秒只读确认一次官网是否已经返回会话页，不再点击挂断。官网延迟结束后会自动关闭悬浮球并刷新当前会话，超过上限则停止观察，避免无限轮询和发热。状态没有变化时不会重复触发无障碍播报，也不使用持续动画。
