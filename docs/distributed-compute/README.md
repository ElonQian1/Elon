...
| Attempt 挑战终态决议与历史 | v197、追加式 Store、角色 open/历史队列、HTTP 与 PC 控制面已写；消费者在 `/compute-challenges` 撤回并看本人历史，Provider 在 `/my-compute-settlement` 看影响本人收益的历史，管理员在 `/compute-challenge-resolution` 裁决并看全局历史。历史逐项重审计 v195-v199 引用和状态组合；available 仅指内部余额且不证明外部付款，尚未编译、执行迁移、接口联调、页面验收或发布 |
