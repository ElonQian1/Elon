...
- 双APK设备导航已在正式主APK1.1.1577补验：量化入口正常打开管理页，选择当前子账户
  的1个网格后，MCP详情结果verified且detail_current=true；返回量化收到未提交回执。
  量化自身的授权、非空列表与详情也已实测。临时语义探针本次安装成功，验收后已卸载。
  新安装身份、实际范围及恢复过程见[只读调试验收](android-binance-grid-read-debug-v1-20260908.md)。
