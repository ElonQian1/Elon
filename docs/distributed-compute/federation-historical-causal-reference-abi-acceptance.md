...
Domain、Store resolver、source-contract、golden与negative test源码已存在，Service、HTTP/MCP、客户端、table、migration、
caller仍不存在，也没有 compile或runtime证据：
...
F0 的 Service、HTTP/MCP、客户端采用与已运行合同测试退出门仍未达到。不得把本文 matrix写成已运行结果。
...
本批五类所需源码已同时落盘，因此只把 Domain/Store记为 `source_written`；验证强度仍严格是
...
本批唯一诚实结果是 Domain/Store=`source_written`，Service/HTTP/MCP/client=`absent`，migration/table=`none`，
