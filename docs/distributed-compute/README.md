...
| 节点插件治理合同 | Signed Manifest/InstallPlan、双 keyring、authority v3/PlanApply、可恢复受管取数、候选 exact closure/hash 与原子 resolution、schema 定义指纹、可信时间、ZIP 安全物化和 staging Store 均已写。staged 候选可进入有界健康状态机；成功路径写不可变健康回执但不改槽位，终止失败路径封存 canonical observation，并在 fresh authority/receipt/owner/fence 对账后原子把槽从 `staged` 推进为 `failed`、令三类 fence 各精确 `+1` 且写不可变 quarantine receipt。两类健康 Store 均有不确定提交恢复。失败候选现另有私有 cleanup authorization Store：消费 exact quarantine 与 retained custody，在同一事务写授权并把 owner 推进为 `cleanup_pending`；不确定结果只接受 `NotCreated` 或 exact `Authorized`，exact adoption 会双读并重哈希 retained 文件和 seal。远程 canonical 又实现只消费固定句柄的进程内物理执行器，失败保留已完成证据和剩余 custody，成功只形成等待 completion Store 的物理证据；相关组合已通过 `elon-pc-node` 编译和 4 项定向测试。本批 topology/journal schema 增量未重新编译或执行 Store/DDL 夹具。当前仍不产生 completion/retry、installed、ready 或商业 Verification。默认关闭的 Bootstrap 已挂入 NodeRuntime，但 sharing-on、生产数据库/时间源/回滚见证、真实 HTTPS downloader、Sidecar/IPC/探针调度、目录耐久/跨重启清理、completion receipt、installed/promotion、ReadyCapability 上报和 Attempt 接线仍缺 |
...
节点失败候选清理的最新 schema-only 增量已加入 expected-object topology、candidate-parent identity anchor、plan seal 与四阶段 hash-chain step journal，并要求 completion 同时绑定 plan digest 和 terminal namespace-durability digest。该增量未编译、未运行 DDL 或测试；typed Store、目录/父级 identity API、真实 namespace durability、既有执行器对 durable-intent/journal 的适配、跨重启恢复与 completion Store 均未实现，不能理解为节点已经会自动清理生产失败候选。
...
4. `docs/distributed-compute/node-plugin-local-authority.md`、`docs/distributed-compute/node-ready-capability.md` 与 `docs/distributed-compute/node-plugin-candidate-cleanup.md`：节点 SQLite 真源、候选下载验证、短期技术就绪健康证明与失败候选清理边界。
...
目标流程仍是：共享关闭时不下载重型组件；开启后按硬件和任务选择签名插件、运行时与模型工件。本机耐久真源已选定为独立 SQLite，根签名双 keyring、原子计划应用、候选所有权、三段式下载认领、候选级全工件验证和可信时间边界见 `docs/distributed-compute/node-plugin-local-authority.md`。raw verified 只把本机槽推进到 `verifying`，不授权解包、健康、promotion 或商业 Offer。staged 候选已有进程内健康状态机和耐久健康回执 Store；连续失败达到阈值时，独立 quarantine Store 可在 fresh binding 未漂移时把槽原子推进到 `failed` 并写不可变回执。成功与失败 Store 均有不确定结果恢复，exact adoption 需 fresh read 与 retained-handle 重哈希；`NotCreated` 不恢复写许可。失败候选随后可进入私有 cleanup authorization Store：它 fresh 对账 failed slot/quarantine/staging/owner/fence/可信时间，原子写不可变授权并推进 `owned -> cleanup_pending`，且提供 `NotCreated | Authorized` 结果恢复和 exact adoption。远程 canonical 又实现只消费固定句柄的进程内物理执行器，失败保留全部剩余 custody，成功也不释放 owner 或授予重试；本批只新增持久 topology/journal schema，未重新编译或执行 Store/DDL 事务。候选观察值仍由 Host 调用方提供，尚未证明真实 Sidecar 已运行；生产时间权威 key resolver、真实网络取时、真实下载器、Sidecar/IPC/探针调度、目录耐久/跨重启清理、既有执行器的 journal 化、completion receipt、installed/promotion、云端 capability gate 和通用 Attempt 协议接线仍未实现。
