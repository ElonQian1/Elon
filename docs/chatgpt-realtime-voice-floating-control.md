...
- 观测可信度：`CONFIRMED`、`STALE`。它只描述 APK 是否及时读到官网状态，不改变官方语音连接本身。
...
- 通话中：启动命令已经被官网接受，但 APK 暂时没有读到新的麦克风或语音控件证据。该状态不停止网页语音、不显示红色故障卡；悬浮球低频校准，后续证据到达后自动转为已确认状态。
...
同样，启动后的观测超时也不是连接失败。只有官网命令明确失败、麦克风权限明确拒绝或网页会话确实不可用时才进入“连接异常”；状态证据缺失只进入非阻断的“通话中”。低频校准最多主动检查约两分钟，之后停止定时读取以控制发热，但仍继续接受页面事件并可随时确认状态。
...
已通过状态模型、全局恢复、会话归属、动作分发和协调器定向测试。观测超时分层、后到证据自动确认、明确权限拒绝失败关闭和低频看门狗均有独立回归；本轮真机敏感语音验收仍需用户在场。`v1.1.1278 (1288)` 小米真机已验证两条结束路径：在原生聊天页展开并挂断，以及离开“一龙 AI”回到普通消息首页后展开并挂断；两次均在结束后移除悬浮层并保留当前原生页面。悬浮球在语音期间跨页面保持可见。
