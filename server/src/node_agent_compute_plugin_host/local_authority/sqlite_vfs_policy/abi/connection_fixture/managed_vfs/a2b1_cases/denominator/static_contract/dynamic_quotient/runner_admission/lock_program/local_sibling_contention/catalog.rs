//! Exact frozen-member authority for completed local sibling-contention outcomes.
...
use super::{range_mask_v1, LOCAL_SIBLING_CONTENTION_MEMBER_COUNT};

const HEADER: &str =
    "action\tfirst\tcount\tmask\tcontention\tcompletion\tcase_key_sha256\tfull_record_sha256";
const MEMBER_CATALOG: &str = include_str!("local_sibling_contention_completed_members.v1.tsv");
...
pub(super) fn exact_member_v1(
    ...
    for line in lines {
        ...
        let Some(contention_text) = fields.next() else {
            return Err(LockRunnerExecutionViolationV1::MemberCatalogInvalid);
        };
        ...
        if fields.next().is_some()
            ...
            || contention_text != contention_tag_v1(expected.action)
    ...
    if observed_rows != LOCAL_SIBLING_CONTENTION_MEMBER_COUNT {
        return Err(LockRunnerExecutionViolationV1::MemberCatalogInvalid);
    }
...
fn expected_row_v1(row: usize) -> Option<ExpectedRowV1> {
    if row == 0 || row > LOCAL_SIBLING_CONTENTION_MEMBER_COUNT {
        return None;
    }
    let ordinal = row - 1;
...
const fn contention_tag_v1(action: LockActionV1) -> &'static str {
    match action {
        LockActionV1::LockShared => "sibling-exclusive-busy",
        LockActionV1::LockExclusive => "sibling-overlap-busy",
        LockActionV1::UnlockShared | LockActionV1::UnlockExclusive => "unsupported",
    }
}
...
pub(super) const fn catalog_row_count_for_test() -> usize {
    LOCAL_SIBLING_CONTENTION_MEMBER_COUNT
}
