...
`staged` 只表示“平台允许继续准备这份精确候选”，不表示 artifact 已下载、字节摘要已重算、签名或供应链已验证、实现已加载、协议能力已验证、credential verifier 已存在或 route 已获授权。当前只有本文设计，没有领域、迁移、Store、HTTP/MCP/PC、编译、测试或运行实现。
...
未来 submit 入口只允许当前 `admin/owner` 提交，不接受 Provider owner 以本人身份自助登记 release。请求必须采用拒绝未知字段的版本化信封、RFC 8785 JCS、SHA-256 摘要和稳定幂等键，并只携带：
...
- 明确确认语、有界说明、提交管理员、提交时间和幂等键。

`candidate_artifact_ref` 不是 URL、下载授权或 bearer secret。`declared_implementation_sha256` 只是预期值；Store 保存它不等于读取过 artifact。`expected_credential_verifier` 只是未来绑定意图；即使字段完整，也不证明 verifier 已注册、当前有效或执行过认证。
...
reviewer 必须是另一名当前 `admin/owner`，且 `reviewed_by_user_id != submitted_by_user_id`。`approved`、`changes_requested` 与 `rejected` 都生成绑定 exact `request_id/request_digest` 的不可变 review receipt；退回或拒绝必须给出有界原因。
...
apply 只消费仍为 `approved` 的 exact request/review、稳定幂等键、预期摘要和固定确认语，不接受任何新的 release 字段。未来 Store-private 事务必须重新核对四眼、摘要、时间、投影、`adapter_id/release_version` 冲突和幂等重放，然后只完成：

1. 把 request 从 `approved` 推进到 `staged`；
2. 写入一份不可变 staged admission；
3. exact readback request、review 与 admission 摘要。

任一步失败必须零提交。apply 执行者可以是已批准 reviewer 或另一名管理员，但只能机械消费既有决定，不能在 apply 时更改 artifact、capability 或 verifier intent。
...
- 禁止开放 HTTP/MCP/PC 写入口，直到领域、v222、Store-private、权限与脱敏合同另批形成。
- 禁止把 staged admission 描述为 Adapter 可执行、外部矿池在线或商业可用。

## 6. 与后续批次的截止线

本批只冻结 release staging 权威。credential resolver/verifier、verifier registry、service actor issuance、Adapter registry activation、route issuance、Provider activation、transport 与 authenticated ACK/event 均属于后续独立批次。
...
本批可证明的只有本文及索引/当前事实已冻结。允许的验收是 Markdown 模块化、链接/术语搜索、行数和 `git diff --check`；不执行 Rust、编译、测试、迁移、SQLite trigger、权限、并发、HTTP、artifact 或网络验证。

状态固定为 `design_only`。不得以“v222”“staged”“admission”或未来表名宣称任何代码、数据库或生产能力已经存在。
