...
use super::{
    ...
    create_schema_objects_v6, create_schema_objects_v6_additions, schema_integrity,
};

/// The singleton row shape remains V3. Database `user_version` advances independently because V4
/// through V6 add append-only journals and exact transition triggers without rebuilding that row.
...
pub(in crate::node_agent_compute_plugin_host) fn ensure_schema(
    ...
    match version {
        ...
        COMPUTE_PLUGIN_LOCAL_AUTHORITY_DATABASE_VERSION_V6
            if application_id == COMPUTE_PLUGIN_LOCAL_AUTHORITY_APPLICATION_ID =>
        {
            verify_required_objects_v6(connection)
        }
        COMPUTE_PLUGIN_LOCAL_AUTHORITY_SCHEMA_VERSION
        | COMPUTE_PLUGIN_LOCAL_AUTHORITY_DATABASE_VERSION_V4
        | COMPUTE_PLUGIN_LOCAL_AUTHORITY_DATABASE_VERSION
        | COMPUTE_PLUGIN_LOCAL_AUTHORITY_DATABASE_VERSION_V6 => bail!(
...
fn migrate_schema_v3_to_v6(connection: &mut Connection) -> Result<()> {
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .context("COMPUTE_PLUGIN_AUTHORITY_SCHEMA_V6_MIGRATION_BEGIN")?;
    ...
    transaction
        ...
        .context("COMPUTE_PLUGIN_AUTHORITY_SCHEMA_V6_VERSION_WRITE")?;
    transaction
        .commit()
        .context("COMPUTE_PLUGIN_AUTHORITY_SCHEMA_V6_MIGRATION_COMMIT")?;
    verify_required_objects_v6(connection)
}

fn migrate_schema_v4_to_v6(connection: &mut Connection) -> Result<()> {
    let transaction = connection
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .context("COMPUTE_PLUGIN_AUTHORITY_SCHEMA_V6_MIGRATION_BEGIN")?;
