...
| 商户数据来源、健康度和同步回执 | 控制面已实现；能力与合格内部回执的显式绑定已通过定向 Rust/SQLite、集成上下文和消费者发现 MCP 回归，HTTP 实例与 PC 浏览器仍未实测；具体平台适配器和外部核验待逐项验收 | `docs/decisions/open-commerce-integration-control-plane.md`、`docs/decisions/open-commerce-capability-source-link-v1.md`、`docs/open-commerce-integration-control-plane-acceptance.md`、`docs/open-commerce-capability-source-link-v1-acceptance.md` |
| 连接器 SDK 与兼容性门禁 | 已实现 V1，尚不包含具体大厂适配器 | `sdk/open-commerce-connector/`、`docs/open-commerce/connector-sdk.md` |
| 消费者发现、关系授权与第三方应用接入 | 跨项目目录、商户声明来源与新鲜度、能力与内部同步回执来源绑定、五种用户可选透明非付费排序器、可复核未签名单次排序凭证、匿名关系、低敏偏好披露、客户端加密敏感数据保险箱、删除请求、有界催办与升级关注、商户提交的未核验外部证明、V5 可验证导出、隔离导入、单包字段级采用、人工关系映射与重新授权、多来源偏好冲突选择、Schema 调用、动作确认、授权与预算、调用事件和封禁主干已形成；单包字段级采用和人工关系映射与重新授权已通过 Rust/SQLite、进程内 Axum、PC 类型检查和生产构建，来源绑定和删除请求有界跟进已通过定向 Rust/SQLite 回归。Webhook 的验证前门卫、App/事件隔离、耐久入队、租约、死信确认与人工重试已通过本地 Rust/SQLite 子集验证；真实 DNS/TLS/网络、历史升级、生产资格和 UI 仍未验证。V5、多来源合并与生产准入等其他新增批次仍有未编译项，生产能力默认关闭。外部平台签名与官方回读证明、运营方签名排序凭证、第三方排序器 SDK、自动删除通知与仲裁、密码找回、多设备密钥同步、商户或 AI 授权解密、完整订单迁移、外部删除执行与回执核验、跨运营方身份互认与生产公共网络仍未完成 | `docs/decisions/open-commerce-directory-publication-v1.md`、`docs/decisions/open-commerce-public-data-provenance-v1.md`、`docs/decisions/open-commerce-capability-source-link-v1.md`、`docs/decisions/open-commerce-pluggable-ranking-v1.md`、`docs/decisions/open-commerce-consumer-ranking-receipts-v1.md`、`docs/decisions/open-commerce-consumer-relationships-v1.md`、`docs/decisions/open-commerce-consumer-preference-disclosures-v1.md`、`docs/decisions/open-commerce-consumer-data-vault-v1.md`、`docs/decisions/open-commerce-consumer-data-request-followups-v1.md`、`docs/decisions/open-commerce-consumer-data-erasure-evidence-v1.md`、`docs/decisions/open-commerce-consumer-portability-exports-v5.md`、`docs/decisions/open-commerce-consumer-portability-imports-v1.md`、`docs/decisions/open-commerce-consumer-portability-selective-adoption-v1.md`、`docs/decisions/open-commerce-consumer-portability-reauthorization-v1.md`、`docs/decisions/open-commerce-consumer-portability-multi-source-merge-v1.md`、`docs/decisions/open-commerce-developer-terminal-events-v1.md`、`docs/decisions/open-commerce-developer-production-credentials-v1.md`、`docs/decisions/open-commerce-production-webhooks-v1.md` |
