...
该 capability 仅证明“扫描所得目录表与已验 Manifest 形成精确、可移植且有界的计划”，不读取归档正文、不创建目录、不写入文件，也不授权 promotion。后续受控解包入口会在受管根下创建随机 create-new staging run，二次解析同一已验证 ZIP 句柄，按计划逐项落盘并复核字节数、SHA-256、EOF、flush/fsync 与同句柄身份，再封存 staging seal。retained-handle 重验、fresh trusted-time/authority binding 与单一 Store 事务现可保存不可变 receipt、推进三类 fence 并把槽原子改为 `staged`；结果 reader 还能稳定返回 `NotCreated` 或 exact `Staged` 并让碰撞/漂移失败关闭。exact `Staged` adoption 只消费原 recovery custody，经 fresh read 与原句柄全量重哈希后恢复 typed custody；`NotCreated` 不授予重试。v216 源码已在该 typed custody 之上增加 retained-handle 再验、互相引用的 install/promotion 双回执与 exact recovery；该增量已随节点编译，v7 authority 全新安装、重开及 v3-v6 原子迁移等 11 项测试通过，但生产磁盘迁移与完整 promotion 事务夹具仍未验证。v217 源码再以签名 `reauthorize_existing`、authenticated applied-only PlanApply barrier 和 v8 source/receipt/head/recovery 封存 current installed release 的 launch authorization；它只推进 work-admission head、authority state/epoch 与可信时间，仍不启动 Runtime、不生成活动健康或 Ready。v217 随完整目标编译但未迁移或专项运行；进程绑定 recovery key 仅支持同进程 adoption，重启不能重建 `DurableWorkAdmittedPluginSlot` 或目录 custody。精确边界见 `node-plugin-archive-extraction.md` 与 `node-ready-capability.md`。
...
独立 `compute-plugin-state.sqlite3` 的路径型 facade、WAL/FULL/foreign-key/foreign-key-check 配置、私有 `BEGIN IMMEDIATE` seam、authority meta payload v3 与 inventory payload v2 已形成代码。数据库 `user_version` 现已在源码中推进到 8，但 `authority_meta.schema_version` 仍保持 3，避免为新增账本重建既有 meta 与整条 trigger/FK 链。v3 在原 meta、不可变 keyring/plan journal、candidate owner、download cursor 和 fetch claim 之上加入 candidate verification run、单 candidate prepared/verified 唯一性、跨 authority/process/cancel 的撤销门卫、不可变 `candidate_health_receipts` 及不可变 `candidate_health_quarantine_receipts`；当前必需对象还包括 abort/resolution fence、run authority-state identity、拆分后的 observed/mismatch 结果列、健康回执的写入 fence/重叠有效期阻断、quarantine 写入 fence、两类健康回执的 update/delete 禁止 trigger，以及失败候选清理的 authorization/completion 表和 `owned -> cleanup_pending -> cleaned` 门卫。清理对象的精确边界见 `node-plugin-candidate-cleanup.md`。

v4-v6 采用逐代纯 reference 迁移：旧库先在同一 `BEGIN IMMEDIATE` 内精确验证全部非内建对象、规范 SQL 指纹和 foreign key，再依次追加 policy receipt、prepared-work companion/终结 trigger 与 Control-signed catalog；缺失、额外、同名漂移或迁移中断均失败关闭。现有验证包括8项版本链、4项policy合同、5+6项v5 Store/恢复判定、3+7项v6 binding Store/恢复判定和5项真实Ed25519目录候选测试。它们仍不代表生产磁盘迁移、v5/v6外层bind/adopt完整会话与生产接管、生产root/keyring、打开态VFS或Host联调。
...
v10 preparation 只证明节点观察同一 policy/authorization；v209 保存请求、逐会话 delivery 与 observation，不打开 authority。云端在 v10 accepted observation 耐久后派发协议阈值 12 的 Planning Snapshot V2，v210 绑定 exact v209 observation 与 Bootstrap 代次。V2 work-admission 是可选 current-head：新晋升或失配历史 head 必须为 `None`，不能伪造 generation 1；当前仍固定 `snapshot_ready=false`，generation outcome 只允许 `signer_unavailable/rejected`。账号、secret 或 token 替换会推进不可丢 epoch并撤销旧 Bootstrap custody。无下载 policy-binding 内核则把 exact policy、installation/root、进程与配置/取消代次封成持实例锁 lease 的一次性意图；disabled 走撤销意图。任何非 replay 策略、实例锁或数据根变化都会推进 generation，使旧意图在 Store 事务边界失败关闭。
...
这仍不是生产 planning context：A1 未专项运行；Barrier/RegistrationShutdown/RegistryLifecycle/Unmap/JointClose 分别为 `8/8`、`8/8`、`16/16`、`49/49`、`36/36`，A2b2=`117/117`。Map/Lock static denominator 已闭合为 `43476/43476` 与 `8668/8668`；dynamic quotient projector/manifest 未实现，`Qmap/Qlock=unknown`、`WindowsDynamic=not_opened`，A2 未动态验收，见[`A2验收`](node-plugin-vfs-fault-acceptance.md)。生产 open/VFS/process/root/time/rollback/producer 仍不可达；v14 blocked-only，无 Host/PlanApply/下载/准入/Ready 副作用。
