...
2. 打开数据库、验证 authority schema 与完整性；当前源码只允许先按纯 v3 reference 精确验明旧库，再执行窄范围 v3→v4 迁移；任何旧对象缺失、额外对象或同名异定义都失败关闭；
...
LocalAuthority v4 Store 还要求同一个 `PinnedComputePluginRoot` 的非 Clone root-lock lease、当前 process fence 与 authenticated trusted-time observation。Pin 构造时使用的完整 `NodeDataPaths` 会随 root capability 保留并与 intent、authority parent exact 比较，同一 installation 下另一数据根的锁不能授权当前库。Store 在 `BEGIN IMMEDIATE` 内重算 snapshot/request/receipt JCS 摘要、精确读取 authority 与 inventory、拒绝低修订或同修订冲突，并追加不可变 `sharing_policy_binding_receipts`；唯一 trigger 把 state revision、inventory revision 与 authority epoch 各推进一位，保持 process owner epoch，不允许复用旧 trusted-time high-water。enabled 必须使 authorization revision/digest 精确等于 policy，disabled 必须三项 authorization 全空。每个新修订还会把库存中的既有插件投影为 `desired_activation=disabled`、`admission=revoked`、清空 health，同时保留槽、owner、候选、运行时、active attempts 与 permission grant 等物理/审计事实，避免旧安装记录被新授权自动“洗白”。Store 在投影前永久关闭旧 process fence 的 fetch cancellation source；即使事务随后拒绝或结果不确定，旧 Plan 也不能从同一 source 再铸 fresh guard，后续增权工作必须重新取得 process fence。当前如仍有 prepared fetch/verification，则事务明确阻断，不伪造一个不属于 policy transition 的撤销原因。

同修订 exact replay 会逐字段回读 receipt、inventory JSON/JCS 与当前 meta；commit-uncertain recovery 只在 exact post-state 仍是当前 head 时返回可继续规划的 durable current capability。若不可变 receipt 已存在但 authority 已有合法后继，只返回 `CommittedHistorical` 审计事实；若没有 receipt 且 exact pre-state 尚在，才区分 current `NotCreated` 与已被新 Bootstrap 代次取代的 `NotCreatedSuperseded`，绝不会用过期意图重新启用旧策略。

这仍不是生产可调用的 planning context。当前没有 NodeRuntime 账号/凭证切换到 Bootstrap generation 的失效 hook，没有把路径型 SQLite open 与 pinned-root 句柄身份闭合，也没有生产 trusted-time/rollback witness、专用 policy revocation 终态、已铸 Ready/Attempt 的 authority-epoch/receipt 绑定、已安装插件的 signed reauthorization 语义和与 fetch guard 分离的 cleanup-only 删除 custody。因此 `context_ready`、root/authority/download/Sidecar 标志继续固定为 false，policy-binding Store 没有 Host 调用点；即使外部出现合法签名，也不得把本批源码描述成已经允许 PlanApply、下载或任务准入。
