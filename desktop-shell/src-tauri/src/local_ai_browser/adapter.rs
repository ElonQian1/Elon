...
fn sanitize_protocol_event(event: &Map<String, Value>) -> Result<SanitizedAdapterEvent, String> {
    ...
    let payload = match kind {
        ...
        "realtime_voice_state" => json!({
            ...
            "managedPhase": clean_identifier(event.get("managedPhase"), 40),
            "managedActive": event.get("managedActive").and_then(Value::as_bool).unwrap_or(false),
            "microphoneActive": event.get("microphoneActive").and_then(Value::as_bool).unwrap_or(false),
            "remoteAudio": event.get("remoteAudio").and_then(Value::as_bool).unwrap_or(false),
            "muted": event.get("muted").and_then(Value::as_bool).unwrap_or(false),
            "routeBound": event.get("routeBound").and_then(Value::as_bool).unwrap_or(false),
            "fallbackCode": clean_identifier(event.get("fallbackCode"), 80),
            "lifecycleRevision": bounded_u64(event.get("lifecycleRevision"), 0, 1_000_000_000),
        }),
