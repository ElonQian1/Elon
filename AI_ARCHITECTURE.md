...
Offer 控制面允许当前用户为 active Provider/Pool 创建规范化 draft，服务端从 Provider、Pool 和 open Bucket 当前版本固定供给身份、容量绑定、窗口、区域、meter 与摘要；所有者可用精确版本撤销 draft。管理员 HTTP 可在一个事务中追加 active 连续版本与 v182 不可变发布回执，也可用 v183 将当前 active Offer 原子转为 draining。draining 退出新候选但保留已有 Reservation；两类管理员写入口均不向 MCP 开放，不生成 Price Snapshot、不预留容量或移动资金。当前为 `implementation_uncompiled`。
...
| `server/src/store/compute_capacity_*.rs`、`compute_{provider,offer,job,reservation}_registry*`、`compute_offer_{publications,lifecycle}.rs`、`compute_quote_candidates.rs`、`compute_broker_reservation/`、`compute_federation_{provider,capacity_pool,capacity_bucket,capacity_supply,offer,broker}_{service,api,mcp}.rs`、`compute_federation_offer_{draft,publication,lifecycle}_*.rs`、`compute_federation_mcp.rs` | 共享容量池、版本化合同、Provider/Pool/Bucket/Supply 控制面、本人 draft Offer、管理员原子 active 发布与 draining 安全退场、项目级 Job 创建/既有候选发现与锁价、本人 Job/Reservation 查询、本地原子 Reserve、未执行任务原子 Release/Expire；当前未编译、未迁移、未运行验证，未接新报价生成、自动撮合、Attempt 或运行中结算 |
