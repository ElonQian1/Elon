...
| Sui 凭证投影、链下包与离线预检任务 | 已实现、适配器与任务代码未编译、无网络副作用 | 标准凭证可保存单笔投影包；已过账纠正可把冲销与替换共同保存为不可拆分的原子包。两类包均绑定目标网络和来源摘要、可复核完整性并固定 `not_submitted`；项目可签发限时、限网络和限包类型的机器凭据，并把当前可导出包显式加入短时租约队列。机器领取前由服务端只读重建交接内容，漂移或争议阻断任务，完成时原子追加 `passed/rejected` 报告。SDK 和 CLI 尚未支持任务租约命令；任务和报告不改投影状态，不含签名、广播、最终性或资金移动 | `server/src/task_settlement/sui_adapter_handoff_*.rs`、`server/src/task_settlement/sui_preflight_*.rs`、`server/src/store/task_sui_preflight_*.rs`、`sdk/open-commerce-connector/src/sui-preflight.js`、`docs/decisions/sui-offline-preflight-adapters-v1.md`、`docs/decisions/sui-offline-preflight-job-leases-v1.md`、`docs/sui-offline-preflight-job-leases-v1-acceptance.md` |
