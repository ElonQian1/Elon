//! Retained q17 child fixture for unread truncate and cleanup-release receipts.
...
use super::{
    payload, validate_binding, LockRunnerExistingFirstTruncateErrorReleaseFailedCompletionV1,
...
impl Deref for RetainedInitializationFailureFixture {
    ...
    fn deref(&self) -> &Self::Target {
        self.fixture
            .as_ref()
            .expect("retained q17 Lock initialization fixture")
    }
}
...
impl Drop for RetainedInitializationFailureFixture {
    fn drop(&mut self) {
        if let Some(fixture) = self.fixture.take() {
            // Both native outcomes are uncertain, so the existing physical file, poisoned node,
            // and terminal domain remain child-owned until the isolated process has exited.
...
pub(super) fn exercise_child(
    ...
    let nonce = std::env::var(A2_DYNAMIC_CHILD_NONCE_ENV)
        .context("read parent-created q17 Lock initialization child nonce")?;
    ...
    let observation = fixture
        .route(SELECTED)?
        .observe_main_shm_lock_existing_first_truncate_error_release_failed_v1(
    ...
    let preemption_values = match binding.completion {
        ...
        LockRunnerExistingFirstTruncateErrorReleaseFailedCompletionV1::RetentionRouteUnknown => {
            ...
            if receipt != [1; 5] {
                return Err(anyhow!(
                    "q17 Lock route-unknown preemption receipt mismatch"
                ));
            }
            [
    ...
    {
        return Err(anyhow!(
            "q17 Lock retained registration/root shape mismatch"
        ));
    }

    let cleanup_native = observation
        .initialization
        .cleanup_native_receipt()
        .ok_or_else(|| anyhow!("q17 cleanup native receipt missing"))?;
    let payload = payload::encode(
        ...
        [
            native_observation_tag(cleanup_native.observation),
            cleanup_native.offset,
            cleanup_native.length,
            u64::from(cleanup_native.exact_call_occurrence),
        ],
...
fn prepare(root: &Path) -> anyhow::Result<RetainedInitializationFailureFixture> {
    let fixture = ManagedSqliteMultiConnectionFixture::open_single(root, [0xb1; 16])?;
    ...
    let mode: String = fixture
        ...
        .context("enable WAL for q17 cold initialization")?;
    if !mode.eq_ignore_ascii_case("wal") || !target_absent_before {
        return Err(anyhow!(
            "q17 Lock fixture was not cold before WAL-main attach"
        ));
    }
    route.into_schema_migration()?;
    route.into_runtime()?;
    // Precreate and close only the physical sibling. The production Map below must reopen it with
    // `was_created=false` and perform the first coordinator-node attach.
    ...
    {
        return Err(anyhow!(
            "q17 physical SHM precreation unexpectedly attached a coordinator target"
        ));
    }
    let cold_map = route
    ...
    {
        return Err(anyhow!(
            "q17 Lock existing-file cold attach receipt mismatch"
        ));
    }
...
fn validate_observation(
    ...
    let cleanup = value
        .initialization
        .cleanup_native_receipt()
        .ok_or_else(|| anyhow!("q17 controlled cleanup receipt missing"))?;
    if value.callback.offset() != i32::from(binding.first)
        ...
        || value.before != cold
        || snapshot_values(value.after) != [1, 0, 0, 1, 1, 0, 0, 4, 1, 1, 1, 1, 1, 0]
    ...
    {
        return Err(anyhow!("q17 controlled initialization receipt mismatch"));
    }
    Ok(())
}
...
fn terminal_values(
    ...
    let route = terminal
        .terminal_route()
        .ok_or_else(|| anyhow!("q17 Lock terminal route missing"))?;
    if terminal.joint_close_physical_failure_retention_count() != 0
        || terminal.joint_close_physical_failure().is_some()
    {
        return Err(anyhow!("q17 Lock retained unrelated joint-close custody"));
    }
    let values = [
    ...
    if values != expected {
        return Err(anyhow!("q17 Lock terminal custody ledger mismatch"));
    }
    Ok(values)
}
...
const fn native_observation_tag(
    observation: ManagedSqliteShmTestInitializationNativeObservationV1,
) -> u64 {
    match observation {
        ManagedSqliteShmTestInitializationNativeObservationV1::ReturnReceiptUnavailable => 1,
    }
}
