...
A2c 严格 `cfg(all(test, windows))` 的源码现含八类 foundation 路径：route-exact `Main/ShmMap`、`Main/ShmLock` callback-before；进程隔离的 VFS unregister before/after；以及四条进程隔离的 direct `xShmUnmap(false)` physical-subset runner。registration bridge 对账两个 `RegistrationShutdown/VfsUnregister` key；四个 SHM partial bridge 则都先验证完整 frozen inventory，再分别只选择 `Unmap/FinalConnection/Keep` 下 `ViewUnmap`、`MappingClose`、`DmsSharedRelease` 与 `ShmFileClose` 的 `AfterSuccessKnown` key，用真实 callback result、exact probe pending/trigger 与去敏 observer 前后快照对账可直接观察的物理字段。`ShmFileClose` 桥由仅在 `file.close()` 返回 `Ok(receipt)` 后激活的 sealed trigger 证明控制流已越过成功关闭 seam；observer 本身不独立读取 close receipt 或 OS handle table。它们仍未构造完整 dynamic `Case`；宽范围 `sqlite_vfs_policy` 回归已开始执行，但当前仍有 A2b2 静态不变量与 A2c 进程隔离 runner 失败。因此 117 项 `WindowsDynamic` 验收通过进度仍为 0；native failure adapter、SQLite/indexed route/main/lease/action 完整 custody、竞争和逐 case Windows 动态验证仍缺。测试目标可编译和 5 项最小矩阵通过，都不能替代逐 case Windows 动态证据。
