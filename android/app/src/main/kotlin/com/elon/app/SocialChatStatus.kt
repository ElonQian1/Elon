...
internal fun showSocialChatStatus(binding: ActivityMainBinding, text: String?, retry: (() -> Unit)? = null) {
    ...
    val existing = frame.findViewWithTag<TextView>("social-sync-status")
    if (text == null || (binding.chatList.adapter?.itemCount ?: 0) > 0) {
        existing?.apply {
    ...
    val label = existing ?: TextView(frame.context).also {
