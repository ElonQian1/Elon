//! Exact frozen-member authority for the 88-member q13 initialization-release tranche.
...
use super::{
    range_mask_v1, LockExistingFirstExclusiveReleaseCompletionV1,
...
pub(super) fn exact_member_v1(
    ...
    if observed_rows != EXISTING_FIRST_EXCLUSIVE_RELEASE_ERROR_MEMBER_COUNT {
        return Err(LockRunnerExecutionViolationV1::MemberCatalogInvalid);
    }
...
fn expected_row_v1(row: usize) -> Option<ExpectedRowV1> {
    if row == 0 || row > EXISTING_FIRST_EXCLUSIVE_RELEASE_ERROR_MEMBER_COUNT {
        return None;
    }
    let ordinal = row - 1;
...
const fn completion_tag_v1(
    ...
    match completion {
        LockExistingFirstExclusiveReleaseCompletionV1::RetentionSucceeded => "succeeded",
        LockExistingFirstExclusiveReleaseCompletionV1::RetentionRouteUnknown => {
