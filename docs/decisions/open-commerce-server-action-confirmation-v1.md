...
- 消费者会话 HTTP、开发者凭据 HTTP 和 MCP 取消共用同一最小安全投影；HTTP 不再把主动取消暴露为数据库兼容状态 `expired`。2 项 Axum 路由测试已实际通过，覆盖会话/Token 鉴权、用户与 App 隔离、幂等、安全投影和零 Invocation；PC 静态契约和生产构建通过。
