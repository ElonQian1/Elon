...
`verified_rust_sqlite_service_and_http`
...
- 两项进程内 Axum 专项通过，覆盖缺失/无效凭据、分页续读、跨 App 游标、损坏游标、同 App 平台事件隔离、跨 App/跨环境详情统一未找到和响应脱敏。
...
- 本批未运行真实 TCP 实例、生产开关启用后的 `oc_live_` 事件读取、PC 浏览器或窄屏交互；测试 Token 的进程内 HTTP 鉴权已验证。
...
- 开发者事件进程内 HTTP 两项测试：`65f3e0ce1d8ff6e3e12dafabefccf16018211718e7fe4d61db82d96bd706bf13`。
...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\validate-rust.ps1 -- test --manifest-path server\Cargo.toml --bin elon-server open_commerce_developer_event_tests::event_api_tests
