...
prepared run 成功后，现有线性 hash 编排必须同时消费 Authorized 与一次性本机资源预算，在事务外按严格递增 ordinal 从每个仍打开的文件 byte 0 以最多 64 KiB buffer 重算原始字节 SHA-256。签名闭包总字节必须先低于本机策略及 64 TiB 硬上限；预算从授权被消费后使用单调时钟计时，最长 24 小时，并在预绑定、逐文件每次有界 read、文件间累计及后置绑定检查。逐文件累计必须最终精确等于权威闭包总字节。每次 read 同时检查原 cancellation guard，检查 exact EOF，并在同一句柄复验 FileId、类型、reparse、卷、单链接与长度；不持久化跨版本不可移植的 SHA 内部状态。摘要错配只记录规范顺序中的第一个 mismatch，但仍继续 hash 完整文件集；完整观察投影绑定 verification generation、file-set/expected-set digest、每项 ordinal/item/size/expected/observed/FileId digest，再计算 JCS SHA-256。任何文件 I/O、取消、预算、身份、长度、闭包或后置 binding 失败都消费 Authorized，仅返还带全套句柄的 recovery custody；只有全部文件成功读完才形成不可 Clone、不可序列化的 Hashed capability。该单调预算是有界读取之间的协作门卫，不能中断一个已经阻塞的系统调用，也不等同于尚未实现的 socket-read 取消。
...
已验签 Manifest 现在还可与不可信归档扫描器提供的条目元数据共同生成一份规范解包计划。计划只接受 Manifest 声明的普通文件和其必要父目录，要求每个文件恰好出现一次且大小精确一致，并拒绝符号链接、硬链接、设备、管道、socket、额外文件、额外目录、重复路径、Windows 保留名、大小写折叠冲突和非 portable 相对路径。文件数、目录数与声明解包总字节均受上限和 checked arithmetic 约束；通过后生成绑定 release、publisher、归档身份、目录闭包、文件大小及 executable 标记的 JCS/SHA-256 计划，并只通过不可 Clone、不可序列化的 capability 继续传递。

该 capability 仅证明“扫描所得目录表与已验 Manifest 形成精确、可移植且有界的计划”，不读取归档正文、不创建目录、不写入文件，也不授权 promotion。后续受控解包入口会在受管根下创建随机 create-new staging run，二次解析同一已验证 ZIP 句柄，按计划逐项落盘并复核字节数、SHA-256、EOF、flush/fsync 与同句柄身份，再封存 staging seal。retained-handle 重验、fresh trusted-time/authority binding 与单一 Store 事务现可保存不可变 receipt、推进三类 fence 并把槽原子改为 `staged`；结果 reader 还能稳定返回 `NotCreated` 或 exact `Staged` 并让碰撞/漂移失败关闭。exact `Staged` adoption 只消费原 recovery custody，经 fresh read 与原句柄全量重哈希后恢复 typed custody；`NotCreated` 不授予重试。目录耐久闭包、跨重启恢复和 promotion 权限仍未形成，精确边界见 `node-plugin-archive-extraction.md`。
...
Keyring 安装和活动快照加载现在也必须在事务中读取、拒绝回退并 CAS 推进 `trusted_time_high_water_ms`；这些较早接口传入的 `trusted_now` 仍只是未来可信时间内核的前置条件，不能由 Host 直接拿普通墙钟冒充。逐段取数链已经收紧为 non-Clone、字段私有且只能由已验证证明构造的 `ComputePluginTrustedTimeObservation`：进程先生成不可 Clone/Serialize 的随机 clock epoch，再为每次取时生成最多存活 60 秒的一次性随机挑战；远端签名证明必须以专用 Ed25519 签名域精确绑定安装身份、clock epoch、challenge ID/digest、时间权威、sequence 和规范 UTC 毫秒，专用 Bootstrap-pinned key resolver 验证成功后才产生 observation。process fence 消耗 observation 并固定 installation 与 clock epoch，普通 claim/redirect/abort session 也只能消费同 epoch observation，不再接裸 `DateTime`。fsync 后 observation 的 `Instant` 必须严格晚于 barrier；等值也失败关闭，因为相同 tick 无法证明先后。UTC 值才用于 Store 高水位，两者不能互相替代；若 UTC observation 与 claim 或高水位落在同一毫秒，只能重新取得真实可信观察或失败，禁止自行 `+1`。本合同源码已随 `elon-pc-node` 编译；当前尚未形成生产 key resolver、真实网络 time attestation 请求器、启动基线或 Host 接线，因此 fetch fence/session 与 durable proof 仍没有生产可达路径。
...
每个 `ComputePluginLocalAuthority` facade 还持有不可序列化的进程内 instance binding；只有由该 facade线性派生的能力才共享身份，facade本身不再可Clone。process fence、claim recovery key 和后续 authority session 必须对账同一 binding，防止把标量恰好相同的另一 facade 当成原 Store。该绑定随进程消失，只解决进程内 provenance，不能替代数据库外认证单调锚点。

SQLite 内部 revision、trigger 和 CAS 只能阻止当前数据库中的正常写入回退，不能独立识别“整个数据库文件被替换成一份更旧但内部自洽的副本”。当前已编译的合同代码可在同一 deferred snapshot 中重读 `authority_meta` 与库存 JSON，重算 inventory JCS 摘要并核对插件结构、共享授权、keyring、环境摘要、全部 revision/epoch 和可信时间高水位，再生成不含库存正文及原始授权引用的规范回滚检查点与 JCS 摘要；时钟不可信或任一事实不一致都拒绝生成。

外部锚合同现已形成独立代码边界：节点为每次“查询最新锚”生成最多存活 60 秒的一次性随机挑战，并在挑战生成时固定预期锚 ID，响应不能替节点选择信任对象；见证回执必须用专用 Ed25519 域精确绑定挑战、安装身份、锚 ID/序列和完整规范检查点，经发布内置的专用 key resolver 验证后才成为不可 Clone、不可序列化的已验锚。启动比较只在本机与外部检查点高水位及摘要完全一致时产生线性启动许可；本机落后、同高水位摘要分叉、关键 revision/epoch/keyring 回退均失败关闭。本机领先时不授予启动，只产生携带完整本机检查点的一次性“必须发布新锚”能力，避免后续发布器根据摘要重建权威事实。

“必须发布新锚”能力现在只能被一次性消费为最长 60 秒的规范发布挑战。请求精确绑定前一锚 ID/序列/检查点摘要、安装身份、随机 publication ID/nonce 和完整新检查点；见证签名回执必须回显发布摘要与前一锚事实，并把新序列精确推进一位，才产生不可 Clone、不可序列化的已确认发布能力。序列跳跃、并发 CAS 冲突、摘要或安装身份错配、挑战过期均失败关闭。发布确认不直接产生启动许可，节点仍须按启动合同取得可信最新锚并精确比较。

首次安装不能把 HTTP 404、空响应或本机数据库无记录当作“尚无外部锚”。同一查询挑战只有在预期见证用独立签名域返回精确绑定挑战、安装身份和锚 ID 的 `absent` 证明后，才能形成线性不存在证明；该证明的原单调截止时间会继续约束首次注册，不能通过本机重新包装延长。首次注册请求绑定不存在证明摘要与原 challenge、完整本机检查点和随机 enrollment ID/nonce；签名回执只接受首锚序列 `1` 和完全相同的检查点摘要。注册确认不直接产生启动许可，仍须重新查询最新锚并精确比较。

该合同仍不是可用的数据库外锚服务：仓库尚无生产见证服务器、经身份认证的传输会话、服务端挑战留存与原子消费、真实网络查询与发布协议、生产见证根公钥，也未接入 NodeAgent 启动门卫。客户端 publication/enrollment challenge 只绑定新鲜度与内容，不能替代见证服务对发布节点身份和安装身份的认证。无法取得可信最新锚时仍必须失败关闭；不得把 DTO、验签、比较与回执能力描述成已经解决整库回滚。

恢复顺序固定为：

1. 取得 NodeAgent 独占实例锁；
2. 打开数据库、验证 authority schema 与完整性；当前源码只允许先按纯 v3/v4 reference 精确验明旧库，再在同一 `BEGIN IMMEDIATE` 内升级到 v5；任何旧对象缺失、额外对象或同名异定义都失败关闭；
...
独立 `compute-plugin-state.sqlite3` 的路径型 facade、WAL/FULL/foreign-key/foreign-key-check 配置、私有 `BEGIN IMMEDIATE` seam、authority meta payload v3 与 inventory payload v2 已形成代码。数据库 `user_version` 现已在源码中推进到 5，但 `authority_meta.schema_version` 仍保持 3，避免为新增账本重建既有 meta 与整条 trigger/FK 链。v3 在原 meta、不可变 keyring/plan journal、candidate owner、download cursor 和 fetch claim 之上加入 candidate verification run、单 candidate prepared/verified 唯一性、跨 authority/process/cancel 的撤销门卫、不可变 `candidate_health_receipts` 及不可变 `candidate_health_quarantine_receipts`；当前必需对象还包括 abort/resolution fence、run authority-state identity、拆分后的 observed/mismatch 结果列、健康回执的写入 fence/重叠有效期阻断、quarantine 写入 fence、两类健康回执的 update/delete 禁止 trigger，以及失败候选清理的 authorization/completion 表和 `owned -> cleanup_pending -> cleaned` 门卫。清理对象的精确边界见 `node-plugin-candidate-cleanup.md`。

v4 是第一条显式、窄范围的本机 authority 迁移：旧 `user_version=3` 库必须先用未修改的纯 v3 reference 对全部非 SQLite 内建 table、index 和 trigger 的对象集合、所属表及规范化 SQL 定义 SHA-256 与 foreign key 完整性做精确验证；通过后才追加 `sharing_policy_binding_receipts`、把 PlanApply 的可信时间门从 legacy `>=` 收紧为严格 `>`。v5 保持纯 v3/v4 reference 不变，追加 `sharing_policy_binding_revocation_receipts` 和精确终结 trigger；v3 或 v4 只有在同一 `BEGIN IMMEDIATE` 内通过对应旧指纹与 foreign-key 检查后才能升级，fresh 库则依次构造 exact v3、v4、v5。缺失对象、额外对象、同名异定义或迁移中断均失败关闭。上述 v4/v5 尚未接入生产 NodeAgent、未对真实本地库执行 DDL 或迁移；此前通过的 v3 内存 SQLite 测试不能算作本批迁移验证。v4 policy-binding 源码此前已随 `elon-pc-node` 编译，但尚未运行专项行为测试、v4 DDL/迁移或 Host 联调；本批 v5 按架构铺设策略只做源码与静态审查，未编译、未测试、未运行迁移。

cleanup execution plan、expected objects、plan seal 与 append-only step events 已纳入 v3 必需对象，并已随当前 `elon-pc-node` 编译及 schema 建库与重开指纹定向测试。plan 绑定 authorization、installation、staging root、candidate-parent anchor identity 与完整对象父子拓扑；每个对象只有完成 durable intent、exact disposition/受约束的恢复缺失、exact parent 下的 namespace absence 和独立 namespace-durability fence，completion 才能同时引用 plan digest、既有物理 execution evidence digest 与独立 terminal journal digest。completion Store 内核已按这三类摘要改造，并只接受没有构造入口的终态 journal capability；物理 evidence v1 仍没有不可变记录及 plan/object digest 绑定。当前 topology typed Store 只消费 `AuthorizedCandidateCleanup`，原子写入 plan、全部 expected objects 与 seal；首对象 intent Store 只消费 sealed topology，在独立事务推进可信时间、写入序号 1 的 `delete_intent`，并 exact readback 事件列、JSON 与 JCS 摘要。私有单对象 executor 只消费 durable intent，重新比对 plan 与 retained custody 后对 ordinal 0 原句柄设置强 disposition；失败保留目标和其余 custody，成功返回保留 exact parent 与根锁的物理 capability。首对象 disposition typed Store 再只消费该物理 capability，以 fresh 事务推进可信时间、写入序号 2 的 `exact_handle_disposition_set` 并 exact readback。首对象 parent-absence 路线只消费 durable disposition；只有 exact parent 下的 `Absent` 能进入 typed Store，expected identity、identity conflict 与已经打开同名对象后的不确定检查都保留句柄并失败关闭。absence Store 用独立 fresh 事务精确重验 plan、授权全部列与 JSON/JCS、完整 owner identity、序号 1/2、authority/inventory/epoch 和 `updated_at/high-water`，再写入序号 3 的 `parent_namespace_absence_observed` 并 exact readback。namespace-durability 路线再只消费 durable absence 与不可伪造的线性 mutation fence，以同一 retained parent handle 执行 pre-proof、native barrier、post-proof，并在独立 fresh 事务写入序号 4 的 `namespace_durable` 及 exact readback；结果不确定恢复保留已完成物理 barrier 的 capability，不会重复物理操作。sequence 3/4 已随节点编译，新增测试覆盖 builder hash-chain/篡改拒绝和 SQLite exact-row/列篡改拒绝；相关 executor 与 Store 仍未向 Host 暴露，也没有 authority/owner fresh/recovery 完整事务夹具或真实 Windows barrier 测试。当前 mutation fence 无安全构造器，故序号 4 生产能力不可达；后续 ordinal、物理 evidence v2、真实 fence/parent namespace flush、显式 release 及跨重启物理 recovery 仍未实现。
...
Windows 文件安全层现已形成代码：`NodeDataPaths` 把 `compute-plugin` 作为不会被 cache/temp 清理的第五个受管根；类型化 `ComputePluginInstallationIdentity` 从原始 installation ID 唯一派生 Store digest，installation marker、受管根和 Authority 初始化不能再各自传入互不相干的身份。启动层可在实例锁后构造长期 `PinnedComputePluginRoot`，先取得并校验 Volume GUID 卷根句柄；卷根以下每个目录和最终文件都通过 `NtCreateFile` 的 `OBJECT_ATTRIBUTES.RootDirectory` 相对已钉住父句柄打开或创建，并对最终分量使用 `FILE_OPEN_REPARSE_POINT`。完整字符串路径只用于失败诊断和稳定卷检查，不参与根以下的授权性 lookup；UNC/DFS、reparse、全零 FileId、目录型最终文件、跨卷和 hardlink 均失败关闭。claim 文件路径必须重新精确等于 `compute-plugin/candidates/{candidate}/downloads/{ordinal:04}-{artifact}.part`，文件身份只从同一句柄 `FileIdInfo` 派生并绑定 installation/root。offset 为零只允许 `create_new`；若文件已存在，Authorized 会先被消费，再以同一父目录 capability 安全重开并只返回恢复结果。正游标缺文件或短文件产出不可写 damage evidence，只有专用线性 permit 能将其转化为两个固定终态原因；schema trigger 要求同一 claim、同一下载身份与同一时间的 damage 终态才能首次把 `downloading` 改为 `failed`。长于 Store cursor 的尾部只在同一句柄截断、`sync_all` 并复验身份/长度后返回线性 `ReconciledComputePluginPartFile`。目录创建、最终文件 create-new 或长尾截断中的任一变更会被带入后续 prior-mutation 判断。分段写入现在沿用该同一文件句柄，执行精确 payload/range、每 write buffer 取消检查、flush、fsync 和最终 identity/type/reparse/link-count/volume/length 复验；发生任何既有或本次变更后，失败只保留 recovery key 与原句柄。commit 前还会再次对原句柄做精确复核，不按路径重开。候选验证的 existing-only 目录 capability 也从同一已验 root 派生，但只允许精确 candidate downloads 的直接子文件，并让全套只读独占文件共享父句柄链，不能创建目录或文件。非 Windows 当前明确失败关闭，尚未宣称 portable beneath 实现。候选同句柄删除原语、父级 binding、namespace 观测、失败 custody 和后续授权顺序由 `node-plugin-candidate-cleanup.md` 独立维护。

NodeRuntime 现持有一个与 legacy Host 分离、默认关闭的 `ComputePluginBootstrap`。它只保留经 installation identity 派生的 dormant root/authority 路径和节点状态目录实例锁的 weak liveness witness；witness 本身不能延长锁寿命，其 operation-scoped lease 也只证明该状态目录锁仍由本进程持有。现有 `pin_compute_plugin_root()` 会 existing-only 钉住受管算力目录，在同一父句柄下取得持久锁文件的 share-none 句柄，并把该跨进程独占与 canonical root 一同封装进 `PinnedComputePluginRoot`。下载 reconcile、写入、耐久绑定、Store 终结、损坏/不确定结果 custody 与候选验证链都会线性移动同一 OS 锁的非 Clone lease；恢复文件把 lease 封装在同一个 opaque custody 内，不能单独取走句柄后提前放锁。解包链以 `'root` 借用继续约束根寿命。

当前源码已新增独立于 legacy 模型推理策略的插件共享 consent 与 V1 desired/observed 协议。只有显式携带插件运行意图、期望修订号和幂等请求号的新请求才会追加不可变授权与 delivery intent；旧客户端和历史开启记录不会被升级授权。已有 plugin head 后，旧请求若改变摘要承诺的 enabled、模型清单或限额会失败关闭；当前 mutable head 也必须与不可变 consent 的全部 resolved-policy facts 精确相等，才能重放。能力匹配的节点会在每个新认证会话收到绑定 node、owner、installation、policy digest 与 authorization receipt 的当前快照；历史 ACK 不会抑制节点重启后的重放。Bootstrap 仅校验单调修订、exact replay 和授权高水位，并生成进程内无副作用的 dormant initialization plan；关闭时撤销该计划。该 transition 不会取得 root capability，也不授权执行 signed `InstallPlan`。状态仍固定报告生产 trusted-time authority、生产外部回滚见证、root pin、authority open 与 process fence 未建立，并报告下载、建库、DDL 与 Sidecar 副作用未开始。运行中数据根变化会销毁休眠计划并要求重启，不会热切 authority；会话重放不能替代生产 rollback witness。Runner/Sidecar/节点 Attempt 的高频执行 journal 不得写回 inventory JSON 或复用下载 cancellation；正式增加这些表时应进入 schema v4 及独立执行 fence 域，而不是继续改写同名预生产 v3。

v10 preparation 只证明节点观察了同一 policy/authorization；v209 保存请求、每会话 delivery 与 observation，不打开 authority，也不返回 planning context。云端现在只在同一认证 session 内于 v10 accepted observation 耐久后派发 v11，并由 v210 绑定 exact v209 observation 与 Bootstrap 代次；阻断 observation 到此终止，未来 ready 快照才会生成唯一 request，当前结果仍只允许 `signer_unavailable/rejected`。节点凭据采用不可丢 epoch，账号、secret 或 token 替换会先撤销旧 Bootstrap policy/preparation/planning custody；但仍无 authority producer，固定 `snapshot_ready=false`。独立的无下载 policy-binding 内核则把 exact desired policy、installation/root、进程与配置/取消代次封成持实例锁 lease 的一次性意图；disabled 走无 authorization/preparation 的撤销意图。任何非 replay 策略、实例锁或数据根变化都会先推进 generation，使旧意图在 Store 各事务边界失败关闭。

LocalAuthority v4/v5 Store 还要求同一个 `PinnedComputePluginRoot` 的非 Clone root-lock lease、当前 process fence 与 authenticated trusted-time observation。Pin 构造时使用的完整 `NodeDataPaths` 会随 root capability 保留并与 intent、authority parent exact 比较，同一 installation 下另一数据根的锁不能授权当前库。Store 在 `BEGIN IMMEDIATE` 内重算 snapshot/request/base receipt JCS 摘要、精确读取 authority 与 inventory、拒绝低修订或同修订冲突；v5 还按稳定顺序枚举全部 prepared fetch claim 与 candidate verification，封存 work-set JSON/JCS、数量、统一 verification terminal result 与 companion receipt。companion 先写入并以 deferred FK 等待 base receipt；base receipt 的同一 AFTER trigger 依次把 fetch 终结为 `aborted/sharing_policy_transition_aborted`、把 verification 终结为 `aborted/verification_aborted`，逐类要求 `changes()` 精确等于回执数量、全局 prepared 为零后才把 state revision、inventory revision 与 authority epoch 各推进一位。子 verification 行只说明执行被终结，具体 policy capability 撤销由 companion 的 exact base-receipt/bound-time/work-set 绑定证明，不把它误称为子行 `revoked`。

enabled 必须使 authorization revision/digest 精确等于 policy，disabled 必须三项 authorization 全空。每个新修订还会把库存中的既有插件投影为 `desired_activation=disabled`、`admission=revoked`、清空 health，同时保留槽、owner、候选、运行时、active attempts 与 permission grant 等物理/审计事实，避免旧安装记录被新授权自动“洗白”。Store 在投影前永久关闭旧 process fence 的 fetch cancellation source；即使事务随后拒绝或结果不确定，旧 Plan 也不能从同一 source 再铸 fresh guard。process fence 的重新取得与所有 policy transition 现在都要求可信 UTC 严格晚于旧 high-water/updated time，相同毫秒失败关闭。

同修订 exact replay 会逐字段回读 base+companion receipt、inventory JSON/JCS、终结后的完整 work-set 与当前 meta；历史 v4 base receipt 没有 companion 时失败关闭，必须由更高修订重新绑定，不能伪造空撤销证明。commit-uncertain recovery 只在两份 exact receipt 与终结行同时存在、且 post-state 仍是当前 head 时返回可继续规划的 durable current capability。若不可变 receipt 已存在但 authority 已有合法后继，只返回同时携带两份回执的 `CommittedHistorical` 审计事实。两份 receipt 都不存在时，只有 exact pre-state 与 current intent 同时成立才返回 `NotCreated`；intent 已失效，或 pre-state 已被完整回执链、process owner 或其他合法单调 authority 后继推进时返回 `NotCreatedSuperseded`，回滚、碰撞和无法证明的漂移继续保留 recovery custody。

`user_version=6` 在这些 policy/cleanup 事实之外新增 append-only Manifest catalog binding。目录 revision 必须为正且单调；首个 binding 可用 Control-signed envelope为旧 scalar补足内容证明，后继必须严格前进。Store 只接受空库存、无 `owned/cleanup_pending` candidate owner、无 prepared fetch/verification 的 authority；它在同一 `BEGIN IMMEDIATE` 中从当前 root-revalidated keyring逐个重验完整 Publisher-signed Manifest来源、规范排序去重，再用当前 Control ring和独立 catalog domain验签精确 payload。catalog JSON、Control envelope与摘要、signing key/fingerprint、完整 source envelopes/set digest和 binding receipt全部逐列回读；事务原子推进 state revision、authority epoch与可信时间，不推进 inventory。rollback checkpoint/anchor V2引用 catalog digest与receipt digest，same revision 任一分叉失败关闭，V1 checkpoint/permit不能升级冒充V2。

这仍不是生产 planning context。新的 `OpenedComputePluginLocalAuthority` 只定义保留 pinned root、root lock、NodeAgent instance lock和authority instance的线性合同；本批故意不提供open intent构造入口，未来必须由Bootstrap把同一NodeDataPaths/root与真实instance-lock见证原子绑定，不能由调用方任意拼装lease。生产 `open()` 固定返回 `COMPUTE_PLUGIN_HANDLE_BOUND_SQLITE_VFS_UNAVAILABLE`，legacy path-based `connect/with_deferred`仍可能建目录、开库、切WAL或迁移，禁止用于planning，也必须在VFS启用前退役或迁移。rollback V2 assessment同样只接受未来由同一opened authority/root事务和live process fence铸造的不可复制本机checkpoint custody，不能把远端attestation中的裸checkpoint回传冒充本机事实；未来permit consumer还必须在同一custody下按authority/root/process fence与checkpoint digest重读current head，或持续持有线性custody。catalog binding会在写入、提交与Durable recovery边界重检其trusted-time observation的60秒单调有效期；生产 time/rollback provider、handle-bound SQLite main/journal/WAL/SHM VFS、snapshot producer、Ready/Attempt work-admission与signed `reauthorize_existing`仍缺。cleanup-only deletion domain也仅允许已耐久授权、owner=`cleanup_pending`的线性custody在同authority/root/process fence与retained handles连续存活时继续；授权前竞态、进程轮换、重启和跨进程adoption均失败关闭。因此`context_ready`、`snapshot_ready`及root/authority/download/Sidecar标志保持false，Store无Host调用点，也不允许PlanApply、下载或任务准入。

当前供应链、候选健康与失败候选清理 sequence 1–4 已通过 `elon-pc-node` 编译；候选清理链通过 3 项 Windows 同句柄删除、1 项目录顺序、4 项 completion、5 项 topology builder/Store、4 项首 intent builder/Store、3 项强 disposition、4 项 disposition event builder/Store，以及 sequence 3/4 各 4 项 builder/SQLite exact-row 测试，合计 32 项，另有 1 项 topology/journal schema 建库与重开测试。新增 8 项只证明 hash-chain、时间/绑定拒绝、NULL/durability 列精确回读和列篡改失败关闭，不证明真实 parent-relative OS 观察、authority/owner fresh/recovery 全事务或 native namespace barrier。仍没有 retained custody → cleanup authorization → topology → intent → physical disposition → disposition Store → parent observation → absence Store → namespace barrier → namespace Store 的完整事务夹具。cleanup authorization Store 可推进 `cleanup_pending`，topology Store 可封存 exact execution plan，intent Store 可独立提交第一个物理动作前置授权，私有单对象 executor 可消费该 capability并保留物理结果 custody，disposition、absence 与 namespace durability Store 可分别独立持久化 sequence 2/3/4 并分类不确定提交；这些入口均没有 Host 调用方。由于安全 mutation fence 构造器、后续 ordinal 和 terminal journal producer 仍缺，终态 capability 不可构造，完整私有流水线尚不可达。当前仍未接入生产 Host 或生产本地库，不能授予生产重试、安装、promotion、ReadyCapability 或商业 Verification。多对象 handle-tree 收口、跨重启物理恢复、真实下载/Sidecar/探针、installed/promotion/GC 与 Attempt 接线仍待实现；精确边界见 `node-ready-capability.md` 与 `node-plugin-candidate-cleanup.md`。
