...
const SELECTOR_ENV: &str =
    "ELON_SQLITE_A2_LOCK_EXISTING_FIRST_EXCLUSIVE_RELEASE_ERROR_SELECTOR";
...
fn validate_parent_receipt(
    ...
    if !child.matches_family(SanitizedPayloadFamily::LockQuotient) {
        return Err(anyhow!("q13 Lock initialization child payload family mismatch"));
    }
    ...
    {
        return Err(anyhow!("q13 Lock initialization parent cleanup binding mismatch"));
    }
...
pub(super) fn validate_binding(
    ...
    if binding.count == 0 || binding.first >= 8 || end > 8 {
        return Err(anyhow!("q13 Lock initialization range is outside eight slots"));
    }
    if binding.action == LockRunnerActionV1::LockShared && binding.count != 1 {
        return Err(anyhow!("q13 shared Lock initialization range is not one slot"));
    }
    if !matches!(
        binding.action,
