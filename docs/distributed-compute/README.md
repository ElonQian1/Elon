...
| 外部算力池适配器与统一报价 | v221/v222 管理面、MCP、Store 重开与 PC 静态构建已有专项。v227 raw bytes source 和 v229 三终态/currentness 已通过完整服务端编译及 51 项 Windows 临时 DATA_DIR/SQLite 测试；v230 Artifact signer key 注册表又通过完整编译及 5 项四眼、撤销、并发、迁移/重开和 HTTP 专项，均为 `implementation_partially_verified`。仍缺真实崩溃/断电、Unix/Windows 生产文件边界、真实 TCP、生产升级和部署；v227/v229/v230 无 MCP/PC，active key 也不证明 candidate ref、签名 provenance、verifier、Adapter、v213 route、Pool/Offer 或派发。见 [`external-pool-adapter-release-api-acceptance.md`](external-pool-adapter-release-api-acceptance.md) 与 [`external-pool-adapter-artifact-signing-key-acceptance.md`](external-pool-adapter-artifact-signing-key-acceptance.md) |
...
服务端 Provider Adapter 统一接入公司集群、云 GPU 和其他算力池；每个 Provider 保留自己的内部调度，只向一龙提交标准回执。v221/v222 已通过管理面专项；v227 [`artifact bytes source`](external-pool-adapter-artifact-source-authority.md) 与 v229 [`release-admission lifecycle`](external-pool-adapter-release-lifecycle-authority.md) 又通过 51 项 Windows 本地专项。v230 [`Artifact signer key registry`](external-pool-adapter-artifact-signing-key-authority.md) 已通过完整编译及 5 项迁移/重开、四眼、撤销、并发和 HTTP 专项，只建立后续验签可使用的 current trust root。真实 TCP、生产升级、部署和签名 provenance 仍缺；无 MCP/PC，也不证明 Artifact、verifier、credential、Adapter、route、容量、派发或结算。证据见 [`external-pool-adapter-release-api-acceptance.md`](external-pool-adapter-release-api-acceptance.md) 与 [`external-pool-adapter-artifact-signing-key-acceptance.md`](external-pool-adapter-artifact-signing-key-acceptance.md)。
