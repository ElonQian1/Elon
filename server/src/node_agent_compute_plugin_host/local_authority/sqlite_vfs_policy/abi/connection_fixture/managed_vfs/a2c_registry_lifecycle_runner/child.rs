//! Child-only RegistryLifecycle actual capture and sanitized stdout publication.
...
use super::super::{
    ...
    a2b2_cases::RegistryLifecycleSelector,
    registry_lifecycle_harness::exercise_registry_lifecycle,
};

pub(super) const CHILD_ROOT_ENV: &str = "ELON_SQLITE_A2_REGISTRY_LIFECYCLE_CHILD_ROOT";
...
pub(super) fn selected_child_root() -> anyhow::Result<Option<PathBuf>> {
    ...
    if !root.is_absolute() {
        return Err(anyhow!("RegistryLifecycle child root is not absolute"));
    }
    Ok(Some(root))
}
...
pub(super) fn exercise_child(
    ...
    let actual = exercise_registry_lifecycle(root, selector)?;
    ...
    if !root.is_dir() {
        return Err(anyhow!(
            "RegistryLifecycle child root disappeared before parent observation"
        ));
    }
    Ok(())
}
