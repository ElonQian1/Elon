...
fn assert_rejected(key: DynamicClassKeyV1, member: StaticMemberSealV1, mutation: &str) {
    assert!(
        ...
        "q14 initialization admission accepted {mutation}"
    );
}

#[test]
fn all_88_q14_descriptors_and_exact_catalog_seals_are_source_present() {
    ...
    for &case in leaves.keys() {
        ...
        validate_lock_program_for_test(&key, member, compile_for_test(&key))
            .unwrap_or_else(|error| panic!("exact q14 member {case:?} was rejected: {error:?}"));
    }
}

#[test]
fn q14_is_inventory_present_without_granting_supported() {
...
#[test]
fn q14_keys_reject_sibling_seals_unlock_actions_and_nearby_initialization_shapes() {
    let leaves = frozen_lock_created_first_truncate_error_release_succeeded_leaves_v1();
