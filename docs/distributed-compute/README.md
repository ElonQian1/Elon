...
| 节点端点凭据与认证会话权威 | 服务端 v216 已铺 credential/version/revocation/session receipt/session head 五表，v217/v218 闭合目标绑定的五分钟 owner 重认证与单次 consumption；默认关闭的 direct-TLS owner API、Windows DPAPI issue/recover 与 legacy no-downgrade 栅栏也已写入。当前增量新增独立 v13 compute-inert WSS 子协议：NodeAgent 从唯一 HTTPS origin 派生 `/agent/ws` 并声明 exact credential triple；服务端在同一 IMMEDIATE 事务重验 TLS/credential/owner/install，追加 receipt/head，启动收口旧 boot head，并用独立 socket supervisor 执行 15 分钟绝对期限。该连接不进入 legacy AgentEntry/NodeRegistry，不收 capabilities、CLI、插件、任务或 ACK，节点 `connected=false`；它不等于 Planning、work-admission、Runtime、Ready、route、outbox、command、Lease 或派发。源码尚未编译、测试、运行或执行迁移，状态仍为 `implementation_unwired`。见 `node-endpoint-session-authority.md` |
