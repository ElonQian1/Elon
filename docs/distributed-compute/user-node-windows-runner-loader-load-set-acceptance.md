...
| recursive forward plan/currentness | `system_closure/acquisition/{plan,plan_digest,plan_forwarder_validation,plan_owner_validation,plan_projection,plan_validation}.rs` | A0复用 GrantReady、Ak plans V1、exact-vector currentness-pending→DispatchReady、receipt/output V3；真实 currentness/parser/resolver缺失 |
...
| recursive source review | `runtime_loader_recursive_wave_custody_source_contract_tests.rs` | 未运行 guard，固定 policy/per-wave source shape而非正向 producer |
...
6. source-written launch-path discovery 只从 retained Runner/package-root/全部 plan-directory handles 生成候选观测，
   ...
   GrantReady复用、Ak canonical plans V1、nonempty producer→target / terminal None、exact-vector currentness-pending→DispatchReady、route-specific
   acquisition、partial custody与 V3 receipt/output cross-binding，但 policy/parser/resolver/backend/positive advancer/sealer/query仍
...
9. exact searched-name/launch-path grants合同绑定 session、parent/name、disposition和 generation；terminal/
   ...
   generation；实际 package+system lease owners逐 base/producer wave进入 graph，final aggregate/sealer消费完整 generation set，
   随后 consuming query再确认同一 set；统一 FileId lease-set
...
16. transition 权威顺序要求 borrow-only launch-path discovery/pre-lease PE plan、authenticated exact CWD/unresolved request、
    wave-zero terminal/disposition/external-owner grant-ready resolution、whole GrantReady borrowed validation + exact currentness query、
    PolicyCurrent GrantReady、base grants + package leases + base route-specific owners、
    package same-handle rehash/reparse、producer-wave parse/canonical plans/exact-vector validation/currentness-pending/point-of-use
    authorization/DispatchReady/outgoing grant/candidate/lease advancement、empty frontier、
...
17. `existing_extraction_directory_access_share_compatibility=source_seam_written_windows_dynamic_unverified`；typed
    ...
    selector/policy signature verifier/currentness backend、prelease-or-recursive parser/GrantReady或逐波 resolver、external-directory owner、逐波 acquisition
...
| `[A0 PolicyCurrent GrantReady→base acquisition \| Ak plan V1→currentness-pending→authorization→DispatchReady]` / receipt V3 | 0 | 0 | 1 | source written；currentness/parser/resolver/backend missing |
