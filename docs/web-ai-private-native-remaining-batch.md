...
| Attachment upload | Verified small-text private upload; native byte handoff, current store association and existing ordinary conversation scope now connected in source | Accept the integrated new/existing-chat plain-text paths on-device, then cover images/project/temporary/multipart variants. The original native-picker smoke failed before byte upload; no fresh integrated-device pass is claimed. See [upload contract](chatgpt-private-attachment-upload.md). |
