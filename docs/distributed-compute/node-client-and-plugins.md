...
这只是 PlanApply live-binding 的必要前置，不是可用planning context。v6目录binding、rollback V2和opened-authority合同均保持失败关闭：仅空库存可绑定，V1/V2不互转，裸远端checkpoint不能冒充本机事实，open intent与snapshot producer都不可构造。精确合同与启用前置见[`node-plugin-manifest-catalog-authority.md`](node-plugin-manifest-catalog-authority.md)。当前v4-v6/v210 DDL仍未实际运行，v5也只撤销下载/校验能力；继续保持零PlanApply、零下载、零安装、零Sidecar。
