...
这只是 PlanApply live-binding 的必要前置，不是完整 planning context。当前 Store 没有生产 Host 调用点，v4/v5 DDL 或迁移也未实际运行；NodeRuntime 账号切换尚未接入 Bootstrap 失效代次，SQLite authority open 仍未与 pinned root 句柄闭合。v5 companion 只撤销尚未终结的下载/校验能力，不撤销已发布 Ready 或运行中 Attempt；cleanup 删除链现在有独立的同进程 deletion custody，但跨重启 adoption、后续 ordinal 和 terminal producer仍缺。Ready/Attempt 仍需要独立 work-admission generation、signed `reauthorize_existing` 和 exact receipt 绑定。正确后续顺序是先补齐这些本机门卫与 planning snapshot，再由默认 unavailable 的真实 signer 接口产生计划，节点验签展示并取得本机 exact confirmation；在这些门卫全部闭合前继续保持零下载、零安装、零 Sidecar。
