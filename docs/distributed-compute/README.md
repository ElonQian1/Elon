...
| 节点插件治理合同 | Signed Manifest/InstallPlan、双 keyring、authority v3/PlanApply、可恢复受管取数、候选 exact closure/hash 与原子 resolution、schema 定义指纹、可信时间和回滚锚查询/首次注册/更新合同均已写。`PinnedComputePluginRoot` 先 existing-only 钉住算力目录，再通过父句柄相对 open/create 的持久文件取得 share-none 跨进程独占；当前取数与候选验证流水线返回的 custody 以非 Clone lease 保活同一锁句柄，竞争、reparse、hardlink、跨卷或非 Windows 均失败关闭。已验证 ZIP 还可生成有界规范计划，在随机 create-new run 中逐文件安全物化并封存 seal；retained-handle 重验与 fresh authority binding 后，Store 在一个 `BEGIN IMMEDIATE` 中保存不可变 staging receipt、推进三类 fence，并把槽从 `verifying` 原子推进到 `staged`。结果不确定时可按 recovery key 稳定读取 `NotCreated` 或 exact `Staged`，碰撞或权威漂移失败关闭；exact `Staged` adoption 还会二次读取同一结果并从原句柄重哈希文件/seal 后恢复 typed custody，`NotCreated` 不授予盲重试。进程内候选健康评估合同还能消费 typed staged custody，形成最长五分钟且绑定探针 transcript 与可信时间的发布输入，但不是耐久健康回执或 promotion permit。默认关闭的 Bootstrap 已挂入 NodeRuntime，但 sharing-on 获取根锁、生产时间源/回滚见证、root pin、authority open 和 process fence 仍未建立。全部为 `implementation_uncompiled`；真实 HTTPS downloader、目录耐久/跨重启恢复、真实 Sidecar 探针、health Store/恢复、installed/promotion 和正式 Attempt 接线仍缺，`staged` 不等于 installed、ready 或商业 Verification |
...
4. `docs/distributed-compute/node-plugin-local-authority.md` 与 `docs/distributed-compute/node-ready-capability.md`：节点 SQLite 真源、候选下载验证，以及短期技术就绪健康证明边界。
...
目标流程仍是：共享关闭时不下载重型组件；开启后按硬件和任务选择签名插件、运行时与模型工件。本机耐久真源已经选定为独立 SQLite，根签名双 keyring、原子计划应用、候选所有权、三段式下载认领、候选级全工件验证和可信时间边界见 `docs/distributed-compute/node-plugin-local-authority.md`；其中 Store 认领/终结/恢复、verification schema/fence、candidate closure 双读与全文件 pin、prepared begin/outcome/abort recovery、逐文件 hash/Hashed capability、严格晚于 hash barrier 的 Prepared S3 binder，以及事务内 current-admission replay/S4/verified-or-rejected resolution 已写。可信时间又形成本机随机 clock epoch、60 秒一次性挑战及 JCS/SHA-256/Ed25519 证明验证合同，只有安装身份、clock epoch、挑战、权威和规范 UTC 时间全部匹配才产生 observation。TrustedHashed 只能由私有 resolver 一次消费；raw verified 只把本机槽推进到 `verifying`，不授权解包、健康、promotion 或商业 Offer。staged 候选已有进程内健康评估状态机，但观察值仍由 Host 调用方提供，尚未证明真实 Sidecar 已运行。以上仍为 `implementation_uncompiled`，尚未建库或接线；生产时间权威 key resolver、真实网络取时、真实下载器、Sidecar 进程与 IPC、真实探针调度、health Store/恢复、installed/promotion、云端 capability gate 和通用 Attempt 协议接线目前仍未实现。
