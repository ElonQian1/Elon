//! Canonical q14 controlled-fault payload and independent parent-side validation.

use anyhow::anyhow;
use rusqlite::ffi;
use sha2::{Digest, Sha256};

use super::super::super::child::lock_created_first_truncate_error_release_succeeded::{
...
pub(in super::super) struct ValidatedCreatedFirstTruncateErrorReleaseSucceededPayloadV1 {
...
pub(in super::super) fn validate_payload(
    payload: &str,
    binding: LockRunnerNativeAcquireCreatedFirstTruncateErrorReleaseSucceededBindingV1,
) -> anyhow::Result<ValidatedCreatedFirstTruncateErrorReleaseSucceededPayloadV1> {
    validate_binding(binding)?;
    let selector = exact_selector(binding);
    let mut fields = payload.split(',');
    if fields.next() != Some(REPORT_VERSION) || fields.next() != Some(selector.as_str()) {
        return Err(anyhow!("q14 Lock initialization payload identity mismatch"));
    }
    let values = fields
        .map(parse_canonical_u64)
        .collect::<anyhow::Result<Vec<_>>>()?;
    if values.len() != REPORT_VALUE_COUNT || values[..25] != binding_values(binding) {
        return Err(anyhow!("q14 Lock initialization payload binding mismatch"));
    }
...
fn digest_receipt(values: &[u64]) -> [u8; 32] {
    ...
    hasher.update(
        b"elon-a2-lock-created-first-truncate-error-release-succeeded-controlled-fault-v1\0",
    );
...
fn parse_canonical_u64(value: &str) -> anyhow::Result<u64> {
    ...
    if parsed.to_string() != value {
        return Err(anyhow!(
            "q14 Lock initialization payload scalar is not canonical"
        ));
    }
    Ok(parsed)
}
