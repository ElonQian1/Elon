...
| Attachment upload | Earlier small-text private upload verified; production text-file delivery passed on 1541; native byte handoff and ordinary/temporary/writable-project image/PDF transactions packaged; ingest-project image gate now implemented in source | Confirm integrated private-route provenance, images/PDF, official model/branch/flag-runtime access and temporary/project library behavior; finish read-only permission variants, other documents and multipart. Production file delivery is not a private-route pass. See [upload contract](chatgpt-private-attachment-upload.md), [PDF extension](chatgpt-private-pdf-upload.md) and [scope contract](chatgpt-private-attachment-scopes.md). |
