...
- `Resume`：从已终止、带当前监督协议且保留平台隔离 worktree 的父任务恢复原始 prompt，角色为 `resume_original`。helper 先做只读门禁，节点再以本机任务记录的基础仓库、隔离路径、分支和 `git_head` 做权威校验；不能用参数指定任意 worktree。目录仍在但 Git worktree 注册丢失时，只读门禁会标记 `recovery_required`，节点确认无活跃 prompt/sidecar 占用后重建元数据，原有未提交和未跟踪文件不被 checkout 覆盖。
...
- `resume_original` 必须指向已终止的同节点、同项目受监督父任务，并复用节点记录的隔离现场；正常现场校验 Git worktree 注册，注册丢失的现场只有在基础仓库、平台路径、分支和已记录 `git_head`（旧记录可退化为仍存在且未漂移的分支引用）全部一致时才可原位重建。非法继承、提交身份漂移和并发占用都会以冲突拒绝。
