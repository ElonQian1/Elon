...
one-shot registry现包含状态核与私有generic owner。状态核描述`PendingMain → Opening → Active → Closing → AwaitingRouteRetirement → Retired`及无出口`TerminalQuarantine`，并对main/sidecar lease、单一SHM租约、in-flight callback、单调ID和精确关闭证明做checked不变量；owner用token、session ID和route epoch三元身份原子封存authority-open custody、policy与state，注册前通过私有trait重验真实open intent current，已用nonce即使正常退休也不可复用。不确定entry会被故意泄漏为进程寿命terminal custody，不能靠`Drop`冒充关闭。10项状态测试与7项owner测试覆盖完整/取消退休、精确route/close proof、资源排空、重复/退休token、identity exhaustion、失效custody和terminal保留。当前没有生产registry实例、随机nonce provider、Connection或live callback。
