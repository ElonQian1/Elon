...
1. **不允许制造新的巨型文件**：新建源文件超过 800 行立刻警觉，不允许一次写出 ≥1500 行的文件。
2. **不允许在 ≥1500 行的旧文件里继续叠加新功能**：除小修外，必须先把本次触碰到的成块职责抽到独立模块再加代码。
3. **按职责模块化 + 长期主义思考**：先想"这块逻辑属于哪个领域、应该独立成什么模块"，再动手；不要为了"快"就塞回入口文件。
4. **任务起手 + 提交前都要 `git fetch origin main`**：本项目长期多 AI 并发拆分，避免和其他 AI 重复抽同一块代码。
5. **只 stage 本任务文件，不混合提交**：拆分用 `refactor(...)`，新功能用 `feat(...)`/`fix(...)`，不允许"重构 + 新功能"塞同一个 commit。
