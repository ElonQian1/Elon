...
const privateFinancePeriods = read('desktop-shell/src-tauri/src/local_ai_browser/chatgpt_win_private_finance_periods.js')
const privateSourceGroups = read('desktop-shell/src-tauri/src/local_ai_browser/chatgpt_win_private_source_groups.js')
const privateRichCompatibility = read('desktop-shell/src-tauri/src/local_ai_browser/chatgpt_win_private_rich_compatibility.js')
const googleBootstrap = read('desktop-shell/src-tauri/src/local_ai_browser/google_ai_mode_adapter_bootstrap.rs')
...
assert.match(bootstrap, /WIN_PRIVATE_FINANCE_PERIODS/)
assert.match(privateFinancePeriods, /timeframe_configs/)
assert.match(privateFinancePeriods, /periodViews/)
assert.match(privateFinancePeriods, /MAX_POINTS_PER_PERIOD = 192/)
assert.match(privateSourceGroups, /search_result_groups/)
assert.match(privateSourceGroups, /tool_icons/)
assert.match(privateSourceGroups, /MAX_SOURCES = 12/)
assert.match(privateRichCompatibility, /renderer_upgrade_required/)
assert.match(privateRichCompatibility, /packedWidgetCount/)
assert.match(privateRichCompatibility, /convertedWidgetCount/)
assert.match(bootstrap, /WIN_PRIVATE_RICH_COMPATIBILITY/)
assert.match(googleBootstrap, /WIN_COMMON_RICH_CONTENT_ADAPTER/)
assert.match(googleBootstrap, /WIN_RICH_CONTENT_ADAPTER/)
...
assert.deepEqual(authorizationRegistry.authorizations[0].dataClasses, [
...
assert.match(protocol, /RichContentFinancePeriodView/)
...
assert.match(structuredContent, /renderer_upgrade_required/)
assert.match(renderer, /aria-label="官方行情卡片"/)
assert.match(renderer, /aria-label="官方回答图表"/)
assert.match(renderer, /payload\.points\.map/)
...
assert.match(renderer, /setRequestedPeriodId/)
assert.match(renderer, /aria-pressed=/)
assert.match(renderer, /metrics\.map/)
assert.match(renderer, /activeChart\.points/)
assert.match(renderer, /activeChart\?\.kind === 'candlestick'/)
...
const validPeriodFinance = structuredClone(validFinance)
validPeriodFinance.payload.periodViews = [{
  id: '1d', label: '1D', selected: true, trend: 'positive',
  primaryValue: 'US$77,274.00', secondaryValue: '+US$123.00 (0.16%)',
  chart: { kind: 'line', points: [{ x: '12:00', y: 77274 }, { x: '13:00', y: 77301 }] },
}]
assert.equal(isYilongRichContent(validPeriodFinance), true)
...
const invalidPeriodFinance = structuredClone(validPeriodFinance)
invalidPeriodFinance.payload.periodViews[0].chart.points[0].y = Number.NaN
assert.equal(isYilongRichContent(invalidPeriodFinance), false)
