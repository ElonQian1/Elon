...
use super::{AuthorityPlanApplicationState, ResolutionProjection};

pub(super) fn update_resolution_authority_meta(
    ...
    let updated = transaction
        .execute(
            ...
            WHERE singleton = 1
              ...
              AND trusted_time_high_water_ms = :resolved_at
              AND clock_status = 'trusted' AND updated_at_ms = :resolved_at"#,
            named_params! {
        ...
        .context("COMPUTE_PLUGIN_VERIFICATION_RESOLUTION_AUTHORITY_UPDATE")?;
    if updated != 1 {
        bail!("COMPUTE_PLUGIN_VERIFICATION_RESOLUTION_AUTHORITY_CAS");
    }
    Ok(())
}
