//! Exact frozen-member authority for the 88-member q16 initialization truncate-release tranche.
...
use super::{
    range_mask_v1, LockCreatedFirstTruncateErrorReleaseFailedCompletionV1,
    CREATED_FIRST_TRUNCATE_ERROR_RELEASE_FAILED_MEMBER_COUNT,
};
...
pub(super) fn exact_member_v1(
    ...
    if observed_rows != CREATED_FIRST_TRUNCATE_ERROR_RELEASE_FAILED_MEMBER_COUNT {
        return Err(LockRunnerExecutionViolationV1::MemberCatalogInvalid);
    }
...
fn expected_row_v1(row: usize) -> Option<ExpectedRowV1> {
    if row == 0 || row > CREATED_FIRST_TRUNCATE_ERROR_RELEASE_FAILED_MEMBER_COUNT {
        return None;
    }
    let ordinal = row - 1;
...
const fn completion_tag_v1(
    ...
    match completion {
        LockCreatedFirstTruncateErrorReleaseFailedCompletionV1::RetentionSucceeded => "succeeded",
        LockCreatedFirstTruncateErrorReleaseFailedCompletionV1::RetentionRouteUnknown => {
