...
- 新增本人动作确认读取和 MCP 模块拆分为 `implementation_uncompiled`；未执行 Cargo、MCP 或端到端回归。
- 新增本人动作确认取消与 v166 `canceled_at` 迁移为 `implementation_uncompiled`；未执行迁移、并发或状态机回归。
...
- pending、confirmed 首次取消和重复取消；正确/错误确认短语。
- 他人、其他 App、自然过期和 consumed 确认均不能形成 `canceled_at`。
- 取消与 Invocation 创建并发时至多一方成功，不能出现已消费且同时标记取消。
- 取消后相同幂等请求可重新准备新确认，但不能借旧确认调用。
