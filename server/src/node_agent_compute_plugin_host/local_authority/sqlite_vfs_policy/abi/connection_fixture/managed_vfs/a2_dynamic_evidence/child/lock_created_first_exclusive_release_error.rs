//! Canonical q12 child header for one controlled initialization release uncertainty.

pub(in super::super) const REPORT_VERSION: &str = "a2lockq12";
...
pub(in super::super) fn selector(
    ...
    Ok(format!(
        "initialization-{action}-first-{first}-count-{count}-created-first-exclusive-release-error-{completion}-terminal-route-unknown"
    ))
}
...
mod tests {
    ...
    #[test]
    fn eighty_eight_q12_selectors_are_unique_and_fail_closed() {
