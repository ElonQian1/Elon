...
6. 根据统一权限模式调用 `project_discussions_apply`。`git_backed_full` 为普通 Git 项目保存整理前和整理后提交；托管笔记库创建对应版本。
7. PC 页面刷新讨论图。用户可从任意节点继续、分叉或要求晋升，AI 先用 `project_discussions_get_node` 获取该节点的局部上下文。
