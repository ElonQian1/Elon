...
| Independent native file library | Root/folder/search/cursor browsing, cache, ordinary downloads, confirmed rename and soft-delete; shared provider/byte owner | Targeted protocol/native tests; device pending | Source-only; [browser](chatgpt-private-library-browser.md) and [mutations](chatgpt-private-library-mutations.md); folder and special-source operations remain open |
