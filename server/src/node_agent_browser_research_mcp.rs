...
fn describe() -> Value {
    json!({
        ...
        "commands":{
            "binance_grid_list":{"request_id":"required 8..80 safe identifier; new ID per explicit fresh read",
                "query":"Set exactly start only to begin. Omit to poll/page the same read; polling never starts or repeats a read.",
                "offset":"0..500; first read must start at 0; immutable snapshot pagination",
                "limit":"1..50, default 25",
                "result":"reader.status pending/ready/failed; poll this command with the same request_id, omit query. ready carries observed_at_ms, expires_at_ms, fixed rows, total and next_offset. Five-minute memory-only snapshot. Not a trading command; no desktop clicks or ChatGPT dependency."},
            "binance_grid_detail":{"request_id":"required 8..80 safe identifier; new ID per explicit fresh read",
                "resource_id":"required positive decimal strategy ID, 1..20 digits; exact ownership checked against a fresh list",
                "query":"Set exactly start only to begin. Omit when polling the same request_id and resource_id.",
                "result":"reader.status pending/ready/failed; ready contains one fixed row with exact decimal strings/nulls and observed_at_ms. Reuses the owner's background Binance session. Login/verification remains manual; no credentials returned."},
