...
这仍不是生产可调用的 planning context。v5 只终结 prepared fetch/verification；当前没有 NodeRuntime 账号/凭证切换到 Bootstrap generation 的失效 hook，没有把路径型 SQLite open 与 pinned-root 句柄身份闭合，也没有生产 trusted-time/rollback witness、已铸 Ready/Attempt 的独立 work-admission epoch/receipt 绑定、已安装插件的 signed `reauthorize_existing` 语义和与 fetch guard 分离的 cleanup-only 删除 custody。因此 `context_ready`、root/authority/download/Sidecar 标志继续固定为 false，policy-binding Store 没有 Host 调用点；即使外部出现合法签名，也不得把本批源码描述成已经允许 PlanApply、下载或任务准入。
