...
2. 每个 App 固定返回沙箱和生产两个环境，分别聚合订阅总数、活动数、已验证数、待发、投递中、重试、待处理死信、已确认死信、最早排队时间、最近成功时间和最近错误码。
3. 环境状态只允许 `idle`、`healthy`、`processing`、`attention` 和 `action_required`。待处理死信或活动生产订阅失去生产就绪条件时要求处置，重试需要关注，正常在途任务标记处理中；已确认死信保留可见但不再单独触发告警。
...
- 历史死信持续保留；未确认记录计入待处理，人工确认后转入已确认计数，合法重试会清除确认并重新进入队列。
...
- `docs/decisions/open-commerce-webhook-dead-letter-acknowledgement-v1.md`
