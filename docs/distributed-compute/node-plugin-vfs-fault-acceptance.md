...
- current Lock source 已推进到 q17：q12/q13 release-error、q14/q15 truncate-error/release-succeeded 事实不变；q16/q17 `LockNativeAcquire{CreatedFirst,ExistingFirst}TruncateErrorReleaseFailedV1` 各 88 singleton（16+72，44/44 completions）。q17 先 physical precreation/close、cold `was_created=false`，再按序丢弃 truncate `Result` 与 cleanup `UnlockFileEx` BOOL；同 q16 为 `cleanup_rewrite=false`、terminal=`CleanupRewritten`、DMS uncertain、file retained。`a2lockq17`=172 values（8 precreation+q16双回执）；catalog=88 rows/18,474 bytes，SHA-256=`5ce129843d33b279c9ec70dd282d59cc79455c8f1a1b652718bb04b72777adff`。q1–q17 inventory m/g=`4196/4196 present,4472/3944 missing,8668/8140 total`，q12–q17=`528/528`，initialization remaining=`2904/2376`。均 source-only/uncompiled/unrun、actual=`not_run`；natural/Windows/production/economic=closed。精确边界见[`Lock tranches`](node-plugin-vfs-lock-dynamic-tranches-authority.md)；19-artifact refresh 接受前须含 q15–q17。
...
| Lock | `8668/8668` | `0/8668` | `not_opened` | q16/q17 CreatedFirst/ExistingFirst release-failed 各88；q17 physical precreation+cold `was_created=false` 后有序双 unavailable receipt；m/g=`4196/4196 present,4472/3944 missing,8668/8140 total`，initialization remaining=`2904/2376`；actual=`not_run`、`Qlock=unknown`；19-artifact refresh须含q15–q17；详见 [Lock tranches](node-plugin-vfs-lock-dynamic-tranches-authority.md) |
...
| OS lock outcome | shared/exclusive sibling relation 分别对账 success/contended/error；q5–q11 不变。q12/q13 的 CreatedFirst/ExistingFirst release BOOL unavailable 保留 DMS uncertain；q14/q15 的 truncate Result unavailable 后须观察 release success 与 DMS `Released`。q16/q17 则必须依次形成 truncate Result unavailable、cleanup `UnlockFileEx` BOOL unavailable；q17 此前还须 physical precreation/close 与 cold `was_created=false`。两者同 tuple：`cleanup_rewrite=false`、terminal=`CleanupRewritten`、DMS=`ExclusiveOutcomeUncertain`、mutation/lock uncertain、file retained、native lock/unlock=`1/1`。六条未运行 seam 未来只能形成 `controlled_fault_actual`，不得冒充 natural actual；详见 [Lock tranches](node-plugin-vfs-lock-dynamic-tranches-authority.md)。`SQLITE_BUSY` 不得与 I/O failure、mutation 或 uncertainty 合并。 |
...
当前正式结论：Map/Lock `StaticContract=43476/43476 + 8668/8668`。q12–q17 六个 initialization slices 各为 source-only 88 singleton；q16/q17 是 CreatedFirst/ExistingFirst truncate-error/release-failed 有序双 unavailable receipt 对切，均 `cleanup_rewrite=false`、terminal=`CleanupRewritten`，q17 另有 physical precreation/cold `was_created=false`。q1–q17 inventory m/g=`4196/4196 present,4472/3944 missing,8668/8140 total`，initialization remaining=`2904/2376`。current source=`source_written/source_review_only/implementation_uncompiled/implementation_unrun`，`passed=0 failed=0 actual=not_run`、无 actual/reviewed digest/frozen manifest，coverage=`0/43476 + 0/8668`、`Qmap/Qlock=unknown`、`WindowsDynamic=not_opened`。19-artifact refresh 接受前须含 q15–q17；A2 仍未动态接受，production/Ready/dispatch/market/settlement/economic closed。细节只见[`Lock tranches`](node-plugin-vfs-lock-dynamic-tranches-authority.md)。
