...
| 外部算力池适配器与统一报价 | Provider onboarding v221 与 Adapter release v222 已编译并随完整临时文件 Store 迁移/重开；v222 四眼复核→immutable staged admission Store 状态机另通过 2 项专项，v221 业务状态机与两者 service/API 未运行。它们只保存候选来源，不验证 artifact/verifier、不写 v213、不建 Pool/Offer 或派发。见 [`external-pool-adapter-authority.md`](external-pool-adapter-authority.md)、[`external-pool-adapter-release-authority.md`](external-pool-adapter-release-authority.md) 与 [`external-pool-adapter-release-acceptance.md`](external-pool-adapter-release-acceptance.md) |
