...
当前已形成未编译的 A1 sealed projector 源码合同，不宣称 producer 已实现或可达。生产 `OpenedComputePluginLocalAuthority` 仍不能成功打开，生产 VFS、process owner/fence、root/currentness、trusted-time、rollback 与 node-profile provider 均未接线，因此不存在可生产的 snapshot custody。所有现有节点继续诚实报告 `context_ready=false`、`snapshot_ready=false`，并保持 Runtime stopped。
...
2. 补齐测试 VFS 的 SHM map/lock/unmap、联合 close 可注入平台失败矩阵，并评估同 namespace 多 Connection 竞争；这些证据不能自动提升为生产入口。
