...
一龙主项目现在具备一个默认关闭的 paper 身份桥接：已登录用户可以通过 `POST /api/me/quant/paper-access-grants` 请求最多五分钟有效的 Ed25519 签名授权，量化子项目据此读取该脱敏主体自己的模拟持仓或申请模拟部分/全额退出。
...
量化项目必须固定信任部署配置中的 key ID 和 Ed25519 公钥，不得信任令牌自带或网络临时提供的任意公钥。`contracts/quant/paper-access-keyring-v1.schema.json` 定义可移植的公开 keyring：新 `active` 与旧 `retiring` key 可以按 grant 签发时间窗重叠验证，`revoked` key 始终失败关闭。
...
签名 seed 与 subject 脱敏密钥必须各自独立、均为 32 字节，不能提交 Git、打印日志或发给浏览器。主项目任一时刻只用当前私钥签发新 grant；量化项目推荐把一个 active 公钥和轮换期内的 retiring 公钥配置到 `QUANT_PAPER_ACCESS_KEYRING_JSON`，不获得主项目私钥或 subject 密钥。旧单 key 环境变量继续兼容，但不能与 keyring 混用。

## 当前限制

- 量化 verifier 已实现固定多 key 重叠轮换、签发时间窗和密钥级撤销；尚无单个 grant 持久化吊销表、生产私钥托管或公开部署验收。当前跨站自动接入仍仅限五分钟 Paper grant，失败时不会降级到 URL 传参。
...
- `paper.redemption.request` 只允许减少本人模拟开放份额；追加模拟配额仍需要量化项目独立运营令牌，主项目不会签发用户自增余额 scope。
