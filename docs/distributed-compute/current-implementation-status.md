...
- Attempt v175-v215、v221/v222 管理面及 v227-v251 Adapter 前置门已形成分层权威，生产成功链仍未运行。V239 形成限时动态沙箱签名收据，V241/V242 固定凭据验证器实现与公钥，V243 形成短时凭据验证回执，V244 汇合为可撤销采用授权；V244 migration 定向测试及 2 项真实 HTTP 链路测试通过。V245 legacy 投影加固 3 项迁移测试通过；V246 从 V227 CAS 同一复验句柄按 V232 manifest 安全解包、原子发布内容寻址目录并登记 `installed_inert`，V247 追加安装撤销终态与 sealed current-authority seam，二者文件系统/迁移 10 项及安装/撤销 HTTP 3 项通过，并修复 Windows 深层命名空间的原子发布长路径失败。主线 V248 属于 ERP；V249 用同一 V247 sealed 文件树登记 Provider-neutral release 与每个 Provider 独立的 installed-inert companion，V251 原位修复凭据验证器 JSON 合法键序被误判为冲突的问题；registry/companion migration 6 项与 HTTP 4 项通过。独立的 V250 为 exact V249 release 增加可续签漏洞情报 re-attestation；完整服务端测试目标已编译，registry HTTP 全新库已执行其 migration，但 V250 专属行为仍未运行（`passed=0`）。它不运行扫描、不验证 intelligence 真实性或完整性，也不刷新 V239 sandbox。V249/V251 不删除安装字节、不读取 credential、不启动进程、不激活 Provider、不创建 v213 route、worker/ACK 或付款。生产库原位升级、V250 专项运行验收、V252 sandbox re-attestation、fresh credential、同事务 Provider activation/actor/route、生产 Adapter、Runner、派发、计量结算、生产 TCP/部署/MCP/PC 仍缺。见 `external-pool-adapter-vulnerability-reattestation-authority.md`、`external-pool-adapter-registry-authority.md` 与对应验收文档。
