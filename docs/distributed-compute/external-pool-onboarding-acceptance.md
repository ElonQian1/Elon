...
该证据只证明 Store 元数据和 Provider 注册原子性。它不验证 owner/reviewer 的真实登录角色、不读取 credential、不连接外部矿池、不签发 v213 route，也不创建容量、Offer、任务或结算。
...
- service/API 的 owner/admin 角色证明、鉴权、脱敏和接口幂等；
...
下一步可以实现最窄 service/API 权限闭环；不能因为 Provider 已登记为 `registering` 就把它升级为 active、可报价或可派发。
