...
fn derives_stable_client_request_id_from_trace() {
    ...
    let id = project_client_request_id(&request, "project", "user", "conversation", "build apk");
...
fn derives_fallback_client_request_id_when_trace_missing() {
    ...
    let first = project_client_request_id(&request, "project", "user", "conversation", "build apk");
    let second =
