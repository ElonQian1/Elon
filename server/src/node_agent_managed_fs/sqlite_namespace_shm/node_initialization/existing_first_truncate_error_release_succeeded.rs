//! Q15 production initialization branch: existing-first unread truncate receipt, known release.
...
impl ManagedSqliteShmCoordinator {
    pub(super) fn execute_q15_truncate_release_ok_test_v1(
        &self,
        ...
        if !self.begin_test_initialization_existing_first_truncate_outcome_unavailable_v1(
            state,
            connection_id,
        )? {
            return Ok(file);
        ...
        let platform_receipt = match file.truncate_outcome_unavailable_for_initialization_test_v1(0)
        {
            ...
            Ok(_) | Err(_) => {
                return Err(self.retain_after_q15_controller_rejection(
                    state,
                    connection_id,
                    file,
                    ...
                    false,
                    "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q15_TRUNCATE_SEAM_FAILED",
                ));
            }
        };
        ...
        if let Err(failure) = self
            .record_test_initialization_existing_first_truncate_receipt_unavailable_v1(
                state,
                connection_id,
                native,
            )
        ...
        if let Err(failure) = self
            .begin_test_initialization_existing_first_truncate_cleanup_unlock_v1(
                state,
                connection_id,
            )
        {
        ...
        if platform::unlock_sqlite_byte_range(&file.file, SHM_DMS_OFFSET, 1).is_err() {
            return Err(self.retain_after_q15_controller_rejection(
                ...
                true,
                "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q15_CLEANUP_UNLOCK_FAILED",
            ));
        }
        if let Err(failure) = self
            .record_test_initialization_existing_first_truncate_cleanup_unlock_succeeded_v1(
                state,
                connection_id,
            )
        {
    ...
    fn retain_after_q15_controller_rejection(
        &self,
        state: &mut ManagedSqliteShmCoordinatorState,
        connection_id: u64,
