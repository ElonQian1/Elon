...
| CapacityPool 本人控制面 | HTTP/MCP 已可在本人 Provider 下登记、读取、列出和审计 `registering` Pool，并按稳定序号分页读取脱敏账本历史；PC `/compute-supply` 已写入列表和登记源码。审计健康不等于硬件 verified，历史省略消费者和业务因果字段；尚未编译和运行验证 |
