...
use super::{
    ...
    projection::CandidateHealthQuarantineProjection,
};

pub(super) fn update_quarantine_authority_meta(
    ...
    let updated = transaction
        .execute(
            ...
            WHERE singleton = 1
              ...
              AND trusted_time_high_water_ms = :failed_at
              AND clock_status = 'trusted' AND updated_at_ms = :failed_at"#,
            named_params! {
        ...
        .context("COMPUTE_PLUGIN_CANDIDATE_QUARANTINE_AUTHORITY_UPDATE")?;
    if updated != 1 {
        bail!("COMPUTE_PLUGIN_CANDIDATE_QUARANTINE_AUTHORITY_CAS");
    }
    Ok(())
}
