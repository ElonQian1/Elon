...
主 APK 1.1.1571（源码42d055b668716133d2a49a09332e3fee40bc417d）与量化0.5.0
（源码9af152df993a53658456ad6d23fb19229f0010ea）均已正式签名并安装，实际base.apk
SHA-256与发布文件一致。真机已确认量化创建入口、主 APK 原生创建页和禁用的最终提交按钮。
主 APK MCP确认当前是子账户、列表已验证、创建确认页已打开，MCP交易权限仍为false。

设备随后锁屏，合成参数的只读准备尚未验收，不将界面可打开写成API准备成功。
1571是首次安装证据；独占窗口、受理刷新及提交前账号补强的最终安装版本另随发布回执记录。
真实创建仍待用户本人测试。正式回执与手机检查保存于发布验收产物，不以合成网络测试替代。
