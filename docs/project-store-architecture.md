...
3. 项目广场支持搜索、筛选、已加入、最热门、加载更多；内置官方项目还可以在加入前打开净化详情预览。
...
6. 官方项目预览：`GET /api/store/projects/:id/preview` 从 Server 内置官方目录投影 V1 白名单字段；普通用户项目 landing 不通过此接口公开，下载 URL、资源 URL 和项目私有状态不进入响应。
...
- 官方目录详情预览必须是加入前只读能力；打开、重试和关闭预览不能隐式加入项目或调用业务写接口。
