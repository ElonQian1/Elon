...
| 商户数据自主控制和跨应用授权 | 部分实现 | V1、跨项目脱敏目录、消费者沙盒、限时 App 授权、消费者匿名关系及低敏偏好字段披露、删除请求与商户声明、关系、偏好、披露及调用凭证的可验证导出、调用配额、App 终态结果流、商户业务证据、活动证据、手动 App 封禁和首个商户自有运行时已打通；敏感数据保险箱、完整订单迁移、跨运营方导入、外部主动推送和删除证明、证据到 ERP/CRM 的生产适配器、生产 App 身份互认、自动全网风控与公共互操作治理尚未完成 |
