...
| 消费者发现与第三方应用沙盒 | 已实现，排序器、来源、筛选与凭证新增批次未编译 | 可注册、轮换、停用和重新启用测试 App，一次性显示测试 Token，跨项目发现主动发布商户，查看或撤回申请并在审批后调用；App 身份绑定所有者。发现代码支持五种用户可选、可解释且非付费的内置排序器，显示商户声明来源、新鲜度和可选内部同步回执摘要，允许消费者独立要求声明期内或内部回执来源能力，也可按单个厂商和数据域精确筛选，并可显式生成经 PC 复核的未签名单次凭证；只作用于最多 100 个目录候选，来源固定未外部核验，不是全网公平性、数据真实性、外部平台背书或运营方身份的证明 | `server/src/open_commerce_client_api.rs`、`server/src/open_commerce_client_lifecycle_service.rs`、`server/src/open_commerce_directory_model.rs`、`server/src/open_commerce_consumer_ranking.rs`、`server/src/open_commerce_consumer.rs`、`pc-frontend/src/features/open-commerce/consumerRankingReceipt.ts`、`docs/decisions/open-commerce-pluggable-ranking-v1.md`、`docs/decisions/open-commerce-public-data-provenance-v1.md`、`docs/decisions/open-commerce-capability-source-link-v1.md`、`docs/decisions/open-commerce-consumer-source-requirement-v1.md`、`docs/decisions/open-commerce-consumer-source-filters-v1.md`、`docs/decisions/open-commerce-consumer-freshness-filter-v1.md`、`docs/decisions/open-commerce-consumer-ranking-receipts-v1.md`、`docs/open-commerce/consumer-developer-sandbox.md` |
