...
v177-v178 另行保存 Provider/Pool 激活证据申请及过期批准废止审计。本人 HTTP/MCP 可显式提交、查询和取消，平台 `admin/owner` 只能通过 HTTP 审核或把失效 `approved` 显式转为 `superseded`；双方均可只读预检申请状态、路由、verified 硬件、信任层、精确版本和账本审计并获得稳定阻断码。废止保留原审核并释放重新申请通道；`approved` 与 `ready_for_activation` 都固定返回 `activation_effect=none`，不触发激活、verified 硬件写入、路由配置或 Offer 发布。该控制面为 `implementation_uncompiled`，详细边界见 `docs/distributed-compute/activation-evidence-api.md`。
