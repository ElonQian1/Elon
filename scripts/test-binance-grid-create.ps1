...
if ($AdapterOnly) {
    Write-Output 'BINANCE_ADAPTER_SUITES_PASSED=12 NATIVE_TESTS_EXECUTED=false TRADING_REQUESTS_EXECUTED=false'
    return
}
...
try {
    ...
    & (Join-Path $root 'android/gradlew.bat') -p (Join-Path $root 'android') --no-daemon --max-workers=1 :app:testDebugUnitTest `
        ...
        --tests com.elon.app.grid.create.BinanceReferenceContractTest `
        --tests com.elon.app.grid.create.BinanceReferenceEconomicsTest `
