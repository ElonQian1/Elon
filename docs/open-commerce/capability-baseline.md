...
| 能力与内部同步回执来源绑定 | 代码已形成、未编译 | 编辑者可把当前能力版本绑定到同项目、同商户、同接入的合格业务同步回执和已登记数据域；目录公开厂商、方式、状态、时间与 SHA-256，能力更新或接入停用后失效。所有字段仍是商户项目内部声明，不是外部平台签名或真实回读 | `server/src/open_commerce_capability_source_*.rs`、`server/src/store/open_commerce_capability_sources.rs`、`docs/decisions/open-commerce-capability-source-link-v1.md` |
