...
fn register_and_claim_dispatch_target() {
    ...
    let claim = claim_decision_target("project", "channel", task_id, "tap_1_1", "approve").unwrap();
...
fn claim_prevents_conflicting_concurrent_decision() {
    ...
    let err = claim_decision_target("project", "channel", task_id, "tap_1_1", "deny").unwrap_err();
...
fn duplicate_decision_is_idempotent_after_dispatch_success() {
    ...
    assert!(claim_decision_target("project", "channel", task_id, "tap_1_1", "approved").is_ok());
    assert!(claim_decision_target("project", "channel", task_id, "tap_1_1", "approve").is_err());
