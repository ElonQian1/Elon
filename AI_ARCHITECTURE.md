...
v177 另行保存 Provider/Pool 激活证据申请。本人 HTTP/MCP 可显式提交、查询和取消，平台 `admin/owner` 只能通过 HTTP 审核；提交锁定当前 Provider/Pool 版本和稳定账本审计摘要，批准前重新复核。`approved` 仍固定返回 `activation_effect=none`，不触发激活、verified 硬件写入、路由配置或 Offer 发布。该控制面为 `implementation_uncompiled`，详细边界见 `docs/distributed-compute/activation-evidence-api.md`。
