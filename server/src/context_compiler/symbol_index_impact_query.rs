...
fn load_seed_symbols(conn: &Connection, query: &SymbolImpactQuery) -> Result<Vec<SymbolHit>> {
...
fn load_symbol_by_id(conn: &Connection, symbol_id: &str) -> Result<Option<SymbolHit>> {
...
fn load_symbols_by_path(conn: &Connection, path: &str, limit: usize) -> Result<Vec<SymbolHit>> {
    let sql = format!(
...
fn load_symbols_by_ids(conn: &Connection, ids: &BTreeSet<String>) -> Result<Vec<SymbolHit>> {
    if ids.is_empty() {
        return Ok(Vec::new());
    }
...
fn load_edge_path_symbols(conn: &Connection, edges: &[SymbolEdgeHit]) -> Result<Vec<SymbolHit>> {
    let ids = edges
        .iter()
...
fn collect_impacted_ids(seed_ids: &BTreeSet<String>, edges: &[SymbolEdgeHit]) -> BTreeSet<String> {
    edges
        .iter()
        .flat_map(|edge| [edge.from_symbol_id.as_deref(), edge.to_symbol_id.as_deref()])
        .flatten()
...
fn looks_like_test(symbol: &SymbolHit) -> bool {
...
fn sort_symbols(symbols: &mut [SymbolHit]) {
