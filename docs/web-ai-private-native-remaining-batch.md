...
| Text submit/stream/stop/follow-up | Official runtime send path verified, reused | 1640 passed private model admission (6/6 handles in two documents), but native retry stopped before dispatch at `semantic_control_missing`. Shared native admission removes the mapper/command DOM gates; 41 Android and 79 runtime tests pass. [Normal 1641 is published/installed](chatgpt-official-runtime-regeneration.md#grouped-normal-release-1641); native retry acceptance stopped before navigation/write at the locked-screen precondition and awaits unlock. Earlier post-dispatch `timeout_owner_changed`, independent Android POST and other contexts remain unresolved |
