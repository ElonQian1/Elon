...
export function SourcePreviewModeBar(props: Props) {
  ...
  return (
    <header className={styles.modeBar}>
      <div className={styles.modeBarContextRow}>
          ...
          <div className={styles.projectContext}>
            <label className={styles.projectField}>
              ...
              <input
                ...
                onChange={(event) => props.onProjectRootChange(event.target.value)}
                placeholder="本机 Android 项目目录"
              />
            </label>
            {props.document && (
              <select
                ...
                aria-label="当前布局文件"
                title={props.document.selectedLayout}
                value={props.document.selectedLayout}
                onChange={(event) => props.onLoad(event.target.value)}
              >
                {props.document.layoutFiles.map((file) => {
                  const parts = file.split('/')
                  return <option key={file} value={file}>{parts[parts.length - 1]}</option>
                })}
              </select>
            )}
            <button onClick={() => props.onLoad()} disabled={props.loading}>
              {props.loading ? <LoaderCircle className={styles.spin} size={15} /> : <Code2 size={15} />}
              加载源码
            </button>
          </div>
        )}
      ...
      {props.mode === 'source' && (
        <div className={styles.modeBarActionRow}>
          <div className={styles.rendererControls}>
            ...
            {props.renderer.capabilities?.composePreviews.length ? (
              <select aria-label="Compose Preview" value={props.renderer.selectedPreviewId} onChange={(event) => { void props.renderer.rerender(props.projectRoot, event.target.value) }}>
                {props.renderer.capabilities.composePreviews.map((preview) => <option key={preview.id} value={preview.id}>{preview.composable}</option>)}
              </select>
            ) : null}
          ...
          <div className={styles.editActions}>
            ...
            <div className={styles.buttonGroup}>
              <button aria-label="撤销动态修改" onClick={props.onUndo} disabled={!props.canUndo}><Undo2 size={15} /></button>
              <button aria-label="重做动态修改" onClick={props.onRedo} disabled={!props.canRedo}><Redo2 size={15} /></button>
            </div>
            <div className={styles.zoomGroup}>
              <button aria-label="缩小动态画布" onClick={() => props.onZoom(Math.max(.35, props.zoom - .1))}><ZoomOut size={15} /></button>
              <span>{Math.round(props.zoom * 100)}%</span>
              <button aria-label="放大动态画布" onClick={() => props.onZoom(Math.min(1.5, props.zoom + .1))}><ZoomIn size={15} /></button>
            </div>
