...
协议阈值 12 的 Planning Snapshot V2 仍是独立、默认阻断的能力。请求只在同一认证 session 已耐久接收 V1 policy 与 v10 preparation ACK 后串行派发，并精确引用 session、v209 delivery 与 observation digest。work-admission `None` 只表达新晋升、candidate-only 或历史 head 与 active release/authority 失配，`Some` 才承诺 exact current head；不得伪造 generation 1。当前没有 authority snapshot producer，始终返回 `snapshot_ready=false`、`snapshot=None` 和全部副作用标志 false；它不是签名器、计划投递、本机确认或 PlanApply。
...
这只是 PlanApply live-binding 的必要前置，不是可用 planning context。A1 sealed handle-bound、同一只读事务 projector 源码已随完整测试目标编译，但尚无专项运行或 producer：它从同一个 already-opened connection 闭合 policy/revocation、catalog/keyring、inventory/profile、installed/promotion、可选 exact work-admission 与 rollback，输出不可复制的本机 custody；不使用 legacy path facade、测试 VFS、多事务缓存或裸标量。生产 open/VFS/process fence/root/trusted-time/rollback/node-profile 均不可达。严格依赖顺序与禁线见 [`node-plugin-planning-snapshot-authority.md`](node-plugin-planning-snapshot-authority.md)，VFS/目录细节见 [`node-plugin-manifest-catalog-authority.md`](node-plugin-manifest-catalog-authority.md)。当前继续保持零 PlanApply、零下载、零安装、零 Sidecar。

A2 总合同已冻结；A2b2/A2c 所在完整测试目标已可编译，targeted fault matrix 已通过 5 项，但 117 项逐 case `WindowsDynamic` 仍为 0，宽范围回归也未通过，所以状态只能是 `implementation_not_dynamically_accepted`。精确矩阵见 [`A2 authority`](node-plugin-vfs-fault-authority.md) 与 [`acceptance`](node-plugin-vfs-fault-acceptance.md)，测试 VFS 不得提升为生产入口。
...
本文是已接受的客户端目标设计。NodeAgent legacy Host seam、Manifest/InstallPlan、本机 authority/PlanApply、受管 fetch/verification/staging、健康与失败 cleanup 合同已形成代码；v216 的 v7 installed/promotion 增量已随节点编译并通过 11 项版本链测试。v217/v218 的 v8 work-admission、authenticated PlanApply barrier、retained-handle fresh rehash 与 Planning A1 已随完整测试目标编译，但尚无专项事务测试、迁移、运行或生产调用；A2 同一目标已有 5 项 targeted fault matrix 通过，但 A2b2 的 117 项 Windows dynamic evidence仍为 0。生产 open/VFS、磁盘迁移、完整事务夹具、生产时间源、downloader、跨重启 custody、Sidecar/IPC/沙箱、Host enforcement、Runtime、Ready 和调度仍不可达；节点不直接发布商业 Offer。精确边界见 `node-plugin-archive-extraction.md`、`node-ready-capability.md`、`node-plugin-candidate-cleanup.md`、`node-plugin-vfs-fault-authority.md` 与 `node-plugin-vfs-fault-acceptance.md`。
