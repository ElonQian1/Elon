...
A2c 严格 `cfg(all(test, windows))` 的进程隔离 runner 现覆盖 Barrier、`RegistrationShutdown` 各八项与 RegistryLifecycle 十六项；parent 对 exact selector/81 字段、child exit、环境绑定和测试根删除收据做 fail-closed 验证。exact clean evidence commit `95d910f0dbc167138f913861efafa20ff11295cc` 上三组分别为 `8/8`、`8/8`、`16/16`，A2b2=`32/117`、剩余 85，宽回归=`142/142`。RegistryLifecycle 的 raw `xClose`、route ledger、拒绝 seam 与逐项指纹只由其[`动态权威`](node-plugin-vfs-registry-lifecycle-dynamic-authority.md)和[`动态验收`](node-plugin-vfs-fault-acceptance.md)解释；A2 仍是 `implementation_not_dynamically_accepted`。

A2b1 current source baseline `a75769029ba4abf5e30002f64846c0f7099d9ae7` 的 owner graph 已加入 `AbiRawCloseWitness`，并在上述 evidence commit 通过 `4/4`；state-abandon witness 不改变 fallback、Pending=5 或 resolved cross-links=9，也不开放 map/lock 动态验收。指纹与复测元组见[`动态验收 § 当前 A2b1 静态守卫证据`](node-plugin-vfs-fault-acceptance.md#当前-a2b1-静态守卫证据)。
