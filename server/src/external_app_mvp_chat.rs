...
fn build_messages(app_id: &str, user_message: &str, req: &ExternalAppMvpChatRequest) -> Vec<Value> {
...
fn system_prompt(app_id: &str) -> &'static str {
    if app_id == "bb64a" {
...
fn project_history_message(message: &ExternalAppMvpChatMessage) -> Option<Value> {
...
fn suggest_tools(app_id: &str, message: &str, local_context: &Value) -> Vec<SuggestedTool> {
    if app_id != "bb64a" {
        return Vec::new();
    }
...
fn push_tool(tools: &mut Vec<SuggestedTool>, tool: SuggestedTool) {
