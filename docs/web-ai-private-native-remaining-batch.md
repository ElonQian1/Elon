...
| Text submit/stream/stop/follow-up | Official runtime send path verified, reused | Retry v8 on 1639 used legacy DOM; private acceptance remains failed. Fresh generation-6 diagnostics now prove an unrecognized second September 10 build. Resolver 11 / adapter 325 repairs its exact mappings (132 focused + 304 adjacent checks), pending grouped install/acceptance; [evidence](reports/chatgpt-runtime-bindings-20260910-b.md). Earlier post-dispatch `timeout_owner_changed` is not explained by that pre-invocation finding. Independent Android POST and other contexts remain separate |
...
| Explicit file citations / remaining cloud references | Scoped metadata/download correction is installed on 1639. Source `7b78394da` adds concrete file-URL identity and empty-context handling; 216 checks pass. Adapter 324 awaits grouped build/install and production citation download acceptance | [Boundary and source evidence](chatgpt-private-file-citations.md). URL-only cloud without a ChatGPT ID and nonempty PCA graphs remain incomplete |
