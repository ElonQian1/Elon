...
| Text submit/stream/stop/follow-up | Official runtime path verified, reused | Not independent Android HTTP POST. Native retry on 1638 returned `timeout_owner_changed`; readonly reopen found changed text in the same native row. Regeneration v8 now distinguishes ownership components, passes 141 checks; actual retry acceptance remains open. [Current evidence](reports/chatgpt-regeneration-observation-1637.md). Other contexts remain separate |
