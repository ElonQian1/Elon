...
| Text submit/stream/stop/follow-up | [Independent page-owned HTTP](chatgpt-fresh-text-dispatch.md) completed/default-enabled for existing personal plain text on 1696: first/follow-up, handed-off stream, stop and default routing accepted. Runtime send retained for other scopes; native retry accepted on 1644 | Reuse [native retry](reports/chatgpt-regeneration-store-20260911.md#normal-1644-acceptance). Stopped-turn continuation/lifetime receipts follow-on implemented, grouped acceptance pending. New/tool/file/project/temporary scopes, initial composer-free ownership and real network-loss recovery remain |
