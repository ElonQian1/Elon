...
| Model/effort/tools/temporary mode | Native presets/cache, [search/image private state](chatgpt-private-composer-tools.md), [model/effort/version/tier](chatgpt-private-model-state.md), [normal-chat additional model catalog](chatgpt-private-model-catalog.md), and [temporary-chat transaction](chatgpt-private-temporary-chat.md) implemented in source | Grouped Android/native tests and device acceptance remain. Additional catalog/paging/privacy guards passed 125 related Node cases; work-mode promoted model discovery and remaining temporary contexts are still separate gaps. Cached labels/local readback do not prove asynchronous server preference persistence; these callbacks are not independent generation/privacy POSTs. |
