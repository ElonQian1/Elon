//! Independent typed fixtures for the 88 q19 existing-first shared-busy close-ok terminals.
...
pub(super) const LOCK_EXISTING_FIRST_SHARED_BUSY_CLOSE_SUCCEEDED_MEMBER_COUNT: usize = 88;

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub(super) enum FrozenExistingFirstSharedBusyCloseSucceededCompletionV1 {
    RetentionSucceeded,
    RetentionRouteUnknown,
}

impl FrozenExistingFirstSharedBusyCloseSucceededCompletionV1 {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub(super) struct FrozenLockExistingFirstSharedBusyCloseSucceededCaseV1 {
    pub(super) action: LockActionV1,
    pub(super) first: u8,
    pub(super) count: u8,
    pub(super) mask: u8,
    pub(super) completion: FrozenExistingFirstSharedBusyCloseSucceededCompletionV1,
}

#[derive(Clone)]
pub(super) struct FrozenLockExistingFirstSharedBusyCloseSucceededLeafV1 {
    ...
    LEAVES.get_or_init(|| {
        ...
        super::super::super::source_leaf_authority::validate_lock_graph_with_records(
            ...
            |leaf| {
                ...
                let Some(case) = existing_first_shared_busy_close_succeeded_v1(record, descriptor)
                else {
                    return Ok(());
                };
                ...
                if previous.is_some() {
                    return Err(format!("duplicate frozen q19 member {case:?}"));
                }
                Ok(())
            },
        )
        .expect("the frozen Lock authority must validate before q19 source tests");
        assert_eq!(
            leaves.len(),
...
pub(super) fn lock_existing_first_shared_busy_close_succeeded_descriptor_v1(
    case: FrozenLockExistingFirstSharedBusyCloseSucceededCaseV1,
    capability: RunnerCapabilityV1,
) -> TerminalDescriptorV1 {
    let mut descriptor =
        frozen_lock_existing_first_shared_busy_close_succeeded_leaves_v1()[&case].descriptor;
    let TerminalDescriptorV1::Lock(value) = &mut descriptor else {
        unreachable!("a q19 fixture must have a Lock descriptor")
    };
    value.recipe.capability = capability;
    descriptor
}

pub(super) fn lock_existing_first_shared_busy_close_succeeded_expected_groups_v1(
        ...
        .map(|leaf| {
            (
                prepare_dynamic_terminal_v1(&leaf.record, &leaf.descriptor)
                    .expect("a frozen q19 descriptor must prepare")
                    .key,
                leaf.member,
            )
        })
...
fn existing_first_shared_busy_close_succeeded_v1(
    record: &LeafRecordV1,
    descriptor: &TerminalDescriptorV1,
) -> Option<FrozenLockExistingFirstSharedBusyCloseSucceededCaseV1> {
    ...
    let completion = match completion {
        LockCompletionV1::UnsafeRetentionSucceededThenRouteUnknown => {
            FrozenExistingFirstSharedBusyCloseSucceededCompletionV1::RetentionSucceeded
        }
        LockCompletionV1::UnsafeRetentionRouteUnknownThenRouteUnknown => {
            FrozenExistingFirstSharedBusyCloseSucceededCompletionV1::RetentionRouteUnknown
        }
        _ => return None,
    };
    let case = FrozenLockExistingFirstSharedBusyCloseSucceededCaseV1 {
        action,
        first,
        count,
        mask,
...
#[test]
fn frozen_q19_family_is_exact_unique_and_only_contains_acquire_requests() {
