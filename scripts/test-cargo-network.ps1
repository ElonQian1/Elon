...
if($scenario -eq 'test_failure_stderr'){Write-Output '';Write-Error 'test node_agent_failure_stderr ... FAILED' -ErrorAction Continue;Write-Error 'test result: FAILED. 1 passed; 1 failed' -ErrorAction Continue;Write-Error 'error: test failed, to rerun pass `--bin elon-pc-node`' -ErrorAction Continue;exit 101}
...
'@|Set-Content -LiteralPath $fake -Encoding UTF8
    ...
    Assert-Equal 'CARGO_TEST_FAILURE' $result.diagnostic.code 'test failure must not be misclassified as proxy failure';Assert-True ($capturedText -match 'CARGO_ATTEMPT_LOG_BEGIN stage=locked_offline_check stream=stdout') 'stdout evidence must remain separate from stderr';Assert-True ($capturedText -match 'CARGO_ATTEMPT_TEST_FAILURE_CONTEXT_BEGIN stage=locked_offline_check stream=stdout') 'failed test context must reach outer validation logs';Assert-True ($capturedText -match 'CARGO_ATTEMPT_TEST_FAILURE_CONTEXT test node_agent_failure \.\.\. FAILED') 'failed test name must remain visible in context';Assert-True ($capturedText -match 'CARGO_ATTEMPT_LOG .*test result: FAILED') 'test failure tail must reach outer validation logs'
    Clear-Content $callLog;$env:ELON_TEST_CARGO_SCENARIO='test_failure_stderr';$captured=@(& { param([string[]]$capturedArguments) Invoke-CargoNetworkValidation $repoRoot $fake (Join-Path $temp 'test-failure-stderr') $capturedArguments $cache -DisableSccache -OverrideSources $sources -HttpInvoker $okInvoker } $args 6>&1);$result=@($captured|Where-Object { $_ -is [pscustomobject] -and $_.PSObject.Properties['exit_code'] }|Select-Object -Last 1);$capturedText=@($captured|ForEach-Object {if($_ -is [System.Management.Automation.InformationRecord]){[string]$_.MessageData}else{[string]$_}})-join "`n"
    Assert-Equal 'CARGO_TEST_FAILURE' $result.diagnostic.code 'stderr test failure must be classified';Assert-True ($capturedText -match 'CARGO_ATTEMPT_TEST_FAILURE_CONTEXT_BEGIN stage=locked_offline_check stream=stderr') 'blank stdout must not block stderr failure context';Assert-True ($capturedText -match 'CARGO_ATTEMPT_TEST_FAILURE_SUMMARY .*test node_agent_failure_stderr \.\.\. FAILED') 'wrapped stderr failed test name must remain visible in summary'
