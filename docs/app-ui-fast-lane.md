...
6. 用 `AndroidFeature` 执行统一收尾，并在最终报告中列出 Server、APK 两端发布结果以及“真机视觉验收未执行”。

## 退出快速通道

出现编译/契约失败、两端无法同步、截图问题无法从源码定位，或用户反馈真机显示不对时，才升级为针对性视觉验证。升级后只验证受影响页面；不要为最新主线重复第二次 Debug 真机安装。
