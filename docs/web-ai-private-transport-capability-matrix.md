...
| Independent native file library | ChatGPT | 1.1.1582: root/folder/cache/search and pagination data verified; rendered UI pending; binary download failed `download_source_unsupported`; [evidence](chatgpt-private-library-browser.md) | Retained rows on refresh failure; explicit official library, no automatic navigation |
...
| Native existing-library attachment | ChatGPT | Single ordinary-file reference into the existing composer/send owner; [scope and device-pending evidence](chatgpt-private-library-attachment.md) | Preserve draft; no reupload, automatic send or cross-context association |
