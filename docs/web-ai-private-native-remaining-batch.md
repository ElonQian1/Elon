...
| Independent native file library | Root/folder/search/cursor browsing, bounded cache and ordinary library-file downloads share the production provider and existing downloader | Targeted protocol/native tests; device pending | Source-only; [scope and remaining variants](chatgpt-private-library-browser.md) |
