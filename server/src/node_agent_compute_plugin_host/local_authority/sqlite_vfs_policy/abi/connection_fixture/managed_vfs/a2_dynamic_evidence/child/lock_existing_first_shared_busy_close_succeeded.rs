//! Canonical q19 child header for ExistingFirst DMS shared-busy close-success receipts.

pub(in super::super) const REPORT_VERSION: &str = "a2lockq19";
pub(in super::super) const REPORT_VALUE_COUNT: usize = 194;
...
pub(in super::super) fn selector(
    ...
    Ok(format!(
        "initialization-{action}-first-{first}-count-{count}-existing-first-shared-busy-close-succeeded-{completion}-terminal-route-unknown"
    ))
}
...
mod tests {
    ...
    #[test]
    fn eighty_eight_q19_selectors_are_unique_and_fail_closed() {
