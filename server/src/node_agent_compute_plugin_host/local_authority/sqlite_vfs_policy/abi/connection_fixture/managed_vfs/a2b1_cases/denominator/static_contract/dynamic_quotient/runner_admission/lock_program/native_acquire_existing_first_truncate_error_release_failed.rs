//! Exact q17 source programs for existing-first DMS truncate failure with uncertain cleanup release.
//!
...
pub(super) use source_scope::NATIVE_ACQUIRE_EXISTING_FIRST_TRUNCATE_ERROR_RELEASE_FAILED_PROJECTOR_DELTA_V1;
...
pub(super) const EXISTING_FIRST_TRUNCATE_ERROR_RELEASE_FAILED_MEMBER_COUNT: usize = 88;

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(super) enum LockExistingFirstTruncateErrorReleaseFailedCompletionV1 {
    RetentionSucceeded,
    RetentionRouteUnknown,
}

impl LockExistingFirstTruncateErrorReleaseFailedCompletionV1 {
...
#[derive(Clone, Copy)]
pub(super) struct LockNativeAcquireExistingFirstTruncateErrorReleaseFailedProgramSpecV1 {
    #[cfg(windows)]
    ...
    pub(super) mask: u8,
    #[cfg(windows)]
    pub(super) completion: LockExistingFirstTruncateErrorReleaseFailedCompletionV1,
...
> {
    ...
    if key.schema_version != DYNAMIC_PROJECTOR_SCHEMA_V1
        ...
        || key.phase != PhaseV1::DmsExclusiveRelease
        || key.timing != TimingV1::Cleanup
...
) -> Option<LockExistingFirstTruncateErrorReleaseFailedCompletionV1> {
    match value {
        ReachabilityV1::Reached(LockCompletionV1::UnsafeRetentionSucceededThenRouteUnknown) => {
            Some(LockExistingFirstTruncateErrorReleaseFailedCompletionV1::RetentionSucceeded)
        }
        ReachabilityV1::Reached(LockCompletionV1::UnsafeRetentionRouteUnknownThenRouteUnknown) => {
            Some(LockExistingFirstTruncateErrorReleaseFailedCompletionV1::RetentionRouteUnknown)
        }
        _ => None,
    }
}
