...
V254已用18个具名数据库trigger覆盖external-pool Provider/projection/route/CapacityPool/Offer与direct SQL；它不是runtime readiness证明。V277 docs-first只允许#1/#5-#12九个由同connection non-deterministic pending-plan UDF精确放行；#2-#4/#13-#18九个继续absolute deny，尤其active Provider INSERT和全部market写永不因V277放行。不得复用公开preflight receipt、先写Provider active再补route/runtime、把v213 seal冒充runtime readiness、只在HTTP层检查或删除trigger。V277分支当前严格`source_review_only / implementation_uncompiled / implementation_unrun / passed=0 / failed=0`；V254历史10项证据不覆盖pending-plan/direct-SQL/restart/原子回滚矩阵。
