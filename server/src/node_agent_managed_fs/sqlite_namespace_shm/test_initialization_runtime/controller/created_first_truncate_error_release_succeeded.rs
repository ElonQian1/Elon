//! Case-specific Q14 controller path for an unread truncate receipt and known cleanup release.
...
use super::{
    advance, fail, ordered_values as release_ordered_values,
    validate_exact_counts as validate_release_counts,
    validate_terminal as validate_release_terminal, ArmedInitializationObservationV1, EventCounts,
    ExactTarget, ManagedSqliteShmTestInitializationControllerV1, Stage, TerminalStateV1,
};

impl ManagedSqliteShmTestInitializationControllerV1 {
    pub(super) fn reject_q14_truncate_success_if_selected(
        &mut self,
        target: ExactTarget,
    ) -> Result<(), &'static str> {
        ...
        if is_q14(active) {
            return fail(
                active,
                "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q14_TRUNCATE_SUCCESS_FORBIDDEN",
            );
        }
        Ok(())
    }

    pub(super) fn reject_q14_release_path_if_selected(
        &mut self,
        target: ExactTarget,
    ) -> Result<(), &'static str> {
        let Some(active) = self.active_for_event(target)? else {
        ...
        if is_q14(active) {
            return fail(
                active,
                "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q14_RELEASE_PATH_INVALID",
            );
        }
        Ok(())
    }

    pub(super) fn begin_created_first_truncate_outcome_unavailable(
        &mut self,
        target: ExactTarget,
            ...
            || active.consumed
        {
            return fail(
                active,
                "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q14_TRUNCATE_SEQUENCE_INVALID",
            );
        }
        Ok(true)
    }

    pub(super) fn record_created_first_truncate_return_receipt_unavailable(
        &mut self,
        target: ExactTarget,
            ...
            || active.native.is_some()
        {
            return fail(
                active,
                "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q14_NATIVE_RECEIPT_INVALID",
            );
        }
        advance(
            active,
            ...
            "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q14_NATIVE_RECEIPT_SEQUENCE_INVALID",
        )?;
        active.native = Some(native);
        Ok(())
    }

    pub(super) fn begin_created_first_truncate_cleanup_unlock(
        &mut self,
        target: ExactTarget,
    ) -> Result<(), &'static str> {
        let active = self.require_active_for_event(target)?;
            ...
            != ManagedSqliteShmTestInitializationFailureV1::CreatedFirstTruncateOutcomeUncertainReleaseSucceeded
        {
            return fail(
                active,
                "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q14_CLEANUP_CASE_INVALID",
            );
        }
        advance(
            active,
            ...
            "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q14_CLEANUP_ATTEMPT_SEQUENCE_INVALID",
        )
    }

    pub(super) fn record_created_first_truncate_cleanup_unlock_succeeded(
        &mut self,
        target: ExactTarget,
    ) -> Result<(), &'static str> {
        let active = self.require_active_for_event(target)?;
        ...
        advance(
            ...
            "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q14_CLEANUP_OUTCOME_SEQUENCE_INVALID",
        )?;
        active.pending = 0;
...
pub(super) fn reject_release_receipt_if_selected(
    ...
    if is_q14(active) {
        return fail(
            active,
            "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q14_RELEASE_RECEIPT_INVALID",
        );
    }
    Ok(())
}
...
pub(super) fn validate_finish(
    ...
    if is_q14(active) {
        ...
        validate_release_counts(&active.counts)?;
        validate_release_terminal(terminal)
    }
}

pub(super) fn ordered_values_for_case(
    active: &ArmedInitializationObservationV1,
    native: ManagedSqliteShmTestInitializationNativeReceiptV1,
    terminal: TerminalStateV1,
) -> [u64; 32] {
    if is_q14(active) {
        ordered_values(active, native, terminal)
    } else {
        release_ordered_values(active, native, terminal)
    }
}

fn is_q14(active: &ArmedInitializationObservationV1) -> bool {
...
pub(super) fn record_poisoned(
    ...
    advance(
        ...
        "NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q14_POISON_SEQUENCE_INVALID",
    )
}

pub(super) fn validate_exact_counts(counts: &EventCounts) -> Result<(), &'static str> {
    if counts.request != 1
        ...
        || counts.poisoned != 1
    {
        return Err("NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q14_EVENT_COUNTS_INVALID");
    }
    Ok(())
}

pub(super) fn validate_terminal(terminal: TerminalStateV1) -> Result<(), &'static str> {
    ...
    {
        return Err("NODE_MANAGED_SQLITE_SHM_TEST_INITIALIZATION_Q14_TERMINAL_STATE_INVALID");
    }
    Ok(())
}
