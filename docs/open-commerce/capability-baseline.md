...
| 消费者能力执行计划 | 代码已形成、未编译 | 用实际输入只读复核能力契约、App、封禁和授权状态，并按下一次调用的次数、价格和币种遍历选择可用 Grant；全部不可用时返回稳定预算原因和续额下一步。动作明确拆成准备、用户确认、确认、调用。计划不占预算、不创建业务记录 | `server/src/open_commerce_consumer_execution_plan.rs`、`server/src/open_commerce_grant_readiness.rs`、`server/src/open_commerce_consumer_discovery_mcp.rs`、`docs/decisions/open-commerce-consumer-capability-execution-plan-v1.md` |
