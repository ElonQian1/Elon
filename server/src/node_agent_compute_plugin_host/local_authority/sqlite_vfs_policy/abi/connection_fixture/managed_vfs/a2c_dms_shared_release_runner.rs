//! Windows-only process-isolated direct xShmUnmap DmsSharedRelease physical-subset runner.
//!
...
const CHILD_ROOT_ENV: &str = "ELON_SQLITE_A2C_DMS_SHARED_RELEASE_CHILD_ROOT";
const EXACT_TEST: &str = "node_agent_compute_plugin_host::local_authority::sqlite_vfs_policy::abi::connection_fixture::managed_vfs::a2c_dms_shared_release_runner::final_keep_dms_shared_release_after_success_physical_subset";

#[test]
fn final_keep_dms_shared_release_after_success_physical_subset() -> anyhow::Result<()> {
    ...
    let root = std::env::temp_dir().join(format!(
        "elon-managed-vfs-a2c-dms-shared-release-{}-{}",
        std::process::id(),
    ...
    let status = Command::new(std::env::current_exe().context("resolve current test executable")?)
        ...
        .status()
        .context("run isolated direct xShmUnmap DmsSharedRelease fault child")?;
    let cleanup = fs::remove_dir_all(&root).with_context(|| {
        format!(
            "remove DmsSharedRelease child namespace after process exit at {}",
            root.display()
        )
    });
    assert!(status.success(), "DmsSharedRelease fault child must pass");
    cleanup?;
    Ok(())
}
...
fn exercise_child(root: &Path) -> anyhow::Result<()> {
    ...
    fixture.connection().execute_batch(
        "CREATE TABLE a2c_dms_shared_release_probe (
    ...
    fixture.connection().execute(
        "INSERT INTO a2c_dms_shared_release_probe(probe_id, value) VALUES (1, 74)",
        [],
    )?;
    ...
    // DMS release has already succeeded when this seam fires. Retain before any fallible post
    ...
    let triggered_after = witness
        .was_triggered(phase, 1)
        .map_err(anyhow::Error::msg)?;
    validate_dms_shared_release_after_success_physical_subset(
        DmsSharedReleasePhysicalSubsetActual {
    ...
    assert!(
        root.is_dir(),
        "poisoned DmsSharedRelease child keeps its root"
    );
    Ok(())
}
