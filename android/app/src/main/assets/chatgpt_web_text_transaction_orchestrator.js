(function () {
  ...
  function create(options) {
    ...
    function regenerateResponse(respond, fallback) {
      ...
      options.scheduleSnapshot(true);
      Promise.resolve(transaction.completion).then(receipt => {
        if (receipt?.status === 'unavailable' && transaction.claimFallback?.() === true) return existing();
        const accepted = receipt?.status === 'accepted';
        respond('regenerate_response', accepted, accepted ? 'private_text_v1:regenerate_accepted' :
          (receipt?.status === 'rejected' || receipt?.status === 'unavailable'
            ? 'official_runtime_v1:regenerate_rejected:' : 'private_text_v1:regenerate_unknown:') + safeCode(receipt?.code, 'unknown'));
        options.scheduleSnapshot(true);
      }).catch(() => {
        respond('regenerate_response', false, 'private_text_v1:regenerate_unknown:completion_failed');
        options.scheduleSnapshot(true);
      });
    }
    ...
    function stopGeneration(respond, fallback) {
      ...
      if (fresh?.state?.().pending) {
        const transaction = fresh.stop?.();
        Promise.resolve(transaction?.completion).then(receipt => {
          const accepted = receipt?.status === 'accepted';
          if (accepted) privateStreamTransport?.finishPrivateSend?.();
          respond('stop_generation', accepted, accepted ? 'private_text_v1:stopped' :
            'private_text_v1:unknown:' + safeCode(receipt?.code, 'server_stop_unconfirmed'));
          options.scheduleSnapshot(true);
        }).catch(() => respond('stop_generation', false, 'private_text_v1:unknown:server_stop_unconfirmed'));
        options.scheduleSnapshot(true);
        return;
      }
