...
if ($IsLinux -or $IsMacOS) {
    ...
    chmod +x .githooks/post-commit 2>$null
    chmod +x .githooks/pre-push 2>$null
}

Write-Host "✅ 已启用 .githooks/ 作为 hook 目录" -ForegroundColor Green
Write-Host "   pre-commit：检测巨型正式文档并生成整理信号" -ForegroundColor Gray
Write-Host "   post-commit：把整理信号排入 Windows 节点" -ForegroundColor Gray
