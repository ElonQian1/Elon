...
| 节点插件治理合同 | Signed Manifest/InstallPlan、双 keyring、authority v3/PlanApply、可恢复受管取数、候选 exact closure/hash 与原子 resolution、schema 定义指纹、可信时间、ZIP 安全物化和 staging Store 均已写。staged 候选可进入有界健康状态机；成功路径写不可变健康回执但不改槽位，终止失败路径封存 canonical observation，并在 fresh authority/receipt/owner/fence 对账后原子把槽从 `staged` 推进为 `failed`、令三类 fence 各精确 `+1` 且写不可变 quarantine receipt。两类 Store 均有不确定提交恢复；quarantine 只返回 `NotCreated` 或 exact `Quarantined`，exact adoption 会重哈希 retained 文件和 seal。代码已通过 `elon-pc-node` 编译及 12 项健康/schema 定向测试，schema DDL 仅在内存 SQLite 验证建库与重开，完整 Store 事务夹具尚未执行。quarantine 保留 candidate owner、pointer 与文件 custody，不等于物理清理、重试、installed、ready 或商业 Verification。默认关闭的 Bootstrap 已挂入 NodeRuntime，但 sharing-on、生产数据库/时间源/回滚见证、真实 HTTPS downloader、Sidecar/IPC/探针调度、清理授权/完成回执、installed/promotion、ReadyCapability 上报和 Attempt 接线仍缺 |
...
目标流程仍是：共享关闭时不下载重型组件；开启后按硬件和任务选择签名插件、运行时与模型工件。本机耐久真源已选定为独立 SQLite，根签名双 keyring、原子计划应用、候选所有权、三段式下载认领、候选级全工件验证和可信时间边界见 `docs/distributed-compute/node-plugin-local-authority.md`。raw verified 只把本机槽推进到 `verifying`，不授权解包、健康、promotion 或商业 Offer。staged 候选已有进程内健康状态机和耐久健康回执 Store；连续失败达到阈值时，独立 quarantine Store 可在 fresh binding 未漂移时把槽原子推进到 `failed` 并写不可变回执。成功与失败 Store 均有不确定结果恢复，exact adoption 需 fresh read 与 retained-handle 重哈希；`NotCreated` 不恢复写许可。代码已通过 `elon-pc-node` 编译与 12 项定向测试，但内存库测试不代表生产本地库已建立，完整 Store 事务夹具也未执行。quarantine 不删除文件、不释放 owner，也不授予重试。候选观察值仍由 Host 调用方提供，尚未证明真实 Sidecar 已运行；生产时间权威 key resolver、真实网络取时、真实下载器、Sidecar/IPC/探针调度、清理授权/完成回执、installed/promotion、云端 capability gate 和通用 Attempt 协议接线仍未实现。
