...
以上供应链与候选健康代码已通过 `elon-pc-node` 编译，候选健康状态机、成功/失败观察校验与 schema 定向测试共 12 项通过；新增同句柄删除原语另有 3 项 Windows 文件系统定向测试。它们仍未在 NodeAgent 启动或 Host 路径接线，也未对生产本地库执行 DDL。健康成功 Store 继续只写回执与时间高水位。终止失败则封存独立 canonical observation，并在 fresh staged/receipt/owner/fence 仍精确一致且无未过期健康回执时，以单一 `BEGIN IMMEDIATE` 把槽从 `staged` 推进为 `failed`、令 state/inventory/authority fence 各精确 `+1`，同时写不可变 quarantine receipt。不确定 quarantine 结果只能读取 `NotCreated` 或 exact `Quarantined`；exact adoption 要求 fresh failed authority 对账及 retained-handle 文件/seal 重哈希，`NotCreated` 不恢复写许可。该结果仍保留 candidate owner、candidate pointer 与文件 custody，不删除目录、不授予重试、安装、promotion、ReadyCapability 或商业 Verification。目录树线性清理编排、fresh cleanup authorization、清理完成回执、目录耐久闭包、跨重启恢复、失败候选重试授权、真实 Sidecar 探针、installed/promotion/GC、生产取时/key resolver、认证传输与回滚见证、真实 HTTPS downloader、Host/Sidecar/IPC/沙箱仍待实现。精确健康与 quarantine 边界见 `node-ready-capability.md`。
