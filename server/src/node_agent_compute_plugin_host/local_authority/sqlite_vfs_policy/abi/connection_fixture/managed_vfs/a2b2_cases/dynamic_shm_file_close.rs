//! Physical-SHM-only actual bridge for final Keep/ShmFileClose/AfterSuccessKnown.
//!
//! The runtime seam is `ManagedSqliteShmFailurePhase::FileClose`; this bridge maps it to the
//! frozen `Phase::ShmFileClose` key. A trigger proves the sealed control flow reached the seam
//! only after `file.close()` returned `Ok(receipt)`. The redacted observer does not independently
//! inspect that receipt or the operating-system handle table.
//! `quarantined_file_closes` is only the managed-fs native close-custody count; it neither
//! observes nor denies the registry terminal marker/custody retained for the unsafe SHM failure.
//!
...
pub(in super::super) struct ShmFileClosePhysicalSubsetActual {
...
pub(in super::super) fn validate_shm_file_close_after_success_physical_subset(
    actual: ShmFileClosePhysicalSubsetActual,
) -> Result<(), &'static str> {
...
fn select_frozen_case(inventory: &[Case]) -> Result<&Case, &'static str> {
    ...
    let case = selected.next().ok_or("frozen ShmFileClose case missing")?;
    if selected.next().is_some() {
        return Err("frozen ShmFileClose case is not unique");
    }
    Ok(case)
}
...
fn validate_static_boundary(expected: &Case) -> Result<(), &'static str> {
    ...
    check(expected.retained.node, false, "frozen node custody")?;
    ...
    check(expected.retained.shm_file, false, "frozen SHM-file custody")?;
    check(
...
fn validate_pre(
    ...
    check(
        ...
        0,
        "pre native file-close quarantine count",
    )
}
...
fn validate_post(
    ...
    check(
        actual.topology.dms,
        ManagedSqliteShmTestDmsCustody::Absent,
        "post DMS custody",
    )?;
    check(
        actual.topology.shm_file_present,
    ...
    check(
        actual.topology.quarantined_file_closes,
        0,
        "post native file-close quarantine count",
    )
}
