...
candidate branch-atom scaffold 仍只允许声称一个内部性质：对它自己已 materialize 的不完整 atom 列表，每个 atom 在该表内只出现一次并获得一个 candidate disposition。Map review ledger 进一步要求自己声明的 step ID 全部 materialize、commit-bound anchor 位于对应 symbol span、共享源码分支携带 caller context，保留非空 Pending/open-boundary 集合，并对八个 pending 与六个 resolved 图边界做 exact cross-ledger 关联；ABI/raw reviewed-successor prefix另冻结自己的 case/edge/endpoint、raw slot retention及 open-frontier集合。这些自一致性不证明记录覆盖 production source，不证明记录是完整 terminal leaf，也不证明 ABI/raw之外的 disposition/reachability/exclusion 判定或 candidate axes 正确。当前已知 gaps 至少包括：
...
| Map source-terminal template review ledger v1 | source-written/source-review-only/validator-uncompiled/unrun; ABI/raw successor prefix written; Pending/open frontiers nonempty; graph exact-link written |
