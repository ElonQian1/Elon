...
fn full_lock_program_inventory_accounts_for_every_frozen_member_without_opening_quotient() {
    ...
    assert_eq!(inventory.source_present_member_count, 3_844);
    assert_eq!(inventory.source_present_group_count, 3_844);
    assert_eq!(inventory.planned_missing_member_count, 4_824);
    assert_eq!(inventory.planned_missing_group_count, 4_296);
    let source_groups = bundle
        ...
        .collect::<Vec<_>>();
    assert_eq!(source_groups.len(), 3_844);
