...
消费者发现和执行计划还会按下一次调用检查有效 Grant 的次数、单位价格和预算币种。同一能力存在多张 Grant 时优先选择任意可用授权；全部不可用时返回 `grant_refresh_required` 和稳定 `grant_budget_status`。重新申请仍创建新的 pending 请求并由商户审批，不扩容或改写旧 Grant；真正调用继续以事务内预算预留为最终权威。
...
| `open_commerce_list_my_consumer_authorization_requests` | 读 | 按状态列出当前用户通过当前项目本人 App 发出的授权申请和 Grant 结果；省略内部身份 |
| `open_commerce_list_my_active_grants` | 读 | 列出当前用户在当前项目本人 App 下仍有效的 Grant、期限和剩余调用/计量预算；不执行能力 |
| `open_commerce_cancel_my_consumer_authorization_request` | 写 | 经用户明确确认后撤回本人 pending 申请；不取消其他成员请求，也不撤销已批准 Grant |
