...
| 外部算力池适配器与统一报价 | v221-v256 前置门为 `implementation_partially_verified`。V250-V254 保留主线既有定向证据，V255 inert launch profile 仍不启动或解析 secret。V256 新增 server-only ephemeral operator-mounted bundle resolver 源码：固定 `profile_digest` path、strict manifest、same-`checked_at` current V255 + Store-selected V253 roots，以及 retained handles/locked zeroize memory；raw owner vault-ref 绝不参与 path/env/key。它没有 migration/receipt/HTTP/MCP/PC，不启动进程/网络/probe，不激活或证明 readiness；源码未编译或运行、`passed=0`，V254 的 18 个 temporary market absolute deny 原样保留。见 [`V256 resolver 权威`](external-pool-adapter-runtime-bundle-resolver-authority.md)、[`V256 验收边界`](external-pool-adapter-runtime-bundle-resolver-acceptance.md)、[`V255 launch profile 权威`](external-pool-adapter-runtime-launch-profile-authority.md) 与 [`当前状态`](current-implementation-status.md) |
...
Provider Adapter 的 v227-v256 前置门已形成分层权威；V244-V255 保留采用、安装、registry、动态声明、inert candidate 与 inert launch profile 的既有边界。V256 进一步实现 server-only ephemeral operator-mounted resolver 源码：bundle 只按 profile digest 固定寻址，strict manifest 在同一 `checked_at` 组合 current V255 与 internally selected current V253 head，config/credential 只在 retained handles 与 locked、zeroize-on-drop memory 中短时存在。raw owner vault-ref locator 只作私有 commitment subject，绝不作 path/env/key；V253 仍不证明 exact bundle bytes。V256 无 migration/receipt/HTTP/MCP/PC，SQLite 与 filesystem 非原子，不执行 entrypoint/Sidecar、网络、probe 或 activation，状态仍为 `runtime_launch_ready=false`、`implementation_uncompiled/source_review_only/passed=0`。V254 覆盖 direct SQL 与 versions 的 18 个 temporary absolute deny 原样保留；后续 authenticated supervisor/no-work probe 与 atomic activation 必须用完整 readiness/currentness admission gate 显式替换它。最新状态见 [`V256 runtime bundle resolver`](external-pool-adapter-runtime-bundle-resolver-authority.md)、[`current-implementation-status.md`](current-implementation-status.md) 与 [`V255 runtime launch profile`](external-pool-adapter-runtime-launch-profile-authority.md)。
