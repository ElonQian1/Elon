...
| Model/effort/tools/temporary mode | Native presets/cache and official controls; [search/image private live-state bridge](chatgpt-private-composer-tools.md) implemented in source | Accept the pinned tool-state bridge in grouped device testing; finish model/effort/temporary mode mutation. Cached labels alone are not proof of server selection, and tool selection is not an independent generation POST. |
