...
pub(crate) fn migration_v161(conn: &Connection) -> Result<()> {
    conn.execute_batch(
        "CREATE TABLE IF NOT EXISTS open_commerce_consumer_portability_merge_adoptions (
           id                       TEXT PRIMARY KEY,
         ...
         CREATE INDEX IF NOT EXISTS idx_open_commerce_portability_merge_owner
           ON open_commerce_consumer_portability_merge_adoptions(
