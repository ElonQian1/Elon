...
- 主服务器发布已完成；节点、Win 运行工件及真实同步仍待验证。
...
本批没有创建网格、修改仓位、结束策略、签名交易或移动资金；没有关闭已登录币安窗口、清理或复制登录 Profile。已发布主服务器，节点、Win 和 APK 未发布；没有把本地 queued 回执当作云端持久成功，也没有把私有 API 源码发现当作 APK 已经读到真实数据。
...
现有 IP 证书签发器仍依赖公网 80；外部证书路径可来自 DNS 验证等独立
