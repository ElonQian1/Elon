...
5. 量化页面只在 React 内存中使用 grant 查询本人模拟仓位，刷新或关闭后必须重新签发。
...
| `QUANT_PAPER_ACCESS_KEY_ID` | 信任的签名 key ID | 与主项目一致 |
| `QUANT_PAPER_ACCESS_PUBLIC_KEY_BASE64URL` | Ed25519 公钥 | 量化项目只持有公钥 |
...
- 当前 V1 量化验证端只信任一个 active key。签名 key 轮换采用安全停机窗口：先关闭一键入口，等待旧 grant 的五分钟寿命结束；更新量化公钥/key ID并重启；更新主项目 seed/key ID并重启；再通过 readiness 和合同测试开放。后续多 key 重叠轮换必须另立需求。
...
6. 清除任一关键配置，确认项目主页显示不可用、无 iframe、无签发。
