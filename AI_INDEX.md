...
| 一龙多身份联合登录 / Google OIDC / 密码与恢复码 / 设备会话 / 安全事件、导出与注销预检 / 认证存储能力 / Win、APK、Web 账号安全 / AI CLI Provider 状态机、假协议诊断、Codex 保险箱显式同意与禁用 Web Chat 生命周期 | `docs/unified-account-and-ai-provider-architecture.md`、`server/src/{account_security*,account_lifecycle,auth_request_guard,auth_safety_store,federated_auth*,node_agent_provider_*,node_agent_codex_vault_consent,node_agent_web_chat_adapter}.rs`、`server/src/store/{account_identities,account_security*}.rs`、`pc-frontend/src/features/{auth,account,node/codexVaultConsent}.ts*`、`android/app/src/main/kotlin/com/elon/app/{GoogleFederatedAuth,AccountSecurityApi,AccountSecurityViews}.kt`、`server/src/assets/{federated_auth,account_security,ai_provider_accounts}.*` |
