...
pub fn classify_cargo(args: &[impl AsRef<str>]) -> ResourceClass {
    ...
    if matches!(verb, "test" | "build" | "bench" | "install" | "rustc")
        || values.iter().any(|arg| matches!(*arg, "--release" | "--all-targets"))
    {
        ResourceClass::Heavy
    } else {
        ResourceClass::Light
...
mod tests {
    ...
    fn normalization_is_worktree_path_separator_independent() {
        assert_eq!(normalize_command(&["check", "server\\Cargo.toml"]), "check\nserver/Cargo.toml");
    }
    ...
    fn checks_are_light_but_linking_and_tests_are_heavy() {
        ...
        assert_eq!(classify_cargo(&["check", "--all-targets"]), ResourceClass::Heavy);
    }

    #[test]
    fn only_agent_validation_or_release_disables_incremental() {
        assert!(!agent_validation_disables_incremental("dev-windows-msvc", false));
        assert!(agent_validation_disables_incremental("agent-validation", false));
        assert!(agent_validation_disables_incremental("dev-windows-msvc", true));
    }
}
