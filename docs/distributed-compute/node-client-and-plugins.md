...
第一批内部兼容 seam 仍直接调用现有 Rust 本地推理函数。本机权威库已确定为独立 SQLite，并已形成 root-signed 双 keyring、authority schema v3、inventory payload v2、库存/计划/候选原子事务、PlanApply、三段式 claim/cursor Store 与类型化不确定结果恢复。Windows 文件层已形成 installation identity、长期已验 root capability、Volume GUID 卷根、父句柄相对打开、同句柄游标协调、分段写入/fsync 和 post-sync 可信时间 binder；正游标文件缺失或短于已提交长度时，专用线性 evidence 路径会原子终结精确 claim 并把下载投影为 `failed`。v3 又加入 candidate-level artifact verification journal、prepared/verified 唯一性、跨 authority/process/cancel 的撤销 fence，并在内容与健康回执尚未形成时禁止 promotion；受管文件层已有从 byte 0 有界 SHA-256、每次 read 取消检查、exact EOF 和 hash 后同句柄身份/长度复验底座。候选验证现进一步形成 exact candidate closure S1、一次 existing-only 目录 pin、由最多 4096 个 share-none 文件共享的 `Arc<File>` 父句柄链、内容稳定的 expected artifact-set JCS 摘要、本机 candidate/file identity JCS binding，以及严格晚于 pin barrier 的 S2。线性 begin 消费该能力，在同一 `BEGIN IMMEDIATE` 内第三次完整重读、重算动态 closure、推进可信时间并精确写入/回读 prepared run；mutation error 只返回携带全部句柄的 recovery custody。独立恢复 session 可稳定区分 exact `NotCreated/Prepared/Verified/Rejected/Aborted/Revoked`，并重验 canonical JSON/JCS。hash 编排强制消费一次性本机字节/单调时间预算，完整覆盖全部 ordinal、保留第一个摘要 mismatch，并生成绑定本机 file-set 的 observed artifact-set JCS digest；post-hash binder 再以严格晚于完整 hash barrier 的 authenticated observation 双读保存 exact Prepared S3 和 Bootstrap roots。新增 resolver 通过私有 permit 一次消费 TrustedHashed，在同一 `BEGIN IMMEDIATE` 内重放 current keyring、sealed plan/Manifest、窗口、sharing/live、inventory、candidate 和完整下载闭包，比较 S3/S4 后先终结目标 run，再撤销其余 prepared claim/run，并把 slot 原子推进到 `verifying` 或 `failed`；canonical 回执绑定 observed/mismatch 与 after fence，任何 Store/commit 不确定都只返回 recovery custody。已验证 ZIP 现可进入随机 create-new staging run，按计划逐文件核对长度和 SHA-256、flush/fsync、同句柄复验并保留输出证据；未写入 Store 的结果仍不是 `staged`、健康、promotion 或商业 Verification。所有能力仍未由启动或 Host 接线，真实数据库和网络下载没有发生；真实 downloader、解包证据 Store 登记/恢复、candidate health、Sidecar/IPC/沙箱、`ReadyCapability` 上报与 Attempt 接线仍未实现。
...
本文是已接受的客户端目标设计。NodeAgent 内部 legacy Host seam、版本化合同、签名与摘要验证器、Manifest/InstallPlan 准入、双 keyring、本机 authority v3、PlanApply、逐段 claim Store、Windows pinned-root/游标/耐久写入、已提交游标文件损坏终结、候选验证 schema/hash 底座、候选闭包双读/全文件 pin、prepared begin/outcome/abort recovery、受本机资源预算约束的逐文件 hash/Hashed capability、严格晚于 hash barrier 的 Prepared S3 binder、current-admission S4/verified-or-rejected 原子 resolution、挑战绑定的可信时间证明验证合同，以及已验证 ZIP 的 create-new staging/逐文件摘要与同句柄复验能力已形成代码。整体仍为 `implementation_uncompiled`，尚未编译、启动、建库、迁移或接入 Host。启动路径仍未持有 root/cancellation/clock epoch capability，生产时间权威 key resolver 与真实网络取时尚未实现；真实 HTTPS downloader、socket-read 取消、解包证据 Store 登记与跨重启恢复、candidate health/promotion、Sidecar/IPC/沙箱、typed 事件、`ReadyCapability` 上报和调度接线仍未实现。节点不直接发布商业 Offer，服务端 Offer 生成也未因本批合同自动接通。解包的精确边界见 `node-plugin-archive-extraction.md`。
