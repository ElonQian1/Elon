...
| 业务证据到 ERP/CRM 显式衔接回执 | 已实现、范围受限 | 项目编辑者可把终态证据摘要与同商户接入器的真实处理结果保存为幂等回执；`applied` 必须有有效标准业务回执和目标记录摘要。当前权威仅为项目编辑者声明，不是适配器独立签名或自动入库证明 | `server/src/open_commerce_business_handoff_service.rs`、`docs/decisions/open-commerce-business-handoff-receipts-v1.md` |
