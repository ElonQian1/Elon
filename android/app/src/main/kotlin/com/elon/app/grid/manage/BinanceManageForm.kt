...
internal class BinanceManageForm(private val activity: Activity, private val investmentEnabled:Boolean=false, edited: () -> Unit) {
    ...
    private val operation=select("本次操作", "binance-manage-action", listOf("请选择操作","修改终止时平仓设置","结束网格")+
        if(investmentEnabled)listOf("追加策略投入") else emptyList(),operations)
    ...
    fun action()=when(operation.selectedItemPosition){1->"settings";2->"close";3->"investment".also{require(investmentEnabled)};else->error("请选择本次操作")}
    fun cps()=if(action()=="investment")false else when(mode.selectedItemPosition){1->true;2->false;else->error("请选择终止时的仓位处理")}
