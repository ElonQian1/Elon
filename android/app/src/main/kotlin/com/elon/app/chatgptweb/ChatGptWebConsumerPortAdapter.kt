...
internal class ChatGptWebConsumerPortAdapter(
    ...
    override fun writingBlock() = observedState().let { it.writingBlock.takeIf { _ -> it.adapterCurrent } }
    override fun writingBlock(request: JSONObject, confirmed: Boolean) =
        execute(JSONObject().put("action", "chatgpt_writing_block").put("writing_request", request).put("user_confirmed", confirmed))
    ...
    override fun cancelDirectoryPage(requestId: String) =
        execute(JSONObject().put("action", "chatgpt_cancel_directory_page").put("directory_request_id", requestId))
    ...
    override fun requestLibraryFiles(directory: String, query: String, operation: String) =
        execute(JSONObject().put("action", "chatgpt_list_library_files").put("directory_handle", directory)
            .put("query", query).put("operation", operation))
    override fun cancelLibraryFiles(requestId: String) =
        execute(JSONObject().put("action", "chatgpt_cancel_library_files").put("library_request_id", requestId))
    override fun downloadLibraryFile(handle: String, downloadHandle: String) =
        execute(JSONObject().put("action", "chatgpt_download_library_file").put("file_handle", handle).put("download_handle", downloadHandle))
    ...
    override fun manageAccountShares(offset: Int, selectionTicket: String?): WebChatConsumerCommandResult =
