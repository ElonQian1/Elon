...
当前仍没有可注册的handle-bound SQLite VFS、`sqlite3_io_methods`、byte-range locking、WAL SHM映射/锁、临时文件策略、one-shot VFS token registry、生产trusted-time/rollback provider、opened snapshot producer或consumer。controller与managed-fs namespace只接到不可打开的sealed intent，生产`open()`继续固定unavailable；下一批必须先完成Windows locking、SHM与temp/URI/ATTACH策略，再铺惰性ABI/token callbacks，最后才能单独注册非默认VFS并打开SQLite。v11仍固定`context_ready=false`、`snapshot_ready=false`，并保持root/authority、PlanApply、work-admission、下载、安装和Sidecar标志为false。非空库存还缺installed/promotion/signed-manifest provenance、work-admission generation、Ready/Attempt撤销及signed `reauthorize_existing`，不得由本合同推断为已完成。
