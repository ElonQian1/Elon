...
pub(crate) fn migration_v196(conn: &Connection) -> Result<()> {
    conn.execute_batch(
        "CREATE TABLE IF NOT EXISTS compute_settlement_challenges (
           ...
           reason_code                     TEXT NOT NULL CHECK(reason_code IN (
             'amount', 'metering', 'price_snapshot',
             'execution_evidence', 'provider_identity', 'other'
           )),
           ...
           opened_by_user_id               TEXT NOT NULL CHECK(length(trim(opened_by_user_id)) > 0),
         ...
         CREATE INDEX IF NOT EXISTS idx_compute_settlement_challenges_opened
           ON compute_settlement_challenges(opened_at DESC, challenge_id);
         CREATE TRIGGER IF NOT EXISTS trg_compute_settlement_challenges_no_update
         BEFORE UPDATE ON compute_settlement_challenges
         BEGIN
           SELECT RAISE(ABORT, 'compute settlement challenges are append-only');
         END;
         CREATE TRIGGER IF NOT EXISTS trg_compute_settlement_challenges_no_delete
         BEFORE DELETE ON compute_settlement_challenges
         BEGIN
           SELECT RAISE(ABORT, 'compute settlement challenges are append-only');
         END;",
    )?;
