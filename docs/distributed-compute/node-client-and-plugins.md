...
第一批内部兼容 seam 仍直接调用现有 Rust 本地推理函数。本机权威库已确定为独立 SQLite，并已形成 root-signed 双 keyring、authority schema v3、inventory payload v2、库存/计划/候选原子事务、PlanApply、三段式 claim/cursor Store 与类型化不确定结果恢复。Windows 文件层已形成 installation identity、长期 root capability、Volume GUID 卷根、父句柄相对打开、同句柄游标协调、分段写入/fsync 和 post-sync 可信时间 binder。v3 又加入 candidate-level artifact verification journal、prepared/verified 唯一性、跨 authority/process/cancel 的撤销 fence，并在内容与健康回执尚未形成时禁止 promotion；受管文件层已有从 byte 0 有界 SHA-256、每次 read 取消检查、exact EOF 和 hash 后同句柄身份/长度复验底座。verification 的精确 file-set、prepared claim、post-hash binder、verified/rejected 原子槽转换与不确定结果恢复尚未实现，schema 当前不能写出验证终态。所有能力仍未由启动或 Host 接线，真实数据库和网络下载没有发生；真实 downloader、安全解包/逐文件 closure、candidate health、Sidecar/IPC/沙箱、`ReadyCapability` 上报与 Attempt 接线仍未实现。
...
本文是已接受的客户端目标设计。NodeAgent 内部 legacy Host seam、版本化合同、签名与摘要验证器、Manifest/InstallPlan 准入、双 keyring、本机 authority v3、PlanApply、逐段 claim Store、Windows pinned-root/游标/耐久写入和候选验证 schema/hash 底座已形成代码，但尚未编译、启动、建库、迁移或接入 Host。启动路径仍未持有 root/cancellation capability，可信时间内核尚不能生产 observation；candidate verification 的 file-set/claim/binder/resolution/recovery、真实 HTTPS downloader、socket-read 取消、短文件终结、安全解包、Sidecar/IPC/沙箱、typed 事件、`ReadyCapability` 上报和调度接线仍未实现。节点不直接发布商业 Offer，服务端 Offer 生成也未因本批合同自动接通。
