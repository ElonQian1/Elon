...
v4-v6 采用逐代纯 reference 迁移：旧库先在同一 `BEGIN IMMEDIATE` 内精确验证全部非内建对象、规范 SQL 指纹和 foreign key，再依次追加 policy receipt、prepared-work companion/终结 trigger 与 Control-signed catalog；缺失、额外、同名漂移或迁移中断均失败关闭。现有验证包括8项版本链、4项policy合同、5项v5 Store、6项v5恢复判定、3项v6空目录binding Store和5项真实Ed25519目录候选测试。它们仍不代表生产磁盘迁移、v5外层adopt完整会话/生产接管、v6完整CAS recovery、生产root/keyring、打开态VFS或Host联调。
