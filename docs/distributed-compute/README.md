...
| 节点插件治理合同 | Signed Manifest/InstallPlan、双 keyring、authority v3/PlanApply、受管取数、候选验证、ZIP 安全物化及 staging/健康/quarantine Store 均已写。失败候选已有 cleanup authorization Store；其后 sealed topology 与首对象 intent/disposition/absence/namespace-durability 共五个独立 typed Store，并具备线性 outcome-uncertain recovery 和原子 completion 内核。序号 4 的同句柄 pre-absence、Windows native barrier、post-absence、evidence 与 Store/recovery 形状已写；普通 parent handle 不能排除外部 writer 瞬时 ABA，因此不可构造的 mutation-fence 门当前阻止安全代码签发能力或接 Host。远程 sequence 2 基线通过 `elon-pc-node` 编译、24 项清理定向测试及 1 项 schema 建库重开测试；sequence 3/4 未编译、未测试。真实 ABA fence、旧执行器强路径、后续 ordinal、terminal journal、evidence v2、跨重启物理恢复、完整事务夹具与 Host 均缺，因此当前仍不产生生产可达的 completion/retry、installed、ready 或商业 Verification；生产数据库/时间源/回滚见证、HTTPS downloader、Sidecar/IPC/探针、installed/promotion、ReadyCapability 和 Attempt 接线也仍缺 |
...
节点失败候选清理的 schema v3 已包含 expected-object topology、candidate-parent identity anchor、plan seal 与四阶段 hash-chain step journal，并要求 completion 同时绑定 plan digest 和 terminal namespace-durability digest。当前代码已形成 topology 及首对象四阶段 typed Store、handle-derived identity/parent/name binding、Windows authority-domain parent namespace barrier 和进程内 exact recovery；远程仅验证到 sequence 2，本批 sequence 3/4 未编译或测试。外部 writer ABA fence、后续对象、terminal journal、跨重启物理恢复、旧执行器适配与 Host 仍未实现，不能理解为节点已经会自动清理生产失败候选。
