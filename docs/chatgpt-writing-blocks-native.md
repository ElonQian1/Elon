...
`chatgpt_private_protocol_probe(mode=text_block_inventory)` 仅对当前普通会话执行一次有界 GET，复用正文解析器报告块数量，不输出正文、ID 或请求头。它不刷新页面、不发送消息，也不把读取失败报成官网没有能力；统计仅覆盖选中分支最近 80 条可见消息。
