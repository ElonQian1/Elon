...
以上新增代码仍未编译、未测试、未执行 DDL，也未在 NodeAgent 启动或 Host 路径打开，因此没有真实数据库、下载、文件写入、插件安装或 Sidecar 运行。仓库仍没有生产 root pin；可信时间观察值生成/attestation、数据库外防整库回滚锚点、受管文件安全根、no-follow/no-reparse 钉住打开、文件游标截尾/短文件损坏转换、全工件摘要验证、候选 promotion/GC、跨重启恢复枚举、Host/Sidecar/IPC/沙箱接线仍待实现。`DurablyWrittenComputePluginSegment` 仍无生产构造器，本节的提交与失败恢复能力目前只是不可绕过的内部合同，不代表下载器已经可用。
