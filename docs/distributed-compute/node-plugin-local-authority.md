...
该合同仍不是可用的数据库外锚服务：仓库尚无生产见证服务器、真实网络查询与发布协议、生产见证根公钥，也未接入 NodeAgent 启动门卫。无法取得可信最新锚时仍必须失败关闭；不得把 DTO、验签与比较能力描述成已经解决整库回滚。
