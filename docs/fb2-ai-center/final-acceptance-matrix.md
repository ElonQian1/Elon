...
| live 预检交接 | `status-current.json` 输出 `live_preflight_request`，拿到 `FB2_AI_CENTER_TOKEN` 后按其中命令先跑 no-write direct read 和 `-DataOnlyAcceptance -PreflightOnly` | `ready_without_token=true`、`blocked_by_external_secret=true` 时说明只差外部 token；`evidence_policy.group_chat_test_method=direct_api_read`、`screenshots_accepted=false`，群聊证据必须包含 `message_id/text_len/text_sha256` |
