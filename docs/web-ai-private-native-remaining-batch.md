...
Current implementation audit: 2026-09-12. This is a work list, not a declaration
...
| Text submit/stream/stop/follow-up | Official runtime send reused; ordinary authenticated native retry completed/default-enabled on 1644, adapter 327/resolver 12 | [Native retry accepted](reports/chatgpt-regeneration-store-20260911.md#normal-1644-acceptance); reuse its identity/store confirmation and 1642 transcript reentry fixes. [Independent page-owned HTTP](chatgpt-fresh-text-dispatch.md) has fresh send, owned stop and history recovery in source only; not enabled or device-accepted. Additional reply contexts, stream handoff and measured first-word timing remain |
