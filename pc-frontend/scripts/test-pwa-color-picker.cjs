...
assert.match(controlSource, /document\.addEventListener\('scroll', updatePosition, true\)/)
assert.match(controlSource, /popoverRef\.current\?\.contains/)
...
assert.match(controlSource, /position: 'fixed'/)

const editorSource = read('PwaStyleEditor.tsx')
assert.match(editorSource, /<span>整体透明度<\/span>/)
