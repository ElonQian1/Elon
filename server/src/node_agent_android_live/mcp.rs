...
fn initialize_response() -> Result<Value> {
    ...
    Ok(json!({
        ...
        "serverInfo": { "name": "yilong-ui-live", "version": "1.9.0" },
        "toolContract": tool_contract,
        "instructions": "先调用 ui_get_design_capabilities 证明当前节点 schema；模糊路由任务再调用 ui_confirm_route。无需打开 PC 画布的多端任务按 ui_list_design_targets -> ui_list_design_sessions -> ui_open_design_target 工作；拿到 AI taskId 后用 ui_bind_design_task 绑定 designSession/draft 和有界 lease，消费者以 ui_list_design_events(afterCursor, taskId, waitMs) 增量跟随，运行中续租并在终态调用 ui_settle_design_task_binding。Web/PWA/Tauri 前端可用 ui_prepare_design_browser -> ui_interact_design_browser 持续边看边调，结束调用 ui_stop_design_browser。Tauri 原生证据按 ui_prepare_tauri_runtime -> ui_capture_tauri_host -> ui_capture_tauri_behavior -> ui_stop_tauri_runtime 分层采集；项目插桩 trace 不冒充操作系统证明。设计修改用 ui_create_design_draft 建立可撤销 DraftOperation v2；SET_STYLE 可进入安全临时预览，文字、资源、variant、结构和响应式操作按 capability matrix 交给源码适配器，不把 SOURCE_HANDOFF 冒充 LIVE_PREVIEW。调用 ui_suggest_design_source_binding 获取有界多候选，先保持 CANDIDATE，核对文件哈希和范围后才用 ui_update_design_draft 显式确认 BOUND；再 ui_begin_design_writeback 固定 Git 基线。源码修改与分平台验证后用 ui_complete_design_writeback 形成完成回执，并用 ui_get_design_verification_matrix 查看每个平台的证据缺口。后台证据只返回路径、哈希和紧凑当前状态，不嵌入 Base64/完整历史，不允许秘密表单输入、任意菜单点击或任意 Tauri command。Codex 桌面端 UI 请求先导入任务、读取项目与 Runtime 并检查能力；requiredCapabilities 只能追加系统推导能力。样式优先 Live Patch/FitRun，结构变化使用最小 CODEX_SOURCE_HANDOFF。平台缺口必须声明 deliveryImpact：原业务任务只创建 Worktree handoff，不得就地升级；DELIVERY_NON_BLOCKING 在 businessDeliveryReady=true 后先收尾业务，再由新的 Codex Desktop Worktree 任务后台升级、发布和复检，前台 UI 任务优先使用真机与节点发布资源；DELIVERY_BLOCKING 则暂停业务并分流。全新页面先建骨架和首次构建，再回到真实 Android Renderer。收尾必须调用 ui_check_workflow_completion；仅 completionReady 或经验证的 businessDeliveryReady 允许对应声明。"
    }))
}
