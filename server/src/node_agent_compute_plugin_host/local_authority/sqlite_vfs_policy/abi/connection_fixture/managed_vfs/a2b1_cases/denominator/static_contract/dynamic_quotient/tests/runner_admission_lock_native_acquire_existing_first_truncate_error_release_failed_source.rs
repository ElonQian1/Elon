//! Source-only admission contracts for all 88 q17 initialization cleanup-release members.

use super::super::super::terminal_descriptor::{InitializationFaultSiteV1, InitializationPathV1};
use super::super::runner_admission::{
    compile_for_test,
    native_acquire_existing_first_truncate_error_release_failed_catalog_row_count_for_test,
    validate_lock_program_for_test,
};
use super::lock_native_acquire_created_first_truncate_error_release_failed_cases::frozen_lock_created_first_truncate_error_release_failed_leaves_v1;
...
fn assert_rejected(key: DynamicClassKeyV1, member: StaticMemberSealV1, mutation: &str) {
    assert!(
        ...
        "q17 initialization admission accepted {mutation}"
    );
}

#[test]
fn all_88_q17_descriptors_and_exact_catalog_seals_are_source_present() {
    ...
    for &case in leaves.keys() {
        ...
        validate_lock_program_for_test(&key, member, compile_for_test(&key))
            .unwrap_or_else(|error| panic!("exact q17 member {case:?} was rejected: {error:?}"));
    }
}

#[test]
fn q17_is_inventory_present_without_granting_supported() {
...
#[test]
fn q17_rejects_neighboring_seals_and_every_cleanup_contract_mutation() {
    let leaves = frozen_lock_existing_first_truncate_error_release_failed_leaves_v1();
    ...
    for (neighbor, label) in [
        (
            frozen_lock_existing_first_exclusive_release_error_leaves_v1()
                ...
                .1
                .member,
            "a q13 at-call existing-first release seal",
        ),
        (
            frozen_lock_existing_first_truncate_error_release_succeeded_leaves_v1()
                ...
                .1
                .member,
            "a q15 cleanup-release-succeeded seal",
        ),
        (
            frozen_lock_created_first_truncate_error_release_failed_leaves_v1()
                ...
                .1
                .member,
            "a q16 created-first cleanup-release-failed seal",
        ),
    ] {
