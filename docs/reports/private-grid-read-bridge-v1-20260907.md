...
## 用户批准的一次 Win 重启验收（21:25）

用户明确允许这一次 Win 重启，Chrome 保持。第一次本机助手在提交动作前因
Windows `\\?\` 路径前缀误判项目绑定；仅规范化标准前缀后，bootstrap 与
MCP status 的完整 Git 项目根均匹配。该失败没有提交重启动作。

21:17 通过官方 `win_control_action` 唯一提交精确 abc3 发布身份，动作
`win_act_ee5253b55ac94a4eb82ad0eb5f023028` 取得 succeeded 调度回执。
外部守护器确认旧桌面正常退出并启动正式更新器；没有强杀进程或第二次提交。
完整包下载进入安装门禁后，日志先报告 42 个缺少安全恢复检查点的历史任务，
随后出现 Windows 原子替换“拒绝访问”。更新器自带的单 exe 回退又因
`runtime 状态缺少活动 prompt 身份列表` 停止。错误日志不能定位具体失败
文件，也不能证明永久 ACL 问题；没有为排除错误而修改权限或恢复账本。

自动审计排除了 22 条历史记录，余 20 条：19 条 resume_required、1 条 running；
未检测到活动 runtime handle、journal 进程或 sidecar。8 条存在恢复回执歧义，
11 条审计或不可重复动作证据不足，1 条缺少持久终态；没有把“没有进程”当作
删除门禁的充分证明，也没有自动恢复旧任务或伪造完成记录。

21:23 守护器终态为 `target_not_activated`，已自动重新打开原正式安装的
Win 桌面。能力接口实际仍是 **0.3.69+aa6874c70b6090d13d8f22e8c16f4de55678336d**，
前端与 Tauri 在线，新的私人投影 binding 接口仍返回 404。因此此次重启已经
结束，但新版未激活，不能宣称真实同步或完整客户端升级成功。

通过官方 browser_research MCP 的 sessions 命令重新读到原项目范围的会话
元数据。最新币安研究会话保留 435 份资源和 422 条请求，状态 expired；只读
元数据不代表币安登录已失效，也不证明重启后登录仍有效。原 Profile 范围保留，
没有复制、清理登录目录、读取过期正文、延长授权或新建替代会话。Chrome 未改动。
后续须先解决官方更新兼容性与历史任务恢复门禁，再取得新会话和真实云端 ACK。

量化 worker 此前通过的是测试与 clippy，没有独立可启动的 exe。本轮补做最小
正式构建，缓存盘门槛通过、目标位于托管 C 盘目录；首次 53 秒在 num-traits
0.2.19 的 build-script 遇 `0xc0000005`。只通过官方入口精确定向清理该 crate
10 文件、4.0 MiB 后，唯一一次重建 13.5 秒仍同错；正式日志
`quant-grid-read-worker-build-20260907-212234-774` 与
`quant-grid-read-worker-build-retry-20260907-212549-323`。没有生成或启动 worker、
创建同步 ledger、重跑业务测试、清理未知缓存或降低磁盘门槛。这是另一个本机
交付缺口，不影响已验签的 APK 工件事实，也不证明 Rust 源码编译错误。

## 后续研究恢复（23:29 起）

在仍运行 aa687 的正式 Win 中，原项目、owner、Binance 站点范围的新研究会话
已返回私人列表及详情，并可读取页面采集的创建、修改设置、结束业务响应。
完整分片/哈希校验、参数调用链和限制见[运行合同研究](binance-grid-runtime-contract-20260907.md)。
这补充了上文“先升级再取新会话”的判断：现有旧版可继续研究；新版 binding、
同步 worker 和真实 APK 读取仍未通过验收。本轮没有再提交重启或交易动作。

## 9 月 8 日同步准备进展

worker 已在原 f28 源码上完成实际构建和无参数启动拒绝检查，补齐可执行工件。
Win 状态过大时丢失更新身份的问题已复现并修复，10 项实际生产模块测试通过。
构建收据、哈希、完整节点编译与运行验收限制见
[状态修复与同步准备](node-status-runtime-identity-20260908.md)。
尚未启动账户同步或再次重启 Win；APK 的真实网格显示仍未完成验收。

## 9 月 8 日现场接收验收

上述运行阻断现已有后续证据：经本次明确授权安装修复后，c87 新版已实际运行，
binding HTTP 200，真实 24 条币安网格已上传并由云端确认。手机授权和显示仍待
验收；一次上传不代表持续采集。详见[安装修复与云端接收](private-grid-cloud-acceptance-20260908.md)。
