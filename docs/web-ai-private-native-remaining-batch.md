...
| Model/effort/tools/temporary mode | Native presets/cache, [search/image private state](chatgpt-private-composer-tools.md), [model/effort/version/tier](chatgpt-private-model-state.md), [additional model catalog](chatgpt-private-model-catalog.md), and [temporary-chat transaction](chatgpt-private-temporary-chat.md) implemented | Tool runtime v2 shipped in 1561, but guest catalog acceptance failed before selection; current website assets were confirmed. Context diagnostics/cache-layout correction is the next checkpoint, not a completed capability. Other model/effort/temporary and account-scope cases remain unaccepted. Cached labels/local readback do not prove server preference persistence; these callbacks are not independent generation/privacy POSTs. |
