...
implementation_status: map_q4_lock_q19_source_written_uncompiled_unrun
...
## 4. Lock q5–q19 exact tranches

q5–q19 的精确成员、lower/receipt source contract、catalog 与隔离约束由
...
仍在 q11 外；q12–q19 各承接 88 个，余下 2,728 members / 2,200 groups 保持 planned-missing。
...
q16 的第五个冻结纵切命名为 `LockNativeAcquireCreatedFirstTruncateErrorReleaseFailedV1`，只承接
initialization namespace 中 `dms.created-first.truncate-error.release-failed` 的 88 个 singleton：仍由相同 44 个
合法 native-acquire request 乘两个 unsafe terminal 组成。完整 typed shape 固定为
`phase/fault_site=DmsExclusiveRelease`、`path=CreatedFirst`、`timing=Cleanup`、`occurrence=Natural`、
`class=OutcomeUncertainPoisoned`、`cleanup_rewrite=false`、terminal disposition=`CleanupRewritten`、
`mutation=Uncertain`、`lock_uncertain=true`、file retained、`DMS=ExclusiveOutcomeUncertain` 与 DMS native
lock/unlock=`1/1`。future controlled-fault actual 必须先在 production truncate validation 点真实调用一次
`File::set_len(0)` 并故意不读 `Result`，再在 cleanup exclusive-release 点真实调用一次 `UnlockFileEx` 并故意
不读 BOOL；两个 typed unavailable receipt 的顺序、同一 case/stage 与 requested Lock range 的 native/local
no-entry ledger 必须同时闭合。它仍只能标记 `controlled_fault_actual`，不得冒充 natural actual。q16 catalog
为 88 rows / 18,386 bytes，SHA-256=`f51b7d0df30afda4a8844ee4c4e227a6cf757b2f7b0c16e0bc0e8dbfaad05718`。

q17 的第六个冻结纵切命名为 `LockNativeAcquireExistingFirstTruncateErrorReleaseFailedV1`，只承接
initialization namespace 中 `dms.existing-first.truncate-error.release-failed` 的 88 个 singleton：8 个 shared
单槽与 36 个 exclusive 连续非空 range 各配两个 unsafe terminal，即 `16+72`，两个 completion 各 `44/44`。
exact selector 冻结为
`initialization-{lock-shared|lock-exclusive}-first-{first}-count-{count}-existing-first-truncate-error-release-failed-{retention-succeeded|retention-route-unknown-prior-quarantine}-terminal-route-unknown`；
typed shape 与 q16 相同但固定 `path=ExistingFirst`；`cleanup_rewrite=false`、terminal disposition=
`CleanupRewritten`、mutation/lock uncertain、file retained、DMS=`ExclusiveOutcomeUncertain`、native lock/unlock=`1/1`。
future controlled-fault actual 必须先物理预创建并关闭 SHM，再从 cold WAL-main attach 观察
`was_created=false`；随后同一 tuple 内先丢弃真实 `File::set_len(0)` 的 `Result`，再丢弃 cleanup
`UnlockFileEx` 的 BOOL，两份 unavailable receipt 必须有序且不得与 q16 换 case。`a2lockq17` payload 预期
172 values，机械分区为 `25 binding + 5 metadata + 8 physical-precreation + 25 cold + 8 callback + 14 after +
32 initialization + 4 cleanup-native + 18 requested-lock + 1 pending + 18 terminal + 6 preemption + 4 registration +
3 route + 1 root`；绝对位置依次为 `[0,25) [25,30) [30,38) [38,63) [63,71) [71,85) [85,117)
[117,121) [121,139) [139] [140,158) [158,164) [164,168) [168,171) [171]`。它仍只能标记 `controlled_fault_actual`，不得冒充
natural actual。q17 catalog 为 88 rows / 18,474 bytes，SHA-256=
`5ce129843d33b279c9ec70dd282d59cc79455c8f1a1b652718bb04b72777adff`。

q18 的第七个冻结纵切命名为 `LockNativeAcquireCreatedFirstSharedBusyCloseSucceededV1`，只承接
initialization namespace 中 `dms.created-first.shared-busy.close-ok` 的 88 个 singleton：同一 8 个 shared 单槽与
36 个 exclusive 连续非空 range 各配两个 terminal，即 `16+72`，两个 completion 各 `44/44`。exact selector
冻结为
...
typed shape 固定 `phase/fault_site=DmsSharedAcquire`、`path=CreatedFirst`、`timing=AtCall`、
`occurrence=Natural`、`class=BusyAfterKnownMutation`、`cleanup_rewrite=false`、source disposition=`Returned`、
mutation known、lock certain、file/DMS=`Released`、DMS native lock/unlock=`2/1`，unsafe retention 后 terminal=
`Quarantined`、callback/payload retained。future controlled-fault actual 必须经 test-only contention source seam
让同一 `FileId` 的 distinct holder/target handles 真实争用：cold CreatedFirst target 完成 exclusive lock/truncate/
unlock 后，holder 在 `SHM_DMS_OFFSET` 持 exclusive lock，target shared `LockFileEx` 恰一次 Contended，再显式
成功关闭 target；holder/target attempts 与 cleanup 分账，terminal ledger 读取后才释放 holder。target DMS
lock=`2/1/1`、unlock=`1/1`、close=`1/1`，holder acquire/unlock=`1/1`，requested-range native/local=`0/0`、
managed=`1/0`、callback=`1/1`。`a2lockq18` 精确为 186 scalars，分区为
`25+5+25+8+14+43+15+18+1+18+6+4+3+1`，绝对位置为 `[0,25) [25,30) [30,55) [55,63)
[63,77) [77,120) [120,135) [135,153) [153] [154,172) [172,178) [178,182) [182,185) [185]`；它仍只能标记
`controlled_fault_actual`，不得冒充 natural actual。q18 catalog 为 88 rows / 18,122 bytes，SHA-256=
`4f78ff1678c93b1c06bad92e838423e4202598fd8e0b5b83f79cde0c528a07cd`。

q19 的第八个冻结纵切 `LockNativeAcquireExistingFirstSharedBusyCloseSucceededV1` 是 q18 的 path 对切，
只承接 `dms.existing-first.shared-busy.close-ok` 的同一 88 个 request/completion singleton。它先物理预建并
关闭 exact SHM、绑定 `[1,1,1,4,1,1,4,1]` receipt，再由 cold attach 观察 `was_created=false`；随后 target
完成 ExistingFirst exclusive/truncate/unlock，同 `FileId` distinct holder 持 DMS exclusive，target production
shared `LockFileEx` 恰一次真实 Contended，target close 显式成功，terminal ledger 后 holder 才 unlock。
`a2lockq19` 精确为 194 scalars，分区 `25+5+8+25+8+14+43+15+18+1+18+6+4+3+1`，绝对位置为
`[0,25) [25,30) [30,38) [38,63) [63,71) [71,85) [85,128) [128,143) [143,161) [161]
[162,180) [180,186) [186,190) [190,193) [193]`。catalog 为 88 rows / 18,210 bytes / 89 LF，SHA-256=
`eb318d91edbd0bbcd7e68ff626504a007a3f3c96d5eb60b965c9e362a421eee8`；源码仍 uncompiled/unrun，不是 actual。

完整 `LockNativeAcquireInitializationFailureV1` 仍是 3,432 members / 2,904 normalized groups，而不是
q12–q19 八段的别名。其机械分期为 Open `792/792`、DMS exclusive acquire `528/440`、first-process
truncate/release `528/528`、shared acquire `1,584/1,144`；八批共 704 个 source-only member/group 只建立可扩展 typed
controller 的八个连续 vertical slice，后续 2,728 members / 2,200 groups 继续 planned-missing，不得由已写纵切推导完成。

聚合未运行 inventory 预期现为 `4,372 present members /
4,372 present groups / 4,296 missing members / 3,768 missing groups / 8,668 total members /
...
`4,372 present members/groups`、`4,296 missing members / 3,768 missing groups`；两根 reviewed inventory
...
refresh 已逾期并继续独立 deferred，未来编译或 runtime acceptance 前的重生成/复核必须覆盖 q15–q19。
