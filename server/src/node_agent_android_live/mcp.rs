...
fn initialize_response() -> Result<Value> {
    ...
    Ok(json!({
        ...
        "serverInfo": { "name": "yilong-ui-live", "version": "1.12.0" },
        "toolContract": tool_contract,
        "instructions": "先调用 ui_get_design_capabilities 证明当前节点 schema；自然语言多端设计任务先用 ui_plan_design_intent 固化 platform/route/state/sessionAction，再按计划恢复或打开 designSession；模糊路由任务再调用 ui_confirm_route。打开匹配会话后用 ui_start_design_intent_plan 绑定 task lease，每个动作调用 ui_record_design_intent_action 写回回执；暂停、恢复、取消、失败、完成或意图变化分别使用 transition/replan 工具和 expectedRevision。无需打开 PC 画布的多端任务也可完整工作。拿到 AI taskId 后用 ui_bind_design_task 绑定 designSession/draft 和有界 lease；消费者先读 ui_get_design_event_checkpoint，再以 ui_list_design_events(afterCursor, taskId, waitMs) 增量跟随，成功处理后单调提交 ui_commit_design_event_checkpoint，运行中续租并在终态调用 ui_settle_design_task_binding。Web/PWA/Tauri 前端可用 ui_prepare_design_browser -> ui_interact_design_browser 持续边看边调，结束调用 ui_stop_design_browser。Tauri 原生证据按 ui_prepare_tauri_runtime -> ui_capture_tauri_host -> ui_capture_tauri_behavior -> ui_stop_tauri_runtime 分层采集；项目插桩 trace 不冒充操作系统证明。设计修改用 ui_create_design_draft 建立可撤销 DraftOperation v2；SET_STYLE 可进入安全临时预览，文字、资源、variant、结构和响应式操作按 capability matrix 交给源码适配器，不把 SOURCE_HANDOFF 冒充 LIVE_PREVIEW。修改前用 ui_create_design_regression_baseline 固化已验证 PNG/UI tree。调用 ui_suggest_design_source_binding 获取有界多候选，先保持 CANDIDATE，核对文件哈希和范围后才用 ui_update_design_draft 显式确认 BOUND；写回前必须 ui_check_design_source_binding 校验 SHA/range，并用 ui_plan_design_writeback 审查分平台适配器、影响和风险，显式 APPROVE 后携带 planId 调用 ui_begin_design_writeback 固定 Git 基线。源码变更优先用精确 byte range/SHA 的 ui_propose_design_source_patch，读取 review artifact 并再次显式批准后才能 ui_apply_design_source_patch；应用后生成可审查回滚计划，不自动回滚。重新捕获修改后证据并创建 regression comparison，优先调用 ui_run_design_regression_comparison 让节点本机复核输入 SHA、生成紧凑 diff artifact 并按阈值结算；外部比较器才用 ui_complete_design_regression_comparison 提交完整 artifact SHA 回执。最后用 ui_complete_design_writeback 形成完成回执，并用 ui_get_design_verification_matrix 查看每个平台的证据缺口。后台证据只返回路径、哈希和紧凑当前状态，不嵌入 Base64/完整历史，不允许秘密表单输入、任意菜单点击或任意 Tauri command。Codex 桌面端 UI 请求先导入任务、读取项目与 Runtime 并检查能力；requiredCapabilities 只能追加系统推导能力。样式优先 Live Patch/FitRun，结构变化使用最小 CODEX_SOURCE_HANDOFF。平台缺口必须声明 deliveryImpact：原业务任务只创建 Worktree handoff，不得就地升级；DELIVERY_NON_BLOCKING 在 businessDeliveryReady=true 后先收尾业务，再由新的 Codex Desktop Worktree 任务后台升级、发布和复检，前台 UI 任务优先使用真机与节点发布资源；DELIVERY_BLOCKING 则暂停业务并分流。全新页面先建骨架和首次构建，再回到真实 Android Renderer。收尾必须调用 ui_check_workflow_completion；仅 completionReady 或经验证的 businessDeliveryReady 允许对应声明。"
    }))
}
