...
| Text submit/stream/stop/follow-up | Official runtime send path verified, reused | Retry v8 on 1639 used legacy DOM. Fresh diagnostics proved an unrecognized second September 10 build. Resolver 11 / adapter 325 repairs its mappings (132 focused + 304 adjacent checks); installed normal 1640 awaits unlock/private acceptance, [evidence](reports/chatgpt-runtime-bindings-20260910-b.md). Earlier post-dispatch `timeout_owner_changed` remains unexplained. Independent Android POST and other contexts remain separate |
...
| Explicit file citations / remaining cloud references | Metadata/scope and source `7b78394da` file-URL identity/empty-context corrections are included in installed 1640 / adapter 325; 216 checks pass. Production citation download acceptance awaits unlock | [Boundary and source evidence](chatgpt-private-file-citations.md). URL-only cloud without a ChatGPT ID and nonempty PCA graphs remain incomplete |
