...
| 节点插件治理合同 | Signed Manifest、InstallPlan、双槽 lifecycle 与短期 ReadyCapability 合同已写；独立 SQLite authority v3、root-signed 双 keyring、库存/计划/候选原子应用、三段式取数、跨代撤销与不确定结果恢复已形成代码。Windows Volume GUID 卷根、父句柄相对打开、同句柄游标协调和耐久分段写入也已形成。候选验证现覆盖 sealed closure S1、全文件 share-none pin、内容与本机 binding、pin 后 S2、事务内第三读/prepared begin、稳定 outcome/recovery abort、完整 ordinal hash，以及严格晚于 hash barrier 的 purpose-specific trusted-time binder；binder 对账 cancellation source，双读保存 exact Prepared S3 快照并保留 Bootstrap roots，任何错误只返还 recovery custody，成功才形成线性 TrustedHashed capability。该能力未重放当前 keyring/plan/window，不是 resolution 授权。缺少新增必需 abort trigger 的旧 v3 失败关闭，同名定义 fingerprint 仍待补齐；verified/rejected resolution 和槽转换仍未实现，全部未编译、建库或接线 |
...
| Attempt Execution Receipt | v193、追加式 Store 与管理员 HTTP 已写；仅 accepted Verification 可签发，固定执行身份、工件、用量和证明，但不改状态、容量和资金，尚未编译、执行迁移或运行验证 |
...
19. `docs/distributed-compute/attempt-terminal-candidate-api.md`：Provider 首次终态候选、输出合同、不可覆盖与无状态/无资金效果边界。
20. `docs/distributed-compute/attempt-consumer-review-api.md`：消费者首次终态审核、证据引用、不可覆盖与非验证/非结算边界。
21. `docs/distributed-compute/attempt-platform-observation-api.md`：平台首次终态观测、累计 meter 差异、不可覆盖与非验证/非结算边界。
22. `docs/distributed-compute/attempt-verification-api.md`：保守 Verification policy、verified/compensable usage、不可覆盖与非状态/非结算边界。
23. `docs/distributed-compute/attempt-execution-receipt-api.md`：accepted Verification 的执行回执、完整源证据重审计与非状态/非结算边界。
...
28. `docs/distributed-compute/attempt-settlement-correction-api.md`：accepted 挑战向下金额纠正、消费者退款和 pending 冲减边界。
...
目标流程仍是：共享关闭时不下载重型组件；开启后按硬件和任务选择签名插件、运行时与模型工件。本机耐久真源已经选定为独立 SQLite，根签名双 keyring、原子计划应用、候选所有权、三段式下载认领、候选级全工件验证和可信时间边界见 `docs/distributed-compute/node-plugin-local-authority.md`；其中 Store 认领/终结/恢复、verification schema/fence、candidate closure 双读与全文件 pin、prepared begin/outcome/abort recovery、逐文件 hash/Hashed capability，以及严格晚于 hash barrier 并保存 exact Prepared S3 快照的 post-hash trusted binder 已写但未编译、建库或接线。TrustedHashed 仍不是完整准入或 resolution 授权；verified/rejected resolution、下载器、Sidecar 进程与 IPC、动态健康状态、云端 capability gate 和通用 Attempt 协议接线目前仍未实现。
