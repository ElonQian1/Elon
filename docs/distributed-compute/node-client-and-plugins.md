...
本文是已接受的客户端目标设计。NodeAgent 内部 legacy Host seam、版本化合同、签名与摘要验证器、Manifest/InstallPlan 准入、双 keyring、本机 authority v3、PlanApply、逐段 claim Store、Windows pinned-root/游标/耐久写入、候选验证 schema/hash 底座、候选闭包双读/全文件 pin、prepared begin/outcome/abort recovery、逐文件 hash/Hashed capability、严格晚于 hash barrier 的 Prepared S3 binder，以及 current-admission S4/verified-or-rejected 原子 resolution 已形成代码，但尚未编译、启动、建库、迁移或接入 Host。启动路径仍未持有 root/cancellation capability，可信时间内核尚不能生产 observation；真实 HTTPS downloader、socket-read 取消、短文件终结、安全解包、candidate health/promotion、Sidecar/IPC/沙箱、typed 事件、`ReadyCapability` 上报和调度接线仍未实现。节点不直接发布商业 Offer，服务端 Offer 生成也未因本批合同自动接通。
