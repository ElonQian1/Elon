//! Retained child fixture for the q13 existing-first DMS release uncertainty.
...
use super::{
    payload, validate_binding, LockRunnerExistingFirstExclusiveReleaseCompletionV1,
...
impl Deref for RetainedInitializationFailureFixture {
    ...
    fn deref(&self) -> &Self::Target {
        self.fixture
            .as_ref()
            .expect("retained q13 Lock initialization fixture")
    }
}
...
impl Drop for RetainedInitializationFailureFixture {
    fn drop(&mut self) {
        if let Some(fixture) = self.fixture.take() {
            // The controlled native release has no return receipt. Retrying SQLite teardown could
            // repeat an uncertain unlock, so all unsafe custody remains child-owned until exit.
...
pub(super) fn exercise_child(
    ...
    let nonce = std::env::var(A2_DYNAMIC_CHILD_NONCE_ENV)
        .context("read parent-created q13 Lock initialization child nonce")?;
    ...
    let observation = fixture
        .route(SELECTED)?
        .observe_main_shm_lock_existing_first_exclusive_release_failure_v1(
...
fn prepare(root: &Path) -> anyhow::Result<RetainedInitializationFailureFixture> {
    let fixture = ManagedSqliteMultiConnectionFixture::open_single(root, [0xad; 16])?;
    ...
    {
        return Err(anyhow!(
            "q13 physical SHM precreation unexpectedly attached a coordinator target"
        ));
    }
    // The first out-of-budget Map call performs production WAL-main promotion and connection
    // attach, then stops before coordinator-node creation. This is the exact cold initialization
    // prestate; it does not synthesize a node or call the q13 native seam.
    let cold_map = route
...
fn validate_observation(
    ...
    {
        return Err(anyhow!("q13 controlled initialization receipt mismatch"));
    }
    Ok(())
}
...
fn initialization_expectation(
    ...
    ManagedSqliteShmTestInitializationExpectationV1 {
        case_v1: ManagedSqliteShmTestInitializationFailureV1::ExistingFirstExclusiveReleaseOutcomeUncertain,
...
fn terminal_values(
    ...
    let route = terminal
        .terminal_route()
        .ok_or_else(|| anyhow!("q13 Lock terminal route missing"))?;
    if terminal.joint_close_physical_failure_retention_count() != 0
        || terminal.joint_close_physical_failure().is_some()
    {
        return Err(anyhow!("q13 Lock retained unrelated joint-close custody"));
    }
    let values = [
    ...
    if values != expected {
        return Err(anyhow!("q13 Lock terminal custody ledger mismatch"));
    }
    Ok(values)
}
