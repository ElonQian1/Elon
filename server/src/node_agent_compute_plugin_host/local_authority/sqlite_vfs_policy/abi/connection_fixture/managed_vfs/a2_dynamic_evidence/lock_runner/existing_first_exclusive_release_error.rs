//! Process-isolated q13 receipts for one controlled existing-file initialization release uncertainty.
//!
...
const SELECTOR_ENV: &str = "ELON_SQLITE_A2_LOCK_EXISTING_FIRST_EXCLUSIVE_RELEASE_ERROR_SELECTOR";

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in super::super::super) enum LockRunnerExistingFirstExclusiveReleaseCompletionV1 {
    RetentionSucceeded,
    RetentionRouteUnknown,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in super::super::super) struct LockRunnerNativeAcquireExistingFirstExclusiveReleaseErrorBindingV1
{
    ...
    pub(in super::super::super) completion: LockRunnerExistingFirstExclusiveReleaseCompletionV1,
...
pub(in super::super::super) fn run_lock_native_acquire_existing_first_exclusive_release_error_program_isolated(
    exact_test: &str,
    ...
    if let Some(root) = super::selected_child_root()? {
        let selected = std::env::var(SELECTOR_ENV)
            .context("read parent-selected q13 Lock initialization program")?;
...
fn run_parent(
    ...
    if exact_test.is_empty() {
        return Err(anyhow!("q13 Lock initialization exact test name is empty"));
    }
    let executable =
        std::env::current_exe().context("resolve q13 Lock initialization test executable")?;
...
fn validate_parent_receipt(
    ...
    if !child.matches_family(SanitizedPayloadFamily::LockQuotient) {
        return Err(anyhow!(
            "q13 Lock initialization child payload family mismatch"
        ));
    }
    ...
    if !child.matches_registration_id(payload.registration_id) {
        return Err(anyhow!(
            "q13 Lock initialization child registration binding mismatch"
        ));
    }
    let environment =
    ...
    {
        return Err(anyhow!(
            "q13 Lock initialization parent cleanup binding mismatch"
        ));
    }
...
pub(super) fn validate_binding(
    ...
    let end = binding
        ...
        .ok_or_else(|| anyhow!("q13 Lock initialization range overflow"))?;
    if binding.count == 0 || binding.first >= 8 || end > 8 {
        return Err(anyhow!(
            "q13 Lock initialization range is outside eight slots"
        ));
    }
    if binding.action == LockRunnerActionV1::LockShared && binding.count != 1 {
        return Err(anyhow!(
            "q13 shared Lock initialization range is not one slot"
        ));
    }
    if !matches!(
        binding.action,
        ...
        return Err(anyhow!("q13 Lock initialization action is not an acquire"));
    }
    let mask = (((1_u16 << binding.count) - 1) << binding.first) as u8;
    if binding.mask != mask {
        return Err(anyhow!("q13 Lock initialization range mask mismatch"));
    }
    super::super::child::lock_existing_first_exclusive_release_error::selector(
...
pub(super) fn exact_selector(
    ...
    .expect("validated q13 Lock initialization selector")
}

pub(super) const fn completion_tag(
    completion: LockRunnerExistingFirstExclusiveReleaseCompletionV1,
) -> u64 {
    match completion {
        LockRunnerExistingFirstExclusiveReleaseCompletionV1::RetentionSucceeded => 1,
        LockRunnerExistingFirstExclusiveReleaseCompletionV1::RetentionRouteUnknown => 2,
    }
}

#[cfg(all(test, windows))]
pub(in super::super::super) fn selected_lock_native_acquire_existing_first_exclusive_release_error_selector_for_test(
) -> Option<String> {
...
#[cfg(all(test, windows))]
pub(in super::super::super) fn lock_native_acquire_existing_first_exclusive_release_error_selector_for_test(
    ...
    let binding = LockRunnerNativeAcquireExistingFirstExclusiveReleaseErrorBindingV1 {
        action,
        first,
        count,
        mask,
    ...
    validate_binding(binding)
        ...
        .map_err(|_| "q13 Lock initialization selector binding invalid")
}
