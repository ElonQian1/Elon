...
A2 总合同已经冻结。源码现推进到 `implementation_uncompiled`、`implementation_unrun` 的 A2b2 静态闭合：A2a/A2b1 的 registration exact route、共享 runtime 多 Connection、exact route→live WAL-main 私有脚本桥与 map/lock 内部 phase 仍保留；117 项 inventory 已覆盖 `xShmBarrier`、非末/末 `xShmUnmap`、WAL-main 联合 `xClose`、route lifecycle 与 VFS unregister。A2c 严格 `cfg(all(test, windows))` 的源码现含八类 foundation 路径：route-exact `Main/ShmMap`、`Main/ShmLock` callback-before；进程隔离的 VFS unregister before/after；以及四条进程隔离的 direct `xShmUnmap(false)` physical-subset runner。registration bridge 对账两个 `RegistrationShutdown/VfsUnregister` key；四个 SHM partial bridge 则都先验证完整 frozen inventory，再分别只选择 `Unmap/FinalConnection/Keep` 下 `ViewUnmap`、`MappingClose`、`DmsSharedRelease` 与 `ShmFileClose` 的 `AfterSuccessKnown` key，用真实 callback result、exact probe pending/trigger 与去敏 observer 前后快照对账可直接观察的物理字段。`ShmFileClose` 桥由仅在 `file.close()` 返回 `Ok(receipt)` 后激活的 sealed trigger 证明控制流已越过成功关闭 seam；observer 本身不独立读取 close receipt 或 OS handle table。它们都未构造完整 dynamic `Case`，也未编译、测试或运行（`passed=0`、`WindowsDynamic=0`），117 项动态通过进度仍为 0；native failure adapter、SQLite/indexed route/main/lease/action 完整 custody、竞争和逐 case Windows 动态验证仍缺。既有历史证据及这些未执行源码都不能替代逐 case Windows 动态证据。
