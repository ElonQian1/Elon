...
Offer 控制面允许当前用户为 active Provider/Pool 创建规范化 draft，服务端从 Provider、Pool 和 open Bucket 当前版本固定供给身份、容量绑定、窗口、区域、meter 与摘要；所有者可用精确版本撤销 draft。管理员 HTTP 可读取待审队列，并在一个事务中复用 Offer Registry 校验 Provider、Pool、Bucket、已发行容量和有效期，追加 active 连续版本与 v182 不可变发布回执。管理员发布不向 MCP 开放；没有 Price Snapshot 时候选查询仍不会返回该 Offer，也不会预留容量或移动资金。当前为 `implementation_uncompiled`。
...
| `server/src/store/compute_capacity_*.rs`、`compute_{provider,offer,job,reservation}_registry*`、`compute_offer_publications.rs`、`compute_quote_candidates.rs`、`compute_broker_reservation/`、`compute_federation_{provider,capacity_pool,capacity_bucket,capacity_supply,offer,broker}_{service,api,mcp}.rs`、`compute_federation_offer_{draft,publication}_*.rs`、`compute_federation_mcp.rs` | 共享容量池、版本化合同、Provider/Pool/Bucket/Supply 控制面、本人 draft Offer 与管理员原子 active 发布、项目级 Job 创建/既有候选发现与锁价、本人 Job/Reservation 查询、本地原子 Reserve、未执行任务原子 Release/Expire；当前未编译、未迁移、未运行验证，未接新报价生成、自动撮合、Attempt 或运行中结算 |
