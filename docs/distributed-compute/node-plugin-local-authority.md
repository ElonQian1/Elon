...
共享策略 ACK 之后现有独立 v10 preparation 握手只证明节点观察了同一 policy snapshot/authorization，并通过 v209 云端追加式账本保存请求、每会话 delivery 与 observation；它明确不打开本机 authority，当前也固定不返回 planning context。源码现已补上其后的无下载 policy-binding 内核：Bootstrap 只能在同一状态锁内把 exact desired snapshot、authorization、installation/root、进程实例、配置/取消代次和最新 preparation 封装成不可 Clone、不可序列化、持有实例锁 lease 的一次性本机意图；显式 disabled snapshot 走独立撤销意图，不要求 enabled authorization 或 preparation。任何非 replay 策略、实例锁或数据根代次变化都会先推进 live generation，使旧意图在 Store preflight、事务写前、insert 前、exact readback 后及返回 capability 前失败关闭。
...
这仍不是生产可调用的 planning context。v5 只终结 prepared fetch/verification；当前没有 NodeRuntime 账号/凭证切换到 Bootstrap generation 的失效 hook，没有把路径型 SQLite open 与 pinned-root 句柄身份闭合，也没有生产 trusted-time/rollback witness、已铸 Ready/Attempt 的独立 work-admission epoch/receipt 绑定或已安装插件的 signed `reauthorize_existing` 语义。源码已增加与 fetch guard 分离的 cleanup-only deletion domain，但只允许 exact 耐久授权且 owner=`cleanup_pending` 的线性 custody，在同 LocalAuthority 实例、同 pinned root、同 process-owner fence 与 retained handles 连续存活时跨单调 authority 后继继续；授权前策略竞态、进程轮换、重启和跨进程重新采用仍失败关闭。因此 `context_ready`、root/authority/download/Sidecar 标志继续固定为 false，policy-binding Store 没有 Host 调用点；即使外部出现合法签名，也不得把本批源码描述成已经允许 PlanApply、下载或任务准入。
