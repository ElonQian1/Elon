...
| 外部算力池适配器与统一报价 | v221-v246 前置门为 `implementation_partially_verified`；V243/V244 保留短时凭据与可撤销采用的局部通过证据，V245 加固 legacy 投影，V246 新增从 V227 CAS 按 V232 manifest 形成 `installed_inert` 内容寻址安装实例的源码。V245/V246 均为 `implementation_uncompiled`、`passed=0`；Provider 仍未激活，Adapter 未启动。见 [`当前状态`](current-implementation-status.md)、[`V246 安装权威`](external-pool-adapter-installation-authority.md)、[`V244 采用权威`](external-pool-adapter-adoption-authority.md) 与 [`V243/V245 凭据回执权威`](external-pool-adapter-credential-verification-authority.md) |
...
Provider Adapter 的 v227-v246 前置门已形成分层权威；V244 汇合当前 V239/V243 为可撤销采用授权，V245 加固 legacy 投影，V246 再增加从隔离 CAS 同一复验句柄安全解包并登记 `installed_inert` 内容树的源码。V245/V246 尚未编译或运行。最新状态见 [`current-implementation-status.md`](current-implementation-status.md)，安装边界见 [`V246 惰性安装`](external-pool-adapter-installation-authority.md)，签名沙箱边界见 [`V239 动态沙箱符合性证据`](external-pool-adapter-artifact-sandbox-conformance-authority.md)，采用边界见 [`V244 采用授权`](external-pool-adapter-adoption-authority.md)。服务器仍不启动或执行制品，不返回原始凭据，也未激活 Provider 或启用生产 Adapter。
