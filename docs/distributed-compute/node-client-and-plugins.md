...
这只是 PlanApply live-binding 的必要前置，不是可用 planning context。v6 目录 binding、rollback V2 和 opened-authority 合同均保持失败关闭：仅空库存可绑定，V1/V2 不互转，裸远端 checkpoint 不能冒充本机事实，open intent 与 snapshot producer 都不可构造。v3-v6 版本链已通过 8 项内存 SQLite 测试，v4 policy-binding 通过 4 项投影/回执测试；v5 companion receipt 通过 5 项 Store 测试，其中 3 项覆盖零 work-set 的原子提交、孤立提交失败和不可变性，另 2 项覆盖同一事务终结 1 个 prepared fetch 与 1 个 prepared verification，以及孤立 companion 回滚后两者不变；v6 空目录 binding 写入层通过 3 项原子提交/精确回读、过期 authority fence 零残留和不可变测试，候选层另有 5 项真实 Ed25519 测试覆盖正常双钥、Manifest/catalog 域隔离、目标绑定和 Publisher/Control 原始公钥指纹隔离。尚未覆盖 v5 双回执完整恢复/接管、v6 完整 bind/CAS/recovery、生产 root/keyring、生产磁盘库或 Host 联调。精确合同与启用前置见 [`node-plugin-manifest-catalog-authority.md`](node-plugin-manifest-catalog-authority.md)。v5 当前也只撤销下载/校验能力；继续保持零 PlanApply、零下载、零安装、零 Sidecar。
...
本文是已接受的客户端目标设计。NodeAgent 内部 legacy Host seam、版本化合同、Manifest/InstallPlan 与双 keyring、本机 authority v3/PlanApply、可恢复分段取数、Windows 受管文件、候选原子验证、可信时间证明和回滚锚回执合同已形成代码。ZIP staging Store、候选健康成功 Store、终止失败 quarantine Store 及各自不确定结果恢复/adoption 合同均已形成，默认关闭的 Bootstrap 已挂载到 NodeRuntime；失败候选还形成 cleanup authorization Store、typed permit 与进程内 outcome recovery。当前 NodeAgent canonical 已通过 `elon-pc-node` 编译；v3-v6 schema 版本链通过 8 项内存 SQLite 测试，v4 policy-binding 通过 4 项投影/回执测试，v5 companion receipt 通过 5 项 Store 测试（含非零 prepared fetch/verification 的同事务终结与孤立回滚），v6 空目录 binding 写入层通过 3 项事务测试，目录候选通过 5 项真实 Ed25519 验签测试。v5 双回执完整恢复/接管、v6 完整 bind/CAS/recovery、生产 root/keyring、生产磁盘库或 Host 联调仍未专项验证；显式插件共享 consent、耐久策略下发/ACK、v10/v11 与 v209/v210 账本的云端路径仍未运行迁移或联调。v5/v6 测试不证明生产启动、Control signer、planning facts、Host 接线、signed reauthorization、Ready/Attempt 即时撤销或 cleanup 独立续行。生产时间源与回滚见证、真实 HTTPS downloader/socket-read 取消、跨重启本机策略见证与目录耐久恢复、真实 Sidecar 探针、线性删除 journal/executor、completion Store、installed/promotion、IPC/沙箱、`ReadyCapability` 上报和调度接线仍未实现。节点不直接发布商业 Offer；精确物化、健康和清理边界见 `node-plugin-archive-extraction.md`、`node-ready-capability.md` 与 `node-plugin-candidate-cleanup.md`。
