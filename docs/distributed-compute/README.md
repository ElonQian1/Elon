...
| 外部算力池适配器与统一报价 | v221-v255 前置门为 `implementation_partially_verified`。V250 scanner、V252 sandbox、V253 Provider-specific credential 与 V254 inert candidate 均保留主线既有定向证据；它们不执行扫描、真实沙箱或 secret 解析，V254 closure 固定未实现且 Provider 保持 `registering`。V255 仅追加 Provider-specific、server-fixed inert runtime launch profile，fresh 路径重审 V249 sealed tree 并消费 V249/V254；它不启动、不解析 secret、不激活或证明 readiness，也不消费 V250/V252/V253。V255 源码未编译或运行、`passed=0`，V254 的 18 个 temporary market absolute deny 原样保留。见 [`V255 launch profile 权威`](external-pool-adapter-runtime-launch-profile-authority.md)、[`V255 验收边界`](external-pool-adapter-runtime-launch-profile-acceptance.md)、[`V254 候选权威`](external-pool-provider-activation-candidate-authority.md) 与 [`当前状态`](current-implementation-status.md) |
...
Provider Adapter 的 v227-v255 前置门已形成分层权威；V244-V249 保留采用、安装、撤销、sealed live-FS seam 与 registry/companion 的既有边界。V250 scanner、V252 sandbox 与 V253 Provider-specific credential 三条可续签声明链以及 V254 inert candidate 均保留对应本地定向验收；它们不运行扫描、真实沙箱或 secret resolver，也不激活 Provider。V255 新增 Provider-specific、server-fixed inert runtime launch profile，owner/admin 可创建、检查 currentness 或撤销；fresh 路径重审 V249 sealed tree，但不执行 entrypoint、不解析 secret、不调用 resolver，也不消费 V250/V252/V253 短 TTL evidence。状态固定 `launch_profile_current_inert`，V255 源码未编译或运行、`passed=0`。V254 覆盖 direct SQL 与 versions 的 18 个 `external_pool` CapacityPool active、Offer draft/active temporary absolute deny 原样保留；后续 atomic activation 必须显式把它替换为完整 readiness/currentness admission gate。最新状态见 [`current-implementation-status.md`](current-implementation-status.md)、[`V255 runtime launch profile`](external-pool-adapter-runtime-launch-profile-authority.md) 与 [`V254 activation candidate`](external-pool-provider-activation-candidate-authority.md)。
