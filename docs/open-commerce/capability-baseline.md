...
| Sui 凭证投影、链下包与离线预检任务 | 已实现、适配器与任务代码未编译、无网络副作用 | 标准凭证可保存单笔投影包；已过账纠正可把冲销与替换共同保存为不可拆分的原子包。两类包均绑定目标网络和来源摘要、可复核完整性并固定 `not_submitted`；项目可签发限时、限网络和限包类型的机器凭据，并把当前可导出包显式加入短时租约队列。机器领取前由服务端只读重建交接内容，漂移或争议阻断任务，完成时原子追加 `passed/rejected` 报告。SDK、CLI 和自动续租工作器已写入但尚未运行；任务和报告不改投影状态，不含签名、广播、最终性或资金移动 | `server/src/task_settlement/sui_adapter_handoff_*.rs`、`server/src/task_settlement/sui_preflight_*.rs`、`server/src/store/task_sui_preflight_*.rs`、`sdk/open-commerce-connector/src/sui-preflight.js`、`sdk/open-commerce-connector/src/sui-preflight-job-client.js`、`sdk/open-commerce-connector/src/sui-preflight-worker.js`、`docs/decisions/sui-offline-preflight-adapters-v1.md`、`docs/decisions/sui-offline-preflight-job-leases-v1.md`、`docs/sui-offline-preflight-job-leases-v1-acceptance.md` |
...
| 服务端动作能力一次性确认 | 主干已实现；本人读取与取消代码未编译 | `action` 必须先生成 5 分钟确认并由当前用户独立确认；凭证绑定用户、App、商户、能力、Grant、幂等键和输入摘要，Invocation 创建与确认消费原子完成，同一幂等请求可重放但不能产生第二次动作。消费者 AI 可按当前用户和 App 重新读取脱敏状态，并在 Invocation 创建前主动取消；服务端不保存或返回原始输入，取消不回滚已创建调用。当前不含 WebAuthn、跨设备可信展示或商户运行时内部订单确认 | `server/src/open_commerce_action_confirmation_service.rs`、`server/src/open_commerce_action_confirmation_mcp.rs`、`server/src/store/open_commerce_action_confirmations.rs`、`docs/decisions/open-commerce-server-action-confirmation-v1.md`、`docs/open-commerce-server-action-confirmation-v1-acceptance.md` |
...
| 能力与内部同步回执来源绑定 | 已实现并通过定向 Rust/SQLite 回归 | 编辑者可把当前能力版本绑定到同项目、同商户、同接入的合格业务同步回执和已登记数据域；绑定、重绑、解绑及审计原子提交。目录公开厂商、方式、状态、时间与 SHA-256，能力更新或接入停用后失败关闭并回退普通声明。所有字段仍是商户项目内部声明，不是外部平台签名或真实回读；HTTP 实例和 PC 浏览器仍未实测 | `server/src/open_commerce_capability_source_*.rs`、`server/src/store/open_commerce_capability_sources.rs`、`docs/decisions/open-commerce-capability-source-link-v1.md`、`docs/open-commerce-capability-source-link-v1-acceptance.md` |
...
| 消费者发现与第三方应用沙盒 | 已实现；来源、价格、能力、偏好、声明期、排序器、候选范围和服务端凭证均已定向验证；HTTP、PC 与 Web Crypto 仍未验收 | 可注册、轮换、停用和重新启用测试 App，一次性显示测试 Token，跨项目发现主动发布商户，查看或撤回申请并在审批后调用；App 身份绑定所有者。发现代码支持五种用户可选、可解释且非付费的内置排序器，显示商户声明来源、新鲜度和可选内部同步回执摘要，允许消费者独立要求声明期内或内部回执来源能力，也可按单个厂商、数据域和 1 秒至 365 天的最大内部回执年龄精确筛选；PC 可从当前非穷尽候选窗口选择厂商和数据域建议，也可继续手输。价格上限先精确匹配三位币种再比较微单位，省略币种时兼容默认 CNY；消费者还能只查信息查询或经营操作、公开调用或需授权调用，并把城市、类别和全部标签从软偏好提升为硬约束。普通响应直接公开候选上限、目录候选数、合格数、返回数、截断状态和非全网穷尽声明；消费者也可显式生成经 PC 复核的未签名单次凭证。所有行为只作用于最多 100 个目录候选，来源固定未外部核验，筛选不绕过授权或动作确认，也不是全网公平性、数据真实性、外部平台背书或运营方身份的证明 | `server/src/open_commerce_client_api.rs`、`server/src/open_commerce_client_lifecycle_service.rs`、`server/src/open_commerce_directory_model.rs`、`server/src/open_commerce_consumer_ranking.rs`、`server/src/open_commerce_consumer_source_options.rs`、`server/src/open_commerce_consumer_constraints.rs`、`server/src/open_commerce_consumer.rs`、`server/src/open_commerce_consumer_source_tests.rs`、`server/src/open_commerce_consumer_filter_tests.rs`、`server/src/open_commerce_consumer_ranking_tests.rs`、`pc-frontend/src/features/open-commerce/consumerRankingReceipt.ts`、`pc-frontend/src/features/open-commerce/ConsumerCandidateScopeSummary.tsx`、`pc-frontend/src/features/open-commerce/ConsumerSourceFilterFields.tsx`、`pc-frontend/src/features/open-commerce/ConsumerPriceFilterFields.tsx`、`pc-frontend/src/features/open-commerce/ConsumerCapabilityFilterFields.tsx`、`pc-frontend/src/features/open-commerce/ConsumerPreferenceConstraintFields.tsx`、`docs/decisions/open-commerce-consumer-candidate-scope-v1.md`、`docs/decisions/open-commerce-pluggable-ranking-v1.md`、`docs/decisions/open-commerce-public-data-provenance-v1.md`、`docs/decisions/open-commerce-capability-source-link-v1.md`、`docs/decisions/open-commerce-consumer-source-requirement-v1.md`、`docs/decisions/open-commerce-consumer-source-filters-v1.md`、`docs/decisions/open-commerce-consumer-source-age-v1.md`、`docs/decisions/open-commerce-consumer-source-filter-options-v1.md`、`docs/decisions/open-commerce-consumer-price-currency-v1.md`、`docs/decisions/open-commerce-consumer-capability-filters-v1.md`、`docs/decisions/open-commerce-consumer-preference-constraints-v1.md`、`docs/decisions/open-commerce-consumer-freshness-filter-v1.md`、`docs/decisions/open-commerce-consumer-ranking-receipts-v1.md`、`docs/open-commerce/consumer-developer-sandbox.md` |
| 消费者发现请求输入边界 | 已实现并通过本地 Rust、SQLite 与 MCP 假数据验收 | 搜索词、能力键、App ID 和返回数量由共享领域服务统一规范化；返回数量只接受 1 至 50，未知字段、非法币种和硬约束缺值失败关闭。真实 SQLite 已验证 `%`、`_` 和反斜杠只作字面匹配，105 商户夹具验证固定 100 候选窗口和排序凭证输入指纹；HTTP 服务与 PC 页面未实际启动 | `server/src/open_commerce_consumer.rs`、`server/src/open_commerce_consumer_input_tests.rs`、`server/src/open_commerce_consumer_model.rs`、`server/src/store/open_commerce_directory.rs`、`docs/decisions/open-commerce-consumer-discovery-inputs-v1.md` |
| 消费者 AI 完整发现 MCP | 已实现并通过本地 Rust、SQLite、MCP 与领域服务同源假数据验收 | 消费者 AI 复用同一消费者发现服务的透明排序、来源、价格、能力和偏好约束；默认身份只做公开发现，显式 App 必须启用且属于当前用户和项目，参数不能覆盖身份。响应不泄露内部身份且无业务写入；工具不申请授权、不调用、不下单 | `server/src/open_commerce_consumer_discovery_mcp.rs`、`server/src/open_commerce_consumer_discovery_mcp_tests.rs`、`server/src/open_commerce_mcp.rs`、`docs/decisions/open-commerce-consumer-discovery-mcp-v1.md` |
| 消费者能力执行计划 | 已实现并通过本地 Rust、SQLite 与 MCP 假数据验收 | 用实际输入只读复核能力契约、App、封禁和授权状态；MCP 显式 App 必须属于当前用户和项目。计划按下一次调用的次数、价格和币种遍历选择可用 Grant；历史批准申请不能替代当前有效 Grant，全部不可用时返回稳定预算原因和续额下一步。动作明确拆成准备、用户确认、确认、调用，不占预算、不创建业务记录 | `server/src/open_commerce_consumer_execution_plan.rs`、`server/src/open_commerce_grant_readiness.rs`、`server/src/open_commerce_consumer_discovery_mcp.rs`、`server/src/open_commerce_consumer_execution_plan_tests.rs`、`docs/decisions/open-commerce-consumer-capability-execution-plan-v1.md` |
| 消费者 AI 单能力授权与续额申请 | 已实现并通过本地 Rust、SQLite 与 MCP 假数据验收 | 用户明确确认后，当前项目内本人启用的注册 App 可向商户申请一个受限能力；任一 Grant 仍能覆盖下一次调用时拒绝重复申请，全部预算不可用时允许新申请。相同待审批请求顺序重放复用记录且审计一次，响应不泄露内部身份；旧 Grant 不改写，成功仅为 pending，商户仍决定批准、期限和预算 | `server/src/open_commerce_consumer_discovery_mcp.rs`、`server/src/open_commerce_consumer_authorization_mcp.rs`、`server/src/open_commerce_consumer_authorization_request_mcp_tests.rs`、`server/src/open_commerce_grant_readiness.rs`、`server/src/store/open_commerce_authorization_requests.rs`、`server/src/store/open_commerce_grants.rs`、`docs/decisions/open-commerce-consumer-authorization-mcp-v1.md` |
| 消费者 AI 本人 App 目录 | 已实现并通过本地 Rust、SQLite 与 MCP 假数据验收 | 只读列出当前项目中本人拥有的 App、当前 MCP 身份和沙箱可用状态；显式身份必须属于当前用户和项目，越权或未知 App 失败关闭。响应不返回测试 Token、提示、生产凭据或所有者内部 ID，读取无业务写入；App 生命周期继续由开发者门户管理 | `server/src/open_commerce_consumer_app_mcp.rs`、`server/src/open_commerce_consumer_app_mcp_tests.rs`、`server/src/open_commerce_mcp_protocol.rs`、`docs/decisions/open-commerce-consumer-app-directory-mcp-v1.md` |
| 消费者本人授权状态、有效 Grant 与撤回 | 已实现并通过定向 Rust 回归 | 按当前用户和当前项目本人 App 隔离列出四种申请状态；另行列出仍有效 Grant 的期限、已用和剩余调用/计量预算，耗尽授权明确标记。项目编辑者经用户确认可撤回本人 pending 请求，非 pending 不变且不撤销 Grant。响应省略内部身份，列表不执行能力，实际输入仍须执行计划复核 | `server/src/open_commerce_consumer_authorization_mcp.rs`、`server/src/store/open_commerce_authorization_requests.rs`、`server/src/store/open_commerce_grants.rs`、`docs/decisions/open-commerce-consumer-authorization-status-mcp-v1.md` |
...
| 开发者 App 限权生产凭据 | 代码已形成、未编译，默认关闭 | 当前资料、域名证明和准入均有效后，平台管理员可显式签发一次性 `oc_live_` 凭据；明文不入库，范围不得超出已审资料，并受风险层级期限、轮换和紧急撤销约束。调用账本、幂等重放、终态事件和 Webhook 按凭据环境隔离，沙箱不得触达真实商户运行时。资料变化、域名重验、App 停用或准入暂停原子撤销，并停用关联生产订阅。PC 已形成只读生产就绪总览，按固定顺序显示 App、资料、域名、准入、凭据和 Webhook 阻断，并分别判断调用与通知是否就绪。它不代表外部身份核验、真实资金或生产部署 | `server/src/open_commerce_developer_credential_*.rs`、`server/src/open_commerce_developer_readiness_*.rs`、`server/src/store/open_commerce_developer_credentials.rs`、`server/src/open_commerce_invocation_service.rs`、`docs/open-commerce-invocation-credential-provenance-v1-acceptance.md`、`docs/open-commerce-developer-production-readiness-v1-acceptance.md` |
...
| 消费者结构化偏好档案与关系披露 | 已实现、范围受限 | 当前用户可保存类别、标签、城市和价格上限等低敏偏好，显式用于发现，并对有效 `preference.remember` 关系选择字段生成匿名快照；商户只见有效关系披露。当前未实现敏感字段披露、自动营销或跨运营方同步 | `server/src/open_commerce_consumer_preference_service.rs`、`pc-frontend/src/features/open-commerce/ConsumerPreferenceProfilePanel.tsx`、`docs/open-commerce-consumer-preference-disclosures-v1-acceptance.md` |
| 消费者客户端加密数据保险箱 | 代码已形成、未编译、范围受限 | PC 端用口令派生密钥并以 AES-GCM 加密敏感自由文本，服务端仅按本人项目保存密文、摘要、大小和修订。当前没有密码找回、多设备密钥同步、商户或 AI 授权解密，也未验证密码学往返 | `server/src/open_commerce_consumer_vault_*.rs`、`pc-frontend/src/features/open-commerce/consumerDataVaultCrypto.ts`、`ConsumerDataVaultPanel.tsx`、`docs/decisions/open-commerce-consumer-data-vault-v1.md` |
| 消费者关联数据删除请求、跟进与外部证明 | 请求已实现；跟进通过 Rust/SQLite、本地 Axum 路由和 PC 构建验证；证明代码未编译；范围受限 | 消费者可针对本人关系发起删除请求并原子撤销关系；商户只见匿名别名，可接单、拒绝或声明完成。消费者可按 24 小时冷却最多催办三次，并在超过 7 天内部目标后升级关注；时间异常失败关闭，商户限量查询先保留升级与逾期请求。HTTP 路由已覆盖鉴权、隔离和幂等，但真实绑定端口及浏览器未实测。它不是法定期限或平台仲裁。完成后商户可追加未核验外部回执摘要，仍不代表外部删除真实发生 | `server/src/open_commerce_data_request_service.rs`、`server/src/store/open_commerce_data_request_followups.rs`、`server/src/open_commerce_data_request_api_tests.rs`、`server/src/open_commerce_data_erasure_evidence_service.rs`、`pc-frontend/src/features/open-commerce/ConsumerDataRequestManager.tsx`、`docs/open-commerce-consumer-data-request-followups-v1-acceptance.md`、`docs/open-commerce-consumer-data-erasure-evidence-v1-acceptance.md` |
| 消费者可携带数据包 | 已实现、范围受限，V5 批次未编译 | V5 可导出本人关系历史、私有续期链、删除请求、商户未核验删除证明、低敏偏好、历史披露、账户级终态调用凭证和引用商户身份声明；接收方可隔离导入、验证来源运营方签名、逐字段选择低敏偏好并安全回滚，也可人工映射关系后重新授权。V1-V4 继续兼容但本批未回归。它仍不迁移原始回执、完整订单、支付、履约或商户私有 ERP 数据 | `server/src/open_commerce_portability_*.rs`、`pc-frontend/src/features/open-commerce/ConsumerPortabilityAdoptions.tsx`、`docs/decisions/open-commerce-consumer-portability-exports-v5.md`、`docs/decisions/open-commerce-consumer-portability-selective-adoption-v1.md` |
...
| 开发者 App 终态调用事件流 | 已实现、Rust/SQLite 服务层已验证、范围受限 | 测试 Token 可按 App 和沙箱环境绑定游标持续读取成功或失败摘要，并读取本 App 单条结果；同 App 平台调用不会进入沙箱列表或详情。序号由数据库终态触发器原子追加，PC 开发者门户可断点续读且不持久化 Token。轮询继续作为恢复与对账入口；真实 HTTP、生产凭据和 PC 浏览器本批未验证 | `server/src/open_commerce_developer_event_service.rs`、`server/src/store/open_commerce_developer_events.rs`、`pc-frontend/src/features/open-commerce/DeveloperInvocationEvents.tsx`、`docs/open-commerce-developer-terminal-events-v1-acceptance.md` |
| 开发者 App 签名 Webhook | 代码已形成、未编译，生产默认关闭 | App 所有者可配置白名单内标准 443 HTTPS 回调，并固定沙箱或生产环境以及成功、失败事件范围；新订阅须完成签名 challenge 才能启用。耐久队列、短租约、指数退避、死信、自动停用、公网地址固定、密钥轮换、单条重试和最多 100 条历史补发均保留。沙箱只接收旧调用和沙箱调用；生产只接收生产调用，并要求双开关和当前有效 App、资料、域名、准入及凭据，资格失效会停用订阅并终止未完成投递。PC 已形成按环境聚合订阅、积压、重试、待处理与已确认死信和生产阻断码的只读健康摘要，并可用不可覆盖的操作人、时间和原因确认单条死信。当前尚未完成编译、迁移、DNS、TLS、并发、网络、性能、跨语言签名和 UI 验证，也不是工商身份、跨运营方总线、支付回调或业务动作重放 | `server/src/open_commerce_webhook_*.rs`、`server/src/open_commerce_production_webhook*.rs`、`server/src/store/open_commerce_developer_webhook*.rs`、`server/src/store/open_commerce_production_webhooks.rs`、`docs/open-commerce-production-webhooks-v1-acceptance.md`、`docs/open-commerce-webhook-operational-health-v1-acceptance.md`、`docs/open-commerce-webhook-dead-letter-acknowledgement-v1-acceptance.md` |
...
| 商户数据自主控制和跨应用授权 | 部分实现 | 跨项目脱敏目录、消费者沙盒、限时 App 授权、匿名关系、低敏偏好披露、客户端加密消费者保险箱、删除请求、有界催办与升级关注、商户未核验证明、V5 可验证导出、隔离导入、调用配额、终态结果流、商户业务证据、人工和受限机器 ERP/CRM 衔接回执、活动证据、手动 App 封禁和首个商户自有运行时已形成；自动外部通知与仲裁、保险箱授权解密、完整订单迁移、真实删除执行与回执核验、生产 ERP/CRM 写入适配器、外部回读、全网签名信任、自动风控与公共互操作治理尚未完成 |
