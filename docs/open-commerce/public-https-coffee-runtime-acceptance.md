...
2026-08-14，一龙当前源码通过真实 HTTP 路由、限权生产凭据、Grant、服务端动作确认和商户运行时出站安全模块，经公网标准 443 HTTPS 调用 `cofficethinking` 真实服务器，并在该商户真实 PostgreSQL 和统一 ERP 订单接口中完成一笔未支付订单。

本次证明的是“一龙平台当前代码可调用公网商户节点并落入真实 ERP”的纵向子链，不是完整生产平台上线、真实支付或 AI 自主选店的证明。
...
- 每次完整验收会在暂停门店新增一个验收商品、报价和未支付订单，必须显式传入确认开关。
...
1. 一龙为隔离开发者 App 签发一次性显示、数据库仅存摘要的 `oc_live_` 凭据，范围只有 `order.quote.create`、`order.commit` 和 `order.status.read`。
2. 一龙登记固定公网运行地址和密钥引用，执行 Manifest/HMAC 健康验证并得到 `runtime_status=active`。
3. 商户发布三项授权能力和目录记录，并向该 App 建立最多 3 次调用、6000 CNY 微单位的一小时 Grant。
4. 消费者应用通过一龙 HTTP 路由创建报价。
5. `order.commit` 先准备动作确认，再由用户确认短语绑定原输入；没有确认回执不能提交。
6. 一龙经公网 HTTPS 转发已签名信封，咖啡运行时按服务端报价事务扣减库存并创建开放商业订单和统一 ERP 订单。
7. 相同提交幂等键重放返回同一 Invocation 和订单，库存不再扣减。
8. `order.status.read` 返回同一开放订单、同一统一 ERP 订单和 `unpaid`。
9. 消费者订单闭环接口返回同一商户业务回执，平台资金移动仍为 false。
...
|---|---|
| 运行 ID | `1786671773634` |
| 验收商品 | `3b2674ab-8e45-4f84-bbbe-e364b2b84ca4` |
| 开放商业订单 | `5ab6987e-d228-4aab-b946-21636e9d93dc` |
| 统一 ERP 订单 | `d61adaf8-2f74-4702-a3b6-2d542788d359` |
| 库存 | `5 -> 4` |
| ERP API 命中数 | `1` |
| 提交调用回执数 | `1` |
...
- 消费者 AI 先通过公开目录自主发现、排序、选择商户再下单；
- 真实 LLM 在无人代填请求参数的情况下自主做出购买决定；
