...
| 通用 ERP 蓝图 / 公共运行时内核 / 版本能力目录 / 独立商户配置 / 初始化 Matter / 物化合同与状态 / AI 通用提案 / 证据化升级与完整回滚 | `docs/erp/README.md`、`docs/erp/operating-workflow-v1.md`、`docs/erp/api-and-agent-tools-v1.md`、`sdk/merchant-erp-kernel/`、`server/src/erp_blueprint/`、`server/src/erp_blueprint_api.rs`、`server/src/erp_blueprint_mcp*.rs`、`server/src/store/erp_*.rs`、`contracts/erp/`、`pc-frontend/src/features/open-commerce/erp-blueprint/` |
...
| 消费者客户端加密敏感数据保险箱 / 密文修订 / 本人下载与删除 / 本地密码学 / HTTP 隔离 / V220 联合主键迁移 | `docs/decisions/open-commerce-consumer-data-vault-v1.md`、`docs/open-commerce-consumer-data-vault-v1-acceptance.md`、`server/src/open_commerce_consumer_vault_{service,api,migration}*.rs`、`server/src/store/open_commerce_consumer_vault.rs`、`pc-frontend/src/features/open-commerce/consumerDataVaultCrypto.js`、`consumerDataVaultCrypto.d.ts`、`ConsumerDataVaultPanel.tsx`、`scripts/test-open-commerce-consumer-data-vault-crypto.mjs` |
...
| 消费者可携带数据包 V5 / 删除证明 / 商户身份声明 / 隔离导入 / 运营方 RSA 签名 / 已验证单包字段级采用 / 多来源偏好冲突选择与回滚 / 本地口令加密归档 / 已验证关系映射与重新授权 | `docs/decisions/open-commerce-consumer-portability-exports-v5.md`、`docs/decisions/open-commerce-consumer-portability-exports-v4.md`、`docs/decisions/open-commerce-consumer-portability-imports-v1.md`、`docs/decisions/open-commerce-consumer-portability-trust-v1.md`、`docs/decisions/open-commerce-consumer-portability-adoption-v1.md`、`docs/decisions/open-commerce-consumer-portability-selective-adoption-v1.md`、`docs/decisions/open-commerce-consumer-portability-multi-source-merge-v1.md`、`docs/decisions/open-commerce-consumer-portability-archive-v2.md`、`docs/decisions/open-commerce-consumer-portability-reauthorization-v1.md`、`server/src/open_commerce_portability_adoption_{service,api}_tests.rs`、`server/src/open_commerce_portability_reauthorization_{service,api}_tests.rs`、`server/src/open_commerce_portability_*.rs`、`server/src/store/open_commerce_consumer_portability*.rs`、`server/src/store/open_commerce_portability_reauthorization.rs`、`sdk/open-commerce-connector/src/portability-signature.js`、`portability-archive.js`、`pc-frontend/src/features/open-commerce/ConsumerPortabilityExports.tsx`、`ConsumerPortabilityImports.tsx`、`ConsumerPortabilityTrustKeys.tsx`、`ConsumerPortabilityAdoptions.tsx`、`ConsumerPortabilityMergePanel.tsx`、`ConsumerPortabilityReauthorization.tsx`、`portabilityArchive.js` |
...
| 任务级分布式算力联邦 / Provider、Offer、Job、Reservation、Attempt、验证回执与 CNY 账本 / external-pool V249-V277 roots、readiness、route source、ELTP、active-successor与atomic activation | `docs/distributed-compute/README.md`、`docs/distributed-compute/current-implementation-status.md`、`docs/distributed-compute/external-pool-adapter-stable-executor-atomic-activation-authority.md`、`docs/distributed-compute/external-pool-adapter-provider-active-successor-authority.md`、`docs/distributed-compute/external-pool-adapter-task-protocol-production-authority.md`、`docs/distributed-compute/external-pool-adapter-task-protocol-conformance-authority.md`、`docs/distributed-compute/external-pool-route-source-projection-bridge-authority.md`、`docs/distributed-compute/external-pool-adapter-provider-runtime-readiness-authority.md`、`docs/distributed-compute/external-pool-adapter-registry-authority.md`、`docs/distributed-compute/market-and-settlement.md` |
| external-pool V270 Provider runtime readiness / cleanup 后短时 receipt / startup custody / 五条 API | `server/src/compute_federation/external_pool_adapter_provider_runtime_readiness.rs`、`server/src/compute_federation/external_pool_adapter_provider_runtime_readiness_api.rs`、`server/src/compute_federation/external_pool_adapter_provider_runtime_readiness_service.rs`、`server/src/store/compute_external_pool_adapter_provider_runtime_readiness.rs`、`server/src/store/compute_external_pool_adapter_runtime_bundle/no_work_probe.rs`、`server/src/store/compute_external_pool_adapter_runtime_bundle/runtime.rs`、`server/src/store_migrations/compute_external_pool_adapter_provider_runtime_readiness.rs` |
| external-pool V271 migration-only route source bridge / V221 logical ID→V249 projection ID / 0 fence opened | `docs/distributed-compute/external-pool-route-source-projection-bridge-authority.md`、`docs/distributed-compute/external-pool-route-source-projection-bridge-acceptance.md` |
| external-pool V272 controlled task-protocol conformance / ELTP v1 14 roots + 8 exchanges / 2 tables + 1 diagnostic view / process HMAC ≤15s / 0 fence opened | `docs/distributed-compute/external-pool-adapter-task-protocol-conformance-authority.md`、`docs/distributed-compute/external-pool-adapter-task-protocol-conformance-acceptance.md`、`server/external-pool-adapter-session-core/src/task_protocol/mod.rs`、`server/src/compute_federation/external_pool_adapter_task_protocol_conformance.rs`、`server/src/compute_federation/external_pool_adapter_task_protocol_conformance_api.rs`、`server/src/compute_federation/external_pool_adapter_task_protocol_conformance_service.rs`、`server/src/store/compute_external_pool_adapter_task_protocol_conformance.rs`、`server/src/store_migrations/compute_external_pool_adapter_task_protocol_conformance.rs` |
| external-pool V273 dormant production transport/ingress / unique default-off env / ELTP v1 8 production roots / exact 6 tables / no public API / eligible_rows=0 | `docs/distributed-compute/external-pool-adapter-task-protocol-production-authority.md`、`docs/distributed-compute/external-pool-adapter-task-protocol-production-acceptance.md`、`docs/distributed-compute/attempt-delivery-outbox-v1.md` |
| external-pool V274 activation-rooted active successor / exact serde_json projected-active Provider / 2 immutable tables + 1 diagnostic view / private `source_written/source_review_only` kernel、无public orchestrator、durable refresh fail-closed | `docs/distributed-compute/external-pool-adapter-provider-active-successor-authority.md`、`docs/distributed-compute/external-pool-adapter-provider-active-successor-acceptance.md` |
| external-pool V277 stable executor + atomic activation `source_written/source_review_only` / 1 immutable receipt table、0 view、0 revocation / V274→V277 immediate witness / 16 INSERT + 1 CAS UPDATE / 9 pending permits + 9 absolute denies / uncompiled、unrun、eligible_rows=0 | `docs/distributed-compute/external-pool-adapter-stable-executor-atomic-activation-authority.md`、`docs/distributed-compute/external-pool-adapter-stable-executor-atomic-activation-acceptance.md` |
| 节点插件候选验证、ZIP 受控解包与失败候选清理 / create-new staging、逐文件 SHA-256/fsync/同句柄复验、线性保管权 / `partial_implementation_compiled` | `docs/distributed-compute/node-plugin-archive-extraction.md`、`docs/distributed-compute/node-plugin-candidate-cleanup.md`、`docs/distributed-compute/node-plugin-local-authority.md`、`server/src/node_agent_compute_plugin_host/candidate_extraction/`、`server/src/node_agent_compute_plugin_host/fetch_file/staging.rs`、`server/src/node_agent_managed_fs/copy.rs`、`server/src/node_agent_managed_fs/delete.rs` |
...
| 一龙多身份联合登录 / Google OIDC / 密码与恢复码 / 设备会话 / 安全事件、导出与注销预检 / 认证存储能力 / Win、APK、Web 账号安全 / AI CLI Provider 状态机、假协议诊断、Codex 保险箱显式同意 / OpenAI ChatKit API 聊天 / 禁用 Web Chat 生命周期 | `docs/unified-account-and-ai-provider-architecture.md`、`server/src/{account_security*,account_lifecycle,auth_request_guard,auth_safety_store,federated_auth*,openai_chatkit_api,node_agent_provider_*,node_agent_codex_vault_consent,node_agent_web_chat_adapter}.rs`、`server/src/store/{account_identities,account_security*}.rs`、`pc-frontend/src/features/{auth,account,chatkit,node/codexVaultConsent}/*`、`android/app/src/main/kotlin/com/elon/app/{GoogleFederatedAuth,AccountSecurityApi,AccountSecurityViews,chatkit/*}.kt`、`server/src/assets/{federated_auth,account_security,ai_provider_accounts}.*` |
