...
| 消费者能力执行计划 | 代码已形成、未编译 | 用实际输入只读复核能力契约、App、封禁和授权状态，返回调用或审批下一步；动作明确拆成准备、用户确认、确认、调用。计划不占预算、不创建业务记录 | `server/src/open_commerce_consumer_execution_plan.rs`、`server/src/open_commerce_consumer_discovery_mcp.rs`、`docs/decisions/open-commerce-consumer-capability-execution-plan-v1.md` |
| 消费者 AI 单能力授权申请 | 代码已形成、未编译 | 用户明确确认后，已注册自有 App 可向商户申请一个受限能力；有效 Grant 拒绝重复申请，相同待审批请求保持幂等。成功仅为 pending，商户仍决定批准、期限和预算 | `server/src/open_commerce_consumer_discovery_mcp.rs`、`server/src/store/open_commerce_authorization_requests.rs`、`docs/decisions/open-commerce-consumer-authorization-mcp-v1.md` |
