...
4. 新来源或 `partial` 来源先调用 `project_discussions_get_source_manifest`，对照图中 `processed_chunk_ids`，再按顺序调用 `project_discussions_read_source_chunk` 读取尚未处理的 chunk。禁止用普通 `project_docs_read` 全文读取聊天。
