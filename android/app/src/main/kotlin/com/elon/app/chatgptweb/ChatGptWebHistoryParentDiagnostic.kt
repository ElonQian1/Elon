...
internal object ChatGptWebHistoryParentDiagnostic {
    ...
    private val kinds = setOf("null", "empty_root", "client_root", "uuid", "other", "paginated_root")
    ...
    fun sanitize(value: JSONObject): String {
        val fields = setOf("schema", "code", "user_found", "nodes", "terminal")
        require(value.keys().asSequence().toSet() == fields + if (value.has("pagination")) setOf("pagination") else emptySet())
        ...
        if (value.has("pagination")) {
            require(observed)
            val pagination = value.getJSONObject("pagination")
            val keys = setOf("present", "complete", "root_owned", "current_leaf_matches")
            require(pagination.keys().asSequence().toSet() == keys)
            for (key in keys) require(pagination.opt(key) is Boolean)
            require(pagination.getBoolean("present") ||
                !pagination.getBoolean("complete") && !pagination.getBoolean("current_leaf_matches"))
        }
