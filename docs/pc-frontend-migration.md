...
| OneNote 式项目文档工作区（按需读写、Markdown 编辑/预览、低 token 项目图谱） | 🔴 高 | `src/features/project-docs/` | ✅ 已实现知识首页、最多四层主/副主题树、四维治理总览和可执行健康中心；项目图谱以 Rust 后端机器契约分别展示产品功能、技术架构和文档主题，文档覆盖与 `file:/test:/route:/symbol:` 实现证据分开；独立“讨论推理”分区可导入聊天、按根主题和分支展示推理过程，并从节点继续、分叉或晋升文档。网页和供应商无关 MCP 共用同一事实来源，目录/图谱阶段零模型 token、零正文读取；聊天原文先以低权威来源保存，再由 Windows 登录账号的 AI CLI 按需读取与编译。右键、`⋯`、`Shift+F10` 与触摸长按共用分区命令，个人排序留在浏览器，共享结构和审计写清单；健康问题支持筛选、分派、期限、原因化忽略/延期、选中后 AI 建议、趋势与评分解释，普通 Git 和托管知识库都可查看差异并安全恢复；默认整理前/后双提交，可安全 rename/move Markdown，不夹带代码、不自动 push |
