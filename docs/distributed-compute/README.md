...
| 激活证据申请控制面 | v177-v178、本人 HTTP/MCP、管理员 HTTP 审核/过期批准废止及双方只读就绪预检已写；申请锁定精确版本和稳定账本审计摘要，失效 approved 可审计转为 superseded 后重新申请，但所有操作 `activation_effect=none`，状态为 `implementation_uncompiled` |
