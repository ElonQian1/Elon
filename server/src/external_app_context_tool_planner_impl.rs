...
pub(super) fn plan_fb2_tools_with_platform_scope(
    context: &Value,
...
pub(super) fn match_evidence(context: &Value, query: &str) -> Option<Vec<String>> {
...
pub(super) fn opinion_memory_arguments() -> Value {
    json!({
...
pub(super) fn audit_evidence(context: &Value) -> Option<Vec<String>> {
    let mut evidence = Vec::new();
    for warning in [
        "fb2_budget_empty",
...
pub(super) fn context_has_warning(context: &Value, warning: &str) -> bool {
    context
        .get("context_quality")
...
pub(super) fn context_has_current_user_orders(context: &Value) -> bool {
    if context
        .get("user_orders")
        .and_then(Value::as_array)
...
pub(super) fn has_any_keyword(text: &str, needles: &[&str]) -> bool {
    keyword_evidence(text, needles).is_some()
}

pub(super) fn keyword_evidence(text: &str, needles: &[&str]) -> Option<Vec<String>> {
    if text.is_empty() {
        return None;
    }
...
pub(super) fn confidence_for(evidence: &[String]) -> u8 {
