...
- 如果改动影响一龙 `pc-frontend/`、`/pc`、`/pc-next` 或用户可见 PC 工作台 UI，不能只用 CodePushed 收尾。除非用户明确说“只同步代码/暂不发布”，否则必须先运行 `pc-frontend` 的构建验证，再 push，之后运行 `scripts/publish-server.ps1` 或 `scripts/publish-server.sh` 上传 `$DATA_DIR/pc-next-dist`，并验证 `/pc` 可访问且 `/api/server/version` 的 `gitSha` 等于本次提交。遇到截图、遮挡、错位、层级、弹窗或按图修复类 UI 问题，先把截图中的区域定位到真实组件/样式文件，再用本地预览、浏览器截图、DOM/坐标/层级检查之一做视觉验收；无法截图时必须说明替代证据，不能只凭 `npm run build` 宣称用户问题已解决。
