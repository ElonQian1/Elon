...
#   .\scripts\setup-node-env.ps1 -CodexOnly         # 只安装/修复 Codex CLI
...
function Add-CodexRuntimeBinsToPath {
    ...
    $entries = @(Get-ChildItem -LiteralPath $root -Directory -ErrorAction SilentlyContinue |
        Where-Object { Test-Path -LiteralPath (Join-Path $_.FullName 'codex.exe') -PathType Leaf } |
        Sort-Object LastWriteTimeUtc -Descending)
    if ($entries.Count -eq 0) { return }

    foreach ($entry in $entries) {
        $dir = $entry.FullName
        if (-not (Test-Path -LiteralPath (Join-Path $dir 'codex.exe') -PathType Leaf)) { continue }
        $exists = $false
        foreach ($part in @($env:PATH -split [System.IO.Path]::PathSeparator | Where-Object { $_ })) {
    ...
    if ($PersistUser) {
        $preferred = $entries[0].FullName
        $userPath = [System.Environment]::GetEnvironmentVariable('PATH', 'User')
        if ($null -eq $userPath) { $userPath = '' }
        $parts = @($userPath -split [System.IO.Path]::PathSeparator | Where-Object { $_ })
        $withoutPreferred = @($parts | Where-Object { $_.TrimEnd('\') -ine $preferred.TrimEnd('\') })
        $nextParts = @($preferred) + @($withoutPreferred | Where-Object { $_ })
        ...
        if ($nextUserPath -ne $userPath) {
            ...
            Ok "Codex CLI 路径已写入用户 PATH：$preferred"
        }
    }
}
...
function Install-CodexCli {
    ...
    if (-not $shell) {
        Warn "未找到 PowerShell，无法运行官方 Codex CLI 安装器。"
        return $false
    }

    $installer = 'https://chatgpt.com/codex/install.ps1'
    $installCommand = "`$ErrorActionPreference='Stop'; "
    if ($Silent) {
        $installCommand += "`$env:CODEX_NON_INTERACTIVE='1'; "
    }
    $installCommand += "try { irm '$installer' | iex; exit 0 } catch { Write-Host ('Codex installer failed: ' + `$_.Exception.Message) -ForegroundColor Yellow; exit 1 }"
    & $shell -NoProfile -ExecutionPolicy Bypass -Command $installCommand
    $ok = ($LASTEXITCODE -eq 0)
    Refresh-Path
    Add-CodexRuntimeBinsToPath -PersistUser
    return $ok
}
...
function Ensure-CodexCli {
    ...
    if (Has 'codex') {
        Warn "检测到 codex 命令但无法运行；常见原因是 PATH 指到了 Codex 桌面端的 WindowsApps 受保护资源。将继续安装/修复真正的 Codex CLI。"
    }

    $installerOk = Install-CodexCli
    if (RunsVersion 'codex') {
        Ok "Codex CLI 安装成功：$(codex --version)"
        return
    }

    if (Has 'npm') {
        if (-not $installerOk) { Warn "官方安装器未成功，尝试 npm 备用安装方式。" }
        Step "备用安装 Codex CLI（npm install -g @openai/codex）"
    ...
    Warn "Codex CLI 官方安装器未完成，且 npm 不可用；请稍后重新点击安装/修复。"
}
...
if ($CodexOnly) {
    ...
    Write-Host "`nCodex CLI 暂未就绪。请查看上方安装错误后重试。" -ForegroundColor Yellow
    exit 1
}
...
$checks = @(
    ...
    [pscustomobject]@{ Name = 'Codex CLI';       Ok = (RunsVersion 'codex'); Note = 'AI 编码代理' }
