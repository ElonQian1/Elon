...
- current Lock source 已推进到 q19：q18/q19 CreatedFirst/ExistingFirst shared-busy/close-ok 各承接 88 singleton（16+72，44/44 completions）。q19 先物理预建 SHM，再由 cold `was_created=false` 进入 ExistingFirst；两批均要求同 `FileId` 的 distinct holder/target handles 真实争用、attempt 分账和 explicit target close success。q19 `a2lockq19`=194，catalog=88 rows/18,210 bytes，SHA-256=`eb318d91edbd0bbcd7e68ff626504a007a3f3c96d5eb60b965c9e362a421eee8`。q1–q19 inventory m/g=`4372/4372 present,4296/3768 missing,8668/8140 total`，q12–q19=`704/704`，initialization remaining=`2728/2200`。均 source-only/uncompiled/unrun、actual=`not_run`；coverage=0、`Qlock=unknown`，natural/Windows/production/economic closed。精确边界见[`shared-busy tranches`](node-plugin-vfs-lock-shared-busy-tranches-authority.md)；19-artifact refresh 接受前须含 q15–q19。
...
| Lock | `8668/8668` | `0/8668` | `not_opened` | q18/q19 CreatedFirst/ExistingFirst shared-busy/close-ok各88；q19含物理预建与cold ExistingFirst，均以same-FileId distinct holder/target真争用、分账、explicit close success；m/g=`4372/4372 present,4296/3768 missing,8668/8140 total`，init remaining=`2728/2200`；actual=`not_run`、`Qlock=unknown`；19-artifact refresh须含q15–q19；详见 [shared-busy tranches](node-plugin-vfs-lock-shared-busy-tranches-authority.md) |
...
| OS lock outcome | shared/exclusive sibling relation 分别对账 success/contended/error；q5–q11 不变，q12–q17 保留 release/truncate 不可观测合同。q18/q19 只能由 same-FileId distinct holder/target 的真实 lock contention 得到 Busy，holder/target attempts 必须分账且 target close success 显式可见；q19 另须证明物理 SHM 预建、cold ExistingFirst 与 `was_created=false`。Expected=`BusyAfterKnownMutation`、mutation known、lock certain、file/DMS Released、native=`2/1`、`cleanup_rewrite=false`。八条未运行 seam 未来只能形成 `controlled_fault_actual`，不得冒充 natural actual；详见 [shared-busy tranches](node-plugin-vfs-lock-shared-busy-tranches-authority.md)。`SQLITE_BUSY` 不得与注入结果、I/O failure、mutation uncertainty 或 close failure合并。 |
...
当前正式结论：Map/Lock `StaticContract=43476/43476 + 8668/8668`。q12–q19 八个 initialization slices 各为 source-only 88 singleton；q18/q19 只接受 CreatedFirst/ExistingFirst shared-busy/close-ok、same-FileId distinct holder/target 真争用、分账与 explicit close success，q19 另含物理预建和 cold ExistingFirst。q1–q19 inventory m/g=`4372/4372 present,4296/3768 missing,8668/8140 total`，initialization remaining=`2728/2200`。current source=`source_written/source_review_only/implementation_uncompiled/implementation_unrun`，`passed=0 failed=0 actual=not_run`、无 actual/reviewed digest/frozen manifest，coverage=`0/43476 + 0/8668`、`Qmap/Qlock=unknown`、`WindowsDynamic=not_opened`。19-artifact refresh 接受前须含 q15–q19；A2 仍未动态接受，production/Ready/dispatch/market/settlement/economic closed。细节只见[`shared-busy tranches`](node-plugin-vfs-lock-shared-busy-tranches-authority.md)。
