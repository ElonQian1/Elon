...
| Provider / Offer / Job / Reservation / Lease / Receipt 统一领域合同 | 基础代码、v169-v175 注册表与首版 Broker Reserve 已写，状态为 `implementation_uncompiled`，尚未迁移、接线和运行验证 |
...
| 消费者余额预授权 | v175 Broker 已将兼容余额表的显式到期预授权与 Job/Claim/Reservation 在同一事务内编排，并拒绝非 `reserved` 结果；仅支持 `platform_balance_cny`，退款/结算终态仍不可组合 |
| Broker 原子 Reserve | v175 schema、不可变回执、严格请求重放与历史绑定审计已写；单一事务内完成余额预授权、容量 Hold、pending/active Reservation 和 quoted/reserved Job，状态为 `implementation_uncompiled`，尚未执行迁移或接入 HTTP/MCP |
...
v173 为 Capacity Claim 的每次 revision 保存完整不可变 JSON、状态和规范摘要，数据库拒绝修改或删除历史版本，历史 Reservation 不必依赖后来已变化的 Claim 当前投影。v174 ComputeReservation Registry 在此基础上精确绑定 Job、Offer、Price Snapshot 和 Claim 历史版本；创建和更新使用消费者级幂等、`expected_revision`、revision/digest CAS 与受限状态机，当前和历史读取都会重新审计全部依赖。注册表单独调用时只登记已存在的合同，不创建 Claim、不冻结预算或移动容量；v175 Broker 才负责组合这些事务内入口。两者同样为 `implementation_uncompiled`。

后续实现可选择 Offer 查询与撮合、价格源/期货曲线和报价生成、容量自动调度与受控修复、Broker HTTP/MCP 接线、带 `fencing_generation` 的尝试租约、预留退款与最终结算、多源计量、挑战任务、争议状态和可提取收益账本。
