...
结束期间挂断按钮禁用，重复点击不会重复发送挂断命令。两轮挂断确认仍未完成时，界面收起为“仍在通话”悬浮球；已有的版本化网页快照会在官网持续返回会话页两秒后立即完成挂断确认。没有快照事件时，单飞看门狗在最多两分钟内只执行十次递增间隔的只读核对，不再点击挂断。官网延迟结束后会自动关闭悬浮球并刷新当前会话，超过上限则停止观察，避免无限轮询和发热。短时间重复快照不会误判挂断，状态没有变化时也不会重复触发无障碍播报或持续动画。
