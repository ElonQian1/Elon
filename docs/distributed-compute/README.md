...
| UserNode Windows Runner loader/进程监管前置草案 | 未登记且不可达：typed chain 已从 retained discovery 铺到 exact context、package-only pre-lease PE、preliminary requests、私有 GrantReady wave-zero prefix、package+去重 system-image custody、pre/post cross-binding、post-lease recursive system-image final projection envelope、QueryVerified lineage与 process pre-create projection；nested API-set DAG与 Shadow positive fail-closed，projection不是 live machine/WOW64 receipt。真实 selector、prelease/recursive parser、resolver、positive-consuming、逐波 custody advancer、sealer/query、live-OS/create/resume/recovery producers仍缺；Windows dynamic=`0`，严格 `source_written/source_review_only/implementation_uncompiled/implementation_unrun`、`passed=0/failed=0`、`migration/table/writer=none/none/none`、四 gap=`missing`、loader 18 effects=`none`。见 [launch-context authority](user-node-windows-runner-launch-context-selection-authority.md)、[recursive closure authority](user-node-windows-runner-recursive-system-image-closure-authority.md)、[recursive closure acceptance](user-node-windows-runner-recursive-system-image-closure-acceptance.md)、[loader](user-node-windows-runner-loader-load-set-authority.md) 与 [process](user-node-windows-runner-process-custody-authority.md) 专题 |
