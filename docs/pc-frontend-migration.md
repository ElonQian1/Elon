...
| 多端后台微调画布 | 🔴 高 | `src/features/ui-tuner/headless-design/` | 🟡 已接入 Web/PWA/Tauri/Android designSession、聊天前 DesignIntentPlan、平台/route 自动切换、AI task lease、cursor + checkpoint 断点续读、DraftOperation v2、源码多候选与漂移健康、分平台写回风险/适配器审批、Tauri 分层证据、验证矩阵和右侧默认对话；画布跟随指定任务。代码尚未编译或平台运行验收，安装节点仍待升级到 MCP schema v1.10 |
