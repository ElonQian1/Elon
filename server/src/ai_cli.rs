    ...
    R: AsyncRead + Unpin,
{
    ...
    while let Ok(Some(line)) = lines.next_line().await {
        ...
        if let Some(tx) = progress_tx.as_ref() {
            for message in stream_event_to_ws_messages(&line) {
...
/// 将 codex --json 事件行翻译成 0~N 条 WS 消息（JSON 字符串）。
///
/// 同一个事件可能同时输出 `tool_call`/`tool_result`/`usage` 结构化事件
/// 以及对应的人类可读 `progress` 文本，前者用于结构化时间线展示，后者
/// 保留给只关心文本进度的旧客户端。
fn stream_event_to_ws_messages(line: &str) -> Vec<String> {
    ...
    match event_type {
        "item.started" => {
            ...
            match item_type {
                "agent_reasoning" => push_progress(&mut out, "AI 正在思考……".to_string()),
                "command_execution" => {
                    let cmd = item
                        .get("command")
                    ...
                    if snippet.is_empty() {
                        push_progress(&mut out, "AI 正在执行命令……".to_string());
                    } else {
                        push_progress(&mut out, format!("AI 执行命令：{}", snippet));
                    }
                }
                "file_change" => {
                    let mut args = serde_json::Map::new();
                    ...
                    push_progress(&mut out, "AI 正在修改文件……".to_string());
                }
                _ => {}
            }
        }
            ...
            match item_type {
                "agent_message" => {
                    if let Some(text) = item.get("text").and_then(Value::as_str) {
                        ...
                        if !snippet.is_empty() {
                            push_progress(&mut out, format!("AI 回复片段：{}", snippet));
                        }
                    }
                }
                "command_execution" => {
                    ...
                    push_progress(&mut out, "命令执行完毕".to_string());
                }
                "file_change" => {
                    let status = item
                        .get("status")
                    ...
                    push_progress(&mut out, "文件修改完毕".to_string());
                }
                _ => {}
            }
        }
        // codex --json 在每次 turn 完成时会发出 token_count 事件，
        // 或在 turn.completed 里携带 usage 字段。这里两种格式都尝试解析。
