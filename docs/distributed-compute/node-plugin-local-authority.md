...
begin 调用 Store 前失败只留下不可再授权的 unclaimed custody；Store 一旦被调用，任一错误都返回 recovery custody，verification ID、非序列化 recovery key、初始 absence 证明、cancellation guard 和全部 share-none 句柄不分离。Store 成功后必须先删除 absence 证明，再做可能失败的句柄/取消复验。恢复读取先按 verification ID 分流：只有 ID 缺失且 authority revision/time/epoch、inventory、candidate/完整动态 closure、generation 与 prepared fetch/run absence 全部仍等于 mutation 前快照，才返回 `NotCreated`；行存在但任一不可变字段不同、ID 缺失伴随漂移或 generation 被消费都作为损坏。当前稳定结果只接受 exact `Prepared/Aborted/Revoked`；abort/revoke 的 typed JSON 必须规范重序列化相等并重算 JCS SHA-256，verified/rejected 在 hash binder 同批落地前失败关闭。

prepared run 成功后，未来 hash binder 必须在事务外从每个仍打开的文件 byte 0 以有界 buffer 重算原始字节 SHA-256，每次 read 前检查同一 cancellation guard，检查 exact EOF，并在同一句柄复验 FileId、类型、reparse、卷、单链接与长度；不持久化跨版本不可移植的 SHA 内部状态。当前 Authorized capability 已线性持有 prepared facts、recovery key 和全文件句柄，但尚未开放 hash 方法。
