...
> v218 合同修正：历史“v11 Planning Snapshot V2”现以协议阈值 12 解释；work-admission `None` 允许新晋升槽首次重授权，`Some` 才承诺 current head。A1 同事务 projector 已随完整测试目标编译，但无专项运行或 producer；A2 总合同已冻结，测试目标可编译且 targeted fault matrix 5 项通过。Barrier 与 RegistrationShutdown 已分别形成 `WindowsDynamic=8/8`，RegistryLifecycle 已形成 `16/16`，Unmap 已形成 `49/49`，JointClose 又在 clean evidence commit `83ed2a33c3e5e7dcfdecd253d94670eb0a78d71d` 上形成 `36/36`，A2b2 当前为 `WindowsDynamic=117/117`；clean wide regression `266/266` 已通过。Map/Lock 独立 denominator 仍未开放，状态仍为 `implementation_not_dynamically_accepted`。v14 永久 blocked-only，生产 VFS/open/Connection/Runtime/Ready/派发不可达。
