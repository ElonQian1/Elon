...
| 激活证据申请与计划控制面 | v177-v180、本人 HTTP/MCP、管理员 HTTP 审核/过期批准废止、申请/计划预检、不可变计划和受控应用回执已写；`admin/owner` 显式应用精确计划时，单一事务登记 active Provider 下一版本、激活 Pool、终结申请/计划并保存回执。该效果不连接节点、不发布 Offer、不移动资金，状态为 `implementation_uncompiled` |
