...
| 外部算力池适配器与统一报价 | v221-v266 保留各层证据，V261-V265 runtime fixture 只作历史 provenance。V267/V268 写入 current V2 hardening 与 Provider-neutral signed compatibility；V269 以默认关闭的一条 admin POST、启动托管 cgroup FD 与 exact V249 binding 同步运行 V268，并返回管理员 courier 所需最小 signer payload。没有私钥托管、signer transport、worker/outbox、新表或 migration；V267-V269 均为 `source_review_only/implementation_uncompiled/implementation_unrun`、`passed=0`。证据不是 production readiness；没有第三方 binary、生产 upstream/Secret、route/activation/market admission，Provider 仍 `registering`，V254 18 deny 不变。见 [`V269 权威`](external-pool-adapter-runtime-compatibility-signing-handoff-authority.md)、[`验收`](external-pool-adapter-runtime-compatibility-signing-handoff-acceptance.md) 与 [`当前状态`](current-implementation-status.md) |
...
Provider Adapter v227-v266 的证据只在各自历史版本内有效。V267 写入 current V2 hardening，V268 写入 Provider-neutral signed compatibility，V269 再接一条默认关闭、同步、管理员 courier 的 signing handoff；三批都未编译或运行，`passed=0`。V269 没有 signer transport/私钥托管、worker/outbox 或新 durable schema，V260-V265 current 全链仍须重验。V268/V269 不能与 atomic activation/admission 同批闭合：后续事务必须组合 current V268 durable release root 与 fresh V265 Provider-specific no-work/target/Secret availability，并先成功 shutdown/reap；V254 18 deny 在完整替代 gate 动态验收前逐字保留。见 [`V269 signing handoff`](external-pool-adapter-runtime-compatibility-signing-handoff-authority.md)、[`V268 signed compatibility`](external-pool-adapter-signed-runtime-compatibility-authority.md)、[`V267 hardening`](external-pool-adapter-post-exec-supervisor-hardening-authority.md) 与 [`当前状态`](current-implementation-status.md)。
