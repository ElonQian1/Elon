...
- 只需要验证“能直接读取群聊功能，不用截图，也不写群”时，跑 `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\smoke-fb2-visible-chat.ps1 -ReadOnlyDirectRead -Fb2Username 123qwe -Fb2Password 123qwe`。该模式只做 fb2 session bridge、`chat-bootstrap`、群成员检查和 baseline 消息读取，必须输出 `direct group message read baseline`、`read-only direct group read text fingerprint`、`writes=false`，并写出 `fb2.main_project.visible_chat_readonly.v1` summary JSON；不会发送 `@EL`、不会调用 selected-message `/ai-reply`、不会创建 summary post。截图只能作为 APK UI 辅助材料，不能替代这份 API 直读 summary。
- 需要快速交接当前进度时，跑 `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\smoke-fb2-ai-center-status.ps1 -OutputPath target\fb2-ai-center\status-current.json`。该脚本只读取本地 summary、git SHA 和环境变量是否存在，输出 `blockers`、`next_actions` 和 `validation_scope.group_chat_evidence=api_direct_read_summary_only`；它不会访问 live fb2 Context Pack、不会写群、不会保存消息正文，也不能替代 `-DataOnlyAcceptance` 或 `-FinalAcceptance`。
