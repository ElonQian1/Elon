...
| 消费者 AI 单能力授权与续额申请 | 代码已形成、未编译 | 用户明确确认后，已注册自有 App 可向商户申请一个受限能力；任一 Grant 仍能覆盖下一次调用时拒绝重复申请，全部预算不可用时允许新申请，相同待审批请求保持幂等。旧 Grant 不改写，成功仅为 pending，商户仍决定批准、期限和预算 | `server/src/open_commerce_consumer_discovery_mcp.rs`、`server/src/open_commerce_grant_readiness.rs`、`server/src/store/open_commerce_authorization_requests.rs`、`server/src/store/open_commerce_grants.rs`、`docs/decisions/open-commerce-consumer-authorization-mcp-v1.md` |
