...
fn generation_mode(plan: &SymbolPatchPlan) -> PatchGenerationMode {
...
fn blocked_reasons(plan: &SymbolPatchPlan, mode: PatchGenerationMode) -> Vec<String> {
...
fn action_for_target(target: &PatchEditTarget, plan: &SymbolPatchPlan) -> String {
    let symbol = target
        .qualified_name
        .as_deref()
...
fn constraints_for_target(target: &PatchEditTarget, plan: &SymbolPatchPlan) -> Vec<String> {
...
fn same_change_target(change: &ProposedPatchChange, target: &PatchEditTarget) -> bool {
...
fn diff_contract(plan: &SymbolPatchPlan, ready_to_generate: bool) -> PatchDiffContract {
