...
6. For `needs_follow_up`, submit a narrow follow-up requirement. For `blocked_capability`, run `-Action Improve -BlockingImprovement`, review that repair, then run `-Action Resume` against the terminated original `requirement` (or an existing `resume_original` generation), never against the `capability_repair` task. `Resume` fails closed unless the parent role is `requirement` or `resume_original`, has the current supervision protocol, and has a platform-recorded isolated worktree; the node independently enforces the same role allowlist before workspace resolution, lease admission or migration, Git metadata repair, and child-task creation, then revalidates Git identity and exclusive occupancy.
