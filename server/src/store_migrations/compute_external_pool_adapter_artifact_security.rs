...
pub(crate) fn migration_v233(conn: &Connection) -> Result<()> {
...
mod tests {
    ...
    #[test]
    fn artifact_security_migration_is_repeatable() {
        ...
        .unwrap();
        crate::store_migrations::compute_external_pool_adapter_artifact_signing_key::migration_v230(&connection).unwrap();
        crate::store_migrations::compute_external_pool_adapter_artifact_signed_provenance::migration_v231(&connection).unwrap();
        crate::store_migrations::compute_external_pool_adapter_artifact_package::migration_v232(
            &connection,
        )
        .unwrap();
        migration_v233(&connection).unwrap();
        migration_v233(&connection).unwrap();
        for object in [
            "compute_external_pool_adapter_artifact_security_receipts",
            "compute_external_pool_adapter_artifact_security_current",
        ] {
