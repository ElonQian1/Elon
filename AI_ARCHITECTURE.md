...
v177-v179 另行保存 Provider/Pool 激活证据申请、过期批准废止审计和不可变激活计划。本人 HTTP/MCP 可显式提交、查询和取消，平台 `admin/owner` 只能通过 HTTP 审核、废止失效 `approved` 或准备计划；双方均可只读预检申请状态、路由、verified 硬件、信任层、精确版本和账本审计并获得稳定阻断码。计划写入在单一事务内重新审计依赖，固定下一 Provider revision、路由引用、verified 硬件摘要和规范计划摘要；废止申请会同步废止 prepared 计划。`approved`、`ready_for_activation` 与 `prepared` 都固定返回 `activation_effect=none`，不触发 Provider/Pool 激活、Offer 发布或资金移动。该控制面为 `implementation_uncompiled`，详细边界见 `docs/distributed-compute/activation-evidence-api.md`。
...
| `server/src/store/compute_activation_{requests,plans}.rs`、`compute_federation_activation_{service,api,mcp}.rs`、`compute_federation_activation_plan_{model,service}.rs` | v177-v179 激活证据申请、本人控制面、管理员 HTTP 审核、废止、预检和不可变 prepared 计划；不激活 Provider/Pool、不发布 Offer，当前未编译、未迁移、未运行验证 |
