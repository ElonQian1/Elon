...
| 节点端点凭据与认证会话权威 | 服务端 v216 已铺默认空的 credential/version/revocation/session receipt/session head 五表及 sealed Store currentness 内核；零 legacy backfill，未接 HTTP、WS 认证、NodeAgent 或协议，也没有 secure transport proof producer。后续源码为 legacy WS 增加 process-local exact key、AgentManager→NodeRegistry 固定锁序及三条插件 ACK 提交 read fence，只阻止同进程旧 reader 越过 replacement；它不读取耐久 session head，也不等于在线 socket、Ready、route、outbox、command、Lease 或派发权威。源码尚未编译、测试或执行迁移。见 `node-endpoint-session-authority.md` |
