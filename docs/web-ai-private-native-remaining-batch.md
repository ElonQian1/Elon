...
| Share/delete/conversation files | Native pin/rename/archive/move; private file index plus ordinary/project/library-linked, [simple image-pointer](chatgpt-private-image-download.md) and [imported connector-copy download candidates](chatgpt-private-connector-file-download.md); guarded flag-selected private delete and current-chat reset; [ordinary full-conversation private share candidate](chatgpt-private-conversation-share.md) | Verify saved-file downloads, live flag binding, current/noncurrent deletion and share creation/update. Complete shared/mounted-library, connector-only cloud-reference and parameterized-image downloads, plus remaining sharing variants. System distribution alone is not official share creation. |
...
Grouped device transfer and saved-byte verification remain pending; the distinct
shared-library binary route and parameterized pointers remain code gaps.
