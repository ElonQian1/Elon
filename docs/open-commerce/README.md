...
| 消费者发现、关系授权与第三方应用接入 | 跨项目目录、商户声明来源与新鲜度、透明非付费排序、匿名关系、低敏偏好、客户端加密数据保险箱、删除请求、可验证导出与隔离导入、动作确认、授权预算、调用事件和封禁主干已形成，多项子集已通过 Rust/SQLite、进程内 Axum 或 PC 构建验收。开发者资料、域名本地状态、准入、一次性生产凭据哈希、轮换和撤销生产 Webhook 的联动现已通过全新 SQLite 专项测试；生产能力仍默认关闭，测试不代表真实域名验证、生产鉴权或网络调用。外部平台官方回读、真实 DNS/TLS、历史升级、第三方排序器 SDK、自动删除仲裁、多设备密钥同步、完整订单迁移、跨运营方身份互认、真实资金和生产公共网络仍未完成 | `docs/open-commerce/capability-baseline.md`、`docs/decisions/open-commerce-developer-production-credentials-v1.md`、`docs/decisions/open-commerce-production-webhooks-v1.md` |
