...
A2a/A2b1 源码已把一个 registration 扩为 exact logical-name route 集合，每条 Connection 独立 route/authorizer/custody 并共享一个 WAL/SHM runtime；exact route 绑定 runtime generation + SHM connection ID，map 初始化与 lock 平台动作进入相邻 test-only hook。早期 owner graph、Map template ledger 与局部 ABI/raw fragments 只是在 static closure 前定位 gaps 的 provenance；完整 terminal projection、`SourceBranch`、`Expected`、`CaseKey`、exclusion 与 StaticContract 已由独立 static denominator authority 闭合为 Map `43476/43476`、Lock `8668/8668`。typed dynamic quotient projector/candidate 已有前序编译与定向测试基线；本批 sealed runner-admission current source 已写但未编译、未运行。Map 在原三预算 6-member program 之外新增 Empty/Reuse/TargetMissing × Observe/Extend 的 exact 6-member single-region lifecycle 源级程序，绑定 installed `xShmMap`、same-target pre/post、一次性 ledger、NotPresent sentinel clear、mapped output↔selected pointer 仅进程内相等性与 parent/child/cleanup；Lock 保留 10 个 request-validation 与 104 个 positive lifecycle program。这些都只是源级 observation/native-receipt 接线，`passed=0 failed=0 actual=not_run`。当前真实缺口是 Map 其余 `43,464`、Lock 其余 `8,554` 个 planned-missing member 的 program source、current-source 验证、frozen manifests、`Qmap/Qlock` 和逐 class Windows evidence；A2b2=`117/117` 不能替代该动态门，也不启用生产入口。
...
当前静态验收已经闭合：Map `StaticContract=43476/43476`，Lock `StaticContract=8668/8668`。类型化 projector/candidate 只保留前序验证基线，sealed runner-admission current source 为 `source_written/source_review_only/implementation_uncompiled/implementation_unrun`。Map 新增 6 个 lifecycle 加原三预算 6 个 program 后，未运行 inventory 预期为 `12` source-present、`43,464` planned-missing；Lock 10+104 后预期为 `114` source-present、`8,554` planned-missing。observer/native-receipt 源码链不能替代 actual execution。后续门槛是验证 current source、补齐剩余 Map/Lock program source并冻结 exact quotient manifests，使 `DynamicQuotientMemberCoverage` 分别达到 `43476/43476` 与 `8668/8668`；再在同一 exact clean Windows cohort 下让每个 class 恰好产生一条正式记录，原子形成 `Qmap/Qmap` 与 `Qlock/Qlock`。
