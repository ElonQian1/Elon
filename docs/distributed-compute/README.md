...
| UserNode Ready source lineage 草案 | 未登记 API-free source draft：shared contract 与 Node owner adapter 只把 v217 work-admission、局部 Ready-health 和明确 `Untrusted` 的 Host runtime observation 投影为六键 JCS lineage；CPU-only 保持 accelerator=0，不虚构 GPU。固定缺 local currentness、runtime transition、Host runtime 与 v15 session 四项权威，当前 `implementation_uncompiled/implementation_unrun`、`passed=0/failed=0`、`migration/table/writer=none/none/none`；不生成 Ready、route、Offer、Attempt、Lease 或下游效果。见 [authority](user-node-ready-source-lineage-authority.md) 与 [acceptance](user-node-ready-source-lineage-acceptance.md) |
