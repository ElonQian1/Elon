...
| 消费者可携带数据包 | 已实现、范围受限，新增批次未编译 | V4 可导出本人关系历史、私有续期链、删除请求、低敏偏好、历史披露、账户级终态调用凭证和引用商户身份声明；接收方可隔离导入、验证来源运营方签名、逐字段选择低敏偏好并安全回滚，也可人工映射关系后重新授权。旧 V1/V2/V3 摘要保持兼容。它仍不迁移完整订单、支付、履约或商户私有 ERP 数据 | `server/src/open_commerce_portability_*.rs`、`pc-frontend/src/features/open-commerce/ConsumerPortabilityAdoptions.tsx`、`docs/decisions/open-commerce-consumer-portability-exports-v4.md`、`docs/decisions/open-commerce-consumer-portability-selective-adoption-v1.md` |
