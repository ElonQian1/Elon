...
cleanup execution plan、expected objects、plan seal 与 append-only step events 已纳入 v3 必需对象，并已随当前 `elon-pc-node` 编译及 schema 建库与重开指纹定向测试。plan 绑定 authorization、installation、staging root、candidate-parent anchor identity 与完整对象父子拓扑；每个对象只有完成 durable intent、exact disposition/受约束的恢复缺失、exact parent 下的 namespace absence 和独立 namespace-durability fence，completion 才能同时引用 plan digest、既有物理 execution evidence digest 与独立 terminal journal digest。completion Store 内核已按这三类摘要改造，并只接受没有构造入口的终态 journal capability；物理 evidence v1 仍没有不可变记录及 plan/object digest 绑定。当前 topology typed Store 只消费 `AuthorizedCandidateCleanup`，原子写入 plan、全部 expected objects 与 seal；首对象 intent Store 只消费 sealed topology，在独立事务推进可信时间、写入序号 1 的 `delete_intent`，并 exact readback 事件列、JSON 与 JCS 摘要。两类 Store 的不确定结果只能分别查询 exact `NotCreated/Sealed` 与 `NotCreated/Durable`，所有能力继续保留原始句柄和根锁。新增私有单对象 executor 只消费 durable intent，重新比对 plan 与 retained custody 后对 ordinal 0 原句柄设置强 disposition；失败保留目标和其余 custody，成功返回保留 exact parent 与根锁的物理 capability。新旧 executor 均未向 Host 暴露。disposition/absence/durability journal Store、后续 ordinal、物理 evidence v2、真实 parent namespace flush 及跨重启物理 recovery 均未实现。
...
当前供应链、候选健康与失败候选清理代码已通过 `elon-pc-node` 编译；清理链通过 3 项 Windows 同句柄删除、1 项目录顺序、4 项 completion、5 项 topology builder/Store、4 项首 intent builder/Store 及 3 项强 disposition 测试，合计 20 项，另有 1 项 topology/journal schema 建库与重开测试。前三项只覆盖旧删除语义，新增强 disposition 测试覆盖原句柄设置后保留 exact parent、父目录别名失败保管与 pending-to-plan 身份绑定；仍没有 retained custody → cleanup authorization → topology → intent → disposition 的完整事务夹具。cleanup authorization Store 可推进 `cleanup_pending`，topology Store 可封存 exact execution plan，intent Store 可独立提交第一个物理动作前置授权，私有单对象 executor 可消费该 capability 并保留物理结果 custody；新旧 executor 均没有 Host 入口。由于 disposition/absence/durability journal Store、后续 ordinal 和目录 durability 仍缺，终态 capability 不可构造，完整私有流水线尚不可达。当前仍未接入生产 Host 或生产本地库，不能授予生产重试、安装、promotion、ReadyCapability 或商业 Verification。多对象 handle-tree 收口、跨重启物理恢复、真实下载/Sidecar/探针、installed/promotion/GC 与 Attempt 接线仍待实现；精确边界见 `node-ready-capability.md` 与 `node-plugin-candidate-cleanup.md`。
