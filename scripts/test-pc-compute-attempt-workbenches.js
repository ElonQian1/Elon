...
assert.ok(lineageContracts.includes("const CARRIER_DIGEST_DOMAIN = 'ELON-COMPUTE-CORE-HISTORICAL-LINEAGE-V1'"), 'digest domain must stay frozen')
...
assert.match(lineageButton, /useLayoutEffect\(\(\) => \{[\s\S]*requestGeneration\.current \+= 1[\s\S]*setState\(\{ status: 'idle' \}\)[\s\S]*\}, \[leaseId, scope\]\)/, 'Lease or scope changes must invalidate and clear historical lineage evidence before paint')
...
for (const phrase of ['并行核验因果链', '重试因果链核验', '双响应摘要与跨链等式已核验']) {
...
assert.match(lifecycleHistory, /FederationHistoricalLineageButton leaseId=\{item\.settlement\.lease_id\} scope=\{scope\}/, 'each settlement history row must pass its Lease and scope')
