...
| `candidate_verification_runs` | candidate 级 verification ID/generation、完整工件集摘要、本机 pinned file-set binding、原 authority state revision 与 authority/process fence、prepared/terminal 状态和不可变结果；当前 schema 只开放 abort/revoke，verified/rejected 仍不可达 |
...
begin 调用 Store 前失败只留下不可再授权的 unclaimed custody；Store 一旦被调用，任一错误都返回 recovery custody，verification ID、非序列化 recovery key、初始 absence 证明、cancellation guard 和全部 share-none 句柄不分离。Store 成功后必须先删除 absence 证明，再做可能失败的句柄/取消复验。恢复读取先按 verification ID 分流：只有 ID 缺失且 authority revision/time/epoch、inventory、candidate/完整动态 closure、generation 与 prepared fetch/run absence 全部仍等于 mutation 前快照，才返回 `NotCreated`；行存在但任一不可变字段不同、ID 缺失伴随漂移或 generation 被消费都作为损坏。当前稳定结果只接受 exact `Prepared/Aborted/Revoked`；abort/revoke 的 typed JSON 必须规范重序列化相等并重算 JCS SHA-256。schema 仍拒绝进入 verified/rejected；其 transition、resolution fence 和结果 reader 必须与后续 resolver 原子落地。
...
以上新增代码仍未编译、未测试、未执行 DDL，也未在 NodeAgent 启动或 Host 路径接线，因此没有真实数据库、网络下载、插件安装或 Sidecar 运行。代码层已形成 fence-owned cancellation、分段耐久写入与 commit 恢复边界；authority v3 还形成 candidate verification journal、abort/revoke fence、promotion 禁入门卫，以及受管独占句柄从 byte 0 有界 SHA-256、每次 read 取消检查、exact EOF 和 hash 后身份复验底座。verification 现已形成 S1 exact closure、已验 root 下全文件 pin、两类规范摘要、post-pin S2、消费线性能力的 prepared begin、精确 absence/outcome reader、canonical terminal result validator、prepared recovery abort、完整 ordinal hash、首个 mismatch 证据、observed artifact-set JCS digest、线性 Hashed capability，以及严格晚于 hash 的 authenticated time、cancellation source、exact Prepared S3 双读与完整 custody 绑定。begin/outcome/abort 的每次 Store 写都由第三次读或双读 permit、完整身份 CAS 和写后重读包围；commit error 仍只能恢复查询，不能盲重试。post-hash binder 本身只读且不是 current-admission replay；verified/rejected 原子 resolution 与槽转换仍未形成，hash 目前也没有独立于签名闭包的总 I/O 字节或墙钟预算。可信时间 observation 生成/attestation、数据库外防整库回滚锚点、短文件 damage 终结、socket-read 取消、真实 HTTPS downloader、安全解包/逐文件 closure、candidate health/promotion/GC、跨重启恢复和 Host/Sidecar/IPC/沙箱仍待实现；本节不代表下载器或插件系统已经可用。
