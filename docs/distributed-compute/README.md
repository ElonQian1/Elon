...
| 外部算力池适配器与统一报价 | v221-v258 前置门为 `implementation_partially_verified`。V255 inert profile 为 `12 passed`；V256 已通过完整目标编译和 `14 passed`，覆盖 strict manifest、摘要、locked-memory 行为与私有边界，但 Windows protected-DACL、Linux syscall 和生产 mount/secret 未验收。V257 已随完整 Windows 目标编译，专项/Linux syscall 仍 `passed=0`。V258 又在 exact V255 profile 下封存 hostname/port/expected SPKI 与 server-fixed future broker TLS policy，公开 summary 隐藏 endpoint；完整目标编译与 11 项 migration/Store/源码/owner-admin HTTP 专项已通过，但仍不做 DNS/TLS/network/spawn/secret/session/probe/route/activation，readiness 全 false。V254 18 deny 逐字保留；V258 文件升级、重开、并发/崩溃、真实 TCP 和生产数据库仍未验收，下一批仍先缺 supervisor/session policy companion。见 [`V258 target 权威`](external-pool-adapter-upstream-transport-target-authority.md)、[`V258 验收边界`](external-pool-adapter-upstream-transport-target-acceptance.md)、[`V257 capsule 权威`](external-pool-adapter-entrypoint-capsule-authority.md)、[`V256 resolver 验收`](external-pool-adapter-runtime-bundle-resolver-acceptance.md)、[`V255 验收边界`](external-pool-adapter-runtime-launch-profile-acceptance.md) 与 [`当前状态`](current-implementation-status.md) |
...
Provider Adapter 的 v227-v258 前置门已形成分层权威。V255 durable inert profile 已通过本地 `12 passed / 0 failed` 定向验证，但仍不代表 runtime ready。V256 保留按 profile digest 寻址的 strict bundle manifest、retained handles 与 locked/zeroize config/credential；V257 再从 fresh V249 retained entrypoint 形成 Linux-only exact `0500` sealed memfd，并用独立 ephemeral companion policy 审计 static ELF64 `ET_EXEC`、no `PT_INTERP/PT_DYNAMIC/ET_DYN` 与 no W+X；它不回写或重解释 V255 durable profile。V258 又在 exact V255 profile 下追加 canonical hostname/port/expected SPKI 与 future broker TLS policy root，但只是一条 inert durable target；完整目标编译与 11 项本地专项已通过。V257/V258 均不 spawn、probe、deliver secret 或 activation；V258 还不做 DNS/TLS/network/session/route，`runtime_launch_ready=false`、`activation_ready=false`。V255 UTF-8 JCS framing 仍与 V256 arbitrary secret bytes 不兼容；下一批必须先形成 supervisor/session policy companion，再实现 child-only IPC，之后才可将 V258 target 绑定进真实 broker TLS/upstream probe。V254 18 deny 逐字保留，完整 admission gate 只能与 atomic activation 同批替换。见 [`V258 upstream target`](external-pool-adapter-upstream-transport-target-authority.md)、[`V257 entrypoint capsule`](external-pool-adapter-entrypoint-capsule-authority.md)、[`V255 runtime launch profile`](external-pool-adapter-runtime-launch-profile-authority.md)、[`V256 runtime bundle resolver`](external-pool-adapter-runtime-bundle-resolver-authority.md) 与 [`current-implementation-status.md`](current-implementation-status.md)。
