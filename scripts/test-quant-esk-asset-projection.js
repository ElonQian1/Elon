...
assert.match(launchSchema.$defs.launchTicket.properties.esk_asset_projection.pattern, /^\^yep1/)
assert.match(launchSchema.$defs.grantMessage.properties.esk_asset_projection.pattern, /^\^yep1/)
assert.deepEqual(
  launchSchema.$defs.readyMessage.properties.capabilities.items.enum,
  ['yilong.esk.asset_projection.v1'],
)
...
assert.ok(bridge.includes('capabilities: [ESK_PROJECTION_SCHEMA]'))
