//! Retained child fixture for the q12 created-first DMS release uncertainty.
...
use super::{
    payload, validate_binding, LockRunnerCreatedFirstExclusiveReleaseCompletionV1,
...
impl Deref for RetainedInitializationFailureFixture {
    ...
    fn deref(&self) -> &Self::Target {
        self.fixture
            .as_ref()
            .expect("retained q12 Lock initialization fixture")
    }
}
...
pub(super) fn exercise_child(
    ...
    let nonce = std::env::var(A2_DYNAMIC_CHILD_NONCE_ENV)
        .context("read parent-created q12 Lock initialization child nonce")?;
    ...
    let observation = fixture
        .route(SELECTED)?
        .observe_main_shm_lock_created_first_exclusive_release_failure_v1(
...
fn prepare(root: &Path) -> anyhow::Result<RetainedInitializationFailureFixture> {
    let fixture = ManagedSqliteMultiConnectionFixture::open_single(root, [0xac; 16])?;
    ...
    // prestate; it does not synthesize a node or call the q12 native seam.
    let cold_map = route
    ...
    {
        return Err(anyhow!("q12 Lock cold WAL-main attach receipt mismatch"));
    }
...
fn validate_observation(
    ...
    {
        return Err(anyhow!("q12 controlled initialization receipt mismatch"));
    }
    Ok(())
}
...
fn initialization_expectation(
    ...
    ManagedSqliteShmTestInitializationExpectationV1 {
        case_v1: ManagedSqliteShmTestInitializationFailureV1::CreatedFirstExclusiveReleaseOutcomeUncertain,
...
fn terminal_values(
    ...
    let route = terminal
        .terminal_route()
        .ok_or_else(|| anyhow!("q12 Lock terminal route missing"))?;
    if terminal.joint_close_physical_failure_retention_count() != 0
        || terminal.joint_close_physical_failure().is_some()
    {
        return Err(anyhow!("q12 Lock retained unrelated joint-close custody"));
    }
    let values = [
    ...
    if values != expected {
        return Err(anyhow!("q12 Lock terminal custody ledger mismatch"));
    }
    Ok(values)
}
