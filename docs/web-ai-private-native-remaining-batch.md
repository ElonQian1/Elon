...
| Share/delete/conversation files | Native pin/rename/archive/move; private file index plus ordinary/project/library-linked, [simple image-pointer](chatgpt-private-image-download.md), [imported connector-copy](chatgpt-private-connector-file-download.md) and [shared-library attachment download candidates](chatgpt-private-shared-library-download.md); guarded flag-selected private delete and current-chat reset; [ordinary full-conversation private share candidate](chatgpt-private-conversation-share.md) | Verify saved-file downloads, live flag binding, current/noncurrent deletion and share creation/update. Complete standalone/mounted-library, metadata-only shared references, connector-only cloud-reference and parameterized-image downloads, plus remaining sharing variants. Binary download still lacks in-app cancel when notifications are disabled and process-death cleanup/resume. System distribution alone is not official share creation. |
