...
这仍不是生产 planning context：缺 pinned-root 句柄型 authority open、catalog digest、可信时间/回滚见证、snapshot producer、Ready/Attempt work-admission 与 signed `reauthorize_existing`。cleanup-only deletion domain 也仅允许已耐久授权、owner=`cleanup_pending` 的线性 custody在同 authority/root/process fence 与 retained handles 连续存活时继续；授权前竞态、进程轮换、重启和跨进程 adoption 均失败关闭。因此 `context_ready`、`snapshot_ready` 及 root/authority/download/Sidecar 标志保持 false，Store 无 Host 调用点，也不允许 PlanApply、下载或任务准入。
