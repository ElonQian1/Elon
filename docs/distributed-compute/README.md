...
| 外部算力池适配器与统一报价 | v221/v222 管理面、v227 bytes source、v229 lifecycle、v230 signer key、v231 exact RSA signed provenance 和 v232 bounded static package proof 已分别通过本地专项，均为 `implementation_partially_verified`。v232 证明固定 ZIP/canonical manifest 与 admission/source/provenance 精确一致并拒绝危险静态结构；仍缺 SBOM/安全扫描、conformance、verifier runtime、Adapter 采用、v213 route、真实派发和结算。真实崩溃/断电、生产文件边界、TCP、升级、部署及 v227-v232 MCP/PC 也未验收。见 [`external-pool-adapter-artifact-package-authority.md`](external-pool-adapter-artifact-package-authority.md) 与 [`external-pool-adapter-artifact-package-acceptance.md`](external-pool-adapter-artifact-package-acceptance.md) |
