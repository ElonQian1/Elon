...
4. Call `ui_get_design_task` and bind a `TARGET_DESIGN` attachment with `ui_bind_target_design`. Then call `ui_get_project_profile`, `ui_get_runtime_status`, and `ui_check_capabilities`.
...
8. Accept a candidate only after deterministic/source write-back and patch-free build verification pass.
...
11. Do not treat unrelated repositories or arbitrary filesystem paths as trusted. Stop when a circuit breaker moves the gap to `HUMAN_REQUIRED`, or when product/security judgment is required. Return the gap ID and exact decision needed.
