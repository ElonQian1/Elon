...
foreach ($name in @('test-binance-grid-create-adapter.cjs','test-binance-grid-create-funds.cjs','test-binance-grid-manage-adapter.cjs','test-binance-grid-protection-contract.cjs','test-binance-grid-trailing-rules.cjs','test-binance-grid-trailing-contract.cjs','test-binance-grid-trailing-adapter.cjs','test-binance-grid-read-session.cjs','test-binance-grid-read-diagnostics.cjs','test-binance-grid-read-adapter.cjs')) {
    & node (Join-Path $PSScriptRoot $name)
    if ($LASTEXITCODE -ne 0) { throw "Binance adapter contract failed: $name" }
}
if ($AdapterOnly) {
    Write-Output 'BINANCE_ADAPTER_SUITES_PASSED=10 NATIVE_TESTS_EXECUTED=false TRADING_REQUESTS_EXECUTED=false'
    return
}
...
try {
    ...
    & (Join-Path $root 'android/gradlew.bat') -p (Join-Path $root 'android') --no-daemon --max-workers=1 :app:testDebugUnitTest `
        ...
        --tests com.elon.app.grid.create.BinanceCreateFundsResultTest `
