...
- `rust-shared-target-cache`: never use a relative `CARGO_TARGET_DIR`. Prefer repository scripts for Rust builds; on new machines or suspicious builds, check user/machine `CARGO_TARGET_DIR` and use an absolute Cargo `target-dir`. Do not hardcode one PC's drive letter in shared scripts; use `RUST_SERVER_MUSL_TARGET_DIR` for a machine-wide server-musl cache shared across Rust backend repos (`RUST_MUSL_TARGET_DIR` remains a compatible old name), or `ELON_BUILD_TARGET_DIR` for this repo's legacy per-project cache root.
...
- Per-machine build cache paths must be local configuration, not committed script defaults. `scripts/publish-server.ps1` and `scripts/publish-server.sh` first read `RUST_SERVER_MUSL_TARGET_DIR` as an exact `CARGO_TARGET_DIR` for machine-wide sharing across Rust backend repos, with `RUST_MUSL_TARGET_DIR` as a compatible old name. If neither is set, they read `ELON_BUILD_TARGET_DIR` from the process environment or root `.env.local` and create a **fixed-name** subdirectory `elon-server-musl/` under that root — never a SHA-specific directory. If all variables are unset, Windows uses `%LOCALAPPDATA%\Elon\build-target\elon-server-musl\` and Linux/macOS uses `~/.cache/elon/build/elon-server-musl/` (XDG standard). These paths survive across sessions on the same machine, enabling **incremental compilation**. Never manually set `CARGO_TARGET_DIR` to a SHA-specific path — that defeats the cache.
