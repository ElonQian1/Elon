...
const REPORT_VERSION: &str = "a2b2jc1";
const REPORT_FIELD_COUNT: usize = 83;
const MAX_REPORT_PAYLOAD_BYTES: usize = 1_024;

impl JointCloseActual {
    pub(in super::super::super) fn to_report_payload(&self) -> String {
        ...
        let values: [u64; REPORT_FIELD_COUNT] = [
            identity.path as u64,
            identity.topology as u64,
            identity.mode as u64,
            identity.node as u64,
            u64::from(identity.variant),
            u64::from(identity.pre_shared_mask),
            u64::from(identity.pre_exclusive_mask),
            identity.main_lock_prestate as u64,
            identity.main_lock_offset_class as u64,
            identity.phase as u64,
            identity.cause as u64,
            identity.timing as u64,
            identity.class as u64,
            target.scope as u64,
            ...
            target.role as u64,
            target.callback as u64,
            u64::from(target.occurrence),
            identity.sqlite_outcome as u64,
            u64::from(self.mutation_may_have_occurred),
            u64::from(self.lock_outcome_uncertain),
            u64::from(self.domain_terminal),
            self.registry_route_phase as u64,
            self.logical_route_phase as u64,
            self.registration_phase as u64,
        ...
        let mut report = String::with_capacity(320);
        write!(
            &mut report,
            "{REPORT_VERSION},{}",
    ...
    pub(super) fn from_report_payload(input: &str) -> Result<Self, &'static str> {
        if input.len() > MAX_REPORT_PAYLOAD_BYTES || !input.is_ascii() {
            return Err("JointClose report payload is not bounded ASCII");
        }
        let mut parts = input.split(',');
        if parts.next() != Some(REPORT_VERSION) {
            return Err("JointClose report payload version is unsupported");
        }
        let selector = JointCloseSelector::from_report_name(
            parts
                .next()
                .ok_or("JointClose report payload has no selector")?,
        )
        .ok_or("JointClose report payload selector is unsupported")?;
        let mut fields = Fields { parts };
        let identity = JointCloseActualIdentity {
            path: JointClosePath::try_from(fields.u64()?)?,
            topology: JointCloseTopology::try_from(fields.u64()?)?,
            mode: JointCloseMode::try_from(fields.u64()?)?,
            node: JointCloseNode::try_from(fields.u64()?)?,
            variant: fields.u8()?,
            pre_shared_mask: fields.u8()?,
            pre_exclusive_mask: fields.u8()?,
            main_lock_prestate: JointCloseMainLockPrestate::try_from(fields.u64()?)?,
            main_lock_offset_class: JointCloseMainLockOffsetClass::try_from(fields.u64()?)?,
            phase: JointClosePhase::try_from(fields.u64()?)?,
            cause: JointCloseCause::try_from(fields.u64()?)?,
            timing: JointCloseTiming::try_from(fields.u64()?)?,
            class: JointCloseFailureClass::try_from(fields.u64()?)?,
            target: JointCloseActualTarget {
                scope: JointCloseTargetScope::try_from(fields.u64()?)?,
                registration_id: fields.u64()?,
                route_ordinal: fields.u64()?,
                runtime_generation: fields.u64()?,
                shm_connection_id: fields.u64()?,
                role: JointCloseRole::try_from(fields.u64()?)?,
                callback: JointCloseCallback::try_from(fields.u64()?)?,
                occurrence: fields.u32()?,
            },
            sqlite_outcome: JointCloseSqliteOutcome::try_from(fields.u64()?)?,
        };
        let actual = Self {
            selector,
            identity,
            ...
            registry_route_phase: JointCloseRegistryRoutePhase::try_from(fields.u64()?)?,
            logical_route_phase: JointCloseLogicalRoutePhase::try_from(fields.u64()?)?,
            registration_phase: JointCloseRegistrationPhase::try_from(fields.u64()?)?,
        ...
        if fields.parts.next().is_some() || actual.to_report_payload() != input {
            return Err("JointClose report payload is not canonical");
        }
        Ok(actual)
    }
}
...
impl Fields<'_> {
    fn u64(&mut self) -> Result<u64, &'static str> {
        self.parts
            .next()
            .ok_or("JointClose report payload is truncated")?
            .parse()
            .map_err(|_| "JointClose report field is not an unsigned integer")
    }

    fn u32(&mut self) -> Result<u32, &'static str> {
        self.u64()?
            .try_into()
            .map_err(|_| "JointClose report field exceeds u32")
    }

    fn u8(&mut self) -> Result<u8, &'static str> {
        self.u64()?
            .try_into()
            .map_err(|_| "JointClose report field exceeds u8")
    }
    ...
    fn boolean(&mut self) -> Result<bool, &'static str> {
        match self.u8()? {
            0 => Ok(false),
            1 => Ok(true),
            _ => Err("JointClose report boolean is not zero or one"),
        }
    }

    fn topology(&mut self) -> Result<JointCloseActualTopology, &'static str> {
        Ok(JointCloseActualTopology {
    ...
    fn custody(&mut self) -> Result<JointCloseActualCustody, &'static str> {
        Ok(JointCloseActualCustody {
            node: self.boolean()?,
            views: self.u8()?,
            mappings: self.u8()?,
            dms: JointCloseDmsCustody::try_from(self.u64()?)?,
    ...
    fn counts(&mut self) -> Result<JointCloseActualCounts, &'static str> {
