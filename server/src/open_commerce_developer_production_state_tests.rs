...
fn approved_fixture() -> ApprovedFixture {
    ...
    // These direct store transitions prove only local state-machine behavior.
    // Real DNS/TLS challenge verification remains the service/network layer's job.
    let app = store
...
fn issue_local_credential(fixture: &ApprovedFixture) -> DeveloperProductionCredentialSecret {
    fixture
        .store
