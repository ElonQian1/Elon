...
pub(super) fn execute_in_isolated_worktree(
...
pub(super) fn run_git_worktree_add(source_git_root: &Path, run_workspace: &Path) -> ProcessOutput {
    run_process(
        source_git_root,
        "git",
        &[
...
pub(super) fn run_fixed_command(
    workdir: &Path,
    display_command: &str,
    program: &str,
    args: &[String],
...
pub(super) fn validate_command(command: &PatchVerificationCommand) -> Result<(), String> {
    parse_command(&command.command).map(|_| ())
}

pub(super) fn parse_command(command: &str) -> Result<(String, Vec<String>), String> {
    if command
        .chars()
        .any(|ch| matches!(ch, '&' | '|' | ';' | '<' | '>' | '`' | '"' | '\''))
    {
...
pub(super) fn is_allowed_program_args(program: &str, args: &[String]) -> bool {
    match program {
        "git" => {
...
pub(super) fn process_output(child: std::process::Child, timed_out: bool) -> ProcessOutput {
...
pub(super) fn temp_run_dir() -> PathBuf {
