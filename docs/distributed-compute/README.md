...
| 共享 CapacityPool 与追加式容量账本 | 领域合同、v165-v168 schema、隔离 Store、Store-canonical Supply/Claim 请求摘要、只读审计、到期批处理、状态门卫和 epoch 轮换已写；v173 为 Claim 每次 revision 追加完整不可变历史；Hold 已收紧窗口/TTL，Release/Expire 只接受 Claim 自有 held 容量；尚未编译、执行迁移或接线 |
...
| ComputeReservation 版本注册表 | v174 schema、Job/Offer/Price Snapshot/Claim 精确版本绑定、当前投影、不可变历史、消费者幂等、CAS、状态机和完整依赖审计已写；尚未编译、执行迁移或接入 HTTP/Broker，不移动容量或资金 |
