...
用户可以选择把自己的 Codex ChatGPT / Pro `auth.json` 加密备份到一龙云端服务器，作为“保险箱”。这不是账号转租功能，边界必须固定：

| 场景 | 允许 | 说明 |
|---|---:|---|
| 本人 PC 节点读取本机默认 `~/.codex/auth.json` 并上传备份 | 是 | 由本机节点读取，浏览器前端不接触明文。 |
| 云端保存 `auth.json` 明文 | 否 | 云端只保存 AES-GCM 密文和 nonce，密钥来自服务器环境变量。 |
| 本人节点从云端恢复到托管临时 `CODEX_HOME` | 是 | 需要用户登录 token + 本人节点 secret 双重证明。 |
| 其他用户下载或恢复该 `auth.json` | 否 | 共享算力只允许别人把任务派发到 provider 的节点，不能拿到凭证明文。 |
| 共享节点的 token / 收益统计 | 是 | consumer 侧记 `shared_codex`，provider 侧记节点结算流水。 |

产品文案应使用“共享 PC 节点 Codex 执行能力”或“共享算力”，不要写成“分享 auth.json 账号”。保险箱只解决账号所有者自己的备份和恢复问题。
...
本机节点状态必须把实际读取路径、是否存在、`auth_mode`、是否有 `refresh_token` 和错误原因返回给页面。用户换电脑、换 Windows 账号、手动设置 `CODEX_HOME` 或使用托管临时登录态时，AI 助手只能根据节点状态提示用户去对应电脑重新登录或点击备份，不能猜路径、不能要求用户粘贴明文。

### 普通聊天、MCP 与节点边界

普通聊天 AI 可以帮助用户完成保险箱备份，但方式必须是确定性的本机节点动作，不是让模型自由读文件或执行 shell：

| 能力 | 当前策略 |
|---|---|
| PC 普通聊天触发备份 | 允许。仅在明确备份意图时由前端调用本机 Win 端保险箱 API。 |
| `/api/llm/chat` 云端模型读取本机文件 | 禁止。云端模型没有本机文件权限。 |
| 普通聊天把 prompt 转给本机 CLI 后让 CLI 读 `auth.json` | 不作为推荐路径。CLI 输出和提示词不可作为保险箱工具边界。 |
| 暴露通用 MCP 文件读取给普通 AI | 禁止。`auth.json` 是凭据文件，不能作为泛化文件工具交给模型。 |
| 未来 MCP 化保险箱 | 只允许做窄工具，例如 `codex_vault.backup_default_auth`，工具内部仍由本机节点读默认路径、加密上传，并返回状态摘要。 |

出现问题时，AI 助手的处理顺序固定为：先检测本机 Win 端是否在线和已绑定账号，再检测默认 `auth.json` 是否存在、是否 ChatGPT / Pro 登录态、服务器保险箱是否配置，最后引导用户到 `/pc/node` 一键修复或重新登录 Codex。不要让用户手动打开、复制、粘贴或上传 `auth.json` 明文。

### Pro 包月成本参考

如果用户想按 ChatGPT Pro `$200/月` 粗略估算共享成本，系统只展示参考价，不把它当作真实账单：

```text
周成本 = 200 * 12 / 52 = 46.15 美元/周
如果 5 小时 ≈ 每周额度 30%
5 小时参考成本 = 46.15 * 30% = 13.85 美元
每小时参考成本 = 13.85 / 5 = 2.77 美元
```

真实结算仍以一龙平台账本为准：`node_transactions.billed_cost_rmb_fen` 是消费者真实扣费，`provider_earned_fen` 是按平台分账比例计算的 provider 收益。Pro 成本参考只帮助 provider 理解自己的包月容量价值；在没有 Codex 官方周额度百分比遥测前，不应把 token 数直接换算成“已用 Pro 周额度百分比”。
