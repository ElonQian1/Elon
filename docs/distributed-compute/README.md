...
> v218 合同修正：历史“v11 Planning Snapshot V2”现以协议阈值 12 解释；work-admission `None` 允许新晋升槽首次重授权，`Some` 才承诺 current head。A1 同事务 projector 已随完整测试目标编译，但无专项运行或 producer；A2 总合同已冻结，测试目标可编译且 targeted fault matrix 5 项通过。Barrier 与 RegistrationShutdown 已分别形成 `WindowsDynamic=8/8`，RegistryLifecycle 已形成 `16/16`，Unmap 已形成 `49/49`，JointClose 又在 clean evidence commit `bfa1a1180d220e9a4c8e39251414fc9a1b0a9ace` 上形成 `36/36`，A2b2 当前为 `WindowsDynamic=117/117`；clean wide regression `266/266` 已通过。Map/Lock StaticContract 已为 `43476/43476` 与 `8668/8668`；dynamic quotient/Q/WindowsDynamic 仍未开放，状态仍为 `implementation_not_dynamically_accepted`。v14 永久 blocked-only，生产 VFS/open/Connection/Runtime/Ready/派发不可达。
...
| UserNode Ready source lineage 与本机当前性封印草案 | 未登记 API-free source draft：shared contract 与 Node owner adapter 只把 v217 work-admission、局部 Ready-health 和明确 `Untrusted` 的 Host runtime observation 投影为六键 JCS lineage；CPU-only 保持 accelerator=0，不虚构 GPU。新增 local-currentness seam 只在未来 handle-bound opened authority 的 query-only Deferred 快照中重审 exact head/chain、共享授权、process fence、Ready inventory revision/policy、逐字段相同的 exact plugin record 与 fresh health TTL，并用 HRTB/private-field/non-Clone/non-Serde seal 禁止权威外逃。原 envelope 仍写四项 gap=`missing`，只有 seal lifetime 内可解释第一项已重证；其余 runtime transition、Host runtime、v15 session 仍缺。当前 `implementation_uncompiled/implementation_unrun`、`passed=0/failed=0`、`migration/table/writer=none/none/none`，且 handle-bound open producer 不存在；不生成 Ready、route、Offer、Attempt、Lease 或下游效果。见 [lineage authority](user-node-ready-source-lineage-authority.md)、[local-currentness authority](user-node-ready-local-currentness-authority.md) 与 [acceptance](user-node-ready-local-currentness-acceptance.md) |
| UserNode Windows Runner loader/进程监管前置草案 | 未登记且不可达：typed chain 已从 retained discovery 铺到 exact context、package-only pre-lease PE、preliminary requests、私有 GrantReady、signed recursive-policy verification evidence、`[A0 PolicyCurrent GrantReady→base acquisition \| Ak plan V1→currentness-pending→authorization→DispatchReady]`、acquisition receipt V3、empty frontier/final aggregate、recursive final projection、QueryVerified lineage与 process pre-create projection；policy binding V2、receipt-set/chain V1、parse/closure V2、profile V3。nested API-set DAG与 Shadow positive fail-closed。真实 selector/signature verifier/currentness backend、parser、resolver、grant/candidate/lease backend与 positive advancer、sealer/query、live-OS/create/resume/recovery producers仍缺；Windows dynamic=`0`，严格 `source_written/source_review_only/implementation_uncompiled/implementation_unrun`、`passed=0/failed=0`、`migration/table/writer=none/none/none`、四 gap=`missing`、loader 18 effects=`none`。见 [launch-context](user-node-windows-runner-launch-context-selection-authority.md)、[policy currentness](user-node-windows-runner-recursive-policy-currentness-authority.md)、[acceptance](user-node-windows-runner-recursive-policy-currentness-acceptance.md)、[recursive acquisition](user-node-windows-runner-recursive-system-image-acquisition-custody-authority.md)、[recursive closure](user-node-windows-runner-recursive-system-image-closure-authority.md)、[loader](user-node-windows-runner-loader-load-set-authority.md) 与 [process](user-node-windows-runner-process-custody-authority.md) 专题 |
...
| 节点插件治理合同 | Signed Manifest/InstallPlan、双 keyring、authority、受管取数/验证/staging/cleanup 与 v216 install/promotion 双回执已形成；v216 已随节点编译并通过 11 项版本链及既有 69 项 SQLite 回归，其中 3 项为单 Connection 成功路径。v217/v218 work-admission 与 Planning A1 已随完整测试目标编译，但无专项、迁移或 runtime 验证；A2 targeted fault matrix 5 项通过。A2b1 owner graph 以 source baseline `e3663e109039f38477de4d6ab5cd57483dbd0541` 续绑，并在 clean evidence commit `bfa1a1180d220e9a4c8e39251414fc9a1b0a9ace` 保持 guard `4/4`。Barrier、RegistrationShutdown 与 RegistryLifecycle 已分别以 Windows `8/8`、`8/8`、`16/16` 通过；[Unmap 49](node-plugin-vfs-unmap-dynamic-authority.md) 已在 exact clean commit `da62f95b09287b79bc1f4c23780b95993cdd85a0` 上首次正式 `49/49`，[JointClose 36](node-plugin-vfs-joint-close-dynamic-authority.md) 随后正式 `36/36`，A2b2 为 `117/117`，clean wide regression `266/266` 已通过。Map/Lock StaticContract 已为 `43476/43476` 与 `8668/8668`；[dynamic quotient](node-plugin-vfs-map-lock-dynamic-quotient-authority.md)、Q 与 WindowsDynamic 未开放，A2 仍未动态验收。新增未登记 open-attempt 仅冻结无生产 owner 的 pending→opening custody/失败拓扑；Node target 已编译、registry `45/45` 通过，但 typestate runtime 未运行。生产 VFS/register/live `sqlite3_file`/process owner/open/Connection、A1 producer、root/keyring/time/rollback、Signer、Host、downloader、Sidecar、Runtime、Ready 与 Attempt 接线仍不可达，v14 永久 blocked-only。精确边界见 [A2 acceptance](node-plugin-vfs-fault-acceptance.md) |
...
| `capacity_future` pricing-mode 交付结算谱系 bridge | 已登记并因动态证据推迟而 `blocked`：API-free Domain 与 Lease-rooted Store retained resolver source 在单一 Deferred snapshot 重建 Instrument/Commitment/exercised Allocation/Snapshot/F0 owners；闭合 v193→v192 decision digest，拒绝 v228 Claim index/cardinality splice，并保留 historical Provider settlement-account payee 规则。当前 `source_review_only/implementation_uncompiled/implementation_unrun`、`passed=0/failed=0`，无 Service/API/migration/writer/ClearingReceipt 或 effect 接线 |
...
55. `docs/requirements/compute-capacity-future-settlement-lineage-bridge-v1.md`、`docs/distributed-compute/capacity-future-settlement-lineage-authority.md` 与对应 acceptance：记录已登记/`blocked` 的 v238→v225→v228→F0→v195/v198 reference-only `capacity_future` bridge、双摘要域、decision/payee/Claim index 边界与 Lease-rooted Store retained resolver source；当前未编译/运行，无 Service/API/migration/writer 或 effect 接线。
