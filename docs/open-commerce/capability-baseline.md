...
| 消费者本人授权状态 | 代码已形成、未编译 | 按当前用户和当前项目本人 App 隔离列出四种申请状态及批准后的 Grant 条件；省略内部身份，不取消、重提或调用，历史批准仍须复核当前 Grant | `server/src/open_commerce_consumer_authorization_mcp.rs`、`server/src/store/open_commerce_authorization_requests.rs`、`docs/decisions/open-commerce-consumer-authorization-status-mcp-v1.md` |
