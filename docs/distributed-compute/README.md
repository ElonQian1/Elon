...
| Provider / Offer / Job / Reservation / Lease / Receipt 统一领域合同 | 基础代码、v169-v176 注册表、Broker Reserve/未执行任务终态、v185-v195 Attempt 激活、证据链、可信终态、容量收口与待结算回执已写，状态为 `implementation_uncompiled`，尚未迁移、完整接线和运行验证 |
...
26. 现有兼容实现：`docs/decisions/node-compute-sharing-supply-v1.md`。
...
v195 再基于精确 v194/v193、Broker 预授权和 Price Snapshot 生成不可变 Settlement Receipt：消费者价格腿使用 verified usage 并按快照舍入到人民币分，Provider 价格腿使用 compensable usage；单事务扣结预授权、退回未用余额、登记 Provider/平台 pending 收益并把 Job 推进为 `settled`。首版仅支持 CNY 基础组件，pending 不可提现，不调用真实支付或链上网络。
