//! Independent typed fixtures for the 88 q13 existing-first exclusive-release terminals.
...
pub(super) const LOCK_EXISTING_FIRST_EXCLUSIVE_RELEASE_ERROR_MEMBER_COUNT: usize = 88;

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub(super) enum FrozenExistingFirstExclusiveReleaseCompletionV1 {
    RetentionSucceeded,
    RetentionRouteUnknown,
}

impl FrozenExistingFirstExclusiveReleaseCompletionV1 {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub(super) struct FrozenLockExistingFirstExclusiveReleaseErrorCaseV1 {
    pub(super) action: LockActionV1,
    pub(super) first: u8,
    pub(super) count: u8,
    pub(super) mask: u8,
    pub(super) completion: FrozenExistingFirstExclusiveReleaseCompletionV1,
}

#[derive(Clone)]
pub(super) struct FrozenLockExistingFirstExclusiveReleaseErrorLeafV1 {
    ...
    LEAVES.get_or_init(|| {
        ...
        super::super::super::source_leaf_authority::validate_lock_graph_with_records(
            ...
            |leaf| {
                ...
                let Some(case) = existing_first_exclusive_release_error_v1(record, descriptor)
                else {
                    return Ok(());
                };
                ...
                if previous.is_some() {
                    return Err(format!("duplicate frozen q13 member {case:?}"));
                }
                Ok(())
            },
        )
        .expect("the frozen Lock authority must validate before q13 source tests");
        assert_eq!(
            leaves.len(),
...
pub(super) fn lock_existing_first_exclusive_release_error_descriptor_v1(
    case: FrozenLockExistingFirstExclusiveReleaseErrorCaseV1,
    capability: RunnerCapabilityV1,
) -> TerminalDescriptorV1 {
    let mut descriptor =
        frozen_lock_existing_first_exclusive_release_error_leaves_v1()[&case].descriptor;
    let TerminalDescriptorV1::Lock(value) = &mut descriptor else {
        unreachable!("a q13 fixture must have a Lock descriptor")
    };
    value.recipe.capability = capability;
    descriptor
}

pub(super) fn lock_existing_first_exclusive_release_error_expected_groups_v1(
        ...
        .map(|leaf| {
            (
                prepare_dynamic_terminal_v1(&leaf.record, &leaf.descriptor)
                    .expect("a frozen q13 descriptor must prepare")
                    .key,
                leaf.member,
            )
        })
...
fn existing_first_exclusive_release_error_v1(
    record: &LeafRecordV1,
    descriptor: &TerminalDescriptorV1,
) -> Option<FrozenLockExistingFirstExclusiveReleaseErrorCaseV1> {
    ...
    let case = FrozenLockExistingFirstExclusiveReleaseErrorCaseV1 {
        action,
        first,
        count,
        mask,
...
const fn expected_axes_v1(case: FrozenLockExistingFirstExclusiveReleaseErrorCaseV1) -> LockAxesV1 {
    LockAxesV1 {
...
#[test]
fn frozen_q13_family_is_exact_unique_and_only_contains_acquire_requests() {
