...
Offer 本人控制面在此基础上只允许当前用户的 active Provider/Pool 创建规范化 draft Offer，并要求 Provider 已有路由、verified 硬件摘要和验证时间。服务端从 Provider、Pool 和 open Bucket 当前版本固定供给身份、容量绑定、窗口、区域、meter 与规范摘要；幂等 ID 由用户、Provider、Pool 和幂等键确定。所有者还可用精确版本和摘要将当前 draft 追加为 revoked 下一版本。该路径只写入 v170 Offer Registry，不把状态提升为 active，不生成 Price Snapshot、不暴露候选、不预留容量也不移动资金；当前为 `implementation_uncompiled`。
