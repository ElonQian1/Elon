...
enum RunPreflight {
    ...
    Replay(ExternalPoolAdapterRuntimeCompatibilityRunObservationReceipt),
}
...
impl Store {
    ...
    pub(in crate::store) fn run_external_pool_adapter_runtime_compatibility_verification_challenge(
    ...
    ) -> Result<ExternalPoolAdapterRuntimeCompatibilityRunObservationWriteReceipt> {
        super::read::identifier(challenge_id)?;
        let preflight = {
            let mut conn = self.conn()?;
            let tx = conn.transaction_with_behavior(TransactionBehavior::Immediate)?;
            let challenge = challenge_by_id_on(&tx, challenge_id)?
                .ok_or_else(|| anyhow::anyhow!("V268 challenge was not found"))?;
            if challenge.receipt.challenge_digest != expected_challenge_digest {
                bail!("V268 expected challenge digest is not exact");
            }
            support::audit_prepared_installation(&prepared, &challenge.receipt)?;
            let state =
                if let Some(observation) = run_observation_by_challenge_on(&tx, challenge_id)? {
                    RunPreflight::Replay(observation.receipt)
                } else {
                    require_fresh_current_authority(&tx, &challenge.receipt, &now())?;
                    RunPreflight::Fresh(challenge.receipt)
                };
            tx.commit()?;
            state
        };
        let challenge = match preflight {
            RunPreflight::Replay(run_observation) => {
                let challenge = {
                    let conn = self.conn()?;
                    challenge_by_id_on(&conn, challenge_id)?
                        .ok_or_else(|| anyhow::anyhow!("V268 replay lost its challenge"))?
                        .receipt
                };
        ...
        let started = DateTime::parse_from_rfc3339(&run_started_at)?;
        let completed = DateTime::parse_from_rfc3339(&run_completed_at)?;
        if completed - started
            > chrono::Duration::seconds(RUNTIME_COMPATIBILITY_MAX_RUN_SECONDS as i64)
        {
            bail!("V268 server-owned run exceeded its exact duration window");
        }
        let mut conn = self.conn()?;
        let tx = conn.transaction_with_behavior(TransactionBehavior::Immediate)?;
        let current = challenge_by_id_on(&tx, challenge_id)?
            .ok_or_else(|| anyhow::anyhow!("V268 challenge disappeared after execution"))?;
        if current.receipt != challenge {
            bail!("V268 challenge drifted during execution");
        }
        support::audit_prepared_installation(&prepared, &current.receipt)?;
        if let Some(stored) = run_observation_by_challenge_on(&tx, challenge_id)? {
            let signature_challenge =
        ...
        if run_completed_at >= current.receipt.challenge.expires_at {
            bail!("V268 challenge expired before durable observation commit");
        }
        ...
        insert_run_observation(&tx, &receipt)?;
        let stored = run_observation_by_id_on(&tx, &receipt.run_observation_id)?
            .ok_or_else(|| anyhow::anyhow!("V268 observation disappeared after insert"))?;
        if stored.receipt != receipt {
            bail!("V268 observation readback drifted");
        }
        let signature_challenge =
            runtime_compatibility_signature_challenge(&current.receipt, &stored.receipt)?;
        let output = ExternalPoolAdapterRuntimeCompatibilityRunObservationWriteReceipt {
...
fn require_fresh_current_authority(
