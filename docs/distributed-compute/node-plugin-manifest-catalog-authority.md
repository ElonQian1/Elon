...
当前仍没有可注册或可成功打开的生产handle-bound SQLite VFS、生产process-owner实例、从生产SQLite ABI进入registry的live route、持有私有Rust file-custody的生产live `sqlite3_file`或生产非空`pMethods`、生产authorizer/PRAGMA持续门卫、生产trusted-time/rollback provider、opened snapshot producer或consumer。真实managed-fs句柄与registry租约的不可拆托管状态、受callback lease约束的安全操作门面、raw-state所有权协议、具体ABI回调桥、测试专用真实file-custody回调矩阵和真实Connection安全夹具均已存在，但尚无生产路径构造适配器、安装state或把SQLite `xOpen`接到registry。测试Connection已在同一exact route上安装authorizer，回读五项db_config和两个连接limit，并真实拒绝ATTACH、temp表、运行期PRAGMA、扩展加载、非白名单函数和DQS字符串；这不能证明生产VFS接线。临时文件仍不实现；现有locking/SHM/close、关闭receipt适配、请求投影、owner/状态核、file-custody和ABI表仍不可生产到达，controller与namespace仍只接到不可打开的sealed intent，生产`open()`继续固定unavailable。

该隔离Connection夹具现已完成：它使用独立非默认测试VFS和测试process-owner走真实SQLite open/close，在同一Connection安装route-backed authorizer，关闭extension loading，设置并回读DEFENSIVE、TRUSTED_SCHEMA、DQS、ATTACHED和worker限制；显式close后Windows可删除数据库和残留sidecar。下一批仍应单独设计测试可达的`xOpen` route producer，使命名测试VFS的打开事件真正绑定registry与file-custody，再据此审查生产process-owner、注册/注销所有权和失败保管；不得把当前默认VFS委托层直接提升为生产入口。v11仍固定`context_ready=false`、`snapshot_ready=false`，并保持root/authority、PlanApply、work-admission、下载、安装和Sidecar标志为false。非空库存还缺installed/promotion/signed-manifest provenance、work-admission generation、Ready/Attempt撤销及signed `reauthorize_existing`，不得由本合同推断为已完成。
