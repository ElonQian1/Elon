...
本文是已接受的客户端目标设计。NodeAgent 内部 legacy Host seam、版本化合同、Manifest/InstallPlan 与双 keyring、本机 authority v3/PlanApply、可恢复分段取数、Windows 受管文件、候选原子验证、可信时间证明和回滚锚回执合同已形成代码。ZIP staging Store、候选健康成功 Store、终止失败 quarantine Store 及各自不确定结果恢复/adoption 合同均已形成，默认关闭的 Bootstrap 已挂载到 NodeRuntime。相关代码已通过 `elon-pc-node` 编译和 12 项候选健康/schema 定向测试，但 schema DDL 仅在内存 SQLite 验证建库与重开，完整 Store 事务夹具、生产启动、本地建库/迁移和 Host 接线尚未完成。quarantine 只将槽推进为 `failed` 并留存审计回执与文件 custody，不是物理清理或重试授权。生产时间源与回滚见证、真实 HTTPS downloader/socket-read 取消、跨重启/目录耐久恢复、真实 Sidecar 探针、失败候选清理完成链、installed/promotion、IPC/沙箱、`ReadyCapability` 上报和调度接线仍未实现。节点不直接发布商业 Offer；精确物化与健康边界见 `node-plugin-archive-extraction.md`、`node-ready-capability.md`。
