//! Independent typed fixtures for the 88 q15 existing-first truncate-error release-ok terminals.
...
pub(super) const LOCK_EXISTING_FIRST_TRUNCATE_ERROR_RELEASE_SUCCEEDED_MEMBER_COUNT: usize = 88;

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub(super) enum FrozenExistingFirstTruncateErrorReleaseSucceededCompletionV1 {
    RetentionSucceeded,
    RetentionRouteUnknown,
}

impl FrozenExistingFirstTruncateErrorReleaseSucceededCompletionV1 {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub(super) struct FrozenLockExistingFirstTruncateErrorReleaseSucceededCaseV1 {
    pub(super) action: LockActionV1,
    pub(super) first: u8,
    pub(super) count: u8,
    pub(super) mask: u8,
    pub(super) completion: FrozenExistingFirstTruncateErrorReleaseSucceededCompletionV1,
}

#[derive(Clone)]
pub(super) struct FrozenLockExistingFirstTruncateErrorReleaseSucceededLeafV1 {
    pub(super) record: LeafRecordV1,
    pub(super) descriptor: TerminalDescriptorV1,
    pub(super) member: StaticMemberSealV1,
}

pub(super) fn frozen_lock_existing_first_truncate_error_release_succeeded_leaves_v1(
) -> &'static BTreeMap<
    FrozenLockExistingFirstTruncateErrorReleaseSucceededCaseV1,
    FrozenLockExistingFirstTruncateErrorReleaseSucceededLeafV1,
> {
    ...
    LEAVES.get_or_init(|| {
        ...
        super::super::super::source_leaf_authority::validate_lock_graph_with_records(
            ...
            |leaf| {
                ...
                let Some(case) =
                    existing_first_truncate_error_release_succeeded_v1(record, descriptor)
                else {
                    return Ok(());
                };
                ...
                if previous.is_some() {
                    return Err(format!("duplicate frozen q15 member {case:?}"));
                }
                Ok(())
            },
        )
        .expect("the frozen Lock authority must validate before q15 source tests");
        assert_eq!(
            leaves.len(),
...
pub(super) fn lock_existing_first_truncate_error_release_succeeded_descriptor_v1(
    case: FrozenLockExistingFirstTruncateErrorReleaseSucceededCaseV1,
    capability: RunnerCapabilityV1,
) -> TerminalDescriptorV1 {
    let mut descriptor =
        frozen_lock_existing_first_truncate_error_release_succeeded_leaves_v1()[&case].descriptor;
    let TerminalDescriptorV1::Lock(value) = &mut descriptor else {
        unreachable!("a q15 fixture must have a Lock descriptor")
    };
    value.recipe.capability = capability;
    descriptor
}

pub(super) fn lock_existing_first_truncate_error_release_succeeded_expected_groups_v1(
        ...
        .map(|leaf| {
            (
                prepare_dynamic_terminal_v1(&leaf.record, &leaf.descriptor)
                    .expect("a frozen q15 descriptor must prepare")
                    .key,
                leaf.member,
            )
        })
...
fn existing_first_truncate_error_release_succeeded_v1(
    record: &LeafRecordV1,
    descriptor: &TerminalDescriptorV1,
) -> Option<FrozenLockExistingFirstTruncateErrorReleaseSucceededCaseV1> {
    ...
    let completion = match completion {
        LockCompletionV1::UnsafeRetentionSucceededThenRouteUnknown => {
            FrozenExistingFirstTruncateErrorReleaseSucceededCompletionV1::RetentionSucceeded
        }
        LockCompletionV1::UnsafeRetentionRouteUnknownThenRouteUnknown => {
            FrozenExistingFirstTruncateErrorReleaseSucceededCompletionV1::RetentionRouteUnknown
        }
        _ => return None,
    };
    let case = FrozenLockExistingFirstTruncateErrorReleaseSucceededCaseV1 {
        action,
        first,
        count,
        mask,
...
#[test]
fn frozen_q15_family_is_exact_unique_and_only_contains_acquire_requests() {
