...
6. 主项目 Rust/Axum 在 `/quant/` 托管量化 PWA，并只代理健康状态、运行时、研究快照和回测四个无凭据 Paper 接口；不得代理本人仓位、ESK、订单、运营、导入或任何写资金接口。
...
6. `/quant/` 由主项目 Rust/Axum 返回 PWA；四个公开 Paper 接口可用，未列入白名单的量化 API 返回 `404`，内部 API 只监听 loopback。
