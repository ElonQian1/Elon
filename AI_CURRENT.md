...
- 已实现、范围受限：连接器 SDK、项目内消费者发现与第三方应用沙盒；可以验证非付费发现、显式授权和测试 Token 调用，但它不是公共跨项目网络。
- 已实现、只预演：项目 AI 资源控制面可盘点自己的 Codex、本人节点、已授权共享 Codex 和平台模型并保存策略；它不启动任务、不验证外部额度，也未接管真实调度。
- 已实现、默认关闭：AI 任务与开放商业调用的链外影子经济层，可生成幂等用量凭证、验收后双分录和未提交网络的 Sui 投影信封；它不移动真实资金。
...
- 消费者 AI 的项目内沙盒已实现；跨项目公共发现网络、闲置算力公开市场、自动收费与跨主体结算仍在路线图或提案阶段。
