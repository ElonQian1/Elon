...
#[test]
fn explains_live_patch_as_real_component_redraw() {
    let line = r#"{"type":"item.started","item":{"id":"mcp_2","type":"mcp_tool_call","name":"ui_apply_live_patch","arguments":{"patch":{}}}}"#;
    let messages = stream_event_to_ws_messages(line, None);
    assert!(messages
        .iter()
        .any(|message| message.contains("真实 Android 组件立即重绘")));
    assert!(messages
        .iter()
        .any(|message| message.contains("尚未改源码")));
}
