...
| Attachment upload | Verified small-text private upload; native byte handoff, store association, ordinary/temporary conversation scopes and static-image private transaction now connected in source | Accept integrated new/existing-chat text and static images on-device, including temporary upload/library behavior, then cover project/PDF/multipart variants. No fresh integrated-device pass is claimed. See [upload contract](chatgpt-private-attachment-upload.md), [image extension](chatgpt-private-image-upload.md) and [scope contract](chatgpt-private-attachment-scopes.md). |
