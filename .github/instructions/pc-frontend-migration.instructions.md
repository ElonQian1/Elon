...
- 修改 `pc-frontend/`、`/pc`、`/pc-next` 或用户可见 PC 工作台 UI 时，`CodePushed` 只代表代码同步，不代表用户页面已经更新。仅前端、文档和发布脚本发生变化，且相对线上后端没有 `server/`、`contracts/`、`sdk/` 变化时，push 后运行 `scripts\publish-pc-frontend.ps1`；该脚本以独立前端 SHA、兼容后端 SHA 和远程 CAS 原子上传 `pc-next-dist`。同时包含后端或 API 合同变化时仍运行 `scripts\publish-server.ps1`。最后统一用 `PcFrontend` Kind 校验 `/pc/release.json`、`/pc`、`/api/server/version`、主基线和 worktree 状态。
