...
| 外部算力池适配器与统一报价 | v221-v254 前置门为 `implementation_partially_verified`。V250 scanner 链已通过 migration 4 项与进程内 HTTP 5 项，V252 sandbox 链已通过 migration 7 项与进程内 HTTP 2 项，V253 Provider-specific credential 链已通过 migration/源码合同 5 项与进程内 HTTP 3 项；三者不执行扫描、真实沙箱或 secret 解析。V254 仅追加 owner delegation 与 inert static compatibility candidate，并以 dynamic preflight 同次消费 V249 Prepared + current V250/V252/V253；closure 固定未实现、Provider 保持 `registering`、`activation_ready=false`。V254 已通过 fresh/repeat migration/源码合同 8 项与进程内 Axum HTTP 2 项，并安装 temporary market absolute deny；V253→V254 文件升级/重开、并发/崩溃、真实 TCP 和生产数据库仍未验收。candidate 不等于 activation、route、market 或 runtime readiness。见 [`V254 候选权威`](external-pool-provider-activation-candidate-authority.md)、[`V254 验收边界`](external-pool-provider-activation-candidate-acceptance.md)、[`V253 凭据续签权威`](external-pool-adapter-credential-reattestation-authority.md) 与 [`当前状态`](current-implementation-status.md) |
...
Provider Adapter 的 v227-v254 前置门已形成分层权威；V244-V249 保留采用、安装、撤销、sealed live-FS seam 与 registry/companion 的既有边界。V250 scanner、V252 sandbox 与 V253 Provider-specific credential 三条可续签声明链均已通过对应本地定向验收；它们不运行扫描、真实沙箱或 secret resolver，也不激活 Provider。V254 新增 owner delegation 和静态兼容候选，dynamic preflight 才在同一次检查中消费 V249 Prepared + current V250/V252/V253；即使输入 current，closure 仍为 `activation_closure_not_implemented`、`activation_ready=false`、Provider 仍为 `registering`。V254 已通过 fresh/repeat migration/源码合同 8 项与进程内 Axum HTTP 2 项，但 V253→V254 文件升级/重开、并发/崩溃、真实 TCP 和生产数据库仍未验收。candidate 不创建 v213 route/service actor authority，也不授权 Pool/Offer/Job/Attempt/Start。V254 已安装覆盖 direct SQL 与 versions 的 `external_pool` CapacityPool active、Offer draft/active temporary absolute deny；后续 atomic activation 必须显式把它替换为完整 readiness/currentness admission gate。最新状态见 [`current-implementation-status.md`](current-implementation-status.md)、[`V254 activation candidate`](external-pool-provider-activation-candidate-authority.md) 与 [`V253 credential re-attestation`](external-pool-adapter-credential-reattestation-authority.md)。
