...
| Same-origin text transaction coordinator | ChatGPT | Completed and enabled; retained for contexts outside the independent sender's accepted scope | Official-page transaction without automatic write replay; [contract](chatgpt-same-origin-text-transaction.md) |
| Fresh independent text dispatch | ChatGPT | Completed/default-enabled: existing/new personal plain text, existing personal Search/Image, local TXT/PDF/PNG with prompt, and existing ordinary owned-project plain text through normal 1754. No-usable-composer first send passed on adapter 411 | No uncertain-write replay. New/tool/file combinations, locked/temporary, other MIME, Library/mounted and cold-bootstrap scopes remain separate. [Current evidence](reports/chatgpt-runtime-bindings-20260915.md), [projects](chatgpt-fresh-project-text-dispatch.md) |
...
| Independent native file library | ChatGPT | Catalogue/pagination, 78 saved bytes, native entry/search/file menu verified in normal 1.1.1584; rendered folder/back and additional scopes pending; [evidence](chatgpt-private-library-browser.md) | Retained rows on refresh failure; explicit official library, no automatic navigation |
| Native private library file mutations | ChatGPT | Confirmed rename and soft-delete implemented; [offline evidence, device pending](chatgpt-private-library-mutations.md) | Read-only reconciliation or explicit official library; no automatic write replay |
| Native existing-library attachment | ChatGPT | Ordinary single-file scope completed and enabled: 1588 native card/remove, private IDs/names, single send and server file association passed; [evidence](chatgpt-private-library-attachment.md) | Preserve draft; no reupload, automatic send or cross-context association |
