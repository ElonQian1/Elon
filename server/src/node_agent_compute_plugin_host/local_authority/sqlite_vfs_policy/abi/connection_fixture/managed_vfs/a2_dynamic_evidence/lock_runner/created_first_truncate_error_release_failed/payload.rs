//! Canonical q16 dual-native-receipt payload and independent parent-side validation.

use anyhow::anyhow;
use rusqlite::ffi;
use sha2::{Digest, Sha256};

use super::super::super::child::lock_created_first_truncate_error_release_failed::{
...
pub(in super::super) struct ValidatedCreatedFirstTruncateErrorReleaseFailedPayloadV1 {
...
pub(in super::super) fn validate_payload(
    ...
    if values[25] == 0
        ...
        || values[109..113] != expected_cleanup_native
        || values[113..131] != expected_lock
        || values[131] != 0
        || values[132..150] != expected_terminal
        || values[150..156] != expected_preemption
        || values[156..160] != [1; 4]
        || values[160..163] != [1, 1, 3]
        || values[163] != 1
    {
        return Err(anyhow!(
            "q16 controlled initialization dual-receipt/custody mismatch"
        ));
    }
    Ok(ValidatedCreatedFirstTruncateErrorReleaseFailedPayloadV1 {
...
fn parse_canonical_u64(value: &str) -> anyhow::Result<u64> {
    ...
    if parsed.to_string() != value {
        return Err(anyhow!(
            "q16 Lock initialization payload scalar is not canonical"
        ));
    }
    Ok(parsed)
}
