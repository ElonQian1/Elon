...
(function () {
  ...
  async function loadMe() {
    ...
    currentUser = data.user;
    const name = currentUser.nickname || currentUser.account || '';
    const account = currentUser.account || '';
    const tail = (account && account !== name) ? ' · ' + account : '';
    const lines = [
      '我的开发工作台',
      '登录账号：' + name + tail,
